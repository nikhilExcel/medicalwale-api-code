...
class PharmacyPartner extends CI_Controller
{
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($params['phone'] == "") {
                    ...
                    $res   = $this->PharmacyPartnerModel->login_v1($phone, $token, $agent,$password);
                }
                simple_json_output($res);
            }
        }
                  ...
                  //  $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($this->input->post('user_id') == "") {
                        ...
                        // Both prescription and  general
                          ...
                          {
                            $resp = $this->PharmacyPartnerModel->order_status_common($order_id_data, $prescription_id_data,$order_details,$prescription_details,$delivery_charges,$delivery_charges_by_customer,$sub_total,$discount,$gst);
                            simple_json_output($resp);
                          } 
                    ...
                    // Only general
                      ...
                      {
                        $resp = $this->PharmacyPartnerModel->order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data,$delivery_charges,$delivery_charges_by_customer,$sub_total,$discount,$gst);
                        simple_json_output($resp);
                    ...
                    else if ($order_details =="" && $prescription_details !="")
                    {
                       ...
                       {
                        $resp = $this->PharmacyPartnerModel->prescription_status($order_id, $order_status, $prescription_order, $delivery_time,$listing_id,$delivery_charges,$delivery_charges_by_customer,$sub_total,$discount,$gst);
                        simple_json_output($resp);
                       } 
                    } 
                    }
  ...
  public function order_deliver_cancel()
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($barcode == "") {
                        ...
                        $resp = $this->PharmacyPartnerModel->product_barcode_scanner_v1($user_id,$hub_user_id,$barcode);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function product_inventory_bill_offline()
    {
        $this->load->model('PharmacyPartnerModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($user_id == "" || $hub_user_id == "" ||  $total_quantity == "" || $total_price == "" || $net_amount == "") {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter fields'
                        );
                    } else {
                        $resp = $this->PharmacyPartnerModel->product_inventory_bill_offline($user_id,$listing_name,$hub_user_id,$product_details,$total_quantity,$total_price,$discount,$tax,$net_amount,$payment_method,$gst,$customer_name,$customer_phone,$customer_email,$address1,$address2,$city,$state,$pincode,$lat,$lng,$map_location,$doctor_name,$bhc_no);
                    }
                    json_outputs($resp);
                }
            }
    ...
    public function upload_stock_csv()
    {
        $this->load->model('PharmacyPartnerModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
                ...
                if ($response['status'] == 200) {
                   // $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($this->input->post('user_id') == "" || $this->input->post('staff_user_id') == "") {
                        ...
                        $file1 = $this->input->post('files');
                        $file   = $_FILES['files']['tmp_name'];
                        $resp         = $this->PharmacyPartnerModel->upload_stock_csv($user_id,$staff_user_id,$file);
                    }
                    json_outputs($resp);
                }
            }
                ...
                if ($response['status'] == 200) {
                   // $list     = json_decode(file_get_contents('php://input'), TRUE);
                    ...
                    if ($user_id == "") {
                        ...
                        $resp = $this->PharmacyPartnerModel->monthly_report_v1($user_id,$hub_user_id);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($user_id == "") {
                        ...
                        $resp = $this->PharmacyPartnerModel->inventory_dashboard_v1($user_id,$hub_user_id,$from_date,$to_date);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function inventory_stock_report_list2()
    {
        $this->load->model('PharmacyPartnerModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['staff_user_id'] == "" || $params['filter']=="" || $params['page']=="") {
                        ...
                        //-----------------------------
                        $filter = $params['filter'];// 1.all 2.no_stock 3. surplus 4. lowstock 5. upcoming_expiry_stock-> 6.expired_stock 7.slow_moving 8. below_stock 9.product_group_wise 10.rack_wise 11. distributor_wise
                        $brand = $params['brand'];
                        $purchase_order = $params['purchase_order'];
                        $expiry_date_from = $params['expiry_date_from'];
                        $expiry_date_to = $params['expiry_date_to'];
                        ...
                        if($filter == "surplus" || $filter == "lowstock")
                        {
                            ...
                            {
                                $resp        = $this->PharmacyPartnerModel->inventory_stock_report_list_v2($page,$user_id,$staff_user_id,$find,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to,$warehouse_id,$pid_sort,$pname_sort,$day_count,$product_category,$rack_id,$distributor_id);
                            }
                        }
                        else
                        {
                              if($filter == "upcoming_expiry_stock" || $filter == "slow_moving")
                                {
                                    if(array_key_exists("day_count",$params)){
                  ...
                  if(array_key_exists('user_id',$params)){
                   ...
                   if(array_key_exists('invoice_no',$params)){
                    ...
                    if ($user_id == "" || $invoice_no == "") {
                        ...
                        $res = $this->PharmacyPartnerModel->estimate_cost_mno($user_id,$invoice_no,$bill="");
                        $resp = array(
                            'status' => 200,
                            'message' => 'success',
                            'data' =>   $res
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['invoice_no'] == "" || $params['order_id'] == "") {
                       ...
                       /// echo 'check';
                        $resp         = $this->PharmacyPartnerModel->generate_invoice($order_id,$invoice_no);
                    }
                    json_outputs($resp);
                }
            }
