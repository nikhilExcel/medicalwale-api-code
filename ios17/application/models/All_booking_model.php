...
$order_date1 = date('l j M Y h:i A', strtotime($row1['booking_date']));
        ...
        $vendor_detail = $this->db->query("SELECT lab_center.lab_name,lab_center.contact_no FROM `lab_center` INNER JOIN users on users.id=lab_center.user_id where lab_center.user_id='$vendor_id'")->row_array();
              ...
              if($vid=="39")
                    {
                      ...
                      $querys1 = $this->db->query("SELECT dentists_branch.name_of_hospital as clinic_name, dentists_branch.id as branch_id ,dentists_branch.dentists_branch_user_id as branch_dentist_id, dentists_branch.image, dentists_branch.phone as db_phone, dentists_branch.address as db_address, dentists_branch.pincode, dentists_branch.state as db_state, dentists_branch.city as db_city,booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id, booking_master.user_name, booking_master.user_mobile, booking_master.user_email, booking_master.user_gender, booking_master.branch_id,booking_master.status,booking_master.booking_type,booking_master.payment_mode, booking_master.vendor_id, booking_master.booking_date, booking_master.category_id, booking_master.trail_booking_date, booking_master.trail_booking_time,booking_master.trainer_package_id,booking_master.trainer_id, booking_master.joining_date,dentists_clinic_list.name_of_hospital,dentists_clinic_list.address,dentists_clinic_list.state,dentists_clinic_list.city,dentists_clinic_list.phone,dentists_clinic_list.pincode as pin,booking_master.branch_id,dentists_clinic_list.user_id as dentists_branch_c_id FROM booking_master left JOIN dentists_branch ON booking_master.branch_id=dentists_branch.id left join dentists_clinic_list on dentists_clinic_list.user_id=booking_master.listing_id  
