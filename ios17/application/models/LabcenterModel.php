...
{ 
    ...
    public function test_details($test_id,$vendor_id)
    {
        ...
        if ($count > 0) {
            ...
            $test_query = $this->db->query("select lab_test_master_details.user_id as vendor_id,users.name as vendor_name,lab_test_master_details.price,lab_test_master_details.discounted_price from lab_test_master_details INNER JOIN users on users.id=lab_test_master_details.user_id where lab_test_master_details.test_id='$test_id' and lab_test_master_details.user_id<>'$vendor_id'");
            $num_count  = $test_query->num_rows();
            if ($num_count > 0) {
    ...
    public function update_payment_method($booking_id,$ledger_id,$payment_type)
    {
        ...
        if($num_count>0 && $ledger_id>0){
            ...
            $transaction_date='';
            $res = $this->LedgerModel->create_ledger($user_id,$invoice_no,$ledger_owner_type, $listing_id, $listing_id_type, $credit, $debit, $payment_type, $user_comments, $mw_comments,$vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date);
        ...
        if($ledger_id=='0' && $payment_type!='3'){
            ...
            if($num_count>0){
                ...
                $transaction_date='';
                $res=$this->LedgerModel->create_ledger($user_id, $booking_id, $ledger_owner_type, $vendor_id, $listing_id_type, $credit, $debit, $payment_type, $user_comments, $mw_comments,$vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date);
				$ledger_id=$res['ledger_id'];
				$ledger_type=$res['type'];
            }
        }
    ...
    public function add_order($user_id,$vendor_id,$address_id,$vendor_type,$booking_date,$booking_time,$package_id,$test_id,$member_id,$payment_type,$coupon_id)
    {
        ...
        if(count($test_id>0) || count($package_id>0)){
            ...
            if($invoice_no>0 && $payment_type!='3'){
				...
				$trans_status='';
				$transaction_date='';
                $res=$this->LedgerModel->create_ledger($user_id, $invoice_no, $ledger_owner_type, $vendor_id, $listing_id_type, $credit, $debit, $payment_type, $user_comments, $mw_comments,$vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date);
				$ledger_id=$res['ledger_id'];
				$ledger_type=$res['type'];
            }
    ...
    public function showtestcart($user_id){
		...
		$packagequery = $this->db->query("SELECT pk.id,pk.code,pk.name,pk.description,pk.home_collection_charges,pk.price,pk.discounted_price,lab_center.lab_name as vendor_name,lab_vendor_cart.vendor_id from lab_vendor_cart INNER JOIN lab_package_master pk on pk.id=lab_vendor_cart.package_id INNER JOIN lab_center on lab_center.user_id=lab_vendor_cart.vendor_id where lab_vendor_cart.user_id='$user_id'");
    ...
    public function all_lab_test($page,$category,$most_common_risk,$sort_type,$home_delivery,$vendor_id,$keyword,$per_page){
    ...
    public function lab_search($user_id,$keyword,$page,$lat,$lng){
        ...
        $test_list    = $this->LabcenterModel->all_lab_test($page,$category,$most_common_risk,$sort_type,$home_delivery,$vendor_id,$keyword,$per_page);
