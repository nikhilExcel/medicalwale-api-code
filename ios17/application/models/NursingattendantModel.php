...
else
{
	...
	public function update_book_package_quick($user_id,$listing_id,$package_id,$bid_id)
	{
        $gallery_query = $this->db->query("SELECT listing_id FROM booking_master WHERE booking_id='$package_id'");
        $num_count_data = $gallery_query->num_rows();
        if($num_count_data > 0){
            $listing_id_data = $gallery_query->row()->listing_id; 
           
            $data = array('copy_listing_id'		=> $listing_id_data);
            
            $this->db->where('booking_id',$package_id);
            $this->db->update("booking_master", $data);
            
            $this->db->where('booking_id',$package_id);
            $this->db->update("nursing_booking_details", $data);
            
            //-----------------------------------------
            $data1 = array('listing_id'		=> $listing_id);
            
            $this->db->where('booking_id',$package_id);
            $this->db->update("booking_master", $data1);
            
            $this->db->where('booking_id',$package_id);
            $this->db->update("nursing_booking_details", $data1);
            
            //---------------------------------------------------
            
            $data2 = array('booking_status'		=> 1);
            
            $this->db->where('id',$bid_id);
            $this->db->update("booking_requirment_bids", $data2);
	...
	/*public function nursingattendant_book_package_quick($user_id,$name,$gender, $patiente_name, $patiente_age, $patiente_gender, $city, $patiente_condition, $attendent_time, $attendant_hour, $tentative_intime, $tentative_outtime, $nursing_gender, $attendant_needed, $appoinment_booking_date,$lat,$lng) {
        $lastid ="";
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        $booking_id           = date("YmdHis");
        ...
        // By ghanshyam parihar starts
        $radius = 5;
        $listing_id = [];
        $listing_name = [];
        $sql_ = sprintf("SELECT `id`, `user_id`,`name`, ( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM nursing_attendant WHERE is_active='1' HAVING distance < '%s' ORDER BY distance", ($lat), ($lng), ($lat), ($radius));
        $query_ = $this->db->query($sql_);
        $count_ = $query_->num_rows();
        if($count_>0){
                       foreach ($query_->result_array() as $mrow_) {
                        //   print_r($mrow_);
                        //   echo 'id'.$mrow_['user_id'];
                        //   echo 'name'.$mrow_['name'];
                               $listing_id[] = $mrow_['user_id'];
                               $listing_name[] = $mrow_['name'];
            }
        
            $listing_id =  implode(',', $listing_id);
            $listing_name = implode(',',$listing_name);
        }
        // By ghanshyam parihar ends
        ...
        //if ($booking_master_count == 1) {
            ...
            // else{
                 ...
                 $this->db->insert('nursing_booking_details', $nursing_booking_details);
                 if($lastid){
                     $lID = explode(',',$listing_id);
                     foreach($lID as $d){
                        $this->insert_notification_book_package($user_id, $lastid, 'user',$d);
                     }
                 }
            // }
        //}
    ...
    */
    	// Quick booking api by ghanshyam parihar Starts
public function nursingattendant_book_package_quick($user_id,$name,$gender, $patiente_name, $patiente_age, $patiente_gender, $city, $patiente_condition, $attendent_time, $attendant_hour, $tentative_intime, $tentative_outtime, $nursing_gender, $attendant_needed, $appoinment_booking_date,$lat,$lng) {
        $lastid ="";
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
    ...
    // Quick booking api by ghanshyam parihar Ends
    ...
    public function edit_review($review_id,$user_id, $listing_id, $rating, $review, $service) {
        ...
        $review_array = array(
           ...
           $this->db->update('nursing_attendant_review',$review_array);
