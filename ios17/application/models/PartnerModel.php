...
class PartnerModel extends CI_Model {
    ...
    public function signup($category, $name, $email, $city, $phone, $token, $agent) {
        if ($name != '' && $email != '' && $city != '' && $phone != '') {
            ...
            if ($count > 0) {
                ...
                if ($success) {
                    //$login_url = "http://pharmacy.medicalwale.com";
                    //$this->sendmail($email, $email, $pas, $login_url);
    ...
    public function login($phone, $token, $agent) {
        $query = $this->db->query("SELECT id,name,email,phone,city,vendor_id FROM users WHERE phone='$phone' and vendor_id>'0' and is_active=1");
        $count = $query->num_rows();
        if ($count > 0) {
            ...
            $multi_user = $this->db->query("SELECT id,name,email,phone,city,vendor_id,staff_hub_user_id FROM `users` WHERE  phone='$phone' AND vendor_id>'0'");
    ...
    public function order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data_list) {
        ...
        function send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $order_status, $updated_at, $order_id, $invoice_no, $name, $listing_name) {
        ...
        $customer_token = $this->db->select('token,token_status')->from('users')->where('id', $user_id)->get()->row();
    ...
    public function order_deliver_cancel($order_id, $cancel_reason, $type, $notification_type) {
        ...
        function send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $notification_type, $order_status, $updated_at, $order_id, $invoice_no, $name, $listing_name) {
            define("GOOGLE_GCM_URL", "https://fcm.googleapis.com/fcm/send");
            $fields = array(
                'to' => $reg_id,
                'priority' => "high",
        ...
        if ($token_status > 0) {
            ...
            send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $notification_type, $order_status, $updated_at, $order_id, $invoice_no, $name, $listing_name);
        }
    ...
    public function pharmacy_partner_profile_list($listing_id) {
        ...
        if ($count > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $free_start_time = str_replace("null", "", $row['free_start_time']);
                $free_end_time = str_replace("null", "", $row['free_end_time']);
            ...
            $data = array();
            return array("status" => 200, "message" => "success", "count" => 0, "data" => $data);
        }
        return $resultpost;
    }
        ...
        function send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $agent) {
            ...
            $headers = array(
                ...
                $agent === 'android' ? 'Authorization: key=AIzaSyCQLqGPoUzIKmh3qZHLFUHFnfzLTXsp2C4' : 'Authorization: key=AIzaSyCN1vVES0Jb3zVHeNCYPAMNnPYTl96H1uE'
                    ...
                    //'Authorization: key=AIzaSyBQDOIo8uwvF9ppdPK-mM-1bVGJKs70jk4' android users
