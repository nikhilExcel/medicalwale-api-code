...
class LedgerModel extends CI_Model {
    ...
    //   have to check booking_date and appointment_date - pending
        ...
        $sql_query = "SELECT uvl.transaction_of,uo.name as customer_name,uo.listing_name as vendor_name,bm.booking_date, dbm.booking_date as doctor_booking_date, hbm.booking_date as hospital_booking_date, pbm.appointment_date as pestcontrol_booking_date, uo.order_date , uvl.ledger_id,  uvl.user_id,  uvl.ledger_owner_type,  uvl.invoice_no,  uvl.transaction_id,  uvl.listing_id,  uvl.listing_id_type,  uvl.credit,  uvl.debit,  uvl.balance,  uvl.payment_method,  uvl.user_comments,  uvl.mw_comments,  uvl.vendor_comments,  uvl.verified,  uvl.trans_status,  uvl.order_type,  uvl.created_at,  uvl.created_by,  uvl.modified_at,  uvl.modified_by,  uvl.settled FROM `user_vendor_ledger` as uvl left join user_order as uo on ((uo.invoice_no = uvl.invoice_no OR uo.order_id = uvl.invoice_no) AND uvl.order_type = 1) left join booking_master as bm on (uvl.invoice_no = bm.booking_id AND uvl.order_type = 2) left join doctor_booking_master as dbm on (uvl.invoice_no = dbm.booking_id AND uvl.order_type = 3) left join hospital_booking_master as hbm on (uvl.invoice_no = hbm.booking_id AND uvl.order_type = 4) left join pestcontrol_booking_master as pbm on (uvl.invoice_no = pbm.booking_id AND uvl.order_type = 5) left join users as u on (uvl.user_id = u.id)  WHERE $whereConditionUser $searchWhereUser

        UNION 
        SELECT vvl.transaction_of,uo.name as customer_name,uo.listing_name as vendor_name,bm.booking_date, dbm.booking_date as doctor_booking_date, hbm.booking_date as hospital_booking_date, pbm.appointment_date as pestcontrol_booking_date, uo.order_date , vvl.ledger_id,  vvl.user_id,  vvl.ledger_owner_type,  vvl.invoice_no,  vvl.transaction_id,  vvl.listing_id,  vvl.listing_id_type,  vvl.credit,  vvl.debit,  vvl.balance,  vvl.payment_method,  vvl.user_comments,  vvl.mw_comments,  vvl.vendor_comments,  vvl.verified,  vvl.trans_status,  vvl.order_type,  vvl.created_at,  vvl.created_by,  vvl.modified_at,  vvl.modified_by,  vvl.settled FROM `vendor_vendor_ledger` as vvl left join user_order as uo on ( (uo.invoice_no = vvl.invoice_no OR uo.order_id = vvl.invoice_no)  AND vvl.order_type = 1) left join booking_master as bm on (vvl.invoice_no = bm.booking_id AND vvl.order_type = 2) left join doctor_booking_master as dbm on (vvl.invoice_no = dbm.booking_id AND vvl.order_type = 3) left join hospital_booking_master as hbm on (vvl.invoice_no = hbm.booking_id AND vvl.order_type = 4) left join pestcontrol_booking_master as pbm on (vvl.invoice_no = pbm.booking_id AND vvl.order_type = 5) left join users as u on (vvl.user_id = u.id)  WHERE $whereConditionVendor $searchWhereVendor
...
// $user_id : user_id ;  $invoice_no :  invoice_no or booking id ; $ledger_owner_type : user id type type ;  $listing_id : listing id ;  $listing_id_type : listing id vendor type ;  $credit : credit amount ;  $debit : debit amount;  $payment_method : payment_method ; $user_comments : user comments ;  $mw_comments : mw comments ;  $vendor_comments : vendor comments  ; $order_type : where 1 : service ; 2 : appointment ; 3 : doctor booking master ; 4 : hospital booking master ; 5 : pestcontrol booking master ,  transaction_of where 1: package; 2: order amount; 3: delivery charges; 4: ledger balance; 5 : points
    public function create_ledger($user_id, $invoice_no, $ledger_owner_type, $listing_id, $listing_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status)
    {
        ...
        if($user_id == "" || $invoice_no == "" ||   ($credit  == "" && $debit  == "" ) || ($credit  == 0 && $debit  == 0 )  ||  $order_type == "" || $order_type > 5 || $order_type < 1 || $transaction_of == "")
        {
