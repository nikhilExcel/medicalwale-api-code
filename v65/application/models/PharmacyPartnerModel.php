...
class PharmacyPartnerModel extends CI_Model
{
 ...
 public function elasticsearchsize3($index_id,$keyword,$listing_id){
            ...
            foreach ($query1->result_array() as $row1) {
                    ...
                    if ($product_count > 0) {
                        foreach ($product_query->result_array() as $product_row) {
                            ...
                            $final_order_total = $order_total + ($product_price * $product_quantity);
                            $disc=($product_price * $product_quantity)/100;
                                $order_total =$final_order_total-$product_discount;
                            $product_resultpost[] = array(
                                "order_id" => $order_id1,
                                "product_order_id" => $product_order_id,
                    ...
                    if ($product_count1 > 0) {
                        ...
                        if ($prescription_count > 0) {
                            foreach ($prescription_query->result_array() as $prescription_row) {
                                ...
                                $prescription_discount = $prescription_row['prescription_discount'];
                                $prescription_status   = $prescription_row['prescription_status'];
                                $final_order_total = $order_total + ($prescription_quantity * $prescription_quantity); 
                                $disc1=($prescription_quantity * $prescription_quantity)/100;
                                $order_total =$final_order_total-$prescription_discount;
                                $prescription_result[] = array(
                                    "prescription_name" => $prescription_name,
                                    "prescription_quantity" => $prescription_quantity,
    ...
    public function order_status_common($order_id_data, $prescription_id_data,$order_details,$prescription_details,$delivery_charges,$delivery_charges_by_customer,$sub_all_total,$discount,$gst)
    {
        $title = "";
       ...
       $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',listing_id='$listing_id',delivery_charge = '$delivery_charges',actual_cost='$sub_all_total',order_total='$sub_all_total',discount='$discount',gst='$gst' where order_id='$order_id_data'");
       ...
       $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',delivery_charge = '$delivery_charges',listing_id='$listing_id',actual_cost='$sub_all_total',order_total='$sub_all_total',discount='$discount',gst='$gst' where order_id='$prescription_id_data'");
    ...
    public function order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data_list,$delivery_charges,$delivery_charges_by_customer,$sub_all_total,$discount,$gst)
    {
         $title = "";
       ...
       $this->db->update('user_order',array('listing_name' => $medical_name));
        ...
        $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',delivery_charge = '$delivery_charges',listing_id='$listing_id',actual_cost='$sub_all_total',order_total='$sub_all_total',discount='$discount',gst='$gst' where order_id='$order_id'");
    ...
    {
        $query = $this->db->query("select * from user_pharmacy_detail where pharmacy_id='$listing_id' and status='0' order by name");
      ...
      public function delete_address($listing_id,$user_id)
    {
     ...
     public function prescription_status($order_id, $order_status, $prescription_order, $delivery_time,$listing_id,$delivery_charges,$delivery_charges_by_customer,$sub_all_total,$discount,$gst)
    {
        // echo $order_id; die();
        $this->load->model('OrderModel');
          $this->load->model('PartnermnoModel');
    ...
    public function order_deliver_cancel($order_id, $cancel_reason, $type, $notification_type)
    {
        ...
        if ($type == 'Order Delivered') {
            ...
            $invNoArray = $this->db->query("select `user_id`,`listing_type`,`listing_id`,`invoice_no` , `total_without_dc` , `listing_name` from `user_order` WHERE invoice_no = '$order_id'")->row_array();
           ...
           // add for each transaction
            ...
            $trans_status = 1; // default success
                ...
                $res = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $listing_id, $listing_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status);
  ...
  public function pharmacy_partner_profile_list($listing_id)
    ...
    public function update_pharmacy($mobile, $store_name, $store_manager_name, $store_since, $address_line1, $address_line2, $state, $city, $pincode, $listing_id)
    {
        ...
        $query = $this->db->query("SELECT id from users WHERE phone='$mobile' AND id <> '$listing_id' AND vendor_id='13'");
        $count = $query->num_rows();
...
//$stock =  $this->Inventory_master_m->get_warehouse_stock($q->id,$pack->id); 
    ...
    {
        $query = $this->db->query("SELECT * FROM inventory_stock WHERE (barcode='$barcode' or product_id='$barcode') AND user_id = '$user_id'");
        ...
        if ($count > 0) {
                ...
                if ($count1 > 0) {
                        $row1 = $query1->row_array();
            	       	...
            	       	$sql1="SELECT sum(stock_quantity) as stock_quantity FROM inventory_stock where user_id='$user_id'  AND (barcode='$barcode' or product_id='$barcode') AND warehouse_id='0'";
                        $stock=$this->db->query($sql1)->row();
                                if(!empty($stock))
        	       	            {
    ...
    public function product_barcode_scanner_v1($user_id,$hub_user_id,$barcode)
    {
        $query = $this->db->query("SELECT * FROM inventory_stock WHERE (barcode='$barcode' or product_id='$barcode') AND user_id = '$user_id'");
    ...
    public function stock_availability($user_id,$hub_user_id,$product_id,$quantity)
    {
        $query = $this->db->query("SELECT * FROM inventory_product WHERE id='$product_id' AND (user_id = '$user_id' OR user_id = '$hub_user_id')");
    ...
    {
        $query = $this->db->query("SELECT * FROM inventory_stock WHERE product_id='$product_id' AND user_id = '$user_id'");
    ...
    public function order_on_call($delivery_by,$service,$delivery_time,$delivery_charge,$listing_id,$listing_name,$staff_user_id,$product_details,$total_quantity,$total_price,$discount,$net_amount,$payment_method,$gst,$customer_name,$customer_phone,$customer_email,$address1,$address2,$city,$state,$pincode,$lat,$lng,$map_location,$address_id,$delivery_charges_by_customer)
    {
        ...
        $user_order = array(
            ...
            'delivery_time' => $delivery_time,
            'delivery_charge' => $delivery_charge,
            'delivery_charges_by_customer' => $delivery_charge,
            'order_status' => $order_status,
            'listing_id' => $staff_user_id,
            'listing_name' => $listing_name,
            'listing_type' => $listing_type,
            'user_id' => $user_id,
            'lat' => $lat,
        ...
        // render the view into HTML
                ...
                $pdfFilePath = "/home/h8so2sh3q97n/public_html/vendorsandbox.medicalwale.com/pharmacy.medicalwale.com/bill/".$output;
        	   ...
        	   $bill_url = "http://vendorsandbox.medicalwale.com/pharmacy.medicalwale.com/bill/".$output;                 
    	    }
    	    
             if($delivery_by == "mno")
        	   {
        	     
        	       $resultpost = $this->estimate_cost_mno($listing_id,$invoice_no,$bill_url);
        	        // call delivery charge api
                    $offer_id = 0;
    ...
    public function product_inventory_bill_offline($listing_id,$listing_name,$staff_user_id,$product_details,$total_quantity,$total_price,$discount,$tax,$net_amount,$payment_method,$gst,$customer_name,$customer_phone,$customer_email,$address1,$address2,$city,$state,$pincode,$lat,$lng,$map_location,$doctor_name,$bhc_no)
    {
        ...
        */
        ...
        // render the view into HTML
               ...
               // $pdf->Output($output, 'F');    
        	    ...
        	    $resultpost = array(
                                'bill_link' => "http://vendorsandbox.medicalwale.com/pharmacy.medicalwale.com/bill/".$output
                                ); 
    ...
    public function inventory_dashboard_v1($user_id,$hub_user_id,$from_date,$to_date)
    {
        ...
        $gross_amount1  = $this->db->query("select * from user_order where listing_id='$user_id' AND order_date between '$new_from_date' AND  '$new_to_date'");
    ...
    public function stock_inventory_dashboard_v1($user_id,$hub_user_id,$page,$date_from,$date_to)
    {
        ...
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
        	    ...
        	    $sql1="SELECT sum(stock_quantity) as stock_quantity FROM inventory_stock where user_id='$user_id'  AND product_id='$id' AND warehouse_id='0' $w ";
                $stock=$this->db->query($sql1)->row();
                                if(!empty($stock))
        	       	            {
    ...
    public function all_order_list2($user_id,$staff_user_id,$from_date,$to_date,$filter,$amount_from,$amount_to)
    {
                if($filter == "all")
                {   
                    $order_total=0;
                    ...
                    $user_orders = $this->db->query("select * from user_order where listing_id='$user_id' AND order_date>='$from_date' AND order_date<='$to_date' AND order_status='Order Delivered'");
                    ...
                    $bill_order = $this->db->query("select * from inventory_bill where user_id='$user_id' AND bill_date>='$from_date' AND bill_date<='$to_date'"); 
                ...
                else if($filter == "generic")
                {
                    ...
                    $sub_total_discount=0;
                    $sub_total_discount1=0;
                    $user_orders = $this->db->query("select * from user_order where listing_id='$user_id' AND order_date>='$from_date' AND order_date<='$to_date' AND order_status='Order Delivered' AND order_type='order'");
                    $user_orders_rows = $user_orders->num_rows(); 
                    if($user_orders_rows>0)
                    {
                        foreach ($user_orders->result_array() as $row) {
                ...
                else if($filter == "prescription")
                {
                    ...
                    $sub_total_discount=0;
                    $sub_total_discount1=0;
                    $user_orders = $this->db->query("select * from user_order where listing_id='$user_id' AND order_date>='$from_date' AND order_date<='$to_date' AND order_status='Order Delivered' AND order_type='prescription'");
                    $user_orders_rows = $user_orders->num_rows(); 
                    if($user_orders_rows>0)
                    {
                        foreach ($user_orders->result_array() as $row) {
                            ...
                            $order_total=$sub_total_sum2;
                            $order_total_discount=$sub_total_discount1;
                            if($order_total_discount=="")
                            {
                                $order_total_discount=0;
                            }
                ...
                else if($filter == "staff" || $filter == "deliver_by")
                {
                     $order_total=0;
                          $sub_total_sum2=0;
                          $sub_total_sum1=0;
                          $sub_total_discount=0;
                          $sub_total_discount1=0;
                   $bill_order = $this->db->query("select * from inventory_bill where user_id='$user_id' AND bill_date>='$from_date' AND bill_date<='$to_date' AND created_by='$staff_user_id'"); 
                    $bill_orders_rows = $bill_order->num_rows();
                ...
                else if($filter == "bill_amount" || $filter == "discount")
                {
                     $order_total=0;
                          $sub_total_sum2=0;
                          $sub_total_sum1=0;
                          $sub_total_discount=0;
                          $sub_total_discount1=0;
                          if($filter == "bill_amount") {
                             $bill_order = $this->db->query("select * from inventory_bill where user_id='$user_id' AND bill_date>='$from_date' AND bill_date<='$to_date' AND (final_amount >= '$amount_from' OR final_amount <= '$amount_to' )"); 
                          }
                           if($filter == "discount") {
                             $bill_order = $this->db->query("select * from inventory_bill where user_id='$user_id' AND bill_date>='$from_date' AND bill_date<='$to_date' AND (discount >= '$amount_from' OR discount <= '$amount_to' )"); 
                          }
                    $bill_orders_rows = $bill_order->num_rows();
    ...
    {
        $resultpost = array();
        $query = $this->db->query("SELECT * FROM product WHERE (created_by='$user_id' OR created_by='0') AND product_name LIKE '%" .$find."%' ");
        $count = $query->num_rows();
            if ($count > 0) {
                foreach ($query->result_array() as $row) {
                    $id                     = $row['id'];
    ...
    public function inventory_stock_report_list($page,$user_id,$staff_user_id,$find,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to,$warehouse_id)
    {
        $resultpost = array();
        $whr ="";
        if($filter =="all")
        {   if($find !="")
            ...
            $query = $this->db->query("SELECT * FROM inventory_product WHERE user_id='$user_id' $whr");
            $count = $query->num_rows();
            if ($count > 0) {
                foreach ($query->result_array() as $row) {
                    $id                     = $row['id'];
                    ...
                    foreach($warehouse as $q) {
                        if($q->id==0)
            	       	...
            	       	{
                	       	$sql1="SELECT sum(stock_quantity) as stock_quantity,expiry_date FROM inventory_stock where user_id='$user_id'  AND product_id='$id' AND warehouse_id='$w_id' $where";
                            $stock=$this->db->query($sql1)->row();
...
SELECT * FROM `other_notifications` where user_id = '$user_id' order by notification_date  desc  LIMIT $start, $limit");
  ...
  /*  public function product_inventory_bill_v1($user_id,$hub_user_id,$date,$invoice_no,$product_details,$discount,$payment_method,$customer_name,$customer_phone,$customer_email,$customer_address,$doctor_name,$bhc_no,$customer_lat,$customer_lng){
       
       $data = array();
       $distanceInKm = $drivingTime = 0;
       $completedKms = $DeliveryCharges = $drivingDistance = $delivery_time = $delivery_time_in_hrs_mins = "";
       $this->load->model('PartnermnoModel');
        date_default_timezone_set('Asia/Kolkata');
        $created_date = date('Y-m-d H:i:s');
        
        if($invoice_no == ""){
           $invoice_no = $created_date = date('YmdHis');
       }
       
        //Customer details
       $email= $name = "";
        if(!empty($bhc_no)) {
            // get customer details by bhc number
            $query = $this->db->query("SELECT * FROM user_priviladge_card_new WHERE card_no='$bhc_no'")->row();
            if(!empty($query)){
                $cust_user_id = $query->user_id;
                 ...
                 if(!empty($query)) {
                    ...
                    if(!empty($query1))  {
                        ...
                        $cust_data1 = array(
                    	    ...
                    	    $this->db->insert('users',$cust_data1);
                            $cust_user_id = $this->db->insert_id();  
                    }
                    $cust_data = array(
            	                'vendor_id'     => '13',
            ...
            //  if bhc num,ber is not given then search customer using BHC number
             $query = $this->db->query("SELECT id,cust_user_id,cemail,address,cname FROM inventory_customer WHERE cmobile='$customer_phone'")->row();
             if(!empty($query))  {
                $cust_id = $query->id;
                $address = $query->address;
        ...
        $total_quantity = 0;
        $net_amount = 0;
        $discountInRupees = $tax = 0;
        
        $totalQuantity = 0;
        
        $product_details_new = json_decode($product_details,TRUE);
        $ordered_products = $product_details_new['product'];
        foreach($ordered_products as $pro_details){
           
            $current_quantity = $pro_details['quantity'];
            $current_total_price = $pro_details['total_price'];
            
            
            $total_price = $total_price + $current_total_price;
        ...
        $pharmacyInfo = $this->db->query("SELECT * FROM `medical_stores` WHERE `user_id` = '$user_id' ")->row_array();
            if(sizeof($pharmacyInfo) > 0){
                $pharma_lat = $pharmacyInfo['lat'];
                $pharma_lng = $pharmacyInfo['lng'];
            } else {
                $pharma_lat = "";
                $pharma_lng = "";
            }
            ...
            if($pharma_lat != "" && $pharma_lng != ""){
                ...
                if($drivingDistance > 0){
                    ...
                    $chargesTable = $this->db->query("SELECT * FROM `mno_delivery_charges` WHERE `start_time` < '$current_time' AND `end_time` > '$current_time' AND `listing_id`  != '0' " )->result_array();
          ...
          // tax calculation default 18% => as discussed with vaibhav
            
            $tax = $total_price * 0.18;
            
          
           // dicount always in percent
            if($discount > 0){
                $discountInRupees =  $total_price * ($discount / 100);
            }
           
           
            if (strpos($payment_method, 'Cash') !== false) {
                $chc = $total_price * 0.02;
            }

            // die();
            // final amount
            $net_amount = $total_price + $tax + $chc - $discountInRupees  + $DeliveryCharges ;
            
            
            
            // get distance and charge - done - delivery_charge
            
            // dont know how pharmacy branches are handled in this module
            
            // get pharm,acy lat lng
            
            // delivery charge - done
        ...
        // render the view into HTML
               ...
               // $pdf->Output($output, 'F');    
                $data_add_pdf =array(
        	                     ...
        	                     $this->db->insert('inventory_customer_pdf',$data_add_pdf);
        	                     if($delivery_time_hrs != ""){
        	                         $estimated_delivery_time = $delivery_time_hrs;
        	                     } else if($delivery_time_mins != ""){
        	                          $estimated_delivery_time = $delivery_time_mins;
        	                     } else {
        	   ...
        	   if($delivery_by == "mno")
        	   {
        	       $bill = "http://vendorsandbox.medicalwale.com/pharmacy.medicalwale.com/bill/".$output;
        	       $resultpost = $this->estimate_cost_mno($user_id,$invoice_no,$bill);
        	   }
        	   else
        	   {
        	        $resultpost = array(
                                ...
                                'approx_distance_meter' => $drivingDistance,
                                'sub_total' => $total_price,
                                'gst' => $tax,
                                'cash_handling_charges' => $chc,
    ...
    public function estimate_cost_mno($user_id,$invoice_no,$bill){
        $this->load->model('PartnermnoModel');
        $this->load->model('PaymentModel');
        $delivery_time_in_hrs_mins = $delivery_time = $completedKms = $drivingDistance = $drivingTime = $estimated_delivery_time = $drivingDistance = $total_price = $tax = $chc = $discountInRupees = $DeliveryCharges = $net_amount =  0;
    ...
    public function responce_user_behalf($invoice_no, $order_status, $cancel_reason, $payment_method) {
        ...
        $date = date('Y-m-d');
        $updated_at = date('Y-m-d H:i:s');
        $order_type_query = $this->db->query("select order_type,user_id,listing_type,listing_name from user_order where invoice_no='$invoice_no' ");
   ...
   public function order_tracking_mno($user_id, $invoice_no){
    ...
    public function payment_accepted($user_id,$invoice_no, $payment_id, $amount){
        ...
        //   if amount is greater than 0 then go to next step else goto X
        // first check is any order exists
        // check payment is exists
        // create ledger : in user order as user debited given $amount
        ...
        if(sizeof($existingOrder) > 0){
            ...
            if($amount > 0){
                ...
                if($listing_id != "" && $listing_id != 0 && $listing_id != ""){
                    ...
                    $this->PharmacyPartnerModel->send_gcm_notify_pharmacy($title, $ph_token, $msg, $img_url, $tag, $ph_agent, $invoice_no,$notification_type);
                }
