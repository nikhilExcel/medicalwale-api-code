...
class LabcenterModel extends CI_Model
{
    ...
    public function update_order($booking_id,$ledger_id,$transaction_id,$payment_status,$ledger_type)
    {
        ...
        $this->db->where('invoice_no', $booking_id);
        $rst = $this->db->update('lab_test_order', $update_data);
        return array(
            'status' => 200,
            'message' => 'success'
        );
    ...
    public function add_order($user_id,$vendor_id,$address_id,$vendor_type,$booking_date,$booking_time,$package_id,$test_id,$member_id,$payment_type)
    {
        ...
        if(count($test_id>0)){
            ...
            $this->db->insert('lab_test_order', $lab_test_order);
            
            $test_id = rtrim($test_id, ',');
            $test_list = explode(',',$test_id);
            foreach ($test_list as $tid) {
                if($tid>0){
                    $test_query = $this->db->query("SELECT code,type,home_collection_charges,price,discounted_price,discount,medicalwale_discount,home_delivery from lab_test_master_details where test_id='$tid' and user_id='$vendor_id' limit 1");
                    ...
                    $this->db->insert('lab_test_order_details', $lab_test_order_details);
                }
            }
            $response[] = array(
