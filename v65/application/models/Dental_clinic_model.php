...
class Dental_clinic_model extends CI_Model {
    ...
    public function view_package($user_id,$listing_id,$package_id,$type)
    {
        ...
        if($type == 1)
        {
            ...
            if($count1>0)
            {
                ...
                $packages[] = array( 'package_id'             => $package_id,
                                                           'package_name'           => $package_name,
                                                            'package_details'       => $package_details,
                                                            'price'                 => $price,
                                                            'discount'              => $discount);
    ...
    public function nearby_dental_clinic_list($user_id,$lat,$lng,$page,$listing_id1,$type)
    {
               $radius = '5';
        $data = array();
        function GetDrivingDistance($lat1, $lat2, $long1, $long2) {
        ...
        if($type==1 || $type==3 || $type==4)
        {
            ...
            elseif($type==4)
            {
                ...
                $result_data= array();
                $sql = sprintf("SELECT *,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_clinic_list  WHERE $wh is_active='1' HAVING distance < '5' ORDER BY distance $limit", ($lat), ($lng), ($lat), ($radius));
                $query = $this->db->query($sql);
                $count = $query->num_rows();
        ...
        if($type==5)
        {
            $sql = "SELECT * FROM dentists_branch  WHERE hub_user_id='$listing_id1' $limit";
            $query = $this->db->query($sql);
            $count = $query->num_rows();
