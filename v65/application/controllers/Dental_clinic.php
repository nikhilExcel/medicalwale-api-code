            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['lat'] == "" || $params['lng'] == "") {
                        ...
                        $resp = $this->Dental_clinic_model->nearby_dental_clinic_list($user_id,$lat,$lng,$page,$listing_id1,$type);
                    }
                    json_outputs($resp);
                }
            }
                                ...
                                if ($img_size < (50000 * 50000)) {
                                    if (in_array($ext, $img_format)) {
                                        ...
                                        if ($s3->putObjectFile($img_tmp, $bucket, $actual_image_path, S3::ACL_PUBLIC_READ)) {
                                            $this->db->query("INSERT INTO `Dental_clinic_prescription`(`order_id`,`user_id`,`listing_id`, `image`,`status`, `date`) VALUES ('$order_id','$user_id','$listing_id', '$actual_image_name','$order_status', '$date')");
                                        }
                                    }
                                }
                            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['listing_id'] == "") {
                        ...
                        $resp = $this->Dental_clinic_model->dentist_time_slot($user_id);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['rating'] == "" || $params['review'] == "" || $params['service'] == "") {
                        ...
                        $branch_id = $params['branch_id'];
                        $resp = $this->Dental_clinic_model->add_review($user_id, $listing_id, $rating, $review, $service, $branch_id);
                    }
                    simple_json_output($resp);
                }
            }
