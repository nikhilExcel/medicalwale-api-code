...
class Elastic extends CI_Controller
{
 ...
 //  exit();
    ...
    {
    $this->elasticsearch->delete_index("dentist_clinic_list");
 ...
 //  exit();
    ...
    public function insert_pharmacy()
    {
     ...
     {
	     $this->elasticsearch->delete_index("lab");
        $labs_query = $this->db->query("SELECT id,user_id,profile_pic,lab_name,address1,latitude,longitude,recommended,certified,mba FROM lab_center WHERE `is_active` = 1  ");
            ...
            if ($labs_count > 0) {
                foreach ($labs_query->result_array() as $labs_row) {
                    ...
                    //print_r($labs);
                      $this->elasticsearch->add("lab","labs", $id, $labs);
                }
            }
     ...
     {
	   $this->elasticsearch->delete_index("pharmacy_order_list");
    ...
    public function insert_hmproduct(){
	 ...
	 public function insert_lab_test(){
        
		 
		 $this->elasticsearch->delete_index("hm_product");
         $lab_query = $this->db->query("SELECT lab_test_master.id as test_id,lab_test_master.name,lab_test_master.aliasname,lab_test_master.description,lab_test_master.instructions,lab_test_master.components,lab_test_master.fasting FROM `lab_test_master` INNER JOIN lab_test_master_details on lab_test_master_details.test_id=lab_test_master.id where lab_test_master.name<>'' group by lab_test_master_details.test_id");
                    $lab_count = $lab_query->num_rows();
                    if ($lab_count > 0) {
                        foreach ($lab_query->result_array() as $lab_row) {
				
				print_R($lab_row);
				
                            $pd_id = $lab_row['test_id'];
                          
                           $data[] =$this->elasticsearch->add("lab_test","lab_test", $pd_id, $lab_row);
	 ...
	 public function insert_lab_package(){
        
		 
		 $this->elasticsearch->delete_index("lab_package");
         $lab_query = $this->db->query("SELECT lab_package_master.*,lab_center.lab_name as vendor_name from lab_package_master INNER JOIN lab_center on lab_center.user_id=lab_package_master.user_id where lab_package_master.user_id>0");
                    $lab_count = $lab_query->num_rows();
                    if ($lab_count > 0) {
                        foreach ($lab_query->result_array() as $lab_row) {
                            $pd_id = $lab_row['id'];
                          
                           $data[] =$this->elasticsearch->add("lab_package","lab_package", $pd_id, $lab_row);
    ...
    public function insert_substitute(){
	   ...
	   public function insert_jobs(){
         $this->elasticsearch->delete_index("jobs");
         $sub_query = $this->db->query(" SELECT * from jobs_description");
                    $sub_count = $sub_query->num_rows();
                    if ($sub_count > 0) {
                        foreach ($sub_query->result_array() as $sub_row) {
                            $id = $sub_row['job_id'];
				 $user_id = $sub_row['user_id'];
                            $job_position = $sub_row['job_position'];
                           $state= $sub_row['state'];
                           $city= $sub_row['city'];
                           $company_name= $sub_row['company_name'];
			$job_desc=$sub_row['job_desc'];
			$employment_type=$sub_row['employment_type'];
				...
				'employment_type'=>$employment_type
                           ...
                           // print_r($sub_product);
                           $data[] =$this->elasticsearch->add("jobs","jobs", $id, $sub_product);
