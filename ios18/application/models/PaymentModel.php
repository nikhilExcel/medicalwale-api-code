...
class PaymentModel extends CI_Model {
    ...
    //   echo 'dsfnhdsfh'.$payment_type;
        ...
        if($payment_type != '3'){
            ...
            if($type=='5'){
            				...
            				$transaction_id='';
            				$trans_status='';
                            $res=$this->LedgerModel->create_ledger($user_id, $booking_id, $ledger_owner_type, $listing_id, $listing_id_type, $credit, $debit, $payment_type, $user_comments, $mw_comments,$vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status);
    ...
    public function get_ledgerBal_Points($user_id){
        ...
        //points
        $query1 = $this->db->query("SELECT SUM(points) as total_points FROM user_points WHERE user_id='$user_id' and status='active' order by id DESC");
 ...
 public function convert_user_points_to_amount($user_id){
    ...
    public function add_delivery_charges($delivery_charges, $invoice_no, $delivery_charges_by_customer, $offer_id){
        ...
        if(sizeof($order_details) > 0){
            ...
            if($listing_type == 13 || $listing_type == 44){
                ...
                $this->db->query("UPDATE user_order SET `delivery_charge` = '$delivery_charges' , `delivery_charges_by_customer` = '$delivery_charges_by_customer', `delivery_charges_by_vendor` = '$delivery_charges_by_vendor' , `delivery_charges_by_mw` = '$delivery_charges_by_mw' , `delivery_charges_to_mno` = '$delivery_charges_to_mno' WHERE invoice_no = '$invoice_no' ");
    ...
    public function get_invoice_costing($invoice_no){
        $cost  = $data = array();
        $order_found = 0;
        $user_order = $this->db->query("SELECT * FROM user_order WHERE invoice_no = '$invoice_no' Group by invoice_no")->row_array();
        if(sizeof($user_order) > 0){
           $order_found = 1; 
            ...
            $grand_total_for_vendor = $amount_with_discount + $gst_in_rupees + $chc - $delivery_charges_by_vendor;
       ...
       $invoice_no = "";
        ...
        if($trans_status == 1){
            ...
            if($amount > 0){
                ...
                $res = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $mw_id, $mw_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date);
             //   print_r($res); die();
            }
