           ...
           /*    $query = $this->db->query("SELECT invoice_no,discount,reward_amount,cash_cheque_reward.status,used_amount FROM `user_order` join cash_cheque_reward on user_order.invoice_no=cash_cheque_reward.txn_reward where user_order.user_id='$user_id'");
    	    ...
    	    else
    	    {
    	      return  $resultpost; 
    	       ...
    	       // $query = $this->db->query("SELECT invoice_no,discount,reward_amount,cash_cheque_reward.status,used_amount FROM `user_order` join cash_cheque_reward on user_order.invoice_no=cash_cheque_reward.txn_reward where user_order.user_id='$user_id'");
                $query = $this->db->query("SELECT invoice_no,order_date,listing_name,discount,reward_amount,cash_cheque_reward.status,used_amount FROM `user_order` left join cash_cheque_reward on user_order.invoice_no=cash_cheque_reward.txn_reward where user_order.user_id='$user_id'"); 
                if($query->num_rows()>0)
                {
                $resultpost=array();
                $adddis="";
