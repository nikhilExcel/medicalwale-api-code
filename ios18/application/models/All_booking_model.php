           ...
           $query2 = $this->db->query("select * from rides where user_id = '$user_id' and ride_id='$invoice_no' ORDER BY ride_id DESC");
            ...
            if ($count2 > 0) {
                ...
                $resultpost234 = array();
                    ...
                    $booking_id = $row1['booking_id'];
                    $listing_id = $row1['branch_dentist_id'];
                    $branch_id = $row1['branch_id'];
                        $branch_name = $row1['name_of_hospital'];
                    $branch_image = $row1['image'];
                    $branch_phone = $row1['phone'];
                    $branch_address = $row1['address'];
                    $branch_pincode = $row1['pincode'];
                    $branch_city = $row1['city'];
                    $branch_state = $row1['state'];
                    $appointment_user_name = $row1['user_name'];
...
usort($resultpost, "cmp");   
           ...
           $query2 = $this->db->query("select * from rides where user_id = '$user_id'  ORDER BY ride_id DESC LIMIT $start, $limit");
            ...
            if ($count2 > 0) {
             ...
             if($vid=="39")
                    {
                        $user_order_query = "SELECT dentists_branch.name_of_hospital, dentists_branch.id as branch_id ,dentists_branch.dentists_branch_user_id as branch_dentist_id, dentists_branch.image, dentists_branch.phone, dentists_branch.address, dentists_branch.pincode, dentists_branch.state, dentists_branch.city,booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id, booking_master.user_name, booking_master.user_mobile, booking_master.user_email, booking_master.user_gender, booking_master.branch_id,booking_master.status, booking_master.vendor_id, booking_master.booking_date, booking_master.category_id, booking_master.trail_booking_date, booking_master.trail_booking_time,booking_master.trainer_package_id,booking_master.trainer_id, booking_master.joining_date FROM booking_master left JOIN dentists_branch ON booking_master.listing_id=dentists_branch.hub_user_id
                                                 WHERE booking_master.user_id='$user_id' AND booking_master.vendor_id='39'  ORDER BY booking_master.id DESC LIMIT $start, $limit"; 
                        // die(); 
              ...
              if($vid=="39")
                    {
            			...
            			$lat = $lng = "";
                         ...
                         $sql_query = "SELECT dentists_clinic_list.lat as clinic_lat,dentists_clinic_list.lng as clinic_lng, dentists_branch.lat as dentists_branch_lat,dentists_branch.lng as dentists_branch_lng, payment_method.payment_method as payment_method_name,booking_master.sub_booking_type,dentists_branch.name_of_hospital as clinic_name, dentists_branch.id as branch_id
,dentists_branch.dentists_branch_user_id as branch_dentist_id, dentists_branch.image, dentists_branch.phone as db_phone,
dentists_branch.address as db_address, dentists_branch.pincode, dentists_branch.state as db_state, dentists_branch.city
as db_city,booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id,
...
booking_master.joining_date,dentists_clinic_list.name_of_hospital,dentists_clinic_list.address,dentists_clinic_list.state,dentists_clinic_list.city,dentists_clinic_list.phone,dentists_clinic_list.pincode
as pin,booking_master.branch_id,dentists_clinic_list.user_id as dentists_branch_c_id 
FROM booking_master left JOIN
dentists_branch ON booking_master.listing_id=dentists_branch.hub_user_id left join dentists_clinic_list on
dentists_clinic_list.user_id=booking_master.listing_id left join payment_method on (booking_master.payment_mode = payment_method.id) 
WHERE booking_master.user_id='$user_id' AND booking_master.vendor_id='39' and booking_master.booking_id='$booking_id' group by booking_master.booking_id  ORDER BY booking_master.id DESC";
                 ...
                 //  WHERE booking_master.user_id='$user_id' AND booking_master.vendor_id='39' and booking_master.booking_id='$booking_id' ORDER BY booking_master.id DESC";
                    ...
                    $branch_id = $row1['branch_id'];
                    if($branch_id=="0"){
                        $listing_id = $row1['dentists_branch_c_id'];
                        $branch_name = $row1['name_of_hospital'];
                        $branch_image = $row1['image'];
                        $branch_phone = $row1['phone'];
                        $branch_address = $row1['address'];
                        $branch_pincode = $row1['pin'];
                        $branch_city = $row1['city'];
                        $branch_state = $row1['state'];   
                        $lat = $row1['clinic_lat'];
                        $lng = $row1['clinic_lng'];
                        ...
                        $listing_id = $row1['branch_dentist_id'];  
                        $branch_name = $row1['clinic_name'];
                        $branch_image = $row1['image'];
                        $branch_phone = $row1['db_phone'];
                        $branch_address = $row1['db_address'];
                        $branch_pincode = $row1['pincode'];
                        $branch_city = $row1['db_city'];
                        $branch_state = $row1['db_state'];
                        $lat = $row1['dentists_branch_lat'];
                        $lng = $row1['dentists_branch_lng'];
                    }
