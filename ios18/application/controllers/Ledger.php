        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
               ...
               if($user_id == "" || $invoice_no == "" ||   ($credit  == "" && $debit  == "" )  ||  $order_type == "" || $order_type > 9 || $order_type < 1 || $transaction_of == ""){
                    ...
                    $res = $this->LedgerModel->create_ledger($user_id, $invoice_no, $ledger_owner_type, $listing_id, $listing_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date);
                ...
                if ($response['status'] == 200) 
                {
                   $params = json_decode(file_get_contents('php://input'), TRUE);
                    ...
                    else
                    {
                        ...
                        $resp = $this->LedgerModel->ledger_select($user_id);
                    }
                    json_outputs($resp);
                }
            }
    ...
    public function get_points() { 
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if(array_key_exists('listing_id',$params ) && $params['user_id'] != "" ){
                     $listing_id = $params['listing_id'];
                } else {
                     $listing_id = 0;
                }
               ...
               if($user_id == "" ||    $listing_id < 0   )  {
            
                    $resp = array(
                        'status' => 400,
                        'message' => 'Please enter user_id, listing_id'
                    );
                    
                } else {
                    $res = $this->LedgerModel->get_points($user_id, $listing_id);
                    $message = $res['message'];
                    if($res['insert_id'] > 0){
