...
//        $allOrders = $this->db->query("SELECT mo.id as mno_order_id,mo.updated_at as last_updated,uo.order_date, uo.listing_name,uo.listing_type, mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE mo.`mno_id` = '$mno_id' $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) ORDER BY uo.invoice_no desc $limit")->result_array();
        $allOrders = $this->db->query("SELECT mo.id as mno_order_id,mo.updated_at as last_updated,uo.order_date, uo.listing_name,uo.listing_type, mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE mo.`mno_id` = '$mno_id' AND mo.order_type = '0'  $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) ORDER BY mo.created_at desc $limit")->result_array();
        $allOrders_count = $this->db->query("SELECT mo.id FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE mo.`mno_id` = '$mno_id'  AND mo.order_type = '0' AND (uo.listing_type = 44 OR uo.listing_type = 13) $typeWHere")->result_array();
    ...
    public function order_accept_reject($mno_id,$invoice_no, $accepted, $cancel_reason_id,$mno_lat,$mno_lng){
     ...
     public function assign_mno($user_id,$invoice_no,$lat,$lng){
        ...
        $getPincode = $this->db->query("select listing_type,pincode from user_order where invoice_no = '$invoice_no'")->row_array();
            ...
            // else {
                ...
                if(!empty($busyMnos) &&  sizeof($busyMnos) > 0 && $mno_id == ''){
                    ...
                    if($orderInsertedButNotAssiged == 0){
                        $insertInOrder = $this->db->query("INSERT INTO `mno_orders`(`invoice_no`, `mno_id`, `ongoing`, `created_at`, `updated_at`,`vendor_type`) VALUES ('$invoice_no','$mno_id','1','$order_date','$order_date','$vendor_type')");
...
//   get_ongoing_order
   ...
   //     return $data;
    ...
    public function assign_mno_v1($user_id,$invoice_no,$lat,$lng,$order_vendor_type){
        ...
        /*52 : distributor to night owl(inven_user_order_invoice); 13: Pharmacy to night owl(user_order); 44: user to night owl(user_order)*/
        if($order_vendor_type == 52){
            $getPincodes = $this->db->query("SELECT * FROM `inven_user_order_invoice` WHERE `invoice_no` IN ($invoice_no)  AND listing_type = '52'")->result_array();
