             ...
             foreach ($query1->result_array() as $row1) 
                     {
                      ...
                      if($count>0)
                      {
                          foreach($query->result_array() as $row)
                          {
                            ...
                            $vendor_status = $this->db->select('name,avatar_id')->from('users')->where('id', $event_user_id)->get()->row();
                            if(!empty($vendor_status)) {
                            $user_name = $vendor_status->name;
                            $avatar_id=$vendor_status->avatar_id;    
                            }
                            else
                            {
                                $user_name = "";
                              ...
                              if(empty($vendor_name))
                              {
                                ...
                                $query_img = $this->db->query("SELECT * FROM media WHERE  id='$avatar_id'  order by id desc");
                         $count_img = $query_img->num_rows();
                         if($count_img > 0)
                           {
                             $media=$query_img->row();
                         $img_file = $media->source;
                           $image_pro = 'https://d2c8oti4is0ms3.cloudfront.net/images/healthwall_avatar/' . $img_file;
                          }
                          else
                             {
                                $image_pro='https://d2c8oti4is0ms3.cloudfront.net/images/img/default_user.jpg';
                             }

                              $result_array[] = array(
                                      'id' => $id,
                                      'user_image'=>$image_pro,
                                      'image' => $image,
