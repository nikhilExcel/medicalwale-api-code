...
class OfwModel extends CI_Model
{
        ...
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
               ...
               $img_file_b=$row['big_image'];
                if ($img_file != '') {
                    $image = 'https://medicalwale.s3.amazonaws.com/images/healthwall_media/Gossip/' . $img_file;
                } else {
                    $image = '';
                }
                 if ($img_file_b != '') {
        ...
        "ad_colour_code"=> ""
		     ...
		     if(count($resultpost) % 5 == 0 && count($resultpost)> 0)  {
                    if($page>1){
                        ...
                        $this11 = new \stdClass();
                       $url="https://dhlr-bot.medicalwale.com/ads/asksaheli/";
  ...
  public function menstural_cycle_notification_on_off($user_id,$profile_id,$is_notification)
    ...
    public function get_menstrual_cycle_data_calendar_wise($user_id, $profile_id)
    {
        ...
        $count = $query->num_rows();
        if ($count > 0) {
            $query_temp = $this->db->query("SELECT id,user_id,profile_id,name,pcod,start_periode_data,period_start_from,cycle_length,tracking_reson,total_periode_day,born_year,tutual_cycle,tutual_cycle,relationship from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' order by id desc limit 1");
