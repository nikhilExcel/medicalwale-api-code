...
class NotificationcontrolModel extends CI_Model {
    ...
    public function Notification_read_update_v1($user_id,$noti_id,$type)
    {
         $updated_at = date('Y-m-d H:i:s');
          if($type=="promo"){
         $query = $this->db->query("UPDATE other_notifications SET is_read='1' WHERE id = '$noti_id'");
          }
        if($this->db->affected_rows()>0)
        {
                 return array(
...
UNION ALL
SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) order by notification_date  desc LIMIT 0, 10");
        $count = $query->num_rows();
        if ($count > 0) {
     ...
     public function Notification_All_list_v1($user_id,$page,$type)
    {
      ...
      //$radius = $page*5;
        ...
        if($type=="promo"){
           // echo "dinehs";
        $query = $this->db->query("SELECT * FROM `other_notifications` order by notification_date desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
            ...
            if($type="trans"){
                ...
                $query = $this->db->query("SELECT * FROM `trans_notification` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order') order by id desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
            
            $query1 = $this->db->query("SELECT * FROM `trans_notification` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order') ");
            $count1 = $query1->num_rows();
