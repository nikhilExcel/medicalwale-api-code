...
class PharmacyPartnerModel extends CI_Model
{
    ...
    public function order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data_list,$delivery_charges,$delivery_charges_by_customer,$sub_all_total,$discount,$gst,$order_deliver_by)
    {
       ...
       $this->db->update('user_order',array('listing_name' => $medical_name));
        ...
        $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',delivery_charge = '$delivery_charges',listing_id='$listing_id',actual_cost='$sub_all_total',order_total='$sub_all_total',discount='$total_discount',gst='$gst' where order_id='$order_id'");
        ...
        if(!empty($order_id))
    	{
    	    ...
    	    if(count($ex)==1) {
    	        ...
    	        $sql="SELECT * FROM medical_stores WHERE user_id = '".$medical_id."' || pharmacy_branch_user_id = '".$medical_id."'";
                    ...
                    if($order_type == "order" || $order_type == "")
                    {
                        $bill_detail1 = $this->db->select('l.*,u.gst')->from('user_order_product as l')->join('user_order as u','u.order_id=l.order_id','left')->where('l.order_id',$order_id)->where('l.product_status','Available')->get()->result();
                        // echo $this->db->last_query();
                    }
                    else
                    {
                        $bill_detail = $this->db->select('l.*,u.gst')->from('prescription_order_list as l')->join('user_order as u','u.order_id=l.order_id','left')->where('l.order_id',$order_id)->where('l.prescription_status','Available')->get()->result();
                        //echo $this->db->last_query();
                    }
