<?php
   ...
   function send_gcm_notify($title, $msg, $reg_id, $img_url, $tag, $agent, $type) {
    ...
    $dictionary_sql = "SELECT *,DATEDIFF(CURRENT_DATE(),booking_to) differ FROM diet_user_package_history WHERE booking_to <= CURRENT_DATE() AND complete_status=0 ";
    ...
    if($count_data>0)
    {
        ...
        while ($list = mysqli_fetch_array($dictionary_res)) {
            ...
            $differ  = $list['differ']; 
            if($differ == 5 || $differ == 4 || $differ == 3 || $differ == 2 || $differ == 1)
            {
                $user_id = $list['user_id']; 
              
                $userql = "SELECT token,agent,name FROM users where id='$user_id' limit 1";
                ...
                $msg ="";
                 if($differ==1)
                {
                     $msg ='Dear '.$name.', 1 day left for the expiry of your Miss Belly Diet Package, please renew it..';
                }
                  if($differ==2)
                {
                     $msg ='Dear '.$name.', 2 days left for the expiry of your Miss Belly Diet Package, please renew it..';
                }
                  if($differ==3)
                {
                     $msg ='Dear '.$name.', 3 days left for the expiry of your Miss Belly Diet Package, please renew it..';
                }
                  if($differ==4)
                {
                     $msg ='Dear '.$name.', 4 days left for the expiry of your Miss Belly Diet Package, please renew it..';
                }
                  if($differ==5)
                {
                     $msg ='Dear '.$name.', 5 days left for the expiry of your Miss Belly Diet Package, please renew it..';
                }
                ...
                if($reg_id == '')
                {
                    ...
                    $title = 'Miss Belly Package Reminder';
                    $type= 'Missbelly_Package_reminder';
                    $packageid = $list['id'];
                    $booking_id = $list['booking_id'];
                    
                    $s = "INSERT INTO diet_plan_notifications (user_id, package_id, booking_id, title, msg, notification_type, created_at)VALUES ('$user_id', '$packageid', '$packageid', '$title', '$msg', '$type', '$current_date')";
