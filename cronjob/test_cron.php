...
if($word != '') {
	...
	$title1   = 'Word of the Day';
        $msg1     = $word .':-'. ' ' . $meaning . '';        
        $date11   = date('Y-m-d');
        $img_url1 = 'https://medicalwale.s3.amazonaws.com/images/Icons+for+Apps/Word-of-the-day.jpg';
        
        $tag1 = 'text';        
        $type1 = 'HealthDictionary'; 
	
 $s1 = "INSERT INTO other_notifications (user_id, package_name, booking_id, title, msg, notification_type, img_url,tag,order_date,notification_date) VALUES ('0', '', '', '$title1', '$msg1', '$type1','$img_url1','$tag1','$date11','$date11')";

$r1 = mysqli_query($hconnection, $s1);  


	
$sql        = "SELECT id as user_id,agent,token FROM users WHERE vendor_id='0' and token != '' and id='28488' ";
$res        = mysqli_query($hconnection, $sql);
$count_data = mysqli_num_rows($res);
$reg_id_android = array();
if ($count_data > 0) {
    $list = mysqli_fetch_array($res);
       echo  $user_id = $list['user_id'];
        $agent   = $list['agent'];
        ...
        $msg     = $word .':-'. ' ' . $meaning . '';        
        $date1   = date('Y-m-d');
        $img_url = 'https://medicalwale.s3.amazonaws.com/images/Icons+for+Apps/Word-of-the-day.jpg';
