<?php
   ...
   function send_gcm_notify($title, $msg, $reg_id, $img_url, $tag, $agent, $type) {
    ...
    $dictionary_sql = "SELECT * FROM medicalbot_dietleadsbybot";
    ...
    if($count_data>0)
    {
        ...
        while ($list = mysqli_fetch_array($dictionary_res)) {
                ...
                $dictionary1 = "SELECT * FROM diet_user_package_history WHERE user_id = '$user_id' AND complete_status = 0 ";
                $dictionarys = mysqli_query($hconnection, $dictionary1);
                $counts = mysqli_num_rows($dictionarys);
                ...
                if($counts == 0)
                {
                    ...
                    $msg ='Dear '.$name.', for more effective diet plan please buy diet package';
                    $msg ='Dear '.$name.', Want to get fit? Upgrade to Miss Belly diet packages today. Buy now!';
                    ...
                    if($reg_id == '')
                    {
                       /* $img_url = 'https://medicalwale.com/img/noti_icon.png';
                        $tag = 'text';
                        $agent ='android';
                        $title = 'Missbelly Package Reminder';
                    ...
                    else
                    {
                        ...
                        $title = 'Buy Miss Belly Package';
                        $type= 'Buy_Missbelly_Package';
   ...
   VALUES ('$user_id', '', '', '$title', '$msg', '$type','$img_url','$tag','$date1','$date1')";
