...
$date=date('Y-m-d');
  $dictionary_sql  = "SELECT Health_Quote,id FROM Health_quote_dictionary WHERE date='$date' limit 1";
$dictionary_res  = mysqli_query($hconnection, $dictionary_sql);
$dictionary_list = mysqli_fetch_array($dictionary_res);
 $word1            = $dictionary_list['Health_Quote'];
 $id              = $dictionary_list['id'];

$word=str_replace('<br>', '', $word1);



if($word != '') {

	$title1   = 'Quote of the Week';
        $msg1     = $word;        
        $date11   = date('Y-m-d');
        $img_url1 = 'https://medicalwale.s3.amazonaws.com/images/Icons+for+Apps/Quote-of-the-week.jpg';
        
        $tag1 = 'text';        
        $type1 = 'quoteweek'; 
	$s2 = "INSERT INTO other_notifications (user_id, package_name, booking_id, title, msg, notification_type, img_url,tag,order_date,notification_date) VALUES ('0', '', '', '$title1', '$msg1', '$type1','$img_url1','$tag1','$date11','$date11')";
        $r2 = mysqli_query($hconnection, $s2); 
	
 $s1 = "UPDATE `Health_quote_dictionary` SET `status`='0' WHERE `id`='$id'";
...
if ($count_data > 0) {
    while ($list = mysqli_fetch_array($res)) {
        ...
        $msg     = $word ;        
        $date1   = date('Y-m-d');
        $img_url = 'https://medicalwale.s3.amazonaws.com/images/Icons+for+Apps/Quote-of-the-week.jpg';
