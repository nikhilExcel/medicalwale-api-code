...
if (isset($_POST['post_id'])) {
    ...
    $roots_herbs_reviewQuery = mysqli_query($conn, "SELECT id,comment,user_id,date FROM `child_physiotherapy_review_comment` WHERE post_id='$post_id' order by id desc");
    ...
    if ($roots_herbs_review_count > 0) {
        while ($row = mysqli_fetch_array($roots_herbs_reviewQuery)) {
            ...
            $count_query = mysqli_query($conn, "SELECT id FROM `child_physiotherapy_review_comment_like` where comment_id='$id'");
            $like_count = mysqli_num_rows($count_query);

            $like_count_query = mysqli_query($conn, "SELECT id FROM `child_physiotherapy_review_comment_like` where user_id='$user_id' and comment_id='$id'");
