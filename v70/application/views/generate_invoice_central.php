        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan='2' style="vertical-align:top;" >
                    <?php if($logo_url != ''){ ?>
                        <img  src="<?php echo $logo_url; ?>" width="80px" height="110px"  style="margin-top:-25px">
                    <?php  } ?>
                </td>
                <td colspan='6' style="vertical-align:top;text-align:center" >
                    <p style="font-size:16px;text-align:center;text-transform: uppercase;">
            ...
            <tr>
                ...
                <td colspan="2" style="vertical-align:top;padding: 1px 10px;"><?php if($bill->auto_invoice_no==NULL ) { echo $bill->invoice_no; } else { echo $bill->auto_invoice_no; } ?> </td>
                
            </tr>
            <!--address-->
            <?php 
		 
$ok1 = preg_match("/(\d\d\d\d\d\d)/", $bill->address1, $matches);
    if (!$ok1) {
        $abc1 = 0;
    }else{
     $abc1 =  $matches[count($matches) - 1];
    }
    if($abc1 > 0){
        
         $add1in =  str_replace($abc1," ",$bill->address1);
	       $add1 =  str_replace($bill->city," ",$add1in);
    }else{
      $add1 =    $bill->address1;
    }
    
    $ok2 = preg_match("/(\d\d\d\d\d\d)/", $bill->address2, $matches);
    if (!$ok2) {
        $abc2 = 0;
    }else{
     $abc2 =  $matches[count($matches) - 1];
    }
    if($abc2 > 0){
        
         $add2in =  str_replace($abc2," ",$bill->address2);
	     $add2 =  str_replace($bill->city," ",$add2in);
    }else{
      $add2 =    $bill->address2;
    }
    
             
            if($bill->address1 == $bill->address2){
               if($bill->address1 != ""){$address .= $add1 ;}
                if($address != ""){ $address .= ', ';}
...
// 				}
            ...
            <tr>
                ...
                ?>
                <td style="padding: 1px 10px;color: #333333;font-weight: 900;vertical-align:top;">Delivery</td>
		   ...
		   $selling_price_discount = $selling_price - (($dat->prescription_discount/100)*($selling_price));
			
                   
                   $sell_price_sum += $selling_price;
	           $sell_price_sum_discount += $selling_price_discount;
                   $total = $selling_price;
	           $total_discount = $sell_price_sum_discount;
                   $tot_sum += $total;
                   $tot_sum_discount += $total_discount;
                    /*for gst*/
                    $gst_per = $dat->gst;
                    if($gst_per > 0){
                        $sgst = $cgst = $gst_per / 2;
                        $gst_rup = $selling_price_discount - ($selling_price_discount / ( 1+ (0.01 * $gst_per))) ;
...
<!--                     <td style="font-size:10px;padding: 2px 5px;color: #555;text-align: center;vertical-align:top;"><?//php echo $dat->mgf_date;?></td> -->
		   ...
		   $selling_price1_discount = $selling_price1 - (($dat->product_discount/100)*($selling_price1));
                   $sell_price_sum_discount += $selling_price1_discount;
                   $total1_discount = $selling_price1_discount;
                   $tot_sum_sum += $total1_discount;
                    ...
                    if($gst_per > 0){
                        $sgst = $cgst = $gst_per / 2;
                        $gst_rup = $selling_price1_discount - ($selling_price1_discount / ( 1+ (0.01 * $gst_per))) ;
                        $sgst_amount =  $cgst_amount = $gst_rup / 2;
                        $sgst_amount_total = $sgst_amount_total + $sgst_amount; 
                        $cgst_amount_total = $cgst_amount_total + $cgst_amount; 
                    } else {
...
<!--                     <td style="font-size:10px;padding: 2px 5px;color: #555;text-align: left;vertical-align:top;"><?//php echo $dat->mgf_date; ?></td> -->
            ...
            <tr style="background:#d0e6d5">
			   ...
			   <?php if($detail->gst_no != ""){ ?>
                        <p style="font-size:8px">
                            GST No.: <?php echo $detail->gst_no; ?>
                        </p>
                    <?php } ?>
                    
                         <?php if($detail->CIN != ""){ ?>
                        <p style="font-size:8px">
                            CIN No.: <?php echo $detail->CIN; ?>
                        </p>
                    <?php } ?>
...
<div style="width:10%;float:left;">
    <table>
        <tbody>
            <tr>
                <td style="text-align:right;text-rotate:-90;vertical-align:middle;"><p style="font-size:7px;">Conceptualised, Designed, Developed & Managed By Medicalwale.com | +918695886958</p></td>
            </tr>
        </tbody>
    </table>
</div>    
