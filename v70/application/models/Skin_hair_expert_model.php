...
class Skin_hair_expert_model extends CI_Model {
   ...
   public function add_comment($data,$comment1,$user_id){
    ...
    public function skin_hair_add_question($type, $user_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous) 
    {
       ...
       /* $field_de = array(
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                if($params["$variables"] == "--Select--")
                {
                    $vvv = "";
                }
                else
                {
                    $vvv = $params["$variables"];
                }
                $resultpost1 = array(
                    'type' => $type,
                    'field_id' => $id,
     ...
     public function skin_hair_add_question_v1($type, $user_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous) 
    {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
       ...
       /* $field_de = array(
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                if($this->input->post("$variables") == "--Select--")
                {
                    $vvv = "";
                }
                else
                if($variables == 'image')
                {
                    if (isset($_FILES["image"]) && !empty($_FILES["image"]["name"])) {
                        ...
                        $targetPath = "images/healthwall_media/image/".$fiel_name;
                        $s3->putObjectFile($sourcePath, $bucket, $targetPath, S3::ACL_PUBLIC_READ);
                        $vvv='https://medicalwale.s3.amazonaws.com/'.$targetPath;
                        ...
                        $date =  date('Y-m-d H:i:s');                    
                        $media  = array('title' => $fiel_name,'type'=>"image",'source'=>$fiel_name,'created_at'=>date('Y-m-d H:i:s'));
    ...
    public function skin_hair_edit_question_v1($type, $user_id,$post_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous) 
    {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        ...
        if ($query->num_rows() > 0) {
			...
			$this->db->select('media_id');
                        ...
                        $this->db->where('post_id',$post_id);
                        $del_med = $this->db->get()->row() ;
                        if(!empty($del_med))
                        {
                            $medi_id = $del_med->media_id;
...
//echo "SELECT skin_hair_expert_question.id,skin_hair_expert_question.age,skin_hair_expert_question.weight,skin_hair_expert_question.diet_preference,skin_hair_expert_question.height,skin_hair_expert_question.user_image,skin_hair_expert_question.user_name,skin_hair_expert_question.user_id,skin_hair_expert_question.question,skin_hair_expert_question.date,IFNULL(skin_hair_expert_question.post_location,'') AS post_location,user_character.image AS c_image  FROM  `skin_hair_expert_question` INNER JOIN `user_character` ON skin_hair_expert_question.user_image=user_character.id  WHERE skin_hair_expert_question.id NOT IN (SELECT post_id FROM miss_belly_hide WHERE post_id=skin_hair_expert_question.id AND user_id='$user_id')  order by skin_hair_expert_question.id DESC limit $start, $limit";
    ...
    {
        $query = $this->db->query("SELECT gender FROM `users` where id = $user_id");
            if ($query->num_rows() > 0)
            {
                foreach ($query->result_array() as $row) {
                        $query = $this->db->query("SELECT * FROM `askexpert_details` where is_active = 1 order by level");
                    $gender = $row['gender'];
                    if($gender == 'Male'){
                        $gender = 1;
                        // $query = $this->db->query("SELECT * FROM `askexpert_details` where is_active = 1 && for_gender = '2' order by level");
                    }
                    else{
                        $gender = 2;
                    }
