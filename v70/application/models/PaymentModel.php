...
class PaymentModel extends CI_Model {
 ...
 public function convert_user_points_to_amount_old($user_id){
    ...
    public function get_order_costing_draft($invoice_no){  //get_order_costing_draft replica Hitesh
        $cost  = $data = array();
        $order_found = 0;
        $user_order = $this->db->query("SELECT pm.icon,uo.* FROM user_order_draft as uo left join payment_method as pm on (pm.payment_method = uo.payment_method) WHERE uo.invoice_no  = '$invoice_no' Group by uo.invoice_no ")->row_array();
        if(sizeof($user_order) > 0){
           $order_found = 1; 
