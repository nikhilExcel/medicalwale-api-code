...
class PharmacyPartnerModel extends CI_Model
{
   ...
   /*  public function __construct()
    ...
    public function order_list_draft($status, $listing_id, $listing_type, $order_type, $page,$keyword, $date_from, $date_to,$customer_invoice,$branch_id,$order_deliver_by,$order_status,$payment_mode,$order_created_by)
    {
        ...
        $whereListing = "";
        if($listing_type == 0){
            $whereListing = " AND (user_order_draft.listing_type='13' or user_order_draft.listing_type='44')";
        } else {
            $whereListing = " AND (user_order_draft.listing_type='13' or user_order_draft.listing_type='44')";
        }
        ...
        if($keyword!="")
        {
            $whereListing .=" AND (user_order_draft.mno_bill_no LIKE '%$keyword%' OR user_order_draft.invoice_no LIKE '%$keyword%' OR user_order_draft.order_type LIKE '%$keyword%' OR user_order_draft.order_status LIKE '%$keyword%' 
                                          OR user_order_draft.name LIKE '%$keyword%' OR user_order_draft.mobile LIKE '%$keyword%' OR user_order_draft.actual_cost LIKE '%$keyword%') ";
        }
        ...
        if(sizeof($customer_invoice) > 0){
            ...
            $whereListing .= " AND user_order_draft.invoice_no IN  ($selected_payment_modes) "; 
        }        
         ...
         if($date_from!="" && $date_to!="")
                {
                    //$whereListing .=" AND (order_date >= '$date_from' AND order_date <= '$date_to') ";BETWEEN 
                    if($date_from != $date_to) {
                    $whereListing .=" AND (user_order_draft.order_date BETWEEN '$date_from' AND '$date_to') "; }
                    else if($date_from == $date_to){
                    $whereListing .=" AND user_order_draft.order_date LIKE '%$date_from%' ";
                    }
                }        
         if($status != "")
         {
             $whereListing .= " AND user_order_draft.order_status='$status' ";
         }
        ...
        if(sizeof($order_created_by) > 0){
            ...
            $whereListing .= " AND user_order_draft.created_by IN  ($selected_created_by) ";
        }
        ...
        if(sizeof($order_status) > 0){
            ...
            $whereListing .= " AND user_order_draft.order_status IN ($order_statuses) ";
        ...
        if(sizeof($order_deliver_by) > 0){
            ...
            $whereListing .= " AND user_order_draft.order_deliver_by IN ($deliver_by) ";
        }
       ...
       /* echo "select user_id,order_id,order_type,action_by,delivery_time,cancel_reason,action_by,listing_id,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status from user_order where FIND_IN_SET('" . $listing_id . "',listing_id) and listing_type='$listing_type' group by invoice_no order by order_id desc";*/
        //$query = $this->db->query("select * from user_order where FIND_IN_SET('" . $listing_id . "',listing_id) $whereListing group by invoice_no order by order_id desc $l ");
      //echo "select * from user_order where CONCAT(',', user_order.listing_id, ',') REGEXP CONCAT(',(', REPLACE('$uid', ',', '|'), '),') $whereListing group by invoice_no order by order_id desc $l ";
       $query = $this->db->query("select user_order_draft.*,uvl.payment_method from user_order_draft as user_order_draft left join user_vendor_ledger as uvl on (uvl.invoice_no = user_order_draft.invoice_no AND uvl.transaction_of = 2 AND uvl.trans_status = 1) where CONCAT(',', user_order_draft.listing_id, ',') REGEXP CONCAT(',(', REPLACE('$listing_id', ',', '|'), '),') $whereListing AND user_order_draft.draft='0' group by user_order_draft.invoice_no order by user_order_draft.order_id desc $l ");
        $count = $query->num_rows();
      ...
      if ($count > 0) { 
            foreach ($query->result_array() as $row) {
                ...
                $query1 = $this->db->query("select user_id,order_id,order_type,action_by,delivery_time,cancel_reason,action_by,listing_id,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status from user_order_draft where invoice_no='$invoice_no'");
        $count1 = $query1->num_rows();
    //   print_r($count1); die;
      if ($count1 > 0) {
            foreach ($query1->result_array() as $row1) {
                    $order_id1            = $row1['order_id'];
                    // $product_query = $this->db->query("select id as product_order_id,product_id,product_discount,product_name,product_unit,product_unit_value,product_img,product_quantity,product_price,sub_total,product_status,product_status_type,product_status_value,order_status,stock_id,barcode,batch_no,gst from user_order_product where order_id='$order_id1' order by product_order_id asc");
                    $product_query = $this->db->query("select id as product_order_id,product_id,product_discount,product_name,product_unit,product_unit_value,product_img,product_quantity,product_price,sub_total,product_status,product_status_type,product_status_value,order_status,stock_id,barcode,batch_no,gst from user_order_product_draft where order_id='$order_id1' order by product_order_id asc");
                    $product_count = $product_query->num_rows();
                    if ($product_count > 0) {
                        foreach ($product_query->result_array() as $product_row) {
    ...
    /*  print_r($prescription_resultpost2);
           if(!empty($product_resultpost1) && !empty($prescription_resultpost2))
            ...
            $this->load->model('PaymentModel');
            // $data1 = $this->PaymentModel->get_invoice_costing($invoice_no);
            $data1 = $this->PaymentModel->get_order_costing_draft($invoice_no);
            $data2[] = $data1['data'];
    ...
    public function edit_general_order_draft($order_id, $listing_id, $listing_type, $order_data_list,$delivery_charges,$delivery_charges_by_customer,$sub_all_total,$discount,$gst)
    {
        ...
        // listing_type
        $userOrderDetails = $this->db->query("SELECT * FROM `user_order_draft` WHERE `order_id` = '$order_id' ")->row_array();
        $invoice_no = $userOrderDetails['invoice_no'];
        if($userOrderDetails['listing_type'] == 44 ){
             //   calculate delivery time and cost
        ...
        $this->db->where('order_id',$order_id);
        $this->db->delete('user_order_product_draft');
        ...
        foreach ($order_data_list as $result) {
            ...
            //$query = $this->db->query("UPDATE user_order_product SET product_quantity='$product_quantity',product_price='$product_price',product_discount='$product_discount',sub_total='$sub_total',order_status='$product_order_status',updated_at='$updated_at',product_unit='$product_unit',product_unit_value='$product_unit_value' where product_id='$product_order_id' and order_id='$order_id'");
            $this->db->insert('user_order_product_draft', $arr);
        ...
        $query = $this->db->query("UPDATE user_order_draft SET delivery_charge = '$delivery_charges',listing_id='$listing_id',actual_cost='$grand_total',order_total='$grand_total',discount='$total_discount',gst='$gst' where order_id='$order_id'");
        /*
        $invNoArray = $this->db->query("select `invoice_no`, `listing_name` from `user_order` WHERE order_id = '$order_id'")->row_array();
        $invno = $invNoArray['invoice_no'];
        $listing_name = $invNoArray['listing_name'];
...
For more services regarding Health | Beauty | Fitness download Medicalwale Saathi App from Google Play or Apple App Store $applink and stay healthy";*/
   ...
   public function order_tracking_mno($user_id, $invoice_no){
    ...
    public function product_list($user_id,$page_no,$per_page,$search,$from_date,$to_date)
    {
        ...
        if($check_table_existance['status'] == true){
            ...
            if($from_date != "" && $to_date != ""){
                  if($from_date != $to_date) {
                    $search_where .=" AND (st.created_at BETWEEN '$from_date' AND '$to_date') "; }
                    else if($from_date == $to_date){
                    $search_where .=" AND st.created_at LIKE '%$from_date%' ";
                    }
            }
...
For more services regarding Health | Beauty | Fitness download Medicalwale Saathi App from Google Play or Apple App Store $applink and stay healthy";*/
     ...
     public function order_on_call_draft($delivery_by,$service,$delivery_time,$delivery_charge,$listing_id,$listing_name,$staff_user_id,$product_details,$total_quantity,$total_price,$discount,$net_amount,$payment_method,$gst,$customer_name,$customer_phone,$customer_phone1,$customer_phone2,$customer_email,$address1,$address2,$city,$state,$pincode,$lat,$lng,$map_location,$address_id,$delivery_charges_by_customer,$doctor_name,$landmark,$mno_bill_no)
    {
        ...
        // $this->db->insert('user_order', $user_order);
        $this->db->insert('user_order_draft', $user_order);
        $order_id = $this->db->insert_id(); 
        ...
        if (isset($_FILES["image"]) && !empty($_FILES["image"]["name"]) && $order_id != "")
        {
            ...
            //echo "UPDATE user_order SET bill_url='$final' WHERE order_id='$order_id'";
            // $this->db->query("UPDATE user_order SET bill_url='$final' WHERE order_id='$order_id'");
            $this->db->query("UPDATE user_order_draft SET bill_url='$final' WHERE order_id='$order_id'");
        }
            	        ...
            	        foreach($product_details_new['product'] as $details)
            	        {
            	            ...
            	            else
            	            {
            	                ...
            	                $product_gst = array_key_exists('gst',$details) ? $details['gst'] : 0;
            	                   ...
            	                   // $this->db->insert('user_order_product', $product_order);   
            	                    $this->db->insert('user_order_product_draft', $product_order);   
            	                    $p_insert_id = $this->db->insert_id();
                                    		  ...
                                    		  //  $query1 = $this->db->query("select user_id,order_id,order_type,action_by,delivery_time,cancel_reason,action_by,listing_id,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status from user_order where invoice_no='$invoice_no'");
                                                ...
                                                if ($count1 > 0) {
                                                    foreach ($query1->result_array() as $row1) {
                                                        $order_id1            = $row1['order_id'];
                                                        // $product_query = $this->db->query("select product_quantity,stock_id,barcode,batch_no from user_order_product where order_id='$order_id1'");
                                                        $product_query = $this->db->query("select product_quantity,stock_id,barcode,batch_no from user_order_product_draft where order_id='$order_id1'");
                                                        $product_count = $product_query->num_rows();
                                                        if ($product_count > 0) {
                                                            foreach ($product_query->result_array() as $product_row) {
                                                                $stock_id           = $product_row['stock_id'];
        ...
        if(!empty($order_id))
    	{
            ...
            $this->load->model('PartnermnoModel');
            // $orderDetails = $this->db->query("SELECT uo.*, ms.lat as pharma_lat, ms.lng as pharma_lng FROM user_order as uo left join medical_stores as ms on (uo.listing_id = ms.user_id) WHERE uo.invoice_no = '$invoice_no'  GROUP by ms.user_id")->row_array();
            $orderDetails = $this->db->query("SELECT uo.*, ms.lat as pharma_lat, ms.lng as pharma_lng FROM user_order_draft as uo left join medical_stores as ms on (uo.listing_id = ms.user_id) WHERE uo.invoice_no = '$invoice_no'  GROUP by ms.user_id")->row_array();
