...
//send notification through firebase
  ...
  public function send_gcm_notify_doctor($title, $reg_id, $msg, $img_url, $tag, $agent, $post_id)
    {
    ...
    public function healthwall_category() {
        $query = $this->db->query("SELECT  `id`, `category`,`hindi_healthwall_categorys`, `image`,`big_image` from healthwall_category order by id=31 asc, CASE WHEN LEFT(category, 1) LIKE '[a-Z]' THEN 1 ELSE 2 END ,LEFT(category, 1)");
        ...
        foreach ($query->result_array() as $row) {
            ...
            $img_file = $row['image'];
            $img_file_b=$row['big_image'];
            if ($img_file != '') {
                $s_image = 'https://d2c8oti4is0ms3.cloudfront.net/images/healthwall_media/category/' . $img_file;
            } else {
                $s_image = '';
            }
            if ($img_file_b != '') {
                $b_image = 'https://d2c8oti4is0ms3.cloudfront.net/images/healthwall_media/category/' . $img_file_b;
            } else {
                $b_image = '';
            }
    ...
    //get healthwall's post list
     public function post_list($user_id, $activity_user_id, $healthwall_category, $page) {
        ...
        if ($activity_user_id > 0) {
            $query = $this->db->query("select healthwall_category.category AS healthwall_category,healthwall_category.id as healthwall_category_id, posts.id as post_id,posts.question_type, posts.type as post_type,IFNULL(posts.repost_user_id,'') AS repost_user_id,posts.repost_location,posts.is_repost,posts.repost_time, posts.caption,posts.description as post,posts.is_anonymous,IFNULL(posts.post_location,'') AS post_location,posts.tag,posts.created_at,users.name,posts.user_id as post_user_id,users.vendor_id ,posts.article_title,posts.article_image,posts.article_domain_name,posts.article_url,IFNULL(posts.category,'') AS post_category from posts INNER JOIN users on users.id=posts.user_id LEFT JOIN healthwall_category ON posts.healthwall_category=healthwall_category.id where posts.user_id<>'' and posts.user_id<>'0' and ((posts.user_id='$activity_user_id' and is_repost='0') or posts.repost_user_id='$activity_user_id') AND posts.healthwall_category<>'0' AND  posts.id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=posts.id AND user_id='$user_id') and posts.active='1' and posts.description != 'uhYke2Bh3m/xvqE3VpjGZQ==' and posts.is_anonymous !='1'  order by posts.id DESC limit $start, $limit");
            $count_query = $this->db->query("select id from posts where user_id<>'' and user_id<>'0' and ((user_id='$activity_user_id' and is_repost='0') or repost_user_id='$activity_user_id') AND healthwall_category<>'0' AND  id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=id AND user_id='$user_id') and active='1' and description != 'uhYke2Bh3m/xvqE3VpjGZQ==' and is_anonymous !='1'  order by id DESC ");
            $count_post = $count_query->num_rows();
        } else {
            if ($healthwall_category == '0') {
                $query = $this->db->query("select healthwall_category.category AS healthwall_category,healthwall_category.id as healthwall_category_id, posts.id as post_id,posts.question_type , IFNULL(posts.repost_user_id,'') AS repost_user_id,posts.repost_location,posts.is_repost,posts.repost_time, posts.type as post_type,IFNULL(posts.post_location,'') AS post_location,posts.caption,posts.description as post,posts.is_anonymous,posts.tag,posts.created_at,users.name,posts.user_id as post_user_id,users.vendor_id ,posts.article_title,posts.article_image,posts.article_domain_name,posts.article_url,IFNULL(posts.category,'') AS post_category from posts INNER JOIN users on users.id=posts.user_id LEFT JOIN healthwall_category ON posts.healthwall_category=healthwall_category.id where posts.user_id<>'' and posts.user_id<>'0' AND posts.healthwall_category<>'0' AND  posts.id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=posts.id AND user_id='$user_id') and posts.active='1' and posts.description != 'uhYke2Bh3m/xvqE3VpjGZQ=='  order by posts.id DESC limit $start, $limit");
                $count_query = $this->db->query("select id from posts where user_id<>'' and user_id<>'0' AND healthwall_category<>'0' AND id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=id AND user_id='$user_id') and active='1' and description != 'uhYke2Bh3m/xvqE3VpjGZQ==' order by id DESC");
                $count_post = $count_query->num_rows();
            } else {
                $query = $this->db->query("select healthwall_category.category AS healthwall_category,posts.id as post_id, IFNULL(posts.repost_user_id,'') AS repost_user_id,posts.repost_location,posts.is_repost,posts.repost_time,posts.question_type, posts.type as post_type,healthwall_category.id as healthwall_category_id,IFNULL(posts.post_location,'') AS post_location,posts.caption,posts.description as post,posts.is_anonymous,posts.tag,posts.created_at,users.name,posts.user_id as post_user_id,users.vendor_id ,posts.article_title,posts.article_image,posts.article_domain_name,posts.article_url,IFNULL(posts.category,'') AS post_category from posts INNER JOIN users on users.id=posts.user_id LEFT JOIN healthwall_category ON posts.healthwall_category=healthwall_category.id where posts.user_id<>'' and posts.user_id<>'0' AND posts.healthwall_category<>'0' AND  posts.healthwall_category IN ($healthwall_category) AND  posts.id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=posts.id AND user_id='$user_id') and posts.active='1' and posts.description != 'uhYke2Bh3m/xvqE3VpjGZQ=='  order by posts.id DESC limit $start, $limit");
                $count_query = $this->db->query("select id from posts where user_id<>'' and user_id<>'0' AND healthwall_category<>'0' AND healthwall_category IN ($healthwall_category) AND id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=id AND user_id='$user_id') and active='1' and description != 'uhYke2Bh3m/xvqE3VpjGZQ==' order by id DESC");
                $count_post = $count_query->num_rows();
            }
        }
            ...
            foreach ($query->result_array() as $row) {
              ...
              //  if ($post != '') {
                ...
                $is_post_save = $get_list['is_post_save'];
                    ...
                    $total_query1 = $this->db->query("SELECT count(id) as comment_count FROM skin_hair_answer where post_id='$post_id' LIMIT 1");

                    $get_list1 = $total_query1->row_array();
                    if(!empty($get_list1)) {
                    $comment_count += $get_list1['comment_count']; }
                ...
                $resultpost_new_type=array();
                $resultpost_type = array();
                ...
                if ($repost_vlcc__counts > 0) {
                    ...
                    $etype = $row_vlcc['question_type'];
                    
                    $query_type = $this->db->query("SELECT * FROM `expert_fields` WHERE type='$etype' AND status='1'");
                            ...
                            if ($query_type->num_rows() > 0) {
                                ...
                                foreach ($query_type->result_array() as $row) {
                                    ...
                                    $details_answer = "";
                                      $query_is_vlcc_details = $this->db->query("SELECT * FROM `expert_fields_details`  where post_id='$post_id' AND variables='$variables' AND type='$etype'");
                                      $repost_vlcc__counts_details = $query_is_vlcc_details->num_rows();
                                      if($repost_vlcc__counts_details >0) {  
                                        $newarray=$query_is_vlcc_details->row_array();
                                        $details_answer = $newarray['details'];
                                      }
                     
                                     
                                    $query1 = $this->db->query("SELECT * FROM `expert_fields_dropdown` WHERE type='$etype' AND variable='$variables' AND status='1'");
                                    ...
                                    $resultpost_type[] = array(
                                        ...
                                        'answer' => $details_answer,
                                        'dropdown_fields' =>   $resultpost_new_type              
                ...
                if(count($resultpost) % 5 == 0 && count($resultpost)> 0)  {
                   ...
                   /*$this11 = new \stdClass();
                   $url="http://10.0.1.198:8011/ads/random/ads/";
                   ...
                   $this11 = new \stdClass();
                   //$url="https://dhlr-bot.medicalwale.com/ads/random/ads/";
                   $url="http://dhlr-bot.medicalwale.com/ads/covid/ads/healthwall/";
                   $this11->user_id=$user_id;
                   $data11 = json_encode($this11);
                  ...
                  /* else
                   {
                     ...
                     $query_is_user_details = $this->db->query("SELECT * FROM `users`  where id='$user_id' ");
                     $user_counts_details = $query_is_user_details->num_rows();
                     $new_users=$query_is_user_details->row_array();
                     $gend=$new_users['gender'];
                     $camp_pk_ads=0;
                     if($gend=="Female")
                     {
                         $camp_pk_ads = "48, 42, 44, 45, 43, 49, 64, 65";
                     }
                     elseif($gend=="Male")
                     {
                         $camp_pk_ads = "47,50,51,43,49,65";
                     }
                     else
                     {
                       $camp_pk_ads = "42, 43, 44, 45, 47, 48, 49, 50, 51, 64, 65";
                     }
                    
                       $new_id=explode(",",$camp_pk_ads);
                 ...
                 $ad_img="";
                        ...
                        {
                          $color="#F06D24"; 
                         $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/VLCC(final).png";
                        }
                        elseif($v=="43")
                        {
                          $color="#696B6D";
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/VISSCO(final).png";
                        }
                        elseif($v=="44")
                        {
                          $color="#e155a1"; 
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/SERONA(final).png";
                        }
                        elseif($v=="45")
                        {
                          $color="#808080";
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/BELLA(final).png";
                        }
                        elseif($v=="47")
                        {
                          $color="#666666";
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/BEARD(final).png";
                        }
                        elseif($v=="48")
                        {
                          $color="#ea0b8c"; 
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/EVERTEEN(final).png";
                        }
                        elseif($v=="49")
                        {
                          $color="#346e21"; 
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/VAIDAY(final).png";
                        }
                        elseif($v=="50")
                        {
                          $color="#000000"; 
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/BRAHMA(final).png";
                        
                        }
                        elseif($v=="51")
                        {
                          $color="#1CBECA"; 
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/BOMBAY(final).png";
                        }
                        elseif($v=="64")
                        {
                          $color="#006768"; 
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/HIMALAYA_1.png";
                        }
                        elseif($v=="65")
                        {
                          $color="#313A2E";  
                          $ad_image="https://medicalwale.s3.amazonaws.com/images/Sponsored_files/AAROGYAM_WELLNESS.png";
                        }
                        
                        
                    $query = $this->db->query("SELECT * FROM sponsored_advertisements where camp_id='$v' order by rand()");
                    $count = $query->num_rows();
                    if ($count > 0) {
                        $row  = $query->row_array();
                        //$ad_img      = $row['ad_image']; 
                        $ad_main_cat = $row['main_cat_id'];
                       //$ad_main_cat="10658";
                        $query_product = $this->db->query("SELECT * FROM `product_details_hm` where pd_id ='$ad_main_cat' ");
                        $query_product_details = $query_product->num_rows();
                        if($query_product_details > 0 )
                        {
                        $new_product=$query_product->row_array();
                        ...
                        $resultpost[] = array(
                            ...
                            'vendor_type'    => 34,
                            'ad_type'        => $row['ad_type'],
                            'ad_cat_id'      => (int)$row['main_cat_id'],
                            'ad_title'       => $row['ad_title'],
                            'ad_description' => $row['ad_description'],
                            'ad_link'  => $row['ad_link'],
                            'ad_image' => $image,
                            'post_type'=>'ad',
                            'post_user_id' => (int)$row['user_id'],
                            'listing_type' => '0',
                            'post_location' => '',
                            ...
                            'repost' => array(),
                            'repost_flag' => '0',
                            "new_ad_pd_id" => $row['main_cat_id'],
                            "new_ad_brand_name" => $new_product['brand_name'],
                            "new_ad_pd_added_v_id"=> $new_product['pd_added_v_id'],
                            "new_ad_pd_name" => $new_product['pd_name'],
                            "new_ad_pd_photo_1"=> $new_product['pd_photo_1'],
                            "new_ad_pd_mrp_price"=> intval(round($new_product['pd_mrp_price'])),
                            "new_ad_pd_vendor_price"=> intval(round($new_product['pd_vendor_price'])),
                            "new_ad_background_image"=> $ad_image,
                            "ad_colour_code"=> $color
                        );
                        }
    ...
    public function single_post_list($user_id, $post_id) {
        ...
        if ($count_post > 0) {
            foreach ($query->result_array() as $row) {
                ...
                if($comment_count=='0')
                {
                    $comment_count = $this->db->select('id')->from('skin_hair_answer')->where('post_id', $post_id)->get()->num_rows();
                }
               
                    $query_comment = $this->db->query("SELECT comments.id,comments.post_id,comments.answer as comment,comments.date as date,comments.doctor_id as post_user_id FROM skin_hair_answer as comments  WHERE comments.post_id='$post_id' order by comments.id desc LIMIT 0,3");
                    $comments = array();
                    $comment_counts = $query_comment->num_rows();
                    if ($comment_counts > 0) {
                        foreach ($query_comment->result_array() as $rows) {
 ...
 public function single_post_list1($user_id, $post_id) {
        $query = $this->db->query("select  IFNULL(posts.post_location,'') AS post_location,IFNULL(posts.repost_user_id,'') AS repost_user_id,posts.repost_location,posts.is_repost,posts.repost_time,healthwall_category.category AS healthwall_category, posts.id as post_id,posts.type as post_type,posts.caption,posts.description as post,posts.is_anonymous,posts.tag,posts.created_at,users.name,posts.user_id as post_user_id,users.vendor_id,posts.article_title,posts.article_image,posts.article_domain_name,posts.article_url from posts INNER JOIN users on users.id=posts.user_id  LEFT JOIN healthwall_category ON posts.healthwall_category=healthwall_category.id where posts.user_id<>'' and posts.user_id<>'0' AND posts.healthwall_category<>'0' AND posts.id='$post_id' order by posts.id DESC ");
        $count_post = $query->num_rows();
        $result_post = array();
        if ($count_post > 0) {
            foreach ($query->result_array() as $row) {
             ...
             if(empty($comments))
                {
              //echo "SELECT comments.id,comments.post_id,comments.answer as comment,comments.date as date,comments.doctor_id as post_user_id FROM skin_hair_answer as comments WHERE comments.post_id='$post_id' order by comments.id desc LIMIT 0,3";
                    $query_comment = $this->db->query("SELECT comments.id,comments.post_id,comments.answer as comment,comments.date as date,comments.doctor_id as post_user_id FROM skin_hair_answer as comments WHERE comments.post_id='$post_id' order by comments.id desc LIMIT 0,3");
                    $comments = array();
                    $comment_counts = $query_comment->num_rows();
                    if ($comment_counts > 0) {
    ...
    public function post_video_views($user_id, $media_id) {
        ...
        $total_query = $this->db->query("SELECT views FROM `media` where id='$media_id' ");
        $get_list = $total_query->row_array();
    ...
    public function comment_list($user_id, $post_id) { 
        ...
        if ($comment_count > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $query_posts = $this->db->query("SELECT type,question_type FROM posts WHERE id='$post_id'");
                $query_posts_c = $query_posts->num_rows();
                if ($query_posts_c > 0) {
                    $rowp = $query_posts->row_array();
                    $post_type = $rowp['type'];
                    $question_type = $rowp['question_type'];
            ...
            $query = $this->db->query("SELECT comments.id,comments.post_id,comments.answer as comment,comments.date as date,comments.doctor_id as post_user_id FROM skin_hair_answer as comments WHERE comments.post_id='$post_id' order by comments.id asc");
            foreach ($query->result_array() as $row) {
                $id = $row['id'];
                $comment_id = $row['id'];
                $post_id = $row['post_id'];
