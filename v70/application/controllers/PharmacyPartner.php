...
//                                     simple_json_output($resp);
				 	die('This order status is in draft.');
                                  } 
                ...
                if ($response['status'] == 200) {
                   ...
                   if($params == ""){
                        ...
                        if ($user_id == "" || $page_no == "" || $per_page == "") {
                            ...
                            $resp = $this->PharmacyPartnerModel->product_list($user_id,$page_no,$per_page,$search,$from_date,$to_date);
                            if($resp['status'] == 1){
                                $resp = array(
                                    'status' => 200,
                                    'message' => 'success',
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($params['listing_id'] == "" || $params['user_id'] == "" || $params['vendor_id'] == "" || $params['amount'] == "" || $params['payment_method'] == "" || $params['transaction_id'] == "" || $params['booking_id'] == "") {
                    ...
                    //$res        = $this->PharmacyPartnerModel->payment_procedure($listing_id);
	...
	//Hitesh, Push to orders from draft.
        public function push_to_order()
    {
        $this->load->model('PharmacyPartnerModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
