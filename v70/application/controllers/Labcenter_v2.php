            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['vendor_id'] == "")  {
                        ...
                        $resp = $this->LabcenterModel_v2->lab_test_by_vendor($vendor_id,$per_page,$page_no,$term,$cat_id);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" )  {
                        ...
                        if(array_key_exists("sort_a_z",$params)){
                            $sort_a_z = $params['sort_a_z']; // 1->asc 2-> desc
                        } else {
                            $sort_a_z = 0; //default no sorting  
                        }
                        
                        // price_low_high
                        
                        if(array_key_exists("price_low_high",$params)){
                            $price_low_high = $params['price_low_high']; // 1->asc 2-> desc
                        } else {
                            $price_low_high = 0; //default no sorting  
                        }
                        
                        
                        $resp = $this->LabcenterModel_v2->lab_packages($vendor_id,$per_page,$page_no,$home_delivery,$body_checkup,$lat,$lng,$term,$sort_a_z,$price_low_high);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['lat'] == "" || $params['lng'] == "") {
                        ...
                        $response = $this->LabcenterModel_v2->labcenter_list($lat, $lng, $user_id, $category_id, $hospital_type,$per_page,$page_no,$featured,$recommended,$certified,$mba);
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                //     print_r($params); die();
                    ...
                    if ($params['total_cost'] == "" || $params['discounted_price'] == "" || $params['user_id'] == "" || $params['listing_id'] == "" || $params['address_line1']=="" || $params['address_line2']=="" || $params['user_name'] == "" || $params['user_email'] == "" || $params['user_mobile'] == "" || $params['user_gender'] == ""  ||  $params['vendor_id'] == "" || $params['payment_mode'] == "" ||  $params['status'] == "" ||$params['trail_booking_date'] == "" || $params['trail_booking_time'] == "" ||   $params['city'] == "" || $params['state'] == "" || $params['pincode'] == "" ) {
                        
                        $required = "required fields are :total_cost, discounted_price, user_id, listing_id, address_line1, address_line2, user_name, user_email, user_mobile, user_gender, vendor_id, payment_mode, status, trail_booking_date, trail_booking_time,  city, state, pincode";
                        ...
                        if(array_key_exists('coupon_id',$params) && $params['coupon_id'] != ''){
                            $coupon_id   = $params['coupon_id'];
                        }
                        else{
                            $coupon_id = date('YmdHis');
                        }
...
//   get all offers
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                        ...
                        if(sizeof($params) > 0 && array_key_exists('offer_id', $params)  &&  $params['offer_id'] != null ){
                           $offer_id = $params['offer_id'];  
                        } else {
                             $offer_id = "";
                        }
                         
                        $resp = $this->LabcenterModel_v2->get_offers($offer_id);
    ...
    // get_offer_content
    public function get_offer_content(){
         $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
        } else {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                        ...
                        if(sizeof($params) > 0 && array_key_exists('offer_id', $params)  &&  $params['offer_id'] != null && array_key_exists('user_id', $params)  &&  $params['user_id'] != null  ){
                            
                            $offer_id = $params['offer_id'];  
                            $user_id = $params['user_id'];  
                            
                            $resp = $this->LabcenterModel_v2->get_offer_content($user_id,$offer_id);
                    	    ...
                    	    // $res['data'] =  $offers;                             
                              ...
                              $res['status'] =  400;
                    	        $res['message'] =  "Please add offer_id and user_id";
                        }
                    simple_json_output($res);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                        ...
                        if(sizeof($params) > 0 && array_key_exists('term', $params)  &&  $params['term'] != null && array_key_exists('user_id', $params)  &&  $params['user_id'] != null && array_key_exists('lat', $params)  &&  $params['lat'] != null && array_key_exists('lng', $params)  &&  $params['lng'] != null  && array_key_exists('term', $params)  &&  $params['page_no'] != null && array_key_exists('page_no', $params) && array_key_exists('per_page', $params)  &&  $params['per_page'] != null ){
                            ...
                            if(array_key_exists('search_for',$params)){
                                $search_for = $params['search_for'];
                            } else {
                                $search_for = 0;
                            }
                            ...
                            $resp = $this->LabcenterModel_v2->search_all($user_id,$term,$lat,$lng,$page_no,$per_page,$search_for);
                    	    ...
                    	    // $res['data'] =  $offers;                             
                              ...
                              $res['status'] =  400;
                    	        $res['message'] =  "Please add lat, lng, page_no, per_page, term and user_id";
                        }
                    simple_json_output($res);
                }
            }
