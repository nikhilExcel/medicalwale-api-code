...
class Jobs extends CI_Controller {
   ...
   /* public function add_jobs_user_profile() {
    ...
    /* add user profile details with profile image */
                ...
                if ($result != '') {
                   ...
                   $image ="";
                    ...
                    if(empty($_FILES["image"]["image"])){
                        $previous_image = $this->db->get_where('jobs_user_profile_master',array('user_id' => $user_id))->row()->image;
                        //print_r($previous_image); die();
                        $this->db->set('image', $previous_image); //value that used to update column  
                                ...
                                if ($img_size < (50000 * 50000)) {
                                    if (in_array($ext, $img_format)) {
                                        ...
                                        if ($s3->putObjectFile($img_tmp, $bucket, $actual_image_path, S3::ACL_PUBLIC_READ)) {
                                            $data['caption'] = '';
                                            $data['title'] = $actual_image_name;
                                            $data['type'] = 'image';
                                            $data['source'] = $actual_image_name;
                                        $this->db->insert('media',$data);
                                        $lastID = $this->db->insert_id() ;
                                        $this->db->set('avatar_id', $lastID); //value that used to update column  
    ...
    /*add user profile educational details*/        
 ...
 /*    public function add_jobs_user_profile_education_postgraduate() {
    ...
    // user profile details (view details)
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $page = $params['page'];
                        /* key for sorting salary*/
                        $sort_salary = $params['sort_salary']; 
                        /* filters*/
                        $location = $params['location'];
    ...
    // get particular job question listing
    ...
    // similar jobs based on job position
    ...
    // jobs listing based on category
   ...
   //languages known
    ...
    // application status of a particular job
    ...
    // company job listing based on company id
    ...
    // alert job listing(based on alert created by user)  
    ...
    //   if  id=null add, else edit (multiple review allowed discussed bu subhangi with vaibhav sir)
    // add review
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['company_id'] == "" ||$params['current_employ'] == "" || $params['employment_status'] == "" ||$params['review_title'] == "" || $params['company_pros'] == "" || $params['company_cons'] == "" || $params['comment_or_advise'] == "" ||$params['rating'] == "" || $params['designation'] == "" ||$params['base_salary'] == "" || $params['anonymously'] == ""   ) {
                       ...
                       // $highest_qualification = $params['highest_qualification'];
                        ...
                        $resp = $this->JobsprofileModel->company_review($id,$user_id,$company_id,$current_employ,$employment_status, $review_title, $company_pros, $company_cons,$comment_or_advise,$rating,$designation,$base_salary,$anonymously );
                    }
                    simple_json_output($resp);
               }
            }
  ...
  public function view_company_reviews()
  {
    ...
    public function view_company_review_share()
    {
        $this->load->model('JobsprofileModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ( $params['user_id'] =="" || $params['review_id'] =="" ) {
                        ...
                        $resp = $this->JobsprofileModel->view_company_review_share($user_id,$review_id);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ( $params['id'] =="" || $params['user_id'] =="") {
                        ...
                        $resp = $this->JobsprofileModel->delete_review($id,$user_id);
                    }
                    json_outputs($resp);
                }
            }
    ...
    //university list and filter based on university name entered
    ...
    //board list and filter based on city name entered
    ...
    // state list and filter based on city name entered
    ...
    // city list and filter based on name
    ...
    //get religion list and filter based on name
    ...
    //get country list and filter based on name
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ( $params['user_id'] =="") {
                        ...
                        $user_id = $params['user_id'];
                        $name = $params['name'];
                        $resp = $this->JobsprofileModel->country_list($user_id,$name);
                    }
                    json_outputs($resp);
                }
            }
    ...
    // add multiple languages to user profile 
    ...
    // filters listing (for dynamic filters)
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['review_id'] == "") {
                        ...
                        $user_id = $params['user_id'];
                        $post_id = $params['review_id'];
                        $post_user_id = $params['review_user_id'];
                        $resp = $this->JobsprofileModel->review_like($user_id, $post_id, $post_user_id);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function review_comment() {
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($params['user_id'] == "" || $params['review_id'] == "" || $params['comment'] == "") {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter all fields'
                        );
                        ...
                        $post_user_id = $params['review_user_id'];
                        $resp = $this->JobsprofileModel->review_comment($user_id, $post_id, $comment, $post_user_id);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function jobs_review_all_reply_list() {
        $this->load->model('JobsprofileModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($params['review_id'] == "" || $params['user_id'] == "") {
                        $resp = array('status' => 400, 'message' => 'please enter fields');
                    } else {
                        $comment_id = $params['review_id'];
                        $user_id = $params['user_id'];
                        $resp = $this->JobsprofileModel->jobs_review_all_reply_list($comment_id, $user_id);
                    }
                    json_outputs($resp);
                }
            }
    ...
    /* user details as job user details
    this api is to fill user profile details from user main table */
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $user_id = $params['user_id'];
                        $resp = $this->JobsprofileModel->get_user_profile_data($user_id);
                    }
                    json_outputs($resp);
                }
            }
    ...
    public function get_user_profile_status ()
    {
        $this->load->model('JobsprofileModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $user_id = $params['user_id'];
                        $resp = $this->JobsprofileModel->get_user_profile_status($user_id);
                    }
                    json_outputs($resp);
                }
            }
