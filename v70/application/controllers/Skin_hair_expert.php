...
class Skin_hair_expert extends CI_Controller {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['user_name'] == "" || $params['question'] == "" || $params['type'] == "") {
                       ...
                       /* $age = $params['age'];
                        ...
                        $resp = $this->Skin_hair_expert_model->skin_hair_add_question($type,$user_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous);
                    }
    ...
    //new for image uploading -- Mrunalini -- 24-08-2020
    public function skin_hair_add_question_v1() {
        $this->load->model('Skin_hair_expert_model');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
          ...
          //  $check_auth_client = $this->Skin_hair_expert_model->check_auth_client();
                    ...
                    //$params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($this->input->post('user_id') == "" || $this->input->post('user_name') == "" || $this->input->post('question') == "" || $this->input->post('type') == "") {
                        $resp = array('status' => 400, 'message' => 'please enter all fields');
                    } else {
                        $user_id = $this->input->post('user_id');
                        $type = $this->input->post('type');
                        $for_user_id = $this->input->post('for_user_id');
                       ...
                       /* $age = $params['age'];
                        ...
                        $resp = $this->Skin_hair_expert_model->skin_hair_add_question_v1($type,$user_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous);
                    }
                    simple_json_output($resp);
    ...
    public function skin_hair_edit_question() {
        $this->load->model('Skin_hair_expert_model');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['user_name'] == "" || $params['question'] == "" || $params['type'] == "") {
                        ...
                        $resp = $this->Skin_hair_expert_model->skin_hair_edit_question($type,$user_id,$post_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function skin_hair_edit_question_v1() {
        $this->load->model('Skin_hair_expert_model');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
                    ...
                    if ($this->input->post('user_id') == "" || $this->input->post('user_name') == "" || $this->input->post('question') == "" || $this->input->post('type') == "") {
                        ...
                        $resp = $this->Skin_hair_expert_model->skin_hair_edit_question_v1($type,$user_id,$post_id, $for_user_id, $user_name, $user_image, $question, $post_location, $is_anonymous);
                    }
                    simple_json_output($resp);
