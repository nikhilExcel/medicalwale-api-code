    ...
    //start 
     public function prescription_add_quickbook() {
        ...
        if ($method != 'POST') {
            ...
            if(array_key_exists('bachat_service_id',$_POST)){
                            $bachat_service_id  = $this->input->post('bachat_service_id');
                        } else {
                            $bachat_service_id = "0";
                        }
            ...
            if ($lat == "" || $lng == "") {
              // print_r($lat); die();
                ...
                $data = $this->OrderModel->prescription_add_quickbook($user_id, $address_id, $chat_id, $payment_method, $delivery_charge, $is_night_delivery,$lat,$lng,$product_id,$is_profile,$listing_id,$night_owls,$bachat_service_id);
                  ...
                  //     {
                        ...
                        if($order_id != ""){
                            ...
                            if($get_invoice_no['listing_type'] == 44){
                                ...
                                //print_r($data); die();
                                $data['mno_name'] = $data['mno_name'] ? $data['mno_name'] : "";
                                $data['mno_phpne'] = $data['mno_phpne'] ? $data['mno_phpne'] : "";
                            } else {
                                $data = (object)[];
                            }
                        } else {
                    ...
                    simple_json_output(array("status" => 201, "message" => $message));
                }
            }
        }
    }
                ...
                if ($response['status'] == 200) {
                  //  $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($this->input->post('user_id') == "") {
                        ...
                        $order_from_stack = $this->OrderModel->order_from_stack($user_id, $address_id, $payment_method, $delivery_charges, $is_night_delivery, $lat, $lng, $product_details,$is_profile,$listing_id,$night_owls);
                        
                        $resp1 = $order_from_stack['invoice_no'];
                        $message = $order_from_stack['message'];
