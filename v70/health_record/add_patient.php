...
if (isset($_POST['user_id'])) {
    ...
    if ($user_id != '') {
        ...
        if ($count > 0) {
           ...
           $query = mysqli_query($hconnection, "SELECT users.id FROM `users` WHERE  users.phone='$mobile'");
		   $count =  mysqli_num_rows($query);
           if ($count > 0) {
           $row = mysqli_fetch_array($query);
           $relation_id=$row['id'];
            date_default_timezone_set('Asia/Kolkata');
                ...
                $query = mysqli_query($hconnection, "SELECT * FROM health_record_folder WHERE health_record_id='$relation_id' and user_id='$user_id' order by created_at desc");
        		$count =  mysqli_num_rows($query);
                if (empty($count)) {
                    ...
                    $post_insert = mysqli_query($connection, "INSERT INTO `health_record_folder`(`user_id`,`health_record_id`,`folder_name`,`date`, `created_at`)VALUES ('$user_id','$relation_id','$a','$created_at','$created_at')");
                     $patient_id = mysqli_insert_id($connection);
                     }
             ...
             $query = mysqli_query($hconnection, "SELECT users.id FROM `users` WHERE users.email='$email' ");
		  $count =  mysqli_num_rows($query);
        if ($count > 0) {
            $row = mysqli_fetch_array($query);
           $relation_id=$row['id'];
            ...
            else
            {
             ...
             date_default_timezone_set('Asia/Kolkata');
                    ...
                    $patient_id_user = mysqli_insert_id($connection);
                     ...
                     $query = mysqli_query($hconnection, "SELECT * FROM health_record_folder WHERE health_record_id='$patient_id_user' and user_id='$user_id' order by created_at desc");
        		  $count =  mysqli_num_rows($query);
                if (empty($count)) {
                     $created_at = date('Y-m-d');
                    $folder_array=['Labs','Prescriptions'];
                     ...
                     foreach($folder_array as $a){
                    $post_insert = mysqli_query($connection, "INSERT INTO `health_record_folder`(`user_id`,`health_record_id`,`folder_name`,`date`, `created_at`)VALUES ('$user_id','$patient_id_user','$a','$created_at','$created_at')");
                     $patient_id = mysqli_insert_id($connection);
                     }
