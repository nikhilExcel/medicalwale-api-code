		...
		if($method != 'POST'){
			...
			if($check_auth_client == true){
		        ...
		        if($response['status'] == 200){
					...
					if ($params['user_id'] == "" || $params['latitude'] == "" || $params['longitude'] == "" || $params['category_id'] == "") {
		        		...
		        		$resp = $this->DoctorModel->doctor_list($latitude,$longitude,$user_id, $category_id);
					}
				  json_outputs($resp);
		        }
			}
		...
		if($method != 'POST'){
			...
			if ($params['doctors_type_id'] == "") {
			  ...
			  $doctors_type_id = $params['doctors_type_id'];
				$resp = $this->DoctorModel->doctor_category($doctors_type_id);
					}
				json_outputs($resp);
		        }
			}
