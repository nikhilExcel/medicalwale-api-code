...
class Trackhealthrecord extends CI_Controller {
	...
	public function record_list()
	{
		...
		if($method != 'POST'){
			...
			if($check_auth_client == true){
		        $response = $this->TrackhealthrecordModel->auth();
	...
	public function update_profile()
	{
		...
		if($method != 'POST'){
			...
			if($check_auth_client == true){
		        ...
		        if($response['status'] == 200){
					$params = json_decode(file_get_contents('php://input'), TRUE);
					if ($params['user_id'] == "" || $params['gender'] == "" || $params['height'] == "" || $params['weight'] == "" || $params['weight_date'] == "" || $params['height_date'] == "" || $params['age'] == "" || $params['activity_level'] == "") {
						$resp = array('status' => 400,'message' =>  'please enter fields');
					} else {
					    $user_id = $params['user_id'];
