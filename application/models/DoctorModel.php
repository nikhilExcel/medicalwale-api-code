...
class DoctorModel extends CI_Model
{
    ...
    public function doctor_list($latitude,$longitude,$user_id, $category_id)
    {
         $radius = '5';
       
         $query = $this->db->query("SELECT id,lat,lng,category,user_id,doctor_name,about_us,speciality,address,telephone,medical_college,medical_affiliation,charitable_affiliation,awards_recognition,all_24_hrs_available,home_visit_available,qualification,consultation_fee,experience,website,location,days,timing,image,rating,review,( 6371 * acos( cos( radians($latitude) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM doctor_list WHERE FIND_IN_SET('".$category_id."', category)  HAVING distance <= '$radius'");
        ...
        foreach ($query->result_array() as $row) {
            ...
            $query_sp = $this->db->query("SELECT GROUP_CONCAT(`category`) AS doctor_category FROM `business_category` WHERE  FIND_IN_SET(id,'".$speciality."') ");
            $get_sp = $query_sp->row();
            $speciality_final = $get_sp->doctor_category;
