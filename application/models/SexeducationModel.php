...
class SexeducationModel extends CI_Model {
    ...
    public function kamasutra_category_list() {
        return $this->db->select('kamasutra_category.id as category_id,kamasutra_category.category,count(kamasutra_position_list.position_category) as total_count')->from('kamasutra_category')->join('kamasutra_position_list', 'kamasutra_category.id=kamasutra_position_list.position_category')->group_by('kamasutra_category.category')->order_by('kamasutra_category.id', 'asc')->get()->result();
    }

    public function kamasutra_pickup_lines() {
        $men_query = $this->db->query("select id,kama_text from kamasutra_men_pickup_lines order by id asc");
        $women_query = $this->db->query("select id,kama_text from kamasutra_women_pickup_lines order by id asc");

        foreach ($men_query->result_array() as $row) {
            $id = $row['id'];
            $pickup_lines = $row['kama_text'];
            $menresultpost[] = array('id' => $id, 'kama_text' => $pickup_lines);
        }

        foreach ($women_query->result_array() as $row) {
            $id = $row['id'];
            $pickup_lines = $row['kama_text'];
            $womenresultpost[] = array('id' => $id, 'kama_text' => $pickup_lines);
        }
    ...
    public function kamasutra_dirty_talks() {
        $men_query = $this->db->query("select id,kama_text from kamasutra_men_dirty_talks order by id asc");
        $women_query = $this->db->query("select id,kama_text from kamasutra_women_dirty_talks order by id asc");

        foreach ($men_query->result_array() as $row) {
            $id = $row['id'];
            $dirty_talks = $row['kama_text'];
            $menresultpost[] = array('id' => $id, 'kama_text' => $dirty_talks);
        }

        foreach ($women_query->result_array() as $row) {
            $id = $row['id'];
            $dirty_talks = $row['kama_text'];
            $womenresultpost[] = array('id' => $id, 'kama_text' => $dirty_talks);
        }
    ...
    public function kamasutra_naughty_talks() {
        $men_query = $this->db->query("select id,kama_text from kamasutra_men_naughty_talks order by id asc");
        $women_query = $this->db->query("select id,kama_text from kamasutra_women_naughty_talks order by id asc");

        foreach ($men_query->result_array() as $row) {
            $id = $row['id'];
            $naughty_talks = $row['kama_text'];
            $menresultpost[] = array('id' => $id, 'kama_text' => $naughty_talks);
        }

        foreach ($women_query->result_array() as $row) {
            $id = $row['id'];
            $naughty_talks = $row['kama_text'];
            $womenresultpost[] = array('id' => $id, 'kama_text' => $naughty_talks);
        }
    ...
    public function kamasutra_love_talks() {
        $men_query = $this->db->query("select id,kama_text from kamasutra_men_love_talks order by id asc");
        $women_query = $this->db->query("select id,kama_text from kamasutra_women_love_talks order by id asc");

        foreach ($men_query->result_array() as $row) {
            $id = $row['id'];
            $love_talks = $row['kama_text'];
            $menresultpost[] = array('id' => $id, 'kama_text' => $love_talks);
        }

        foreach ($women_query->result_array() as $row) {
            $id = $row['id'];
            $love_talks = $row['kama_text'];
            $womenresultpost[] = array('id' => $id, 'kama_text' => $love_talks);
        }
    ...
    public function kamasutra_all_positions($user_id) {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $resultpost[] = array('id' => $id, 'position_category' => $position_category, 'kama_audio' => $kama_audio, 'position_category_name' => $position_category_name, 'position_name' => $position_name, 'position_tag' => $position_tag, 'position_description' => $position_description, 'regular_gif_image' => $regular_gif_image, 'regular_jpg_image' => $regular_jpg_image, 'wild_gif_image' => $wild_gif_image, 'to_do_yes_no' => $to_do_yes_no, 'favourite_yes_no' => $favourite_yes_no, 'tried_yes_no' => $tried_yes_no);
        }
        return $resultpost;
    }
    ...
    public function kamasutra_position_list($category_id, $user_id) {
        ...
        foreach ($query->result_array() as $row) {
            ...
            $resultpost[] = array('id' => $id, 'position_category' => $position_category, 'position_category_name' => $position_category_name, 'position_name' => $position_name, 'position_tag' => $position_tag, 'position_description' => $position_description, 'regular_gif_image' => $regular_gif_image, 'regular_jpg_image' => $regular_jpg_image, 'wild_gif_image' => $wild_gif_image, 'to_do_yes_no' => $to_do_yes_no, 'favourite_yes_no' => $favourite_yes_no, 'tried_yes_no' => $tried_yes_no, 'kama_audio' => $kama_audio);
        }
        return $resultpost;
    }

    public function kamasutra_sex_tips() {
        return $this->db->select('*')->from('kamasutra_sex_tips')->order_by('id', 'asc')->get()->result();
    }
    ...
    public function kamasutra_to_do_list($user_id) {
        ...
        if ($to_do_count > 0) {
            foreach ($query->result_array() as $row) {
                ...
                $resultpost[] = array('id' => $id, 'position_category' => $position_category, 'position_category_name' => $position_category_name, 'kama_audio' => $kama_audio, 'position_name' => $position_name, 'position_tag' => $position_tag, 'position_description' => $position_description, 'regular_gif_image' => $regular_gif_image, 'regular_jpg_image' => $regular_jpg_image, 'wild_gif_image' => $wild_gif_image, 'to_do_yes_no' => $to_do_yes_no, 'favourite_yes_no' => $favourite_yes_no, 'tried_yes_no' => $tried_yes_no);
            }
        } else {
            $resultpost = array();
        }
