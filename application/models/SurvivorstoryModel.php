...
class SurvivorstoryModel extends CI_Model {
    ...
    public function story_list($user_id)
   {
        $query = $this->db->query("SELECT `id`, IFNULL(tag,'') AS tag, IFNULL(author,'') AS author, `article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` FROM article where cat_id='37' order by id desc");
            ...
            foreach ($query->result_array() as $row) {
                ...
                if ($is_follow > 0) {
                    ...
                    $is_follow = 'No';
                     ...
                     $share_url= "https://medicalwale.com/inspiring_survivor_stories/".$article_id;
    ...
    {
        $query = $this->db->query("SELECT `id`, IFNULL(tag,'') AS tag, IFNULL(author,'') AS author, `article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` FROM article WHERE id='$post_id'");
    ...
    {
        $query = $this->db->query("SELECT `id`, IFNULL(tag,'') AS tag, IFNULL(author,'') AS author, `article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` FROM article WHERE cat_id='37' AND id<>'$article_id'");
    ...
    public function story_bookmark_list($user_id)
    {
		...
		$query = $this->db->query("SELECT  article_bookmark.id, article_bookmark.user_id, article_bookmark.article_id, article.id, article.article_title, article.article_description, article.tag, article.author, article.image, article.posted FROM `article`
            ...
            WHERE article_bookmark.user_id = '$user_id' AND cat_id='37' order by article_bookmark.id");
      $count= $query->num_rows();
		if($count>0){
   foreach($query->result_array() as $row){
        $id=$row['id'];
