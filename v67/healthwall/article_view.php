...
if (isset($_POST['post_id'])) {
    ...
    function send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $agent, $post_id) {
        ...
        $headers = array(
            ...
            $agent === 'android' ? 'Authorization: key=AIzaSyBQDOIo8uwvF9ppdPK-mM-1bVGJKs70jk4' : 'Authorization: key='
        );
        $ch = curl_init();
    ...
    if ($post_id != '') {
        for ($i = 3680; $i <= 3795; $i++) {
            $sql = "SELECT id FROM `article_views` WHERE user_id='$i' and article_id='$post_id'";
            $res = mysqli_query($hconnection, $sql);
            $count = mysqli_num_rows($res);
            if ($count > 0) {
                
            } else {
                $media = mysqli_query($hconnection, "INSERT INTO `article_views`(`article_id`, `user_id`) VALUES ('$post_id', '$i')");
