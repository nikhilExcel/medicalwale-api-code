...
class Sexeducation extends CI_Controller {
 ...
 /*   public function kamasutra_love_talks_hindi() {
  ...
  public function sex_expert_question_list_web() {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                       ...
                       // $user_id = $params['user_id'];
                        ...
                        $resp = $this->SexeducationModel->add_comment($data,$comment1,$user_id);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function test_stall()
    {
        
       /* function crul($data1)
        {
             
                $u="http://52.66.208.83:8004/doctor/lelo/forum/auto/answer";
               $url=$u;
               $ch = curl_init($url);
               curl_setopt($ch, CURLOPT_POST, true);
               curl_setopt($ch, CURLOPT_POSTFIELDS, $data1);
               curl_setopt($ch, CURLOPT_HTTPHEADER,['Cache-Control:no-cache','Content-Type:application/json','Postman-Token:e3cbb37b-8f6b-42b9-9241-54144f17d27d']);      
       ...
       $ar = array('userId' => 28488,"postId"=> 123,"text" => "test");
       $data1          = json_encode($ar);*/
       
       $new=array(
          "userId"=>28488,
          );
        $url="http://139.59.76.89:8002/user/profile/update/";
            $data=json_encode($new);
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
            curl_setopt($ch, CURLOPT_HTTPHEADER,['Cache-Control:no-cache','Content-Type:application/json','Postman-Token:cf53b3a6-3893-4b6f-9f98-60390c239e70,b7478610-58de-430a-8e6b-c71030ed6774','cache-control:no-cache']);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            $result = curl_exec($ch);
            curl_close($ch);
            print_r($result);
