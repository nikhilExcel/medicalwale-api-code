 ...
 class MedicalMall_model_2 extends CI_Model {
   ...
   /* public function auth() {
    ...
    public function get_featured_products(){
        ...
        WHERE pd.is_featured = '1' AND pd.pd_status = '1' ");
    ...
    public function get_ayurvedic_products(){
        ...
        on vd.v_id=pd.pd_added_v_id
        WHERE 	landing_screen = '1' AND pd_status = '1' AND pd_pc_id = '123'");
        $res = $query->result_array();
        return $res;
    }
     ...
     public function get_fitness_products(){
        ...
        join product_details_hm pd 
        on vd.v_id=pd.pd_added_v_id
        WHERE 	landing_screen = '1' AND pd_status = '1' AND pd_pc_id = '81'");
        $res = $query->result_array();
        return $res;
    }
    ...
    public function get_personal_care_products(){
        ...
        join product_details_hm pd 
        on vd.v_id=pd.pd_added_v_id
        WHERE 	landing_screen = '1' AND pd_status = '1' AND pd_pc_id = '15'");
        $res = $query->result_array();
        return $res;
    }
    ...
    public function get_luxurious_brand_products(){
        ...
        join product_details_hm pd 
        on vd.v_id=pd.pd_added_v_id
        WHERE 	pd_luxurious_brand = '1' AND pd_status = '1' AND landing_screen = '1'");
        $res = $query->result_array();
        return $res;
    }
...
//  		print_r($data); die();
    ...
    $amount = $data['amount'];
    $listing_id = $data['v_id0'];
    $listing_name = $data['v_name0'];
    $listing_type = 34;
// actual_total  '".$data['amount']."',
		$query_for_order = "INSERT INTO user_order
(invoice_no,order_status,listing_id,listing_name,listing_type,name,mobile,user_id,order_total,pincode,address1,landmark,city,state,payment_method,order_date,action_by) 
VALUES
('$invoice_no','$order_status',$listing_id,'$listing_name','$listing_type','$name','$mobileno','$cust_id','$amount','$pincode','$address','$landmark','$city','$state','$paym','$order_date','customer')";
		...
		if($insert_id != ""){
		  ...
		  // for website; in app not calling place_order untill payment is successful
		    ...
		    for($i=0;$i<$data['total_products'];$i++){
		    	...
		    	$query_for_order_data = "INSERT INTO orders_details_hm (order_id,user_id,product_id,product_qty,vendor_id,is_offer,offer_id,offer_price,price)
         VALUES ('".$insert_id."','".$cust_id."','".$data['product_id'.$i]."','".$data['product_qty'.$i]."','".$data['vendor_id'.$i]."','".$data['is_offer'.$i]."','".$data['offer_id'.$i]."','".$data['offer_total'.$i]."','".$data['total_products_price'.$i]."')";
...
// 	}
	...
	public function cancel_order($order_id, $reason){
        ...
        if($count > 0){
            if($checkAvailable->result_array()[0]['order_status'] == 'Cancelled'){
                $res = array(
                    "status" => 400,
                ...
                $this->db->query("UPDATE user_order SET order_status = 'Cancelled', cancel_reason = '$reason' WHERE order_id = '$order_id'");
                $checkAvailable = $this->db->query("SELECT order_id,invoice_no,order_status,user_id,payment_method,order_date,cancel_reason FROM user_order WHERE order_id = '$order_id'");
                $res = array(
                    "status" => 200,
