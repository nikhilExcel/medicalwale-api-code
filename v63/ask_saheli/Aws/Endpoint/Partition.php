...
final class Partition implements ArrayAccess, PartitionInterface {
    ...
    $service, $allowNonRegionalEndpoints = false
        ...
        if (isset($this->data['services'][$service]['endpoints'])) {
            ...
            return $allowNonRegionalEndpoints ? $serviceRegions : array_intersect($serviceRegions, array_keys(
                                    $this->data['regions']
            ));
        }
    ...
    public function __invoke(array $args = []) {
        ...
        return [
            'endpoint' => "{$scheme}://" . $this->formatEndpoint(
                    isset($data['hostname']) ? $data['hostname'] : '', $service, $region
            ),
            'signatureVersion' => $this->getSignatureVersion($data),
            'signingRegion' => isset($data['credentialScope']['region']) ? $data['credentialScope']['region'] : $region,
            'signingName' => isset($data['credentialScope']['service']) ? $data['credentialScope']['service'] : $service,
        ];
    }
    ...
    private function getSignatureVersion(array $data) {
        ...
        $possibilities = array_intersect(
                $supportedBySdk, isset($data['signatureVersions']) ? $data['signatureVersions'] : ['v4']
        );
