...
class DoctorModel extends CI_Model
{
       ...
       if($keyword!=""){
        ...
        if ($count > 0) {
            foreach ($query->result_array() as $row) 
            {
                ...
                $sql2             = "SELECT `id`, `doctor_id`,`contact_no`, `clinic_name`,`open_hours`,`consultation_charges`, `address`, `state`, `city`, `pincode` ,`lat`, `lng`,`image` FROM doctor_clinic WHERE doctor_id='$doctor_user_id'";
  ...
  /* public function add_bookings($user_id, $listing_id, $clinic_id, $booking_date, $booking_time, $user_name, $user_mobile, $from_time, $to_time, $user_email, $user_gender, $is_user, $is_patient_added, $patient_id, $health_condition, $allergies, $heradiatry_problem, $description, $relationship, $date_of_birth, $connect_type,$status)
    ...
    */
    ...
    public function add_bookings_v2($user_id, $listing_id, $clinic_id, $booking_date, $booking_time, $user_name, $user_mobile, $from_time, $to_time, $user_email, $user_gender, $is_user, $is_patient, $patient_id,$allergies, $heradiatry_problem, $description, $relationship, $date_of_birth, $connect_type,$coupon_id,$status)
    {
        date_default_timezone_set('Asia/Kolkata');
        $booking_id           = date("YmdHis");
        $date                 = date('Y-m-d H:i:s');
...
By using these service, you agree that you are bound by the Terms of Service.<br/>Copyright 2017 AEGIS HEALTH SOLUTIONS PVT. LTD. All rights reserved.
    ...
    public function user_read_slot_v2($doctor_id, $consultation_type,$agent)
    {
        if($agent=="ios")
        {
             $clinic1_slott=array();
