...
// remaining 
    ...
    //   print_r($orders_per_day);  die();
    //   echo $totalDaysCounted; die();
        $dateInLoop = $from_date;
        for($d=0;$d<=$totalDaysCounted;$d++){
        
        $getIndex = array_search($dateInLoop, array_column($orders_per_day, 'date'),true);
               
            if($getIndex > -1) {
            //  print_r($orders_per_day); die();
                $ordersFinal[] = $orders_per_day[$getIndex];
            } else {
                
                    $dayLoop = date('D',strtotime($dateInLoop));
                    
              
                $orders_per_day_empty = array("day" => $dayLoop,
                                            "date" => $dateInLoop,
                                            "total_orders" => 0 ,
                                            "billing" => 0 ,
                                            "online_time" => "",
                ...
                // print_r($orders_per_day_empty); die();
                $ordersFinal[] = $orders_per_day_empty;
            }
