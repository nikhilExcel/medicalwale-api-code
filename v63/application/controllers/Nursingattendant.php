...
class Nursingattendant extends CI_Controller {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['package_id'] == "" ||  $params['name'] == "" || $params['gender'] == "" || $params['patient_name'] == "" || $params['patient_age'] == "" || $params['patient_gender'] == "" || $params['city'] == "" || $params['attendent_time'] == "" || $params['attendant_hour'] == "" || $params['tentative_intime'] == "" || $params['tentative_outtime'] == "" || $params['nursing_gender'] == "" || $params['attendant_needed'] == "" || $params['appoinment_booking_date'] == "") {
                        ...
                        $appoinment_booking_date = $params['appoinment_booking_date'];
                          if (array_key_exists("patient_condition",$params))
                   {
    ...
    public function update_book_package_quick() {
        $this->load->model('NursingattendantModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
               ...
               $response = $this->LoginModel->auth();
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    ///print_r($params);
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['booking_id'] == "" ||  $params['bid_id'] == "" ) {
                        $resp = array('status' => 400, 'message' => 'please enter fields');
                    } else {
                        $user_id            = $params['user_id'];
                        $listing_id         = $params['listing_id'];
                        $package_id         = $params['booking_id'];
                        $bid_id             = $params['bid_id'];
                        
                        $resp1               = $this->NursingattendantModel->update_book_package_quick($user_id,$listing_id,$package_id,$bid_id);
                        if($resp1 != "")
                        {
                             json_output(200, array('status' => 200, 'message' => 'Success'));
                        }
                        ...
                        {
                             //$resp = array('status' => 400, 'message' => 'Unable to Book Package');
                             json_output(400, array('status' => 400, 'message' => 'Unable to Update Package'));
                        }
    ...
    public function nursingattendant_book_package_quick() {
        $this->load->model('NursingattendantModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
               ...
               $response = $this->LoginModel->auth();
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    ///print_r($params);
                    if ($params['user_id'] == "" ||  $params['name'] == "" || $params['gender'] == "" || $params['patient_name'] == "" || $params['patient_age'] == "" || $params['patient_gender'] == "" || $params['city'] == "" || $params['attendent_time'] == "" || $params['attendant_hour'] == "" || $params['tentative_intime'] == "" || $params['tentative_outtime'] == "" || $params['nursing_gender'] == "" || $params['attendant_needed'] == "" || $params['appoinment_booking_date'] == "") {
                        $resp = array('status' => 400, 'message' => 'please enter fields');
                        ...
                        $lat = $params['lat'];
                        $lng = $params['lng'];
                        $resp1               = $this->NursingattendantModel->nursingattendant_book_package_quick($user_id,$name,$gender, $patiente_name, $patiente_age, $patiente_gender, $city, $patiente_condition, $attendent_time, $attendant_hour, $tentative_intime, $tentative_outtime, $nursing_gender, $attandant_needed, $appoinment_booking_date, $lat, $lng);
                        if($resp1 != "")
                        {
                             json_output(200, array('status' => 200, 'message' => 'Success'));
                        }
                        ...
                        {
                             //$resp = array('status' => 400, 'message' => 'Unable to Book Package');
                             json_output(201, array('status' => 201, 'message' => 'Unable to Book Package'));
                        }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['rating'] == "" || $params['review'] == "" || $params['service'] == "") {
                        ...
                        $review = $params['review'];
                        $service = $params['service'];
                        $resp = $this->NursingattendantModel->edit_review($review_id,$user_id, $listing_id, $rating, $review, $service);
                    }
                    simple_json_output($resp);
                }
            }
