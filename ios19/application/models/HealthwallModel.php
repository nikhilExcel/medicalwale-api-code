      ...
      /* $this11 = new \stdClass();
        ...
        if ($count_post > 0) {
            ...
            $last_names = array_column($newarray,'details','variables');
                ...
                /*    $query = $this->db->query("SELECT id,user_id,vendor_type,ad_main_cat,main_cat_id,ad_title,ad_description,ad_link,ad_image FROM sponsored_advertisements where NOT FIND_IN_SET(id, '$ad_array') ORDER BY id desc limit $count_limit");
                    $count = $query->num_rows();
                    if ($count > 0) {
                        $row  = $query->row_array();
                        $ad_img      = $row['ad_image'];
