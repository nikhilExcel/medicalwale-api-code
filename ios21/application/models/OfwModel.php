...
class OfwModel extends CI_Model
{
    ...
    public function ask_saheli_post_category()
    {
        ...
        $query = $this->db->query("SELECT id,category,hindi_healthwall_categorys,image,big_image FROM `asksaheli_main_category` order by category asc");
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id                         = $row['id'];
                ...
                if ($img_file != '') {
                    $s_image = 'https://medicalwale.s3.amazonaws.com/images/healthwall_media/Gossip/' . $img_file;
                } else {
                    $s_image = '';
                }
                if ($img_file_b != '') {
                    $b_image = 'https://medicalwale.s3.amazonaws.com/images/healthwall_media/Gossip/' . $img_file_b;
                } else {
                    $b_image = '';
                }
  ...
  public function menstural_cycle_notification_on_off($user_id,$profile_id,$is_notification)
    {
    ...
    public function add_menstrual_cycle_data($user_id, $name, $relationship, $start_period_day, $cycle_length, $total_period_day, $born_of_year, $tutual_cycle, $tracking_resoan, $profile_id, $history_id, $pcod,$mobile)
    {
        ...
        if ($count > 0) {
            ...
            if ($count1 > 0) {
                ...
                $start_period_day = date('d-m-Y', strtotime($start_period_day));
            ...
            //add family
            if($mobile!=''){
                $query = $this->db->query("SELECT users.id FROM `users` WHERE  users.phone='$mobile' limit 1");
                $count = $query->num_rows();
                if ($count > 0) {
                    
                    $row          = $query->row_array();
                    $relation_id  = $row['id'];
                    $health_record_data = array(
                        'user_id' => $user_id,
                    ...
                    $this->db->insert('health_record', $health_record_data);
                    $patient_id = $this->db->insert_id();
                    $query = $this->db->query("SELECT * FROM health_record_folder WHERE health_record_id='$relation_id' and user_id='$user_id' order by created_at desc");
                    $count = $query->num_rows();
                    if ($count==0) {
                        $folder_array=['Labs','Prescriptions'];
                        foreach($folder_array as $a){
                            $health_record_folder_data = array(
                                'user_id' => $user_id,
                                'health_record_id' => $relation_id,
                                'folder_name' => $a,
                                'date' => $created_at,
                                'created_at' => $created_at
                            );
                            $this->db->insert('health_record_folder', $health_record_folder_data);
                        }
                    }
                }
                else{
                    ...
                    $patient_id_user = $this->db->insert_id();
                    
                    $health_record_data = array(
                        'user_id' => $user_id,
                        'relation_id' => $patient_id_user,
                        'patient_age' => '',
                        'patient_name' => $name,
                        'relationship' => $relationship,
                        'date_of_birth' => '',
                    ...
                    $query = $this->db->query("SELECT * FROM health_record_folder WHERE health_record_id='$patient_id_user' and user_id='$user_id' order by created_at desc");
                    $count = $query->num_rows();
                    if ($count==0) {
                        $folder_array=['Labs','Prescriptions'];
                        foreach($folder_array as $a){
                            $health_record_folder_data = array(
                                'user_id' => $user_id,
                                'health_record_id' => $patient_id_user,
                                'folder_name' => $a,
                                'date' => $created_at,
                                'created_at' => $created_at
                            );
    ...
    public function add_menstural_cycle_profile($user_id, $relationship, $name, $age, $height, $weight, $sleep_cycle, $birth_control, $image)
    {
        $query = $this->db->query("SELECT id from `users` WHERE id='$user_id' limit 1");
        $count = $query->num_rows();
        if ($count > 0) {
    ...
    public function update_menstural_cycle_profile($user_id, $profile_id, $relationship, $name, $age, $height, $weight, $sleep_cycle, $birth_control, $image)
    {
        $query = $this->db->query("SELECT id from `users` WHERE id='$user_id' limit 1");
        $count = $query->num_rows();
        if ($count > 0) {
            ...
            $this->db->where($where);
            $this->db->update('menstural_cycle_profile ', $data);
	    $update = $this->db->query("UPDATE menstural_cycle_data SET name = '$name' where user_id='$user_id' and profile_id='$profile_id'");
            return array(
                'status' => 200,
                'message' => 'success'
            );
            ...
            if ($count1 > 0) {
                foreach ($query1->result_array() as $row) {
                    ...
                    $result_array             = array(
                        $period_in_array,
                        $ovulation_array,
                        $ending_of_period_array,
                        $starting_of_period_array,
                        $pms_array,
                        $add_pill_array,
                        $safe_period_array
                    );
        ...
        if ($count > 0) {
            ...
            if ($count_temp > 0) {
                ...
                for ($j = 0; $j < 12; $j++) {
                    ...
                    if (empty($period_days['after_ovulation']) && $j != 0) {
                ...
                // die();
                $latest_date = $this->db->query("SELECT period_start_from from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' order by id desc limit 1")->row_array();
                $start_period_day  = $latest_date['period_start_from'];
                $result_array[] = array(
                    'history_id' => $history_id,
                    'user_id' => $user_id,
                    'profile_id' => $profile_id,
    ...
    public function get_calendar($period_start, $period_start_from, $total_period_day, $cycle_length, $user_id, $profile_id)
    {
        ...
        for ($i = 0; $i < $cycle_length; $i++) {
            ...
            if(!empty($table_id)){
                $table_sql=' and id>'.$table_id;
            }
            else{
                $table_sql='';
            }
            $query_temp = $this->db->query("SELECT id as table_id,start_periode_data,period_start_from,cycle_length,total_periode_day,DATE_FORMAT(STR_TO_DATE(`period_start_from`, '%d-%m-%Y'), '%Y-%m-%d') AS start_period from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' HAVING start_period='$date_array' and start_period<>'$period_start_from' $table_sql order by id desc limit 1");
            $count_temp = $query_temp->num_rows();
            ...
            if ($count_temp > 0) {
                ...
                if ($i < $total_period_day) {
                    if(!empty($date_array) || $date_array!=null){
                        $p_date[] = array(
                            'date' => $date_array
                        );
                    }
        ...
        return array_filter($period_days);
    }
    
    
    
    public function get_menstrual_cycle_data_calendar_list($user_id, $profile_id)
    {
        $query = $this->db->query("SELECT id from `users` WHERE id='$user_id' limit 1");
        $count = $query->num_rows();
        if ($count > 0) {
            $query_temp = $this->db->query("SELECT id,user_id,profile_id,name,pcod,start_periode_data,period_start_from,cycle_length,tracking_reson,total_periode_day,born_year,tutual_cycle,tutual_cycle,relationship from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' order by id asc");
            $count_temp = $query_temp->num_rows();
    ...
    public function update_cycle_category_status($user_id, $main_category, $sub_category, $status, $profile_id,$date)
    {
        $updated_at = date('Y-m-d H:i:s');
    ...
    public function menstrual_cycle_article_list($user_id, $page)
    {
        ...
        if ($count > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $article_image  = str_replace("itÃ¢â‚¬â„¢s", "it's", $row['image']);
	...
	public function get_calendar_by_month($period_start,$period_start_from,$total_period_day, $cycle_length,$user_id,$profile_id,$datelist)
    {
		$p_date           = array();
        $before_ovulation = array();
        $o_date           = array();
        $after_ovulation  = array();
        $period_days  = array();
		$date = explode(',',trim($datelist));
		$lastdate = end($date);
		$next_month = date('F Y', strtotime($lastdate . ' +1 month'));
		$datelist=$datelist.','.$next_month;
		$final_date = explode(',',trim($datelist));
        for ($i = 0; $i < $cycle_length; $i++) {
		    $current_month = date('F Y', strtotime($period_start . ' +' . $i . '  days'));
            ...
            if ($count_temp>0) {			
				...
				if(in_array($current_month,$final_date))
				{
					$period_days = array(
						'period' => $p_date,
						'before_ovulation' => $before_ovulation,
    ...
    public function get_menstrual_cycle_data_by_month($profile_id, $user_id, $date)
    {
        $period_cal_days=array();
		$query = $this->db->query("SELECT id from `users` WHERE id='$user_id' limit 1");
        $count = $query->num_rows();
        if ($count > 0) {
            $query_temp = $this->db->query("SELECT id,user_id,profile_id,name,pcod,start_periode_data,period_start_from,cycle_length,tracking_reson,total_periode_day,born_year,tutual_cycle,tutual_cycle,relationship from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' order by id asc limit 1");
            ...
            if ($count_temp > 0) {
                ...
                for ($j = 0; $j < 12; $j++) {                    
					if($this->session->userdata('status')==TRUE){
                        ...
                        $period_days = $this->get_calendar_by_month($period_start,$period_start_from,$total_period_day, $cycle_length,$user_id,$profile_id,$date);	
						$period_start  = date('Y-m-d', strtotime($period_start . ' +' . $cycle_length . ' days'));
					}
					else{
					    $period_days = $this->get_calendar_by_month($period_start,$period_start_from,$total_period_day, $cycle_length,$user_id,$profile_id,$date);
    ...
    public function fetch_menstural_cycle_details($profile_id, $user_id, $date)
    {
        $array        = $this->get_menstrual_cycle_data_by_month($profile_id, $user_id, $date);
	
        $created_date = date('d-M-Y');
        $name         = $array[0]['name'];
        $relation     = $array[0]['relation'];
        $reason       = $array[0]['reason'];
        $startPdate   = $array[0]['startPdate'];
        $totalPdate   = $array[0]['totalPdate'];
        $trackBorn    = $array[0]['trackBorn'];
        $trackCycle   = $array[0]['trackCycle'];
        $tutual_cycle = $array[0]['tutual_cycle'];
		
		$output = '
  <body>
    <header class="clearfix">
      <div id="logo">
                <img src="'.base_url().'/assets/menstrual_cycle_logo.png">
      </div>
...
/*        for ($i = 0; $i < count($array[0]["calendar"]); $i++) {
            $j = $i + 1;            
            $periodC    = '';
            $ovulationC = '';
            foreach ($array[0]["calendar"][$i]["period"] as $values) {
            ...
            foreach ($array[0]["calendar"][$i]["ovulation"] as $values2) {
        ...
        $output .='<table border="0" cellspacing="0" cellpadding="0">
            ...
            foreach ($array[0]["calendar"][$i]["period"] as $values) {
                $period_date = $values['date'];
                    ...
                    $periodC = date("d-M-Y", strtotime($period_date));
                    $date_format= date("Y-m-d", strtotime($periodC));
                    $activity_query = $this->db->query("SELECT GROUP_CONCAT(CONCAT(sub_category, ':', child_category)) AS activity from `Menstraul_cycle_subcategory_status` WHERE status='on' and date='$date_format' and user_id='$user_id' and profile_id='$profile_id'");
                    $activity_count = $activity_query->num_rows();
                    if ($activity_count > 0) {
                        $activity = $activity_query->row()->activity;
                    }else{
                        $activity = '-';
                    }
			...
			<tbody>
			<tr>
            <td class="desc">' . $periodC . '</td> <td class="desc">Period</td> </td> <td class="desc">' . $activity . '</td>   
			</tr>   
			</tbody>
			 ';
            }
            ...
            foreach ($array[0]["calendar"][$i]["ovulation"] as $values2) {
                ...
                $ovulationC = date("d-M-Y", strtotime($ovulation_date));
                 ...
                 $date_format= date("Y-m-d", strtotime($ovulationC));
                    $activity_query = $this->db->query("SELECT GROUP_CONCAT(CONCAT(sub_category, ':', child_category)) AS activity from `Menstraul_cycle_subcategory_status` WHERE status='on' and date='$date_format' and user_id='$user_id' and profile_id='$profile_id'");
                    $activity_count = $activity_query->num_rows();
                    if ($activity_count > 0) {
                        $activity = $activity_query->row()->activity;
			...
			<tbody>
			<tr>
            <td class="desc">' . $ovulationC . '</td> <td class="desc">Ovulation</td>   <td class="desc">' . $activity . '</td>    
			</tr>   
			</tbody>
			 ';
            }
        ...
        $output .= '</table></main> <footer> Copyright © 2019 Aegis Health Solutions. </footer> </body>';
        return $output;
