...
class DoctoronlineModel extends CI_Model
{
    ...
    public function add_bookings($doctor_id,$user_id,$member_id,$type,$fee,$status,$coupon_id,$trans_id,$payment_method)
    {
        ...
        if($insert)
            {
             ...
             $booking_array = array(
                     ...
                     if ($count2 > 0) 
                        {
                           ...
                           $this->send_gcm_notify_doctor($title, $reg_id, $msg, $img_url, $tag, $agent,$type);
                           $this->send_gcm_web_notify($title, $msg, $web_token, $img_url, $tag, $agent, $type,$click_action);
                             ...
                             if($payment_method != '3' || $payment_method != '14') 
                               ...
                               /* $ledger_id=$ledger_data['ledger_id'];
                    			$ledger_type=$ledger_data['type'];*/
                               /* $ledger_type1='user';*/
                               /* $status='1';
                                $this->LedgerModel->update_ledger($user_id,$ledger_id,$booking_id,$trans_id,$status,$ledger_type);
                ...
                $this->insert_notification_post_question($user_id, $booking_id,$doctor_id);
                $description="";
                 $this->notifyMethod($doctor_id, $user_id, $description, $booking_date, $booking_time, $booking_id, $con_type, $doctor_name);
     ...
     public function send_gcm_notify_doctor($title, $reg_id, $msg, $img_url, $tag, $agent,$type)
    {
