...
else
{
   ...
   else
   {
    ...
    public function patient_search($doctor_id,$keyword) {
        date_default_timezone_set('Asia/Kolkata');
        $date=date('Y-m-d');
        $query = $this->db->query("SELECT user_id,patient_name as keyword FROM doctor_patient WHERE patient_name like '%$keyword%' and doctor_id='$doctor_id'");
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $booking="";
        ...
        if ($count_appointment_slot > 0) {
            foreach ($Appointment_dataList->result_array() as $row) 
          {
           ...
           $is_user='1';
            ...
            {
                $url="https://doctor.medicalwale.com/doctor_prescription.php?id=".$pr_id;
                $pdf="https://doctor.medicalwale.com/prescription/".$pr_id.".pdf";                           
           ...
           if($follow_query){
            ...
            $customer_token       = $this->db->query("SELECT name,token,agent,token_status FROM users WHERE id='$following_id'");
                ...
                if ($customer_token_count > 0) {
                    ...
                    $msg          =  $vendor_name.' has sent you a request.';
...
/************************************end*****************************************/
    ...
    public function event_highlights($user_id,$event_id,$status)
    {
        ...
        if($status =="all")
        {
            $sql    = "SELECT * FROM events_highlights WHERE user_id='$user_id' AND event_id='$event_id'";
            $query = $this->db->query($sql);
        }
        ...
        foreach ($query->result_array() as $row) {
          //  echo "hii";
           ...
           //echo " ";
            ...
            //  echo "UPDATE doctor_booking_master SET status='$status',description='$reason' WHERE id ='$id'";
