...
public function elasticsearchhospital_package($index_id,$keyword){
    ...
    public function keyword_list($user_id,$type,$keyword) {
	  ...
	  public function recent_keyword_list($user_id) {
   
        $query = $this->db->query("SELECT * FROM `user_search_history` WHERE user_id =$user_id order by date DESC limit 25");
        $count = $query->num_rows();
  ...
  public function elastic_jobs($index_id,$keyword){
     ...
     $returnresult = $this->elasticsearch->query_all($index_id,$keyword);
    	    ...
    	    if($returnresult['hits']['total'] < 0){
                	   ...
                	   foreach($returnresult['hits']['hits'] as $hi){
                	      ...
                	      $sim = similar_text($hi['_source']['job_position'], $keyword, $perc[]);
                	   }
                	}
        ...
        if($dataperc >= 70){
             ...
             $returnresult = $this->elasticsearch->query_all($index_id,$keyword);
               foreach($returnresult['hits']['hits'] as $hi){
            	...
            	$data = array("suggest"=>array("my-suggestion"=>array("text"=>$keyword, "term"=>array("field"=>"job_position")))); 
                $data1=json_encode($data);
    ...
    public function search_list($user_id, $keyword) {
        ...
        if ($user_id > 0) {
           ...
           $index_id="hospital";
            ...
            $field3 = '';
           $index_id="jobs";
            $jobs=$this->elastic_jobs($index_id,$keyword);  
            if (!empty($jobs)) {
        ...
        else
        {
            ...
            $resultpost = array_merge($people_array, $doctor_array,$dental_clinic_array, $pharmacy_array,  $labs_array,$labs_test_array,$labs_package_array, $nursing_array, $fitness_array,$spa_center_array,$jobs_array, $hospital_array,$hospital_package_array, $healthwall_array, $article_array,$tips_array,$skin_array,$mom_array, $survivor_array,$hm_product_array);
         ...
         if ($user_id > 0) {
          ...
          //  $query = $this->db->query("select healthwall_category.category AS healthwall_category,healthwall_category.id as healthwall_category_id, posts.id as post_id, IFNULL(posts.repost_user_id,'') AS repost_user_id,posts.repost_location,posts.is_repost,posts.repost_time, posts.type as post_type,IFNULL(posts.post_location,'') AS post_location,posts.caption,posts.description as post,posts.is_anonymous,posts.tag,posts.created_at,users.name,posts.user_id as post_user_id,users.vendor_id ,posts.article_title,posts.article_image,posts.article_domain_name,posts.article_url,IFNULL(posts.category,'') AS post_category from posts INNER JOIN users on users.id=posts.user_id LEFT JOIN healthwall_category ON posts.healthwall_category=healthwall_category.id where posts.user_id<>'' and posts.user_id<>'0' AND posts.healthwall_category<>'0' AND  posts.id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=posts.id AND user_id='$user_id') and posts.active='1' order by posts.id DESC limit 50");
            ...
            return $NewStr;
                         ...
                         // Articles
            $query = $this->db->query("SELECT article.id, `cat_id`, `article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id  where  (article_title like '%$keyword%' or article_description like '%$keyword%') and article_category.type='home_remedies' and limit 20");
                     ...
                     $field1 = '';
                       ...
                       $index_id="jobs";
                        ...
                        if (!empty($jobs)) {
                           ...
                           return $jobs_array = array();
                        }
           
                    
                    
                    }else if($type == 'spa_package'){ 
              // Spa Package
            $field1 = '';
            $field2 = '';
            $field3 = '';
      ...
      $hospital_query = $this->db->query("SELECT * FROM hospitals WHERE user_id='$hospital_id' ORDER BY id desc ");
        ...
        if ($count > 0) {
            ...
            $result_hospital = array();
             ...
             //jobs (added date-3/5/2020 for IOS)
             
             if($listing_type == '46')
              {
                  
        $query = $this->db->query("SELECT * FROM `company_profile` LEFT JOIN company_industry_type ON company_profile.company_industry_type = company_industry_type.id WHERE `user_id`=$listing_id");
        $resultpost = array();
        $job_count = array();
        $profile_view = $this->db->query("SELECT views FROM `company_profile` WHERE `user_id`=$listing_id ");
        ...
        $this->db->where('user_id', $listing_id); //which row want to upgrade  
       ...
       // print_r($pv); die();
        ...
        foreach ($query->result_array() as $row) {
            ...
            $company_data = array('company_id' => $listing_id,
        ...
        $reviews_counting = $this->db->get_where('company_profile',array('user_id' => $listing_id))->row()->views;
        $avg_rating = $this->db->query("SELECT AVG(rating) as avsg FROM jobs_review WHERE company_id = $listing_id")->result_array();
        $avgr = ($avg_rating[0]['avsg']);
        $job_count = $this->db->select('job_id')->from('jobs_description')->where('company_id', $listing_id)->get()->num_rows();
        $review_count = $this->db->select('id')->from('jobs_review')->where('company_id', $listing_id)->get()->num_rows();
        //echo $job_count; die();
