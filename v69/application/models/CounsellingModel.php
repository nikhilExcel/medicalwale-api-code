...
class CounsellingModel extends CI_Model {
    ...
    public function counselling_list($latitude, $longitude, $user_id) {
        $radius = '5';

        $query = $this->db->query("SELECT id,lat,lng,user_id,doctor_name,about_us,speciality,address,telephone,medical_college,medical_affiliation,charitable_affiliation,awards_recognition,all_24_hrs_available,home_visit_available,qualification,consultation_fee,experience,website,location,days,timing,image,rating,review,( 6371 * acos( cos( radians($latitude) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM counselling_list ");
    ...
    public function counselling_details($user_id, $listing_id) {
        $radius = '5';

        $query = $this->db->query("SELECT id,lat,lng,user_id,doctor_name,about_us,speciality,address,telephone,medical_college,medical_affiliation,charitable_affiliation,awards_recognition,all_24_hrs_available,home_visit_available,qualification,consultation_fee,experience,website,location,days,timing,image,rating,review FROM counselling_list WHERE user_id='$listing_id' limit 1");
    ...
    public function add_review($user_id, $listing_id, $rating, $review, $service) {
        ...
        $this->db->insert('counselling_review', $review_array);
        return array(
            'status' => 201,
            'message' => 'success'
        );
...
public function edit_review($review_id,$user_id, $listing_id, $rating, $review, $service) {
        ...
        $review_array = array(
           ...
           $this->db->update('counselling_review',$review_array);
    ...
    public function review_list($user_id, $listing_id) {
        ...
        if ($review_count > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $post_count = $this->db->select('id')->from('counselling_review_comment')->where('post_id', $id)->get()->num_rows();
    ...
    public function review_like($user_id, $post_id) {
        ...
        $count_query = $this->db->query("SELECT id from counselling_review_likes where post_id='$post_id' and user_id='$user_id'");
        $count = $count_query->num_rows();

        if ($count > 0) {
            $this->db->query("DELETE FROM `counselling_review_likes` WHERE user_id='$user_id' and post_id='$post_id'");
            $like_query = $this->db->query("SELECT id from counselling_review_likes where post_id='$post_id'");
            ...
            $this->db->insert('counselling_review_likes', $doctors_review_likes);
            $like_query = $this->db->query("SELECT id from counselling_review_likes where post_id='$post_id'");
            $total_like = $like_query->num_rows();
            return array(
                'status' => 201,
    ...
    public function review_comment($user_id, $post_id, $comment) {
        ...
        $this->db->insert('counselling_review_comment', $doctors_review_comment);
        $doctors_review_comment_query = $this->db->query("SELECT id from counselling_review_comment where post_id='$post_id'");
    ...
    public function review_comment_like($user_id, $comment_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $count_query = $this->db->query("SELECT id from counselling_review_comment_like where comment_id='$comment_id' and user_id='$user_id'");
        $count = $count_query->num_rows();

        if ($count > 0) {
            $this->db->query("DELETE FROM `counselling_review_comment_like` WHERE user_id='$user_id' and comment_id='$comment_id'");
            $comment_query = $this->db->query("SELECT id from counselling_review_comment_like where comment_id='$comment_id'");
            ...
            $this->db->insert('counselling_review_comment_like', $doctors_review_comment_like);
            $comment_query = $this->db->query("SELECT id from counselling_review_comment_like where comment_id='$comment_id'");
    ...
    public function review_comment_list($user_id, $post_id) {
        ...
        $query = $this->db->query("SELECT counselling_review_comment.id,counselling_review_comment.post_id,counselling_review_comment.comment as comment,counselling_review_comment.date,users.name,counselling_review_comment.user_id as post_user_id FROM counselling_review_comment INNER JOIN users on users.id=counselling_review_comment.user_id WHERE counselling_review_comment.post_id='$post_id' order by counselling_review_comment.id asc");
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id = $row['id'];
                ...
                $like_count = $this->db->select('id')->from('counselling_review_comment_like')->where('comment_id', $id)->get()->num_rows();
                $like_yes_no = $this->db->select('id')->from('counselling_review_comment_like')->where('comment_id', $id)->where('user_id', $user_id)->get()->num_rows();
    ...
    { 
          ...
          $time_slots = $this->db->query("SELECT `id`,`consultation_type`,`clinic_id`,`doctor_id`,`day`,`timeSlot`,`time`,`from_time`,`to_time`,`status` FROM `doctor_clinic_timing` WHERE `doctor_id` = '$doctor_id' AND `consultation_type` = '$consultation_type'");

             count($time_slots->result_array());
             
              
              $day_array = array('Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday');
              
               $dayslot_array = array('Morning','Afternoon','Evening','Night');
        ...
        if($day == $day_array[$i])
        {
              ...
              $time_slott = array();
                    ...
                    foreach($time_slots->result_array() as $get_time)
                            {
             ...
             $time_slot = $get_time['timeSlot'];
             
               $test_slot = array();
              
          
                           
                          if($get_time['timeSlot'] == 'Afternoon')
                          {
                              ...
                              $time_slott[]  = array (
                                  'day'  => $get_time['day'],
                                   'from_day' => $from_time,
                                   'to_time' => $to_time,
                                   'timeslot' => $get_time['timeSlot']
                              );
                              
                          }
                          
                          else if($get_time['timeSlot'] == 'Morning')
                          {
                            ...
                            else if($get_time['timeSlot'] == 'Night')
                          {
                            ...
                            else if($get_time['timeSlot'] == 'Evening')
                          {
