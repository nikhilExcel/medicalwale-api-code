...
class PartnermnoModel extends CI_Model {
    ...
    /*created on 5th may 202 by swapnali waghunde*/
...
// ( 6371 * acos( cos( radians(ms.lat) ) * cos( radians( uo.lat ) ) * cos( radians( uo.lng ) - radians(ms.lng) ) + sin( radians(ms.lat) ) * sin( radians( uo.lat ) ) ) ) AS distance_ms
        ...
        $allOrders_count = $this->db->query("SELECT mo.id FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE mo.`mno_id` = '$mno_id'  AND mo.order_type = '0' $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) group by mo.id ORDER BY mo.created_at desc ")->result_array();
                ...
                if($current_status['status'] == 'accepted'){
                    ...
                    return $return;
                     ...
                     if($accepted == false){
                        ...
                        $res = $this->PartnermnoModel->send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status);
    ...
    // order_details
    /*distributor orders not integrated*/
        ...
        if ($token_status > 0) {
          ...
          //  $orderDetails = $this->db->query("SELECT * FROM `mno_orders` WHERE `invoice_no` LIKE '$invoice_no' AND `mno_id` = '$mno_id' ")->row_array();
            ...
            $this->PartnermnoModel->send_gcm_notification_to_mno($mno_id,$invoice_no,$mno_order_id, $notification_type, $img, $title, $msg);
                ...
                $res = $this->PartnermnoModel->send_gcm_notify_user($title1, $reg_id, $msg1, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status);
...
// order_to_pharmacy
   ...
   /*Swapnali waghunde made changes in function WRT whatsapp messages on 11th May 2020*/
     public function send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile = null, $whatsapp_msg = null,$user_id = null,$listing_id=null,$order_date=null, $order_status=null) {
...
// user_id, order_date, order_status, $listing_id
            ...
            $template_type = 'all';
            $res = $this->PartnermnoModel->send_whatsapp_msg($user_mobile,$template_type,$whatsapp_msg);
        }    
        return true;
    ...
    // dashboard
    /*pending : Not integrated distributor orders : Swapnali waghunde  on 9th may 2020*/
...
// remaining 
    ...
    /*Pendig: distributoe orders not integrated : Swapnali Waghunde on 9th May 2020*/
    ...
    public function order_pickedup($mno_id,$invoice_no,$paid_to_pharmacy,$paid_to_pharmacy_comment,$received_from_pharmacy,$received_from_pharmacy_comment,$received_including_delivery_charges,$paid_payment_method, $received_payment_method){
        ...
        $this->load->model('OrderModel');
        $existingOrder = $this->db->query("SELECT uo.order_date,uo.order_status,uo.bill_url,uo.mno_bill_no,uo.listing_name,uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor, uo.delivery_charges_by_mw,  uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id,  uo.delivery_charges_to_mno, uo.suggested_pharmacy_id, ml.mno_name,  ml.phone,mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
...
//   get_ongoing_order
    ...
    public function get_ongoing_order($mno_id){
        ...
        $type = 3; //ongoing 
        $search = "";
          $listOfOrders = $this->PartnermnoModel->order_list($mno_id, $page_no, $per_page,$type,$search);
   ...
   //     return $data;
    ...
    /*testinf pending : done with distributoe orders integration on 9th May 2020 by Swapnali Waghunde*/
        ...
        if (sizeof($pending_orders) > 0) {
            ...
            if($update_ids != ""){
                ...
                foreach($pending_orders as $po){
                    ...
                    $res = $this->PartnermnoModel->send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type_user,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status);
...
//   reahced_store
    /*Done with Distributoe orders integration By swapnali on 9th may 2020*/
                ...
                if (sizeof($orders) > 0) {
                    ...
                    if($update_ids != ""){
                        ...
                        if($cancelReason['cancel_order'] > 0){
                            ...
                            if($update_inv_ids != ""){
                                 $this->db->query("UPDATE user_order SET order_status = 'Order Cancelled' WHERE $update_inv_ids");
                            }
                        }
    ...
    public function send_whatsapp_msg($destinationPhone, $template_type, $body)
    {
        $is_live = $this->check_environment();
        if(strlen($destinationPhone)== '10' && $template_type != ""  && $body != "") {
            $whatsappBody['destinationPhone'] = "91".$destinationPhone;
            $whatsappBody['template_type'] =$template_type;
            $whatsappBody['body'] = $body;
            $data = json_encode($whatsappBody, JSON_UNESCAPED_UNICODE);
...
order by invoice_no desc ";
    ...
    /*Created by Swapnali Waghunde 27th May 2020 to handle distributoe orders*/
    public function order_details_v1($mno_id, $invoice_no,$mno_order_id = null){
        ...
        if($acceped_query['status'] == 'accepted'){
            ...
            $query = $this->db->query("select mno_bill_no, delivery_note, bill_url,user_id,gst,chc,discount,order_total,action_by,order_id,order_type,delivery_time,listing_id,suggested_pharmacy_id,cancel_reason,action_by,delivery_charge,delivery_charges_by_customer,delivery_charges_by_vendor,delivery_charges_to_mno,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status,rxId,delivered_sign, order_received_by from user_order where `invoice_no` = '$invoice_no' group by invoice_no 
                union
                select null as mno_bill_no, null as delivery_note, null as bill_url,user_id,gst,null as chc,discount,order_total,action_by,order_id,order_type,null as delivery_time,listing_id,null as suggested_pharmacy_id,cancel_reason,action_by,null as delivery_charge,null as delivery_charges_by_customer,null as delivery_charges_by_vendor,null as delivery_charges_to_mno,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status,null as rxId,null as delivered_sign, null as order_received_by from inven_distrub_invoice where `invoice_no` = '$invoice_no' group by invoice_no order by order_id desc
            ");
