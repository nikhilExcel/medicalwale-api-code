...
class PharmacyPartnerModel extends CI_Model
{
    ...
    public function list_address_v2($listing_id,$page_no,$per_page,$search)
    {
        ...
        if($search != "")
        {
            $w = "AND (cname LIKE '%$search%' OR cmobile LIKE '%$search%' OR date_of_birth LIKE '%$search%' OR gender LIKE '%$search%' OR cemail LIKE '%$search%' OR blood_group LIKE '%$search%'
             OR address LIKE '%$search%' OR city LIKE '%$search%' OR state LIKE '%$search%' OR pincode LIKE '%$search%')";
        }
        $query1 = $this->db->query("select * from inventory_customer where user_id='$listing_id' AND cname != '' AND lead = '1' $w GROUP BY cust_user_id order by cname");
        $count1 = $query1->num_rows();
        $query = $this->db->query("select * from inventory_customer where user_id='$listing_id' AND cname != '' AND lead = '1' $w GROUP BY cust_user_id order by cname $limit");
     ...
     public function list_address_lead_v2($listing_id,$page_no,$per_page,$search)
    {
        $offset = $per_page*($page_no - 1);
        $limit = "LIMIT $per_page OFFSET $offset"; 
        $resultpost = array();
...
For more services regarding Health | Beauty | Fitness download Medicalwale Saathi App from Google Play or Apple App Store $applink and stay healthy";
    ...
    */
    public function get_report($user_id,$page_no,$per_page,$search,$order_created_by,$payment_mode,$to_date,$from_date,$order_status,$order_deliver_by){
        ...
        $all_orders_query  = "SELECT uvl.payment_method as payment_method_id,uo.order_date,uo.mno_bill_no,uvl.trans_status,pm.payment_method, uvl.ledger_owner_type, uvl.listing_id_type as payment_received_by, uvl.transaction_id,uvl.credit,uvl.debit,   uo.invoice_no, uo.name, uo.bill_url, uo.order_deliver_by, uo.order_status FROM $from  WHERE uo.listing_id IN ($branches_str) $search_where $search_created_by $search_payment_mode $date_search $search_order_status $search_order_deliver_by GROUP BY uo.invoice_no ";
