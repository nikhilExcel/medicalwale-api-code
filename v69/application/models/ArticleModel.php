...
class ArticleModel extends CI_Model {
    ...
    public function article_list($category_id, $user_id, $page) {
     ...
     public function article_list_new($user_id, $page,$type,$sub_type,$sort) {
        $ad_array='0,';
        ...
        if($sort=="3"){
            $sort_artical='posted DESC'; 
             $sort_surgery='created_at DESC'; 
        }else if($sort=="4"){
            $sort_artical='posted ASC';
            $sort_surgery='created_at ASC'; 
        }else if($sort=="1"){
            $sort_artical='article_title ASC'; 
            $sort_surgery='surgery_name ASC'; 
        }else if($sort=="2"){
            $sort_artical='article_title DESC'; 
            $sort_surgery='surgery_name DESC'; 
        }else if($sort=="") {
          $sort_artical='posted DESC';
          $sort_surgery='created_at DESC'; 
        }
        ...
        if($sub_type!=""){
            ...
            $wher.="and fIND_IN_SET('$sub_type',cat_id)";
        }
        
         if($sub_type!=""){
            
            $where.="where fIND_IN_SET('$sub_type',subcategory_id)";
        }
        ...
        //echo "select('language')->from('users')->where('id', $user_id)";
        $lang=$query_lang->language;
     //     echo "SELECT article.id,article.views, article.cat_id as cat_id1, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='app_article' $wher and  article.language='$lang' order by $sort_artical limit $start,$limit";
      
      if($type=='1'){
        $query = $this->db->query("SELECT article.id,article.views, article.cat_id as cat_id1, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='app_article' $wher and  article.language='$lang' order by $sort_artical limit $start,$limit");
      }
    if($type=='4'){
         $query = $this->db->query("SELECT article.id,article.views, article.cat_id as cat_id1, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='home_remedies' $wher and  article.language='$lang' order by $sort_artical limit $start,$limit");
    }if($type=='3'){ 
        $query = $this->db->query("SELECT article.id,article.views, article.cat_id as cat_id1, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='tips' $wher  and article.language='$lang' order by $sort_artical limit $start,$limit");
    }if($type=='2'){
        $query = $this->db->query("SELECT article.id,article.views, article.cat_id as cat_id1, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='skin_hair' $wher  and  article.language='$lang' order by $sort_artical limit $start,$limit");
    }
    if($type=='5'){
        $query = $this->db->query("SELECT surgery_master.id,surgery_master.surgery_name,surgery_master.surgery_image,surgery_master.created_at,surgery_master.subcategory_id,surgery_master_category.category_name FROM `surgery_master` Inner join surgery_master_category  on surgery_master.subcategory_id=surgery_master_category.id $where order by $sort_surgery  limit $start,$limit");
    }
    if($type=='6'){
        $query = $this->db->query("SELECT kids_disorder_master.id,kids_disorder_master.disease_name,kids_disorder_master.surgery_image,kids_disorder_master.created_at,kids_disorder_master.subcategory_id,kids_disorder_master_category.category_name FROM `kids_disorder_master` Inner join kids_disorder_master_category  on kids_disorder_master.subcategory_id=kids_disorder_master_category.id $where order by $sort_surgery  limit $start,$limit");
    }
    
      //  echo "SELECT article.id,article.views, article.cat_id, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article.cat_id='$category_id' AND article.language='$lang' limit $start,$limit";
        $count = $query->num_rows();
        ...
        if ($count > 0) {
            ...
            if($type=="5"){
                   ...
                   foreach ($query->result_array() as $row) {
                    ...
                    $surgery_id = $row['id'];
                    $surgery_title = $this->RemoveBS($row['surgery_name']);
                   /* $surgery_description = $row['surgery_desc'];
                    $surgery_pre_procedure = $row['pre_procedure'];
                    $surgery_conditions = $row['conditions'];
                    $surgery_during_procedure = $row['during_procedure'];
                    $surgery_post_procedure = $row['post_procedure'];
                    $surgery_instruction = $row['instruction'];
                    $surgery_risk_complication = $row['risk_complication'];
                    $surgery_other_details = $row['other_details'];*/
                    $surgery_image = str_replace("itâ€™s", "it's", $row['surgery_image']);
                    $src_file_name = $row['surgery_image'];
                    $surgery_date = $row['created_at'];
                    $author = 'Medicalwale.com';
                    $surgery_image = 'https://medicalwale.s3.amazonaws.com/images/SurgeriesImages/' . $surgery_image;
                    $category_id = $row['subcategory_id'];
                    $cat_name = $row['category_name'];
                    $category_name = (str_replace(' ', '-', strtolower($cat_name)));
                    $surgery_title_final = (str_replace(' ', '-', strtolower($surgery_title)));
                         ...
                         $resultpost[] = array(
                        'article_id' => $surgery_id,
                        'category_id' => $category_id,
                        'article_title' => $surgery_title,
                        'article_description' => "",
                        'short_desc' => $surgery_title,
                        'article_image' => $surgery_image,
                        'type'=>$image_ext,
                        'article_date' => $surgery_date,
                        'author' => $author,
                        'share_url' => "",
                        'like_count' => 0,
                        'like_yes_no' => 0,
                ...
                foreach ($query->result_array() as $row) {
                    ...
                    $surgery_id = $row['id'];
                    $surgery_title = $this->RemoveBS($row['disease_name']);
                   /* $surgery_description = $row['surgery_desc'];
                    $surgery_pre_procedure = $row['pre_procedure'];
                    $surgery_conditions = $row['conditions'];
                    $surgery_during_procedure = $row['during_procedure'];
                    ...
                    $surgery_date = $row['created_at'];
                    $author = 'Medicalwale.com';
                    $surgery_image = 'https://medicalwale.s3.amazonaws.com/images/Kids_Disorders/' . $surgery_image;
                    $category_id = $row['subcategory_id'];
                    $cat_name = $row['category_name'];
                    $category_name = (str_replace(' ', '-', strtolower($cat_name)));
                    $surgery_title_final = (str_replace(' ', '-', strtolower($surgery_title)));
                ...
                $article_id = $row['id'];
                $article_title = $this->RemoveBS($row['article_title']);
                $article_description = $row['article_description'];
     ...
     public function article_sub_cat($user_id,$type) {
        $ad_array='0,';
        //Pagination


        if($type=='1'){
            $query = $this->db->query("SELECT article.id,article.views, article.cat_id, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='app_article' group by category_name ");
        }if($type=='4'){
            $query = $this->db->query("SELECT article.id,article.views, article.cat_id, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='home_remedies' group by category_name  ");
        }if($type=='3'){ 
            $query = $this->db->query("SELECT article.id,article.views, article.cat_id, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='tips' group by category_name ");
        }if($type=='2'){
            $query = $this->db->query("SELECT article.id,article.views, article.cat_id, article.article_title, article.article_title_hindi, article.article_description, article.article_description_hindi, article.image, article.posted, article.is_active, article.updated_date ,article_category.name AS category_name,article_category.type FROM article INNER JOIN article_category ON article.cat_id=article_category.id WHERE article_category.type='skin_hair' group by category_name ");
        }if($type=='5'){
           $query = $this->db->query("SELECT surgery_master.id,surgery_master.surgery_name,surgery_master.surgery_image,surgery_master.created_at,surgery_master.subcategory_id,surgery_master_category.category_name FROM `surgery_master` Inner join surgery_master_category  on surgery_master.subcategory_id=surgery_master_category.id group by category_name ");
        }if($type=='6'){
        $query = $this->db->query("SELECT kids_disorder_master.id,kids_disorder_master.disease_name,kids_disorder_master.surgery_image,kids_disorder_master.created_at,kids_disorder_master.subcategory_id,kids_disorder_master_category.category_name FROM `kids_disorder_master` Inner join kids_disorder_master_category  on kids_disorder_master.subcategory_id=kids_disorder_master_category.id group by category_name ");
    }
            ...
            if($type=='5'){
                ...
                foreach ($query->result_array() as $row) {
                        ...
                        $cat_id=$row['subcategory_id'];
                            $query = $this->db->query("SELECT * FROM surgery_master_category WHERE surgery_master_category.id='$cat_id' ")->row_array();
                ...
                foreach ($query->result_array() as $row) {
                        ...
                        $cat_id=$row['subcategory_id'];
                            $query = $this->db->query("SELECT * FROM kids_disorder_master_category WHERE kids_disorder_master_category.id='$cat_id' ")->row_array();
                ...
                $cat_id=(explode(",",$cat_id));    
                       foreach($cat_id as $c){
                            $query = $this->db->query("SELECT id,name,article_category.type FROM article_category WHERE article_category.id='$c' ")->row_array();
...
public function article_detail($user_id, $post_id) {
    ...
    //get article details
    public function article_details($user_id, $post_id,$type) {
        $share_url ="";
        $query_lang = $this->db->select('language')->from('users')->where('id', $user_id)->get()->row();
        $lang=$query_lang->language;
    ...
    public function your_story($user_id, $title, $description) {
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        /*Added by swapnali - resolved db error*/ 
        if($title == null){
            $title = "";
        } 
        if($description == null){
    ...
    public function surgery_details($user_id,$surgey_id){
        
          $sgquery = $this->db->query("SELECT * FROM surgery_master WHERE id = '$surgey_id'");
          ...
          return $hospital_surgery;
        
        }
    
    
    
      public function article_select($user_id) 
   {
        
        
        $sql = "SELECT * FROM article_select";
	 ...
	 public function article_home_select($user_id) 
   {
            ...
            foreach ($query->result_array() as $row) 
                    {
                      ...
                      $sort=$row['sort'];
$sub_type="";
                        $data_array=$this->article_list_new($user_id,1,$type,$sub_type,$sort);
                         $resultpost[] = array('id'=> $id,
                                                 'image' => $image,
                                                 'name' => $name,
                                                 'type'=>$type,
