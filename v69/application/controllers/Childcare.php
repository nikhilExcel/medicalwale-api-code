...
public function childprofile_update_web() {
    ...
    public function babysitter_add_review() {
     ...
     public function edit_babysitter_add_review() {
        $this->load->model('ChildcareModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array(
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['rating'] == "" || $params['review'] == "" || $params['service'] == "") {
                        ...
                        $review = $params['review'];
                        $service = $params['service'];
                        $resp = $this->ChildcareModel->edit_babysitter_add_review($review_id,$user_id, $listing_id, $rating, $review, $service);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function dai_nanny_add_review() {
     ...
     public function edit_dai_nanny_add_review() {
        $this->load->model('ChildcareModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array(
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['rating'] == "" || $params['review'] == "" || $params['service'] == "") {
                        ...
                        $review = $params['review'];
                        $service = $params['service'];
                        $resp = $this->ChildcareModel->edit_dai_nanny_add_review($review_id,$user_id, $listing_id, $rating, $review, $service);
                    }
                    simple_json_output($resp);
                }
            }
