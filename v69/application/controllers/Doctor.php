...
class Doctor extends CI_Controller
{
    ...
    public function doctor_list()
    {
     ...
     public function doctor_list_v1()
    {
        $this->load->model('DoctorModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['latitude'] == "" || $params['longitude'] == "" || $params['category_id'] == " "   ) {
                        ...
                        $resp        = $this->DoctorModel->doctor_list_v1($latitude, $longitude, $user_id, $category_id,$page,$keyword,$type);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['rating'] == "" || $params['review'] == "" || $params['service'] == "") {
                        ...
                        $review = $params['review'];
                        $service = $params['service'];
                        $resp = $this->DoctorModel->edit_review($review_id,$user_id, $listing_id, $rating, $review, $service);
                    }
                    simple_json_output($resp);
                }
            }
   ...
   /* public function add_bookings()
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                $coupon_id = 0;
                        ...
                        $resp               = $this->DoctorModel->add_bookings($user_id, $listing_id, $clinic_id, $booking_date, $booking_time, $user_name, $user_mobile, $from_time, $to_time, $user_email, $user_gender, $is_user, $is_patient, $patient_id,$allergies, $heradiatry_problem, $description, $relationship, $date_of_birth, $connect_type,$coupon_id,$status);
                    }
                    json_outputs($resp);
                }
            }
        ...
        if ($method != 'POST') {
            json_output(400, array(
              ...
              if ($this->input->post('user_id') == "" || $this->input->post('listing_id') == "" || $this->input->post('clinic_id') == "") {
                    ...
                    else if ($this->input->post('listing_type') == '' && !is_numeric($this->input->post('listing_type'))) {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter listing_type'
                        );
                    } 
                    else if ($this->input->post('type') == '' && !is_numeric($this->input->post('type'))) {
                        $resp = array(
                            'status' => 400,
            ...
            $resp               = $this->DoctorModel->add_bookings_v2($user_id, $listing_id, $clinic_id, $booking_date, $booking_time, $user_name, $user_mobile, $from_time, $to_time, $user_email, $user_gender, $is_user, $is_patient, $patient_id,$allergies, $heradiatry_problem, $description, $relationship, $date_of_birth, $connect_type,$coupon_id,$status,$listing_type,$type);
     ...
     public function user_read_slot_v2()
    {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['doctor_id'] == "" || $params['consultation_type'] == "") {
                        ...
                        if(array_key_exists('listing_type', $params))
                            {
                              ...
                              if(array_key_exists('hospital_id', $params))
                            {
                        ...
                        if(empty($hospital_id))
                            {
                        $resp              = $this->DoctorModel->user_read_slot_v2($doctor_id, $consultation_type,$agent,$listing_type,$hospital_id);
                            }
                            else
                            {
                                $resp              = $this->DoctorModel->user_read_slot_v4($doctor_id, $consultation_type,$agent,$listing_type,$hospital_id);
                            }
                    }
                    json_outputs($resp);
                }
      ...
      public function user_read_slot_v3()
    {
        $this->load->model('DoctorModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($params['doctor_id'] == "" || $params['consultation_type'] == "" || $params['booking_id'] == "" || $params['clinic_id'] == "") {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter all fields'
                        );
                        ...
                        $resp              = $this->DoctorModel->user_read_slot_v3($doctor_id, $consultation_type,$agent,$clinic_id,$booking_id);
                ...
                if ($response['status'] == 200) {
                   ...
                   /*  $order_info = $this->db->select('phone,token,token_status,agent')->from('users')->where('id', $user_id)->get()->row();
                    ...
                    $post_data = array(
                        ...
                        'From' => $from,
                        'CallerId' => "02248933722",
                        'Url' => 'http://my.exotel.com/aegishealthsolutions/exoml/start_voice/220962'
                    );
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                   ...
                   $exotel_sid = "aegishealthsolutions"; 
            $exotel_token = "a642d2084294a21f0eed3498414496229958edc5"; 
            $response1 = file_get_contents("https://$exotel_sid:$exotel_token@api.exotel.com/v1/Accounts/$exotel_sid/Calls/$sid");

                 
                print "Response = ".print_r($response1); 
    ...
    public function coupon_code() {
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                $response = $this->DoctorModel->auth();
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($params['user_id'] == "" || $params['coupon'] == "" || $params['listing_id'] == "" || $params['listing_type'] == "") {
                        ...
                        $resp = $this->DoctorModel->coupon_code($user_id,$coupon,$listing_id,$listing_type,$amount,$card_type);
                    }
                  simple_json_output($resp);
                }
            }
    ...
    public function add_re_bookings()
    {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['patient_id'] == "" || $params['doctor_id'] == "" || $params['clinic_id'] == "" ) {
                        ...
                        $status      = $params['booking_status']; 
                        $listing_type=$params['listing_type'];
                         $resp               = $this->DoctorModel->add_re_bookings($patient_id, $doctor_id, $clinic_id, $booking_date, $booking_time, $from_time, $to_time, $connect_type,$status,$appointment_id,$description,$listing_type);
                    }
                    json_outputs($resp);
     ...
     public function doctor_approval() {
                ...
                if ($response['status'] == 200) {
                    ...
                    $booking_id = $params['booking_id'];
                     if ($params['doctor_id'] == "" || $params['booking_status'] == "" || $params['booking_id'] == "" ) {
                        ...
                        $resp = $this->DoctorModel->doctor_approval($doctor_id, $booking_status, $booking_id);
                    }
                }
      ...
      public function doctor_random_cat() {
        $this->load->model('DoctorModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                     ...
                     if ($params['user_id'] == "" || $params['lat'] == "" || $params['lng'] == "" || $params['type']== "" ) {
                        $resp = array('status' => 400, 'message' => 'please enter fields');
                    } else {
                        $user_id = $params['user_id'];
                        $lat     = $params['lat'];
                        $lng     = $params['lng'];
                        $type    = $params['type'];
                        $resp = $this->DoctorModel->doctor_random_cat($user_id,$lat,$lng,$type);
                    }
                }
     ...
     public function doctor_problem() {
        $this->load->model('DoctorModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                     ...
                     if ($params['user_id'] == ""  ) {
                        ...
                        $resp = $this->DoctorModel->doctor_problem($user_id);
                    }
                }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $resp        = $this->DoctorModel->view_doctor_profile($user_id,$view_yourself,$self_user_id);
                    }
                    json_outputs($resp);
                }
            }
   ...
   public function view_doctor_follower()
    {
        $this->load->model('DoctorModel');
         $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    elseif ($params['user_id'] == "") {
                        $resp = array('status' => 400, 'message' => 'please enter User ID fields');
                    } 
                     elseif ($params['listing_id'] == "") {
                        $resp = array('status' => 400, 'message' => 'please enter Doctor ID fields');
                    }
                     elseif ($params['type'] == "") {
                        $resp = array('status' => 400, 'message' => 'please enter Type fields');
                    }
                    else {
                        $user_id = $params['user_id'];
                        $doctor_id = $params['listing_id'];
                        $type = $params['type'];
                        $page_no = $params['page_no'];
            		    $per_page = $params['per_page'];
                        $resp        = $this->DoctorModel->view_doctor_follower($user_id,$doctor_id,$type,$page_no, $per_page);
                    }
                    json_outputs($resp);
                }
            }
