...
class Ofw extends CI_Controller
{
    ...
    public function menstural_cycle_notification_on_off()
    {
        $this->load->model('OfwModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($params['user_id'] == "" || $params['profile_id'] == "" || $params['is_notification'] == "") {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter fields'
                        );
                        ...
                        $resp       = $this->OfwModel->menstural_cycle_notification_on_off($user_id,$profile_id,$is_notification);
                    }
                    simple_json_output($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $resp  = $this->OfwModel->add_menstrual_cycle_data($user_id, $name, $relationship, $start_period_day, $cycle_length, $total_period_day, $born_of_year, $tutual_cycle, $tracking_resoan, $profile_id, $history_id,$pcod,$mobile);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function add_menstural_cycle_profile()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($user_id == "" || $name=="" || $relationship=="") {
                ...
                $resp = $this->OfwModel->add_menstural_cycle_profile($user_id, $relationship, $name, $age, $height, $weight, $sleep_cycle, $birth_control, $image);
            }
            simple_json_output($resp);
        }
    }
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['profile_id'] == "") {
                        ...
                        $date       = $params['date'];
          	            $html_content =  '<link rel="stylesheet" href="'.base_url().'assets/bootstrap.min.css">';
         	            $html_content .= '<link rel="stylesheet" href="'.base_url().'assets/style.css">';
         	          echo  $html_content .= $this->OfwModel->fetch_menstural_cycle_details($profile_id,$user_id,$date);
         	            $this->pdf_dom->set_paper("A4", "portrait"); 
    		            $this->pdf_dom->set_option('isHtml5ParserEnabled', TRUE);
    		            $this->pdf_dom->load_html($html_content);
    		            $this->pdf_dom->render();
        	            ...
        	            include('s3_config.php');
        	            $file_path = 'images/MenstrualCycle/pdf/' . $pdfname;
                        ...
                        $file_link ='https://d2c8oti4is0ms3.cloudfront.net/images/MenstrualCycle/pdf/'.$pdfname;
                    }
