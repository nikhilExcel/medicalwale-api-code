     ...
     public function create_ledger() { 
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                $user_id = array_key_exists('user_id',$params ) ? $user_id = $params['user_id'] : "" ;
                $invoice_no = array_key_exists('invoice_no',$params ) ? $params['invoice_no'] : "";
                $ledger_owner_type = array_key_exists('ledger_owner_type',$params ) ? $params['ledger_owner_type'] : "" ;
                $listing_id = array_key_exists('listing_id',$params ) ? $params['listing_id'] : "" ;
                $listing_id_type = array_key_exists('listing_id_type',$params ) ? $params['listing_id_type'] : "" ;
                $credit = array_key_exists('credit',$params ) ? $params['credit'] : "";
                $debit = array_key_exists('debit',$params ) ? $params['debit'] : "";
                $payment_method = array_key_exists('payment_method',$params ) ? $params['payment_method'] : "";
                $user_comments = array_key_exists('user_comments',$params ) ? $params['user_comments'] : "";
                $mw_comments = array_key_exists('mw_comments',$params ) ? $params['mw_comments'] : "";
                $vendor_comments = array_key_exists('vendor_comments',$params ) ? $params['vendor_comments'] : "" ;
                $order_type = array_key_exists('order_type',$params ) ? $params['order_type'] : "" ;
                $transaction_of = array_key_exists('transaction_of',$params ) ? $params['transaction_of'] : "" ;
                $trans_status = array_key_exists('trans_status',$params ) ? $params['trans_status'] : "";
                $transaction_id =  array_key_exists('transaction_id',$params ) ? $params['transaction_id'] : "";
                $transaction_date = array_key_exists('transaction_date',$params ) ? $params['transaction_date'] : "" ;
                $vendor_id = array_key_exists('vendor_id',$params ) ? $params['vendor_id'] : $listing_id_type ;
                $submitted_by = array_key_exists('submitted_by',$params ) ? $params['submitted_by'] : "";
                $accepted_by = array_key_exists('accepted_by',$params ) ? $params['accepted_by'] : "" ;
               ...
               if($user_id == "" || $invoice_no == "" ||   ($credit  == "" && $debit  == "" )  ||  $order_type == "" || $order_type > 10 || $order_type < 1 || $transaction_of == ""){
            
                    $resp = array(
                        'status' => 400,
                        'message' => 'Please enter user_ledger : it should be 1 or 0 if 1 means user ledger and if 0 means vendor ledger, user_id, invoice_no : it should be invoice_no from user_order table or booking_id from booking_master / doctor_booking_master / hospital_booking_master / pestcontrol_booking_master , ledger_owner_type i.e. user_id type, listing_id, listing_id_type,  either credit or debit amount ,  payment_method, trans_status should be 0: filed; 1: success ; 2 : pending , order_type should be 1 : service ; 2 : appointment ; 3 : doctor appointment; 4 : hospital appointment; 5 : pestcontrol  appointment ;6: BHC ;  8 :missbelly; 9: points to ledger convert; 10 => distributor, transaction_of where 1: package; 2: order amount; 3: delivery charges; 4: ledger balance; 5 : points '
                    );
