...
// 	get_quiz_questions
    public function get_quiz_questions($user_id,$level_id){
       ...
       // $hintsNlevel = $this->db->query("SELECT * FROM `quiz_users_info` WHERE `user_id` = '$user_id' AND `level` = '$level_id'")->row_array();
        ...
        if(!empty($unlocked) && sizeof($hintsNlevel) > 0){
            ...
            $rows = $this->db->query("SELECT qq.*, ql.level_name  FROM `quiz_questions` as qq LEFT JOIN `quiz_levels` as ql ON (qq.`level` = ql.`level_id`) WHERE qq.`level` = '$level_id' AND qq.`status` = 1 ORDER BY RAND()")->result_array();
            $count = sizeof($rows);
  ...
  public function sponsored_ads_list()
   ...
   /*Created by Swapnali Waghunde on 18th May 2020 to add pagination in score board API*/
   // quiz_leader_board
    public function quiz_leader_board_v1($user_id,$level_id,$page_no,$per_page){
        $user_details = $data = array();
        $level_name = '';
        $limit = "";
        if($page_no > 0 && $per_page > 0){
            ...
            $rank = $page_no = $per_page = 0;
        }
        $query = "SELECT u.id, u.name, m.source , ql.level_name,  qui.* , qui.points/qui.attempts as ratio FROM `quiz_users_info` as qui LEFT JOIN users as u ON( qui.`user_id` = u.id ) LEFT join media as m ON (u.avatar_id = m.id) LEFT JOIN `quiz_levels` as ql ON (qui.`level` = ql.level_id )   WHERE `level` = '$level_id'  and qui.`attempts` != 0 order by ratio DESC  ";
        
        $user_info  = $this->db->query($query . $limit)->result_array();
        $user_info_count  = $this->db->query($query)->num_rows();
        ...
        if($user_info_count > 0){
            $per_page = $per_page > 0 ? $per_page : sizeof($user_details) ;
            $page_no = $page_no > 0 ? $page_no : 1;
            $last_page = strval(ceil($user_info_count/$per_page));
            $data['data_count'] = strval($user_info_count);
            $data['current_page_count'] = strval(sizeof($user_details));
