    ...
    public function check_auth_client()
    {
     ...
     public function get_url_version() {
        //$api_url = "http://sandboxapi.medicalwale.com/v52/";
         $api_url = "https://live.medicalwale.com/ios20/";
        return $api_url;
    }
            ...
            if ($count9 > 0) {
                foreach ($query9->result_array() as $row) {
                    ...
                    if ($booking_id != "" || $booking_id!="0") {
                        $url = $this->get_pdf_version().$booking_id . ".pdf";
                    } else {
                        $url = "";
                    }
...
$count = $query->num_rows();
    ...
    /*Changes by swapnali Waghunde on 14th May 2020: Added mno_tracking key in response (only if vendor type is 13) */
                        ...
                        $rxId;
                                ...
                                $tracker = $this->All_booking_model->pharmacy_tracker($user_id, $invoice_no);
                                 ...
                                 if($listing_type=="38")
                                   ...
                                   {
                                        /*Added by Swapnali Waghunde on 14th MAy 2020 to Get mno details */
                                        $get_mno_info = $this->db->query("SELECT * FROM mno_orders WHERE invoice_no = '$invoice_no' and ongoing = '1' and mno_id > '0'")->result_array();
                                       ...
                                       $resultpost[] = array(
                                        ...
                                        "life_qty"=>"",
                                        "urgent"=>"",
                                        "mno_tracking" => sizeof($get_mno_info) > 0 ? true : false
                                    ); 
                                   }
                                }
                           } 
...
else
{
    $rxId;
            ...
            /*Added by Swapnali Waghunde on 14th MAy 2020 to Get mno details */
            $get_mno_info = $this->db->query("SELECT * FROM mno_orders WHERE invoice_no = '$invoice_no' and ongoing = '1' and ongoing = '1' and mno_id > '0'")->result_array();
            ...
            if ($count2 > 0) {
                ...
                $resultpost234 = array();
                    ...
                    if($vid=="39")
                    {
                        ...
                        $querys1 = $this->db->query("SELECT booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id, booking_master.user_name, booking_master.user_mobile, booking_master.user_email, booking_master.user_gender, booking_master.branch_id,booking_master.status, booking_master.vendor_id, booking_master.booking_date, booking_master.category_id, booking_master.trail_booking_date, booking_master.trail_booking_time,booking_master.trainer_package_id,booking_master.trainer_id, booking_master.joining_date,booking_master.booking_type,booking_master.sub_booking_type FROM booking_master WHERE booking_master.user_id='$user_id' and booking_master.booking_id='$invoice_no' ORDER BY booking_master.id DESC");
                    ...
                    if($row1['sub_booking_type'] == "2")
                    {
                        $package_ida = explode(',',$package_id);
                        $package_id1 = implode("','",$package_ida);
                        
                        $query12 = $this->db->query("SELECT d.*,m.name FROM dental_services_offered as d LEFT JOIN dentist_services_master as m ON m.id=d.category_id  where d.id IN ('$package_id1') ");
                        $row12=$query12->result();
                        $count12 = $query12->num_rows();
                          if(!empty($count12))
                            {
                                foreach ($row12 as $mser1) {
                                    $ids = $mser1->id;
                                    $service_name = $mser1->service_name;
                                    $category_id = $mser1->category_id;
                                    $category_name = $mser1->name;
                                    $price = $mser1->price;
                ...
                if($vid=="13" || $vid=="38" || $vid=="45" || $vid=="44")
                    {
                      ...
                      $dis = 0;
                        /*Pharmacy*/
                        $query = $this->db->query("SELECT user_order.*,medical_stores.medical_name,medical_stores.profile_pic FROM user_order LEFT JOIN medical_stores on user_order.listing_id=medical_stores.user_id WHERE (user_order.listing_type='13' or user_order.listing_type='38' ) AND user_order.user_id='$user_id' and user_order.bachat_service_id='0' group by user_order.invoice_no order by user_order.order_date DESC  LIMIT $start, $limit ");
                        $count = $query->num_rows();
                        if ($count > 0) 
                           { 
            foreach ($query->result_array() as $row) {
           ...
           $query2 = $this->db->query("select doctor_booking_master.booking_id, doctor_booking_master.booking_date,doctor_booking_master.listing_id,doctor_booking_master.user_id, doctor_booking_master.listing_id,doctor_booking_master.clinic_id, doctor_prescription.id as prescription_id, doctor_booking_master.booking_time,doctor_booking_master.status,doctor_booking_master.consultation_type, doctor_clinic.clinic_name,users.name as patient_name, doctor_clinic.address,doctor_clinic.image,doctor_clinic.state,doctor_clinic.city,doctor_clinic.pincode, doctor_clinic.consultation_charges,doctor_clinic.contact_no,dl.doctor_name from doctor_booking_master LEFT JOIN doctor_prescription ON (doctor_booking_master.booking_id = doctor_prescription.booking_id AND doctor_booking_master.listing_id = doctor_prescription.doctor_id AND doctor_booking_master.clinic_id = doctor_prescription.clinic_id)  LEFT JOIN doctor_clinic on (doctor_booking_master.clinic_id = doctor_clinic.id) LEFT JOIN users ON (doctor_booking_master.user_id = users.id) LEFT JOIN doctor_list as dl ON(dl.user_id=doctor_booking_master.listing_id) where doctor_booking_master.user_id = '$user_id'  and doctor_booking_master.vendor_type = '5'  ORDER BY doctor_booking_master.booking_date DESC LIMIT $start, $limit");
            ...
            if ($count2 > 0) {
                ...
                $resultpost = array();
                 ...
                 if($vid=="10" || $vid=="31")
                    {
                       $count_query4 = $this->db->query("SELECT lb.*,bm.vendor_id,bm.status,lab_center.lab_name,lab_center.profile_pic from booking_master as bm LEFT JOIN lab_booking_details as lb ON(bm.booking_id = lb.booking_id) LEFT JOIN lab_center ON(bm.listing_id = lab_center.user_id) where bm.user_id='$user_id' and (bm.vendor_id='10' or bm.vendor_id='31') and bm.bachat_service_id='0' GROUP BY bm.booking_id order by bm.booking_id desc LIMIT $start, $limit");
                        $lab_booked4       = $count_query4->num_rows();
                        if ($lab_booked4 > 0) 
                        {
                          foreach ($count_query4->result_array() as $Lbooked) 
                ...
                if($vid=="34")
                   {
                        ...
                        $pro_list = array();
                        $results = $this->db->query("SELECT vd.delivery_by_medicalwale, uo.* FROM `user_order` as uo left join vendor_details_hm as vd on (uo.`listing_id` = vd.v_id) WHERE uo.`user_id` = '$user_id' AND  uo.`listing_type`='34' and uo.bachat_service_id='0'  ORDER BY order_date DESC LIMIT $start, $limit");
...
usort($resultpost, "cmp");   
           ...
           $query2 = $this->db->query("select * from rides where user_id = '$user_id' and bachat_service_id='0'  ORDER BY ride_id DESC LIMIT $start, $limit");
            ...
            if ($count2 > 0) {
             ...
             if($vid=="39")
                    {
                         $user_order_query = "SELECT booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id, booking_master.user_name, booking_master.user_mobile, booking_master.user_email, booking_master.user_gender, booking_master.branch_id,booking_master.status, booking_master.vendor_id, booking_master.booking_date, booking_master.category_id, booking_master.trail_booking_date, booking_master.trail_booking_time,booking_master.trainer_package_id,booking_master.trainer_id, booking_master.joining_date,booking_master.booking_type,booking_master.sub_booking_type FROM booking_master WHERE booking_master.user_id='$user_id' AND booking_master.vendor_id='39'  and booking_master.bachat_service_id='0' ORDER BY booking_master.id DESC LIMIT $start, $limit"; 
                        // die(); 
  ...
  public function subscription_bachat_booking($user_id,$page)
    {
        $image='';
        $limit = 10;
        $start = 0;
        ...
        $sql1=$this->db->query("SELECT user_id as uid, listing_type as vid, order_date as date, invoice_no as inv_no from user_order where user_id='$user_id' and bachat_service_id!='0' GROUP BY invoice_no
                                UNION all
                                SELECT user_id as uid, vendor_id as vid,booking_date as date, booking_id as inv_no  from booking_master where user_id='$user_id' and bachat_service_id!='0'
                                UNION all
                                SELECT user_id as uid, vendor_type as vid,Create_time as date, ride_id as inv_no  from rides where user_id='$user_id' and bachat_service_id!='0'
                                ");
        $count12 = $sql1->num_rows();
        
        
    
        $sql=$this->db->query("SELECT user_id as uid, listing_type as vid, order_date as date, invoice_no as inv_no from user_order where user_id='$user_id' and bachat_service_id!='0' GROUP BY invoice_no
                                    UNION all
                                    SELECT user_id as uid, vendor_id as vid,booking_date as date, booking_id as inv_no  from booking_master where user_id='$user_id' and bachat_service_id!='0'
                                    UNION all
                                    SELECT user_id as uid, vendor_type as vid,Create_time as date, ride_id as inv_no  from rides where user_id='$user_id' and bachat_service_id!='0'
            ...
            if($vid=="13" || $vid=="38" || $vid=="44")
                {
                  ...
                  $order_amount =  $gst_rs = $gst_per = $chc = $grand_total = "";
                    $dis = 0;
                        $query = $this->db->query("SELECT user_order.*,medical_stores.medical_name,medical_stores.profile_pic FROM user_order LEFT JOIN medical_stores on user_order.listing_id=medical_stores.user_id WHERE (user_order.listing_type='13' or user_order.listing_type='38' ) AND user_order.user_id='$user_id_t' and  user_order.invoice_no='$invoice_no' group by user_order.invoice_no order by user_order.order_id DESC");
                        $count = $query->num_rows(); 
                        ...
                        if ($count > 0) 
                                ...
                                foreach ($query->result_array() as $row) {
                                   ...
                                   /*Added by swapnali on 20th nov 2k19 at 5pm*/
                                    ...
                                    $action_by = $row['action_by'];
                                    $rxId = $row['rxId'];
                                    $bachat_service_id=$row['bachat_service_id'];
                                    if($vid=="13" || $vid=="44"){
                                        $delivery_charge = $delivery_charges_by_customer;
                                    } 
                                 ...
                                 if($listing_type=="38")
                                   {
                                    ...
                                    $resultpost[] = array(
                                        ...
                                        "urgent"=>"",
                                        "mno_tracking" => false,
                                        "bachat_service_id"=>$bachat_service_id
                                           );
                                    }
...
else
{
    $rxId;
           ...
           $resultpost[] = array(
               ...
               if($vid=="10" || $vid=="31")
                    {
                       $count_query4 = $this->db->query("SELECT lb.*,bm.vendor_id,bm.status,bm.bachat_service_id from booking_master as bm LEFT JOIN lab_booking_details as lb ON(bm.booking_id = lb.booking_id) where bm.user_id='$user_id_t' and bm.booking_id ='$invoice_no' and (bm.vendor_id='10' or bm.vendor_id='31') order by bm.id desc");
                        $lab_booked4       = $count_query4->num_rows();
                        if ($lab_booked4 > 0) 
                        {
                          foreach ($count_query4->result_array() as $Lbooked) 
                ...
                'image'=>'',
                "listing_type"=> "31",
                "bachat_service_id"=>$Lbooked['bachat_service_id']
                );
                                 }
                                   }  
                                else
        ...
        $test_count  = $test_query->num_rows();
                ...
                if($vid=="34")
                   {
                        ...
                        $pro_list = array();
                        $results = $this->db->query("SELECT vd.delivery_by_medicalwale, uo.* FROM `user_order` as uo left join vendor_details_hm as vd on (uo.`listing_id` = vd.v_id) WHERE uo.`user_id` = '$user_id_t' AND  uo.`listing_type`='34'  and uo.invoice_no='$invoice_no' ORDER BY order_id DESC");
           ...
           $query2 = $this->db->query("select * from rides where user_id = '$user_id_t' and ride_id='$invoice_no' ORDER BY ride_id DESC");
            ...
            if ($count2 > 0) {
                foreach ($query2->result_array() as $row2) {
                    ...
                    $dirver_id=$row2['driver_id'];
                          $payment_status=$row2['payment_status'];
                          $bachat_service_id=$row2['bachat_service_id'];
                     $amb="";
                    $query1 = $this->db->query("select * from ambulance_fare where driver_user_id = '$dirver_id'");
                    $count1 = $query1->num_rows();
                    if ($count1 > 0) {
                    ...
                    if($vid=="39")
                    {
                        ...
                        $querys1 = $this->db->query("SELECT booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id, booking_master.user_name, booking_master.user_mobile, booking_master.user_email, booking_master.user_gender, booking_master.branch_id,booking_master.status, booking_master.vendor_id, booking_master.booking_date, booking_master.category_id, booking_master.trail_booking_date, booking_master.trail_booking_time,booking_master.trainer_package_id,booking_master.trainer_id, booking_master.joining_date,booking_master.booking_type,booking_master.sub_booking_type,booking_master.bachat_service_id FROM booking_master WHERE booking_master.user_id='$user_id_t' and booking_master.booking_id='$invoice_no' ORDER BY booking_master.id DESC");
                    ...
                    if ($count1 > 0) 
                       {
                        ...
                        $branch_city = "";
                        $branch_state = "";
                        $bachat_service_id=$row1['bachat_service_id'];
                ...
                $final_array=array_merge($resultpost,$resultpost4,$resultpost5,$resultpost6,$resultpost7);
...
$order_date1 = date('l j M Y h:i A', strtotime($row1['booking_date']));
                ...
                if($vid=="36")
                    ...
                    $querys1 = $this->db->query("SELECT payment_method.payment_method as payment_method_name,booking_master.payment_mode,spa_branch.branch_name,spa_branch.user_id as branch_fit_id, spa_branch.branch_image, spa_branch.branch_phone, spa_branch.branch_address, spa_branch.pincode, spa_branch.state, spa_branch.city,booking_master.package_id,booking_master.booking_id, booking_master.listing_id, booking_master.user_id, booking_master.user_name,booking_master.amt, booking_master.user_mobile, booking_master.user_email, booking_master.user_gender, booking_master.branch_id,booking_master.status, booking_master.vendor_id, booking_master.booking_date, booking_master.category_id, booking_master.trail_booking_date, booking_master.trail_booking_time,booking_master.trainer_package_id,booking_master.trainer_id, booking_master.joining_date FROM booking_master
                                                 INNER JOIN spa_branch ON booking_master.branch_id=spa_branch.id left JOIN
payment_method on (booking_master.payment_mode = payment_method.id)
                ...
                foreach ($querys1->result_array() as $row1) {
                    ...
                    $payment_method_name = $row1['payment_method_name'];
                    $trail_booking_date = $row1['trail_booking_date'];
                    $trail_booking_time = $row1['trail_booking_time'];
                    $joining_date = date('j M Y', strtotime($row1['joining_date']));
                    $book_for_date = date('d/m/Y', strtotime($row1['joining_date']));
                    ...
                    $payment_mode = $row1['payment_mode'] != null ? $row1['payment_mode']  : "" ;
           ...
           $query2 = $this->db->query("select doctor_booking_master.booking_id,doctor_booking_master.consultation_charges, doctor_booking_master.old_price,doctor_booking_master.new_price,doctor_booking_master.user_id,doctor_booking_master.patient_id,
           dl.qualification,dl.experience,
                doctor_booking_master.booking_date,doctor_booking_master.listing_id,doctor_booking_master.user_id, doctor_booking_master.listing_id,doctor_booking_master.clinic_id, doctor_prescription.id as prescription_id, doctor_booking_master.booking_time,doctor_booking_master.status,doctor_booking_master.consultation_type,users.name as patient_name, dl.doctor_name from doctor_booking_master LEFT JOIN doctor_prescription ON (doctor_booking_master.booking_id = doctor_prescription.booking_id AND doctor_booking_master.listing_id = doctor_prescription.doctor_id ) LEFT JOIN users ON (doctor_booking_master.user_id = users.id) LEFT JOIN doctor_list as dl ON(dl.user_id=doctor_booking_master.listing_id) where doctor_booking_master.user_id = '$user_id' and doctor_booking_master.booking_id='$booking_id' ");
            ...
            if ($count2 > 0) {
                foreach ($query2->result_array() as $row2) {
                    ...
                    {
                      $get_pract=$query_practices1->row_array(); 
                        $consultation_charges_clinic = $get_pract['consultation_charges'];
                        $branch_name = $get_pract['clinic_name'];
                    ...
                    $get_pract01=$query_practices0->row_array();
                            $consultation_charges_clinic = $get_pract01['consultation'];
                         }
                       }
                ...
                $resultpost = array();
                 ...
                 if($vid=="10" || $vid=="31")
                    {
                       $count_query4 = $this->db->query("SELECT lb.*,bm.coupon_id,bm.coupon_discount,bm.vendor_id,bm.status,bm.bachat_service_id from booking_master as bm LEFT JOIN lab_booking_details as lb ON(bm.booking_id = lb.booking_id) where bm.user_id='$user_id' and (bm.vendor_id='10' or bm.vendor_id='31') and bm.booking_id='$booking_id'");
                        $lab_booked4       = $count_query4->num_rows();
                        if ($lab_booked4 > 0) 
                        {
                          foreach ($count_query4->result_array() as $Lbooked) 
              ...
              if($vid=="39")
                    {
            			...
            			$lat = $lng = "";
                         ...
                         $sql_query = "SELECT dcp.description, dcp.image as prescription_image,payment_method.payment_method as payment_method_name,booking_master.sub_booking_type
...
booking_master.joining_date,booking_master.bachat_service_id
                   ...
                   $booking_date1 = date('j M Y  h:i A', strtotime($row1['booking_date']));
                    ...
                    $resultpost[] = array(
                      ...
                      //  'joining_date' => $joining_date,
                        ...
                        'prescription_remark' => $description,
                        "booking_type" => $booking_type,
                        "sub_booking_type" => $sub_booking_type,
                        "lat" => $lat,
                        "lng" => $lng,
                        'bachat_service_id'=>$bachat_service_id
                    );
                ...
                if ($count2 > 0) 
                {
                 ...
                 foreach ($query2->result_array() as $row2) 
                 {
                    ...
                    if($status == null)
                    {
                     ...
                     if($booking_id!="" || $booking_id!='0')
                     {
                    $url=$this->get_pdf_version().$prescription_id.".pdf";
                     }
                     else
                     {
                         $url="";
