...
if($product['day']===$dfinal)
{
 $current_day_final = $product['time'][0];
    	    ...
    	    if($insert_id)
    	    {
    	       // multiple image/video
                ...
                if (! empty($_FILES["image"]["name"])) {
                    $flag = '1';
                    $video_flag = '1';
                     $image_file_count = count($_FILES['image']['name']);
                    for($i = 0; $i < $image_file_count; $i++){
                   
                        $img_name = $i.$_FILES['image']['name'][$i];
    ...
    public function vendor_list($user_id,$mlat, $mlng,$page,$vendor_id,$keyword)
    {
          $radius = $page*500;
        $limit = 10;
        $start = 0;
       ...
       elseif($vendor_id=='8')
        {
           if($keyword!="")
           {
              $sql = sprintf("SELECT user_id, name_of_hospital,lat, lng,( 6371 * acos( cos( radians($mlat) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($mlng) ) + sin( radians($mlat) ) * sin( radians(lat ) ) ) ) AS distance FROM hospitals WHERE is_approval='1' AND is_active='1' AND name_of_hospital LIKE '%%$keyword%%'  HAVING distance < '%s'  LIMIT $start, $limit ", ($mlat), ($mlng), ($mlat), ($radius));
            }
          else
          {
             
            $sql = sprintf("SELECT user_id, name_of_hospital,lat, lng,( 6371 * acos( cos( radians($mlat) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($mlng) ) + sin( radians($mlat) ) * sin( radians(lat ) ) ) ) AS distance FROM hospitals WHERE is_approval='1' AND is_active='1'  HAVING distance < '%s'  LIMIT $start, $limit", ($mlat), ($mlng), ($mlat), ($radius));
          }
            $query = $this->db->query($sql);
            $count = $query->num_rows();
            if ($count > 0) {
            ...
            if($keyword!="")
              {
               ...
               $sql    = sprintf("SELECT `user_id`, `lab_name`, `latitude`, `longitude`, ( 6371 * acos( cos( radians($mlat) ) * cos( radians( latitude) ) * cos( radians( longitude) - radians($mlng) ) + sin( radians($mlat) ) * sin( radians( latitude) ) ) ) AS distance FROM lab_center  WHERE `is_active` = 1 and lab_name LIKE '%%$keyword%%'   HAVING distance < '%s' ORDER BY distance LIMIT $start, $limit", ($mlat), ($mlng), ($mlat), ($radius));
               
           }
            else
              {
                $sql    = sprintf("SELECT `user_id`, `lab_name`, `latitude`, `longitude`, ( 6371 * acos( cos( radians($mlat) ) * cos( radians( latitude) ) * cos( radians( longitude) - radians($mlng) ) + sin( radians($mlat) ) * sin( radians( latitude) ) ) ) AS distance FROM lab_center  WHERE `is_active` = 1  HAVING distance < '%s' ORDER BY distance LIMIT $start, $limit", ($mlat), ($mlng), ($mlat), ($radius));
               }
                $query  = $this->db->query($sql);
                $count  = $query->num_rows();
       ...
       elseif($vendor_id == '36')    
         {
                if($keyword!="")
              {
                 ...
                 $sql = sprintf("SELECT fcb.user_id, fcb.branch_name,fcb.lat, fcb.lng,( 6371 * acos( cos( radians($mlat) ) * cos( radians( fcb.lat ) ) * cos( radians( fcb.lng ) - radians($mlng) ) + sin( radians($mlat) ) * sin( radians( fcb.lat ) ) ) ) AS distance FROM spa_branch as fcb LEFT JOIN fitness_center as fc ON(fcb.user_id= fc.user_id) WHERE fc.is_active='1'  and fcb.branch_name LIKE '%%$keyword%%'  and fc.vendor_type='Spa' HAVING distance < '%s' ORDER BY distance LIMIT $start, $limit", ($mlat), ($mlng), ($mlat), ($radius));
              }
            else
              {  
        ...
        else if($vendor_id=='6')
        {
             if($keyword!="")
          {
             ...
             $sql =   sprintf("SELECT fcb.user_id, fcb.branch_name,fcb.lat, fcb.lng,  ( 6371 * acos( cos( radians($mlat) ) * cos( radians( fcb.lat ) ) * cos( radians( fcb.lng ) - radians($mlng) ) + sin( radians($mlat) ) * sin( radians( fcb.lat ) ) ) ) AS distance FROM fitness_center_branch as fcb LEFT JOIN fitness_center as fc ON(fcb.user_id= fc.user_id) WHERE fc.is_active='1' AND fcb.branch_name LIKE '%%$keyword%%' HAVING distance < '%s' ORDER BY distance LIMIT  $start, $limit", ($mlat), ($mlng), ($mlat), ($radius));
           }
          else
          {
     ...
     public function all_list($user_id,$mlat, $mlng,$page,$keyword)
    {
      ...
      public function feature_list($user_id,$type)
    {
        
            $resultpost=array();
            $sql1 = "SELECT * FROM feature_category  WHERE status='0' AND type='$type'";
            $query1 = $this->db->query($sql1);
            $count1 = $query1->num_rows();
            if($count1>0){
                foreach ($query1->result_array() as $row) {
