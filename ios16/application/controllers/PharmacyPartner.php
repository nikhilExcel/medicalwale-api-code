                  ...
                  //  $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($this->input->post('user_id') == "") {
                        ...
                        $prescription_details = $this->input->post('prescription');
                          ...
                          {
                            $resp = $this->PharmacyPartnerModel->order_status_common($order_id_data, $prescription_id_data,$order_details,$prescription_details);
                            simple_json_output($resp);
                          } 
                    ...
                    else if ($order_details =="" && $prescription_details !="")
                    {
                       ...
                       {
                        $resp = $this->PharmacyPartnerModel->prescription_status($order_id, $order_status, $prescription_order, $delivery_time,$listing_id);
                        simple_json_output($resp);
                       } 
                    } 
                    }
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['staff_user_id'] == "" || $params['filter']=="" || $params['page']=="") {
                        ...
                        if($filter == "surplus" || $filter == "lowstock")
                        {
                            ...
                            {
                                $resp        = $this->PharmacyPartnerModel->inventory_stock_report_list_v1($page,$user_id,$staff_user_id,$find,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to);
                            }
                        }
                        else
                        {
