...
*/
   ...
   //  echo $searchWhere; die();
        // $pharmacy_id = 38292;
        if($pharmacy_id != ""){
            $parmacyWhere = "AND `user_id` = '$pharmacy_id'";
        } else {
            $parmacyWhere = "";
        }
        date_default_timezone_set('Asia/Calcutta');
        ...
        $days_closed_query = "(`days_closed` Not Like '".$todayDay." Closed' && `days_closed` Not Like '".$todayDay." Half Day' )";
        $store_close_query = "(`store_close` > '$current_time' AND `store_close` > '$currenttime' or `store_close` = '')";
        $store_open_query = "(`store_open` <= '$current_time' AND `store_open` <= '$currenttime' or `store_open` = '')";
       
        $radius = 20;
        $start = 0;
        $data = $resultpost=array();
        //   $sql = sprintf("SELECT mba,recommended,certified,`id`,`store_time`, `user_id`,`reach_area`, `medical_name`, `discount`, `store_manager`, `lat`, `lng`, `address1`, `address2`, `pincode`, `city`, `state`, `contact_no`, IFNULL(whatsapp_no,'') AS whatsapp_no, `profile_pic`,  `day_night_delivery`,  `days_closed`,  `is_24hrs_available`, `payment_type`, `store_open`, `store_close`,    ( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance FROM medical_stores  WHERE   is_approval='1'  AND is_active='1' AND $days_closed_query AND $store_close_query AND $store_open_query AND visible <> '0'  $parmacyWhere $searchWhere ORDER BY distance ASC  $limit", ($lat), ($lng), ($lat));
          
          
          $sql = "SELECT mba,recommended,certified,`id`,`store_time`, `user_id`,`reach_area`, `medical_name`, `discount`, `store_manager`, `lat`, `lng`, `address1`, `address2`, `pincode`, `city`, `state`, `contact_no`, IFNULL(whatsapp_no,'') AS whatsapp_no, `profile_pic`,  `day_night_delivery`,  `days_closed`,  `is_24hrs_available`, `payment_type`, `store_open`, `store_close`,    ( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance FROM medical_stores  WHERE   is_approval='1'  AND is_active='1' AND $days_closed_query AND $store_close_query AND $store_open_query AND visible <> '0'  $parmacyWhere $searchWhere ORDER BY distance ASC  $limit";
          
          
    //  echo $sql; die();
       $all_pharmacies = $this->db->query("SELECT  ( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance FROM medical_stores  WHERE   is_approval='1'  AND is_active='1' AND $days_closed_query AND $store_close_query AND $store_open_query AND visible <> '0'  $parmacyWhere $searchWhere ORDER BY distance ASC  ")->result_array();
...
// echo "SELECT mo.id,  mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE `mno_id` = '$mno_id' $typeWHere AND uo.listing_type = 44 ORDER BY uo.invoice_no desc $limit"; die();
    ...
    public function order_details($mno_id, $invoice_no){
        ...
        if($acceped_query['status'] == 'accepted'){
            ...
            $query = $this->db->query("select user_id,chc,discount,order_total,action_by,order_id,order_type,delivery_time,listing_id,suggested_pharmacy_id,cancel_reason,action_by,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status,rxId from user_order where `invoice_no` = '$invoice_no' group by invoice_no order by order_id desc");
        ...
        if($rxId == "") {
            ...
            $prescription_resultpost_new=array();
               ...
               $estimated_delivery_time = $deliveryTimeAndCostMno['delivery_time'];
                ...
                $total = $order_total + $delivery_charge + $chc - $order_total_discount + $gst;    
                $cost = array(
...
// add_new_pharmacy
    ...
    public function order_status_common($mno_id,$order_id_data, $prescription_id_data,$order_details,$prescription_details)
    {
     ...
     //   echo $order_id_data ;die();
        ...
        $invoiceNo = $getInvNoByOrderId['invoice_no'];
    ...
    public function order_price($mno_id,$order_id, $listing_type, $order_data_list){
        ...
        $order_status = 'Awaiting Customer Confirmation';
        $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',delivery_charge = '$delivery_charges' where order_id='$order_id'");
...
// remaining 
    ...
    public function order_pickedup($mno_id,$invoice_no){
        $this->load->model('OrderModel');
    ...
    // payment_accepted 
        ...
        public function payment_accepted($mno_id,$invoice_no, $payment_id,$amount){
            ...
            // dont know what to do with amount => pending task 
            
            
        $pharmacyDetails = array();
        $paymentMethods = $this->db->query("SELECT * FROM `mno_payment_method` WHERE `parent_id` = '$payment_id' AND listing_type = 44  ")->result_array(); 
        if(sizeof($paymentMethods) > 0){
            return 4; //please send subtype id
        } else {
            
            $paymentMethodsSelected = $this->db->query("SELECT * FROM `mno_payment_method` WHERE `id` = '$payment_id' AND listing_type = 44  ")->row_array(); 
            ...
            if(sizeof($paymentMethodsSelected) > 0){
                ...
                $this->load->model('OrderModel');
                $existingOrder = $this->db->query("SELECT ml.mno_name, mo.*, u.id as user_id,uo.listing_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
   ...
   //     echo "SELECT * FROM `mno_notifications` WHERE `invoice_no` LIKE '$invoice_no' AND `mno_orders_id` = '$mno_orders_id' ORDER BY `id` ASC"; die();
     ...
     public function get_delivery_time_cost($order_id_data){
        ...
        $getMNODetails = $this->db->query("SELECT uo.invoice_no, uo.lat as user_lat, uo.lng as user_lng ,uo.`listing_id`, ms.lat as store_lat, ms.lng as store_lng , uo.`suggested_pharmacy_id`, msp.lat as suggested_pharmacy_lat ,  msp.lng as suggested_pharmacy_lng, mo.mno_id, mo.id as mno_order_id, ma.lat as mno_lat, ma.lng as mno_lng 
            ...
            left join mno_suggested_pharmacies as msp on (uo.suggested_pharmacy_id != '' && uo.suggested_pharmacy_id = msp.id ) 
             WHERE uo.`order_id` = '$order_id_data' AND mo.`ongoing` = 1 AND mo.`status` LIKE 'accepted'")->row_array();
    ...
    public function payment_methods($mno_id){
        ...
        $paymentMethods = $this->db->query("SELECT * FROM mno_payment_method WHERE  `listing_type` = 44")->result_array();   
   ...
   //     return $data;
    ...
    public function responce_behalf_of_user($mno_id, $invoice_no, $confirm, $cancel_reason_id, $payment_method_id) {
       ...
       //     print_r($payment_method); die();
                if($payment_method['payment_method'] != ""){ 
                    ...
                    $update = $this->db->query("UPDATE `user_order` SET `updated_at`='$updated_at',`order_status`='Order Confirmed',`payment_method`='$order_status',`cancel_reason`='',`action_by`='night owl' WHERE invoice_no='$invoice_no'");
                    ...
                    $msg = "Order confirmed by you behalf of customer of order id $invoice_no";
                ...
                $cancel_reason = $getAllReasons['cancel_reason'];
                    ...
                    if ($check_status == 'Order Delivered') {
                       ...
                       $existingOrder = $this->db->query("SELECT ml.mno_name, mo.*, u.id as user_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
                            ...
                            $msg = "Order cancelled by you behalf of customer of order id $invoice_no";
    ...
    public function get_payment_method($payment_id){
       ...
       public function vendor_payment_option($pharmacy_id){
        ...
        $payment_methods = $this->db->query("SELECT mpm.id,mpm.payment_method,mpm.icon,   ppm.`user_id` as pharmacy_id, ppm.phone, ppm.`image` FROM `pharmacy_payment_methods` as ppm left join mno_payment_method as mpm on(ppm.payment_method_id = mpm.id) WHERE `user_id` = '$pharmacy_id' AND `active` = 1 ")->result_array();
      ...
      // print_r($data); die();
        if(sizeof($data) == 0){
            // default CAP
               $CAP = $this->db->query("SELECT * FROM `mno_payment_method` WHERE `id` = 2 ")->row_array();
            $cp['id'] = $CAP['id'];
