    ...
    /*    
     ...
     public function order_tracking_mno($user_id, $invoice_no){
        ...
        if(sizeof($order_details) > 0){
            ...
            $get_mno_details = $this->db->query("SELECT mo.* FROM `mno_orders` as mo WHERE mo.`invoice_no` LIKE '$invoice_no' AND  (mo.status = 'accepted' OR mo.status = '') order by mo.id desc")->row_array();
            ...
            if($status == "accepted"){
                ...
                if($mno_lat != "" && $mno_lng  != "" &&  $store_lat  != "" &&  $store_lng  != "" && $customer_lat  != "" && $customer_lng  != "" ){
                    
                // $customer_lat $customer_lng
                $drivingDistancePharmaToUser = $this->PartnermnoModel->GetDrivingDistance($store_lat, $customer_lat, $store_lng, $customer_lng );
