...
class PharmacyPartnerModel extends CI_Model
{
   ...
   /* public function order_list($listing_id, $listing_type, $order_type)
    ...
    public function order_status_common($order_id_data, $prescription_id_data,$order_details,$prescription_details)
    {
        ...
        $invNoOrder = $userOrderDetails['invoice_no'];
       ...
       $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',listing_id='$listing_id',delivery_charge = '$delivery_charges' where order_id='$order_id_data'");
       ...
       $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',delivery_charge = '$delivery_charges',listing_id='$listing_id' where order_id='$prescription_id_data'");
    ...
    public function order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data_list)
    {
       ...
       $this->db->update('user_order',array('listing_name' => $medical_name));
        ...
        $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',delivery_charge = '$delivery_charges',listing_id='$listing_id' where order_id='$order_id'");
    ...
    {
        $query = $this->db->query("select * from user_pharmacy_detail where pharmacy_id='$listing_id' and status='0'");
      ...
      public function delete_address($listing_id,$user_id)
    {
     ...
     public function prescription_status($order_id, $order_status, $prescription_order, $delivery_time,$listing_id)
    {
        // echo $order_id; die();
        $this->load->model('OrderModel');
          $this->load->model('PartnermnoModel');
...
//$stock =  $this->Inventory_master_m->get_warehouse_stock($q->id,$pack->id); 
    ...
    public function order_on_call($delivery_by,$service,$delivery_time,$delivery_charge,$listing_id,$listing_name,$staff_user_id,$product_details,$total_quantity,$total_price,$discount,$net_amount,$payment_method,$gst,$customer_name,$customer_phone,$customer_email,$address1,$address2,$city,$state,$pincode,$lat,$lng,$map_location,$address_id,$delivery_charges_by_customer)
    {
        ...
        /*print_r($product_details_new['product']);
        die;*/
        ...
        // render the view into HTML
        	   ...
        	   $bill_url = "https://pharmacy.medicalwale.com/bill/".$output;                 
    	    }
	    }else
	    {
		    $bill_url = "";
             ...
             if($delivery_by == "mno")
        	   {
        	       ...
        	       $resultpost = $this->estimate_cost_mno($listing_id,$invoice_no,$bill_url);
        	    	if($delivery_charges_by_customer == 'Yes'){
				$delivery_charges_by_customer_amt = $resultpost['delivery_charge'];
				$delivery_charges_by_vendor_amt = 0;
			    } else {
				$delivery_charges_by_customer_amt = 0;
				$delivery_charges_by_vendor_amt = $resultpost['delivery_charge'];
			    }
			    if($order_id > 0){
    ...
    public function inventory_stock_report_list($page,$user_id,$staff_user_id,$find,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to)
    {
        $resultpost = array();
        $whr ="";
        if($filter =="all")
    ...
    public function inventory_stock_report_list_v1($page,$user_id,$staff_user_id,$find,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to)
    {
        $resultpost = array();
        $limit = 10;
        $start = 0;
...
SELECT * FROM `other_notifications` where user_id = '$user_id' order by notification_date  desc  LIMIT $start, $limit");
  ...
  /*  public function product_inventory_bill_v1($user_id,$hub_user_id,$date,$invoice_no,$product_details,$discount,$payment_method,$customer_name,$customer_phone,$customer_email,$customer_address,$doctor_name,$bhc_no,$customer_lat,$customer_lng){
    ...
    public function estimate_cost_mno($user_id,$invoice_no,$bill){
        ...
        $data = array(
            'bill_link' => $bill,
            'estimated_delivery_time' => $delivery_time_in_hrs_mins,
            'estimated_delivery_time_sec' => $delivery_time,
            'approx_distance_meter' => $drivingDistance,
            'sub_total' => floatval($order_total),
            'gst' => floatval($tax),
            'cash_handling_charges' => 0,
            'discount' => floatval($discountInRupees),
            'delivery_charge' => floatval($DeliveryCharges),
            'grand_total' => floatval($net_amount),
            'invoice_no' => $invoice_no,
            'lat' => $customer_lat,
            'lng' => $customer_lng,
            'order_id' => $order_id,
