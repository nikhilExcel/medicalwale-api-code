...
trait DecryptionTrait {
    ...
    $cipherText, MaterialsProvider $provider, MetadataEnvelope $envelope, array $cipherOptions = []
    ) {
        ...
        $cek = $provider->decryptCek(
                base64_decode(
                        ...
                        $envelope[MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true
                )
        );
    ...
    Psr7\Stream $cipherText, $tagLength
    ) {
        ...
        return (string) new LimitStream(
                $cipherText, $tagLength, $cipherTextSize - $tagLength
        );
    }
    ...
    Psr7\Stream $cipherText, $tagLength
        ...
        return new LimitStream(
                $cipherText, $cipherTextSize - $tagLength, 0
        );
    }
        ...
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['Tag'] = $this->getTagFromCiphertextStream(
                        $cipherTextStream, $cipherOptions['TagLength']
                );

                return new AesGcmDecryptingStream(
                        $this->getStrippedCiphertextStream(
                                $cipherTextStream, $cipherOptions['TagLength']
                        ), $cek, $cipherOptions['Iv'], $cipherOptions['Tag'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'] ?: null, $cipherOptions['KeySize']
                );
            default:
                $cipherMethod = $this->buildCipherMethod(
                        $cipherOptions['Cipher'], $cipherOptions['Iv'], $cipherOptions['KeySize']
                );
                return new AesDecryptingStream(
                        $cipherTextStream, $cek, $cipherMethod
                );
        }
    }
