        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    $po_status      = $list['po_status'];
                    $po_date        = $list['po_date'];
                    $warehouse_id   = $list['warehouse_id'];
                    $distributor_id = $list['distributor_id'];
    ...
    public function product_inventory_bill()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($user_id == "" || $hub_user_id == "" || $date == "" || $invoice_no == "" || count($product_details) <=0 || $total_quantity == "" || $total_price == "" || $net_amount == "") {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter fields'
                        );
                    } else {
                        $resp = $this->PharmacyPartnerModel->product_inventory_bill($user_id,$hub_user_id,$date,$invoice_no,$product_details,$total_quantity,$total_price,$discount,$tax,$net_amount,$payment_method,$customer_name,$customer_phone,$customer_email,$customer_address);
                    }
                    json_outputs($resp);
                }
            }
