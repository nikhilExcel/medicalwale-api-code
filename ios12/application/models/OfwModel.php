...
class OfwModel extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT id from `menstural_cycle_data` WHERE user_id='$user_id' and profile_id='$profile_id' limit 1");
        $count = $query->num_rows();
        date_default_timezone_set('Asia/Kolkata');
        $updated_at = date('Y-m-d h:i:s');
        if ($count > 0) {
            $query1 = $this->db->query("SELECT id from `menstural_cycle_data` WHERE user_id='$user_id' and profile_id='$profile_id' and start_periode_data='$start_period_day'");
            $count1 = $query1->num_rows();
            if ($count1 > 0) {
                $history = 'Cycle data updated';
    ...
    public function add_menstural_cycle_profile($user_id, $relationship, $name, $age, $height, $weight, $sleep_cycle, $birth_control, $image, $profile_id)
    {
        $query = $this->db->query("SELECT id from `users` WHERE id='$user_id' limit 1");
        $count = $query->num_rows();
        if ($count > 0) {
            $pro_query = $this->db->query("SELECT id from `menstural_cycle_profile` WHERE id='$profile_id' AND user_id = '$user_id'  limit 1");
    ...
    public function get_menstrual_cycle_data_calendar_wise($user_id, $profile_id)
    {
        ...
        if ($count > 0) {            
            ...
            $query1 = $this->db->query("SELECT id,user_id,profile_id,name,pcod,start_periode_data,period_start_from,cycle_length,total_periode_day,born_year,tutual_cycle,tutual_cycle,relationship from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' order by id asc limit 1");
            $count1 = $query1->num_rows();
            if ($count1 > 0) {
                foreach ($query1->result_array() as $row) {
                    $history_id        = $row['id'];
                    ...
                    $born_of_year      = $row['born_year'];
                    $tutual_cycle      = $row['tutual_cycle'];
                    $tracking_resoan   = $row['tutual_cycle'];
                    $relationship      = $row['relationship'];
                    $image             = "";
                    $query2            = $this->db->query("SELECT image from `menstural_cycle_profile` WHERE user_id='$user_id' and id='$profile_id'");
                    $count2            = $query2->num_rows();
                    ...
                    $count3       = '0';
                    $key_         = "0";
                    $period_start = $period_start_from;
                    $ss_date      = array();
                    for ($i = 0; $i < 12; $i++) {                        
                        $count3        = '0';
						$period_days[] = $this->get_calendar($period_start, $period_start_from, $i, $total_period_day, $cycle_length);
						$period_start  = date('Y-m-d', strtotime($period_start . ' +' . $cycle_length . ' days'));
						$period_start_ = date('Y-m-d', strtotime($period_start));
                        
                        $query3 = $this->db->query("SELECT start_periode_data,period_start_from,DATE_FORMAT(STR_TO_DATE(`period_start_from`, '%d-%m-%Y'), '%Y-%m-%d') AS start_period from `menstural_cycle_data` WHERE user_id='$user_id' AND profile_id='$profile_id' HAVING start_period>='$period_start_' order by id asc limit 1");
                        $count3 = $query3->num_rows();
                        
                        if (in_array($period_start_, $ss_date)) {
							$key_ = "1";
						} else {
							$key_ = "0";
						}
                        ...
                        if ($count3 > 0 && $key_<1) {
                            $res               = $query3->row_array();
                            $period_start_from = $res['period_start_from'];
                            $period_start = $res['period_start_from'];
							$period_days[] = $this->get_calendar($period_start, $period_start_from, $i, $total_period_day, $cycle_length);
							$period_start  = date('Y-m-d', strtotime($period_start . ' +' . $cycle_length . ' days'));
                        }
                        $ss_date[] = $period_start;
                    }                    
    ...
    public function get_calendar($period_start, $period_start_from, $i, $total_period_day, $cycle_length)
    {
        $c_month = date('M Y', strtotime($period_start_from . ' +' . $i . ' months'));
        $o_days  = date('Y-m-d', strtotime($period_start . ' + 10 days'));
        $p_date  = array();
        $o_date  = array();
        for ($j = 0; $j < $total_period_day; $j++) {
            $p_date_array = date('Y-m-d', strtotime($period_start . ' +' . $j . '  days'));
            $p_date[]     = array(
                'date' => $p_date_array
            );
        }
        for ($k = 0; $k < $total_period_day; $k++) {
            $o_date_array = date('Y-m-d', strtotime($o_days . ' +' . $k . '  days'));
            $o_date[]     = array(
                'date' => $o_date_array
            );
        }
        $p_start                = date('Y-m-d', strtotime($period_start . ' +' . $cycle_length . '  days'));
        $p_end                  = date('Y-m-d', strtotime($p_date_array . ' +1  days'));
        $o_start                = date('Y-m-d', strtotime($o_days . ' -1  days'));
        $before_ovulation_count = abs((strtotime($p_end) - strtotime($o_start)) / 24 / 3600) + 1;
        $after_ovulation_count  = abs((strtotime($o_date_array) - strtotime($p_start)) / 24 / 3600) - 1;
        $before_ovulation  = array();
        $after_ovulation  = array();
        
        for ($l = 1; $l <= $before_ovulation_count; $l++) {
            $before_ovulation_array = date('Y-m-d', strtotime($p_date_array . ' +' . $l . '  days'));
            $before_ovulation[]     = array(
                'date' => $before_ovulation_array
            );
        }
        
        for ($m = 1; $m <= $after_ovulation_count; $m++) {
            $after_ovulation_array = date('Y-m-d', strtotime($o_date_array . ' +' . $m . '  days'));
            $after_ovulation[]     = array(
                'date' => $after_ovulation_array
            );
        }
