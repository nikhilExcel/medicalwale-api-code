...
class SurvivorstoryModel extends CI_Model {
    ...
    public function story_list($user_id, $page) {
        ...
        $query = $this->db->query("SELECT `id`, IFNULL(tag,'') AS tag, IFNULL(author,'') AS author, `views`,`article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` FROM article where cat_id='37' AND language ='$lang' order by id desc limit $start,$limit");
    ...
    public function article_story_list($user_id) {
     ...
     public function story_details($user_id, $post_id) {
        ...
        $query = $this->db->query("SELECT `id`, IFNULL(tag,'') AS tag, IFNULL(author,'') AS author,`views`, `article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` FROM article WHERE id='$post_id' AND language ='$lang'");
    ...
    public function story_views($user_id, $article_id) { 
		$total_query = $this->db->query("SELECT views FROM `article` where id='$article_id' limit 1");
        $get_list = $total_query->row_array();
        $views = $get_list['views'];
        $total_views = $views+1;
        $querys = $this->db->query("UPDATE `article` SET `views`='$total_views' WHERE id='$article_id'");
    ...
    public function related_story_list($article_id, $user_id) {
        ...
        $query = $this->db->query("SELECT `id`, IFNULL(tag,'') AS tag, IFNULL(author,'') AS author,`views`, `article_title`, `article_description`, `image`, `posted`, `is_active`, `updated_date` FROM article WHERE cat_id='37' AND language ='$lang' AND id<>'$article_id'");
    ...
    public function story_bookmark_list($user_id) {
        ...
        $query = $this->db->query("SELECT  article_bookmark.id, article_bookmark.user_id, article_bookmark.article_id, article.id, article.article_title, article.article_description, article.tag,article.views, article.author, article.image, article.posted FROM `article` 
        ...
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                ...
                $res5 = $this->db->query("SELECT id FROM `article_likes` WHERE article_id='$id'");
