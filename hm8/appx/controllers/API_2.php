...
// 	public function rapid_pickup(){
	    
// 	    $invoice_no           =  'meditest12345'; //Your Order Number,
// 	    $name     =  'Ghanshyam';
// 	    $address          =  'Eco space ot park';
// 	    $address1          =  'Eco space ot park';
// 	    $landmark = 'Eco space ot park';
// 	    $pincode           =  '400031';
// 	    $city          =  'Mumbai';
// 	    $state         =  'MH';
// 	    $country       =  'India';
...
// 	    return $result = $this->MedicalMall_model_2->rapidelivery_create($invoice_no,$name,$mobileno,$address,$address1,$pincode,$landmark,$city,$state,$amount,$paym,$ret_add,$ship_pin,$ship_phone,$ship_company,$product_description,$hub_id);
// 	    //rapid_pickup($invoice_no,$name,$mobileno,$address,$address1,$pincode,$landmark,$city,$state,$amount,$paym);
// 	}
	...
	public function get_products123(){
		        ...
		        if($response == 200){
	               ...
	               if($page_no == "" || $per_page == ""){
	                   ...
	                   return $this->output->set_content_type('Content-Type: application/json')->set_output(json_encode($resp));   
    		            ...
    		            $id = $this->input->post('id');
    		            $term = $this->input->post('term');
    		            $resp = $this->MedicalMall_model_2->get_productselastic($type,$id,$term, $page_no, $per_page);
                    	...
                    	if($count > 0){
                            ...
                            $array2 = array_splice($resp, $exclude_page_entries, $per_page);
		        ...
		        if($response == 200){
        		  ...
        		  //   $ledger = $this->input->post('ledger');
		            if($user_id == "" || $amount == "" || ($transaction_id == "" && $transaction_status != 3 ) || ( $transaction_status != 3 && sizeof($ledger) < 1) || $transaction_status == ""){
		              ...
		              $call_post_order = $this->MedicalMall_model_2->call_post_order($user_id, $amount, $transaction_id, $transaction_status, $ledger,$payment_method);
    		             if($call_post_order['status'] == 1 || $call_post_order['status'] == 2){
    		              //  DECLINED
		                    ...
		                    $response = array('status'=> 200 , 'message' => 'success', 'description' => $description);
		                }  else {
