 ...
 class MedicalMall_model_2 extends CI_Model {
    ...
    public function get_api_version() {
       /* $api_url = "http://sandboxapi.medicalwale.com/v52/";*/
        $api_url = "https://live.medicalwale.com/v66/";
        return $api_url;
    }
   ...
   /* public function auth() {
    ...
    public function home_page($user_id){ 
        ...
        $data['get_promotional_advertisements'] = $this->User_model->all_promotional_ads($user_id);
		...
		if($insert_id != ""){
		  ...
		  // for website; in app not calling place_order untill payment is successful
		    ...
		    $getProductMrp = $getProductMrpRow['pd_mrp_price'] * $data['product_qty'.$i];
		    	...
		    	$query_for_order_data = "INSERT INTO orders_details_hm (user_id,order_id,product_id,product_qty,vendor_id,is_offer,offer_id,offer_price,price,product_mrp,variable_pd_id,sku)
         VALUES ('".$cust_id."','".$insert_id."','".$data['product_id'.$i]."','".$data['product_qty'.$i]."','".$data['vendor_id'.$i]."','".$data['is_offer'.$i]."','".$data['offer_id'.$i]."','".$data['offer_total'.$i]."','".$data['total_products_price'.$i]."','$getProductMrp','".$data['variable_pd_id'.$i]."','".$data['sku'.$i]."')";
