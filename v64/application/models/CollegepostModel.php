...
class CollegepostModel extends CI_Model {
            ...
            foreach ($query->result_array() as $row) {
                ...
                if ($comment_counts > 0) {
                 ...
                 //comments Reply query       
                  $comments_reply ='';      
                 $query_comment_reply = $this->db->query("SELECT medical_college_comment_reply.id,medical_college_comment_reply.comment,medical_college_comment_reply.date,medical_college_comment_reply.user_id as post_user_id,medical_college_comment_reply.post_id,medical_college_comment_reply.comment_id,users.name FROM medical_college_comment_reply INNER JOIN users on users.id=medical_college_comment_reply.user_id WHERE medical_college_comment_reply.comment_id='$comment_id' order by medical_college_comment_reply.id DESC LIMIT 0,3");
                $comment_reply_counts = $query_comment_reply->num_rows();
                if ($comment_reply_counts > 0) {
                    $comments = array();
                    foreach ($query_comment_reply->result_array() as $rows) {
                        $comment_reply_id = $rows['id'];
                        $comment_reply_user_name = $rows['name'];
                        $comment_reply_text = $rows['comment'];
                       
                          $comment_reply_text = preg_replace('~[\r\n]+~', '', $comment_reply_text);
                        if ($comment_reply_id > '8') {
                            $comment_reply_decrypt = $this->decrypt($comment_reply_text);
                            $comment_reply_encrypt = $this->encrypt($comment_reply_decrypt);
                            if ($comment_reply_encrypt == $comment_reply_text) {
                                $comment_reply_text = $comment_decrypt;
                            }
                        } else {
                            if (base64_encode(base64_decode($comment_reply_text)) === $comment_reply_text) {
                                $comment_reply_text = base64_decode($comment_reply_text);
                            }
                        }
                         $comment_date = $rows['date'];
                        $reply_post_user_id = $rows['post_user_id'];       
                        $img_count = $this->db->select('media.source')->from('media')->join('users', 'users.avatar_id=media.id')->where('users.id', $reply_post_user_id)->get()->num_rows();

                        if ($img_count > 0) {
                            $profile_query = $this->db->select('media.source')->from('media')->join('users', 'users.avatar_id=media.id')->where('users.id', $reply_post_user_id)->get()->row();
                            $img_file = $profile_query->source;
                            $reply_user_image = 'https://d2c8oti4is0ms3.cloudfront.net/images/healthwall_avatar/' . $img_file;
                        } else {
                            $reply_user_image = 'https://d2c8oti4is0ms3.cloudfront.net/images/img/default_user.jpg';
                        }
                        $comment_date = get_time_difference_php($comment_date);
                         $reply_like_count = $this->db->select('id')->from('medical_college_comment_reply_likes')->where('comment_reply_id', $comment_reply_id)->get()->num_rows();
                        $reply_like_yes_no = $this->db->select('id')->from('medical_college_comment_reply_likes')->where('user_id', $user_id)->where('comment_reply_id', $comment_reply_id)->get()->num_rows();
                        
                        
                         $comments_reply[] = array(
                            'id' => $comment_reply_id,
                            'comment_user_id' => $comment_post_user_id,
                            'username' => $comment_reply_user_name,
                            'userimage' => $reply_user_image,
                            'like_count' => $reply_like_count,
                            'like_yes_no' => $reply_like_yes_no,
                            'comment' => $comment_reply_text,
    ...
    public function college_post_comment_list($user_id, $post_id) {
        ...
        if ($review_count > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $comment = $row['comment'];
                $username = $row['name'];
                 $comment_post_user_id = $row['post_user_id'];
                if(!empty($comment) && $comment !==""){
                  ...
                  //comments Reply query       
                  $comments_reply ='';      
                 $query_comment_reply = $this->db->query("SELECT medical_college_comment_reply.id,medical_college_comment_reply.comment,medical_college_comment_reply.date,medical_college_comment_reply.user_id as post_user_id,medical_college_comment_reply.post_id,medical_college_comment_reply.comment_id,users.name FROM medical_college_comment_reply INNER JOIN users on users.id=medical_college_comment_reply.user_id WHERE medical_college_comment_reply.comment_id='$id' order by medical_college_comment_reply.id DESC LIMIT 0,3");
                $comment_reply_counts = $query_comment_reply->num_rows();
                if ($comment_reply_counts > 0) {
                    $comments = array();
                    foreach ($query_comment_reply->result_array() as $rows) {
                        ...
                        $reply_like_yes_no = $this->db->select('id')->from('medical_college_comment_reply_likes')->where('user_id', $user_id)->where('comment_reply_id', $comment_reply_id)->get()->num_rows();
                         ...
                         $comments_reply[] = array(
                            'id' => $comment_reply_id,
                            'comment_user_id' => $reply_post_user_id,
                            'username' => $comment_reply_user_name,
                            'userimage' => $reply_user_image,
                            'like_count' => $reply_like_count,
                            'like_yes_no' => $reply_like_yes_no,
    ...
    public function college_post_comment_like($user_id, $comment_id,$post_id,$comment_user_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
    ...
    public function college_post_comment_reply_like($user_id, $post_id, $comment_reply_id, $comment_reply_user_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $count_query = $this->db->query("SELECT id from medical_college_comment_reply_likes WHERE comment_reply_id='$comment_reply_id' and user_id='$user_id' and post_id='$post_id'");
        $count = $count_query->num_rows();

        if ($count > 0) {
            $this->db->query("DELETE FROM `medical_college_comment_reply_likes` WHERE user_id='$user_id' and comment_reply_id='$comment_reply_id' and post_id='$post_id'");
            $comment_query = $this->db->query("SELECT id from medical_college_comment_reply_likes WHERE comment_reply_id='$comment_reply_id'");
            $total_comment = $comment_query->num_rows();
            return array(
                'status' => 200,
                'message' => 'deleted',
            ...
            $this->db->insert('medical_college_comment_reply_likes', $ask_saheli_comment_like);
            ...
            if ($customer_token_count > 0) {
                ...
                $msg = $usr_name . ' Beats on your Reply click here to view reply.';
                $this->send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $agent, $post_id);
            }
            // Close notification
    ...
    public function college_post_comment_reply_delete($user_id, $post_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $count_query = $this->db->query("SELECT id from `medical_college_comment_reply` WHERE  user_id='$user_id' and id='$post_id'");
        $count = $count_query->num_rows();

        if ($count > 0) {
            $this->db->query("DELETE FROM `medical_college_comment_reply` WHERE user_id='$user_id' and id='$post_id'");
    ...
    public function college_follow_post($user_id, $post_id,$post_user_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        ...
        if ($count > 0) {
            ...
            //added by zak for for notification 
              //Like Notification
             $post_user_comment_plike = $this->db->query("SELECT name FROM users WHERE id='$user_id'");
            $user_comment_plike = $this->db->query("SELECT name FROM users WHERE id='$post_user_id'");
            $img_count = $this->db->select('media.source')->from('media')->join('users', 'users.avatar_id=media.id')->where('users.id', $post_user_id)->get()->num_rows();
            if ($img_count > 0) {
                $profile_query = $this->db->select('media.source')->from('media')->join('users', 'users.avatar_id=media.id')->where('users.id', $post_user_id)->get()->row();
                $img_file = $profile_query->source;
            ...
            if ($customer_token_count > 0) {
                $token_status = $customer_token->row_array();
                $getusr = $user_comment_plike->row_array();
                $getuser = $post_user_comment_plike->row_array();
                $usr_name = $getusr['name'];
                ...
                $tag = 'text';
                $key_count = '1';
                $title = $user_name . ' follow  your post';
                $msg = $user_name . ' follow  your post click here to view followers.';
                $this->send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $agent, $post_id);
            }
            // Close notification
   ...
   public function medical_college_question($user_id,$post,$post_id) {
    ...
    public function medical_college_comment_reply($user_id, $post_id, $comment_id, $comment, $post_user_id, $post_comment_user_id) {
        ...
        $query_comment_reply = $this->db->query("SELECT medical_college_comment_reply.id,medical_college_comment_reply.date,medical_college_comment_reply.comment_id,medical_college_comment_reply.post_id,medical_college_comment_reply.comment, users.name,medical_college_comment_reply.user_id FROM medical_college_comment_reply INNER JOIN users on users.id=medical_college_comment_reply.user_id WHERE medical_college_comment_reply.id ='$id'")->row();
        ...
        $comment_reply_like_count = $this->db->select('id')->from('medical_college_comment_reply_likes')->where('comment_reply_id', $comment_reply_id)->get()->num_rows();
     ...
     public function post_comment_all_reply_list($comment_id, $user_id) {
        $query_comment_reply = $this->db->query("SELECT medical_college_comment_reply.id,medical_college_comment_reply.comment_id,medical_college_comment_reply.post_id,medical_college_comment_reply.comment,medical_college_comment_reply.date as date,users.name,medical_college_comment_reply.user_id FROM medical_college_comment_reply INNER JOIN users on users.id=medical_college_comment_reply.user_id WHERE medical_college_comment_reply.comment_id='$comment_id' order by medical_college_comment_reply.id desc");
        $this->db->last_query();
        $comment_reply = array();
        ...
        return array(
            'status' => 200,
            'message' => 'success',
            'count' => $comment_count_reply,
            'data' => $comment_reply_array
        );
    }
