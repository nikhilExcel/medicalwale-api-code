...
class HealthrecordModel extends CI_Model {
       ...
       $resultpost = array();
        ...
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                ...
                // print_r($row); die();
                if($relation_record_id=="0")
                {
                    $new_relation_record_id=$user_id;
                }
                else
                {
                    $new_relation_record_id=$relation_record_id;
                }
                $relation_name = $relationship;
                $relation_info = array(
                    'id' => $health_record_id,
                    'member_id'=>$new_relation_record_id,
                    'patient_name' => $patient_name,
                    'folder_id' => $folder_id,
                    'patient_age'=>$patient_age,
                    'image' => $image,
    ...
    public function folder_list_v2($health_record_id) {
        ...
        if ($count > 0) {
            ...
            $query = $this->db->query("SELECT * FROM health_record_folder WHERE user_id='$health_record_id' and health_record_id='0' order by id desc");
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $folder_id        = $row['id'];
