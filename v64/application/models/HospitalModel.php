...
public function list_category($latitude, $longitude, $user_id)
{
    ...
    $nursing_array = array();
    $hospital="select id,category from hospital_category where sub_category='14'";
...
public function list_surgey_category($latitude, $longitude, $user_id)
{
    $data=array();
    $data1=array();
    $hospital_array = array();
...
public function hospital_list_surgery($user_id, $latitude, $longitude, $category_id,$page,$type,$keyword,$sort) 
{
    $surgey_discount=0;
    $radius = $page*500;
    $limit = 10;
    $start = 0;
       ...
       $disc ="ORDER BY hp.package_discount desc , distance desc";
         ...
         $sql = sprintf("SELECT h.id, h.user_id, h.name_of_hospital,h.phone,h.about_us,h.certificates_accred, h.category, h.speciality, h.surgery,h.services,h.address,h.pincode,h.city,h.state,h.email, h.lat, h.lng, h.image, h.rating, h.review,  h.date, h.is_active,h.establishment_year,h.user_discount,h.recommended,h.mba,h.certified,hp.package_name,hp.surgery_id,hp.package_discount,hp.package_amount,( 6371 * acos( cos( radians($latitude) ) * cos( radians( h.lat ) ) * cos( radians( h.lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( h.lat ) ) ) ) AS distance FROM hospitals as h INNER join hospital_packages as hp on (h.user_id = hp.hospital_id ) WHERE h.is_approval='1' AND h.is_active='1' $cwh  $key HAVING distance < '%s'  $disc LIMIT $start, $limit", ($latitude), ($longitude), ($latitude), ($radius));
...
public function hospital_detail_surgery($user_id, $latitude, $longitude, $category_id,$page,$type2,$keyword,$sort,$hospital_id,$sorts) 
{
    ...
    $start = 0;
       ...
       if($keyword!="")
       {
        ...
        $sql = "SELECT * FROM `hospital_packages` WHERE hospital_id='$hospital_id' and FIND_IN_SET('" . $type2 . "', treatment_type) $cwh $key $sorts";
        ...
        $count = $query->num_rows();
        if ($count > 0) {
               foreach($query->result_array() as $qRow)
            {
                $package_id         = $qRow['id'];
