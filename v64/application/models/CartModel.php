...
class CartModel extends CI_Model
{
   ...
   public function getExtension($str)
   {
    ...
    public function order_add_cart($user_id,$cart_details,$description,$pid)
    {
                                ...
                                if ($img_size < (50000 * 50000)) {
                                    if (in_array($ext, $img_format)) {
                                       ...
                                       if ($s3->putObjectFile($img_tmp, $bucket, $actual_image_path, S3::ACL_PUBLIC_READ)) {
                                         ...
                                         $p_results = $this->db->query("INSERT INTO `user_card_order` (`user_id`, `listing_id`, `listing_type`,`product_id`,`offline_prescription`,`date`,`is_prescription`,`description`,`original_prescription`) VALUES ('$user_id', '0', '13','$p','$actual_image_path','$date','YES','$d','$actual_image_path2')");
     ...
     public function order_add_cart_web($user_id,$cart_details,$description,$image)
    {
        ...
        if($count1 != 0){
            //$this->db->query("DELETE FROM `user_card_order` WHERE `product_id` = '$product_id' AND `user_id` = '$user_id'");
            $results = $this->db->query("INSERT INTO `user_card_order` (`user_id`, `listing_id`, `listing_type`, `product_id`,  `quantity`, `prescription_id`, `offline_prescription`,`total_product_price`,`date`,`is_prescription`) VALUES ('$user_id', '$listing_id', '$listing_type', '$product_id',  '$product_qty', '$prescription_id','$prescription_url','$total_product_price','$date','$is_prescription')");
    	    $insert_id = $this->db->insert_id();
    	    if($insert_id){
    	           $checkAvailablec = $this->db->query("SELECT 'user_id' FROM `user_card_order` WHERE `user_id` = '$user_id'");
                  $stack_count = $checkAvailablec->num_rows();
    ...
    public function remove_user_cart($user_id,$product_id,$listing_type)
    {
     ...
     public function remove_user_cart_all($user_id,$listing_type)
    {
         $checkAvailable = $this->db->query("SELECT `user_id` FROM `user_card_order` WHERE `user_id` = '$user_id' AND `listing_type` = '$listing_type'");
      
        $count = $checkAvailable->num_rows();
        if($count != 0){
            $this->db->query("DELETE FROM `user_card_order` WHERE  `user_id` = '$user_id' AND `listing_type` = '$listing_type'");
              return array(
                    "status" => 200,
                    "message" => "success",
                    "description" => "Successfully Deleted from Stack"
    ...
    // Re-order API
    public function reorder_add_cart($user_id,$invoice_no)
    {
       date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');    
       $checkAvailable = $this->db->query("SELECT `user_id` FROM `user_card_order` WHERE `user_id` = '$user_id' AND `listing_type` = '13'");
      
       $count = $checkAvailable->num_rows();
        if($count > 0){
            $this->db->query("DELETE FROM `user_card_order` WHERE  `user_id` = '$user_id' AND `listing_type` = '13'");
            ...
            foreach ($query1->result_array() as $row1) {
                    ...
                    $product_query = $this->db->query("select product_id,product_quantity,product_price,sub_total from user_order_product where order_id='$order_id1' order by id asc");
                    ...
                    if ($product_count > 0) {
                        foreach ($product_query->result_array() as $product_row) {
                            ...
                            $product_qty          = $product_row['product_quantity'];
                            $sub_total            = $product_row['sub_total'];
                            $total=$sub_total/$product_qty;
      $results = $this->db->query("INSERT INTO `user_card_order` 
      (`user_id`, `listing_id`, `listing_type`, `product_id`,  `quantity`, `prescription_id`, `prescription_url`,`total_product_price`,`date`,`is_prescription`) 
      VALUES ('$user_id', '0', '13', '$product_id',  '$product_qty', '','','$total','$date','NO')");
                    ...
                    $product_query1 = $this->db->query("SELECT prescription_image,original_prescription,description FROM prescription_order_details WHERE order_id='$order_id1' order by id asc");
                    ...
                    if ($product_count1 > 0) {
                        foreach ($product_query1->result_array() as $product_row1) {
                            ...
                            {
                              $product_img2=$product_row1['prescription_image'];  
                            }
                            else
                            {
                                $product_img2;
                        ...
                        $p = md5(uniqid(rand(), true));
                            $p_results = $this->db->query("INSERT INTO `user_card_order`
                            (`user_id`, `listing_id`, `listing_type`,`product_id`,`offline_prescription`,`date`,`is_prescription`,`description`,`original_prescription`) 
                            VALUES ('$user_id', '0', '13','$p','$product_img2','$date','YES','$d','$product_img2')");
                        }
            ...
            else
            {
                return array(
                   "status" => 400,
                    "message" => "fail",
                    "description" => "Wrong Inovice Number",
                );
            }
     ...
     public function reorder_edit_cart($user_id,$id,$desc)
    {
       date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');    
       $checkAvailable = $this->db->query("SELECT `user_id` FROM `user_card_order` WHERE `user_id` = '$user_id' AND id='$id' AND `listing_type` = '13'");
                        ...
                        if (strlen($img_name) > 0) {
                            if ($img_size < (50000 * 50000)) {
                                if (in_array($ext, $img_format)) {
                                    ...
                                    $actual_image_path = 'images/prescription_images/' . $image;
                                    $s3->putObjectFile($img_tmp, $bucket, $actual_image_path, S3::ACL_PUBLIC_READ);								

                                  $update="UPDATE `user_card_order` SET offline_prescription='$actual_image_path',description='$desc' where `user_id` = '$user_id' AND id='$id' AND `listing_type` = '13' ";
          ...
          foreach ($checkAvailable->result_array() as $row) {
            ...
            $prescription_array[] = array
                (
                   ...
                   "product_name" => "",
                 //$row['product_image']
                   "product_image" => 'https://d2c8oti4is0ms3.cloudfront.net/images/product_images/'.$images_1,
