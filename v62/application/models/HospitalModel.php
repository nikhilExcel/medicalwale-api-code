...
public function hospital_list_v2($latitude, $longitude, $user_id, $category_name,$page) {
 ...
 $resultcomment = array();
   ...
   public function wellness_clinic_list($latitude, $longitude, $user_id, $category_name,$page,$keyword)
   {
     ...
     if($keyword!=""){
        $sql = sprintf("SELECT `id`, `user_id`, `name_of_hospital`,`phone`,`about_us`, `establishment_year`,`certificates_accred`, `category`, `speciality`, `surgery`,`services`, `address`, `pincode`, `city`, `state`, `email`, `lat`, `lng`, `image`, `rating`, `review`,  `date`, `is_active`,`establishment_year`,`user_discount`,( 6371 * acos( cos( radians($latitude) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM wellness_clinic WHERE FIND_IN_SET('" . $category_name . "', category) AND is_approval='1' AND is_active='1' and name_of_hospital LIKE '%%$keyword%%' HAVING distance < '%s' ORDER BY distance LIMIT $startk, $limitk", ($latitude), ($longitude), ($latitude), ($radius));
    }
   else
   {
        $sql = sprintf("SELECT `id`, `user_id`, `name_of_hospital`,`phone`,`about_us`,`establishment_year`, `certificates_accred`, `category`, `speciality`, `surgery`,`services`, `address`, `pincode`, `city`, `state`, `email`, `lat`, `lng`, `image`, `rating`, `review`,  `date`, `is_active`,`establishment_year`,`user_discount`,( 6371 * acos( cos( radians($latitude) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM wellness_clinic WHERE FIND_IN_SET('" . $category_name . "', category) AND is_approval='1' AND is_active='1' HAVING distance < '%s' ORDER BY distance  LIMIT $start, $limit", ($latitude), ($longitude), ($latitude), ($radius));
