...
class TreeCompiler {
    ...
    private function visit_function(array $node) {
        ...
        return $this->write(
                        '$value = Fn::getInstance()->__invoke("%s", %s);', $node['value'], $args
        );
    }
    ...
    private function visit_slice(array $node) {
        return $this
                        ...
                        ->write('    ? null : Utils::slice($value, %s, %s, %s);', var_export($node['value'][0], true), var_export($node['value'][1], true), var_export($node['value'][2], true)
        );
    }
    ...
    private function visit_comparator(array $node) {
        ...
        if ($node['value'] == '==') {
            ...
            $this->write(
                    '$value = (is_int(%s) || is_float(%s)) && (is_int(%s) || is_float(%s)) && %s %s %s;', $a, $a, $b, $b, $a, $node['value'], $b
            );
        }
