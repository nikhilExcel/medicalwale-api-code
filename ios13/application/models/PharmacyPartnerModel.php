...
class PharmacyPartnerModel extends CI_Model
{
   ...
   /* public function order_list($listing_id, $listing_type, $order_type)
    ...
    public function order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data_list,$user_id1)
    {
        ...
        // if order is by night owls then send this notification to mno
        $mno_details = $this->db->query("SELECT u.token, u.agent, mo.mno_id ,uo.* FROM `user_order` as uo left join mno_orders as mo on (uo.`invoice_no` = mo.invoice_no) left join users as u on (mo.mno_id = u.id) WHERE uo.`order_id` = '$order_id' and uo.`listing_type` = 44 AND mo.ongoing = 1 AND mo.status = 'accepted'")->row_array();
        if(sizeof($mno_details) > 0){
            $this->load->model('PartnermnoModel');
            $reg_id = $mno_details['token'];
            $agent = $mno_details['agent'];
    ...
    public function order_deliver_cancel($order_id, $cancel_reason, $type, $notification_type)
    {
        ...
        if ($token_status > 0) {
            ...
            // if order is by night owls then send this notification to mno  $order_id
                if($cancel_reason != ""){
                    $givenReason = "due to ".$cancel_reason;
                } else {
                    $givenReason = "";    
                }
                
                $mno_details = $this->db->query("SELECT u.token, u.agent, mo.mno_id ,uo.* FROM `user_order` as uo left join mno_orders as mo on (uo.`invoice_no` = mo.invoice_no) left join users as u on (mo.mno_id = u.id) WHERE uo.`invoice_no` = '$order_id' and uo.`listing_type` = 44 AND mo.ongoing = 1 AND mo.status = 'accepted'")->row_array();
                if(sizeof($mno_details) > 0){
                    $this->load->model('PartnermnoModel');
                    ...
                    if ($type == 'Order Cancelled') {
                        $reg_id = $mno_details['token'];
                        $agent = $mno_details['agent'];
                        $mno_id = $mno_details['mno_id'];
                        $invoice_no = $mno_details['invoice_no'];
                        $msg = 'Order #'.$invoice_no.' Cancelled by pharmacy '.$givenReason.' Please send order to another pharmacy ';
                        $img_url = 'https://medicalwale.com/img/noti_pharmacy.png';
                        $tag = 'text';
                        ...
                        $agent = $mno_details['agent'];
                        $invoice_no = $mno_details['invoice_no'];
                        $msg = 'Order #'.$invoice_no.' delivered by night owl';
                        $img_url = 'https://medicalwale.com/img/noti_pharmacy.png';
                        $tag = 'text';
                        $title = 'Order delivered' ;
                        ...
                        $this->PartnermnoModel->send_gcm_notify_mno($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type);
                          ...
                          $this->db->query("UPDATE `mno_orders` SET `ongoing`=0  WHERE `invoice_no`='$invoice_no' and `mno_id`= '$mno_id' AND `ongoing` = '1'");
...
//$stock =  $this->Inventory_master_m->get_warehouse_stock($q->id,$pack->id); 
    ...
    public function product_inventory_bill($user_id,$hub_user_id,$date,$invoice_no,$product_details,$total_quantity,$total_price,$discount,$tax,$net_amount,$payment_method,$name,$customer_phone,$email,$customer_address,$doctor_name,$bhc_no)
    {
        ...
        if(!empty($type))
    	{
                    ...
                    {
                        $image = "https://d2c8oti4is0ms3.cloudfront.net/images/pharmacy_images/".$detail->logo;
                    }
        ...
        <tr>
        <td colspan="2"><img alt="" src="'.$image.'" style="max-height: 48px;height: 48px;"></td>
        <td colspan="4">
    ...
    public function stock_inventory_dashboard($user_id,$hub_user_id,$page,$date_from,$date_to)
    {
        $limit = 10;
        $start = 0;
        if ($page > 0) {
        ...
        if ($count > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $size                   = $row['size'];
        	       	...
        	       	$sql1="SELECT sum(stock_quantity) as stock_quantity FROM inventory_stock where user_id='$user_id'  AND product_id='$id' AND warehouse_id='0' AND created_at>= '$date_from' AND created_at <= '$date_to'";
                    $stock=$this->db->query($sql1)->row();
     ...
     function send_gcm_notify_test($title, $reg_id, $msg, $img_url, $tag, $agent)
        {
            if (!defined("GOOGLE_GCM_URL"))
                define("GOOGLE_GCM_URL", "https://fcm.googleapis.com/fcm/send");
    ...
    public function check_notification($user_id,$title,$msg)
    {
          $mno_details = $this->db->query("SELECT u.token, u.agent FROM  users as u WHERE u.id = '$user_id' ")->row_array();
        if(sizeof($mno_details) > 0){
             $reg_id = $mno_details['token'];
            $agent = $mno_details['agent'];
          
          //  $msg = 'Order received by pharmacy and gave price to customer';
            $img_url = 'https://medicalwale.com/img/noti_pharmacy.png';
            $tag = 'text';
          //  $title = 'Order received by pharmacy and gave price to customer';
            $order_status = 'Awaiting Customer Confirmation';
            ...
            $this->send_gcm_notify_test($title, $reg_id, $msg, $img_url, $tag, $agent);
