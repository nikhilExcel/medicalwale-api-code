            ...
            if ($post_insert) {
                ...
                if ($type == 'question') {
                   ...
                   // print_r($parent_id);
                    ...
                    define("POST_URL", "https://sandbox.medicalwale.com/v44/healthwall/post_comment");
                ...
                if ($image!="") {
                  $imagearray=explode(',', $image);
                    ...
                    foreach ($imagearray as $key => $tmp_name) {
                        ...
                        $ext = getExtension($tmp_name);
                        if ($tmp_name > 0) {
                                if (in_array($ext, $img_format)) {
                                    $actual_image_name = uniqid() . date("YmdHis") . "." . $ext;
                                   
                                        $media = mysqli_query($hconnection, "INSERT INTO `media`(`caption`,`title`, `type`, `source`, `created_at`, `updated_at`) VALUES ('$caption[$key]','$tmp_name', 'image', '$tmp_name', '$date','$date')");
                                ...
                                if (in_array($ext, $video_format)) {
                                        ...
                                        $video_width = '300';
                                        $video_height = '160';
                                        $media = mysqli_query($hconnection, "INSERT INTO `media`(`caption`,`title`, `type`, `source`, `created_at`, `updated_at`) VALUES ('$caption[$key]','$tmp_name', 'video', '$tmp_name', '$date','$date')");
                                        $media_id = mysqli_insert_id($hconnection);
