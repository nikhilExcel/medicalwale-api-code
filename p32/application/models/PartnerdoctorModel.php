...
class PartnerdoctorModel extends CI_Model { 
   ...
   public function payment_process($booking_id,$amount,$discount,$card_type,$card_sub_type,$carditdetails,$debitdetails,$walletdetails,$user_id,$doctor_id,$discount_type,$finalamount,$savedamount)
   {
    ...
    public function addcustomercoupon($data, $data_point, $data_credit)
    {
       ...
       public function patient_appointment($doctor_id, $agent)
    {
        if($agent=="ios")
        {
             $clinic1_slott=array();
...
public function doctor_pic_list111($group_user_id) 
{
    ...
    public function list_patient($doctor_id) 
    {
            ...
            $query1 = $this->db->query("SELECT *  FROM `doctor_patient` WHERE `doctor_id`='$doctor_id' group by contact_no order by created_date desc ");
            $count1 = $query1->num_rows();
            if ($count1 > 0) 
                {
   ...
   /*$data_clinic = array('address'   =>   $address,
    ...
    public function doctor_privacy_profile_list($user_id)
    {
		...
		$arr[] = array(
            ...
            "user_id"=> $user_id,
            "address_privacy" => "0",
            "experience_privacy" => "0",
            "dob_privacy" => "0",
            "email_privacy" => "0",
            "bio_privacy" => "0",
            "language_privacy" => "0",
            "relationship_privacy" => "0",
            "work_privacy" => "0",
            "education_privacy" => "0",
            "onames_privacy" => "0",
            "place_privacy" => "0",
       ...
       $sql1    = "SELECT user_id FROM events WHERE id='$event_id'";
        ...
        foreach($query->result_array() as $row)
        {
            ...
            if(!empty($row['images']))
            {
                ...
                {
                    $image[]= array("image"=>"https://s3.amazonaws.com/medicalwale/images/Event_images/$im[0]");
                }
            }
            ...
            if(!empty($row['videos']))
            {
                ...
                {            
                    $video[] = array("video"=>"https://d2c8oti4is0ms3.cloudfront.net/images/Event_images/$imv[0]");
                }
            }
