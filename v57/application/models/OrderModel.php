...
else
{
   ...
   //start
    public function prescription_add_quickbook($user_id, $address_id, $chat_id, $payment_method, $delivery_charge, $is_night_delivery,$lat,$lng,$product_id,$is_profile,$listing_id) {
        date_default_timezone_set('Asia/Kolkata');
        $order_date = date('Y-m-d H:i:s');
        $invoice_no = date("YmdHis");
        $order_status = 'Awaiting Confirmation';
 ...
 /*  $order_date = date('j M Y h:i A', strtotime($order_date));
            ...
            if ($token_status > 0) {
                ...
                $agent = $order_info->agent;
           //     $msg = 'Thanks uploading your prescription with ' . $listing_name;
                 $msg = 'We will inform you as soon as any one Pharmacy Store accepts it.';
                $img_url = 'https://medicalwale.com/img/medical_logo.png';
                $tag = 'text';
                $key_count = '1';
                $title = 'Your order is placed with ' .$pharmacy_listing_count. ' Pharmacy Stores.';
