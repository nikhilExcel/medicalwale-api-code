   ...
   //  $firstOfMonth = strtotime(date("Y-m-d", $date));
    //Apply above formula.
           ...
           // echo 'test '.$week_number =  intval(date("W", $date)) - intval(date("W", $firstOfMonth)) + 1;
            ...
            $Quatequery = $this->db->query("SELECT * FROM Health_quote_dictionary WHERE date<='$date' ORDER BY id ASC LIMIT 1 ");
...
public function SearchwordModel($keyword)
   ...
   /*    if($count > 0)
       ...
       */
        ...
        if($count1 > 0)
       {
           ...
           foreach($query1->result_array() as $row)
           {
               ...
               {
                 $image[]=array(
                   'image' =>'https://s3.amazonaws.com/medicalwale/images/Health_Dictionary/'.$img[$i],
    ...
    {
             $query = $this->db->query("SELECT * FROM Health_quote_dictionary ORDER BY id DESC");
             $count = $query->num_rows();
             $Health_quate = array();
             if($count>0)
             {
