...
class Dietplan_model extends CI_Model {
            ...
            $count1 = $query1->num_rows();
              if ($count1 > 0) {
                    foreach ($query1->result_array() as $row) {
                        ...
                        {
                            $this->db->select('month,follow_up,price');
    ...
    public function dietplan_booking($user_id, $package_id, $gst, $amount, $booking_id){
        date_default_timezone_set('Asia/Kolkata');
        $created_at   = date('Y-m-d H:i:s');
    ...
    public function renew_package($user_id,$package_id,$from_date,$gst,$amount){
        date_default_timezone_set('Asia/Kolkata');
        $created_at   = date('Y-m-d H:i:s');
        $booking_date = date('Y-m-d');
        $booking_time = date('H:i:s');
