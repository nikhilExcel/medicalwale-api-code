...
class SearchModel extends CI_Model {
 ...
 public function elasticsearchdoctor($index_id,$keyword){
    ...
    public function search_list($user_id, $keyword) {
        ...
        else
        {
            ...
            $resultpost = array_merge($people_array, $doctor_array, $pharmacy_array,  $labs_array, $nursing_array, $fitness_array, $hospital_array, $healthwall_array, $article_array, $survivor_array,$hm_product_array);
   ...
   public function search_list_by_category($user_id, $keyword ,$type)
    {
         if ($user_id > 0) {
            ...
            $field3 = '';
                    $index_id="people";
                    $people=$this->elasticsearch($index_id,$keyword);
            ...
            $field3 = '';
             $index_id="doctor";
            $doctor=$this->elasticsearch($index_id,$keyword);
          ...
          //  $query = $this->db->query("select healthwall_category.category AS healthwall_category,healthwall_category.id as healthwall_category_id, posts.id as post_id, IFNULL(posts.repost_user_id,'') AS repost_user_id,posts.repost_location,posts.is_repost,posts.repost_time, posts.type as post_type,IFNULL(posts.post_location,'') AS post_location,posts.caption,posts.description as post,posts.is_anonymous,posts.tag,posts.created_at,users.name,posts.user_id as post_user_id,users.vendor_id ,posts.article_title,posts.article_image,posts.article_domain_name,posts.article_url,IFNULL(posts.category,'') AS post_category from posts INNER JOIN users on users.id=posts.user_id LEFT JOIN healthwall_category ON posts.healthwall_category=healthwall_category.id where posts.user_id<>'' and posts.user_id<>'0' AND posts.healthwall_category<>'0' AND  posts.id NOT IN (SELECT post_id FROM posts_hide WHERE post_id=posts.id AND user_id='$user_id') and posts.active='1' order by posts.id DESC limit 50");
            ...
            $field3 = '';
            $index_id="medical";
            $pharmacy=$this->elasticsearch($index_id,$keyword);
            ...
            if ($pharmacy_count > 0) {
               $pharmacy_array[] = array(
                ...
                $field3 = '';
                $index_id="lab";
                $labs=$this->elasticsearch($index_id,$keyword);
            ...
            $field3 = '';
            $index_id="fitness";
            $fitness=$this->elasticsearch($index_id,$keyword);
            ...
            $index_id="hospital";
            $hospital=$this->elasticsearch($index_id,$keyword);
            ...
            $field3 = '';
            $index_id="nursing_attendant";
            $nursing=$this->elasticsearch($index_id,$keyword);
 ...
 //********************************************added by jakir on 3 aug 2018 for lab changes*************************************************** 
                  $package_list = array();
                $package_query = $this->db->query("SELECT * FROM `lab_packages1` WHERE user_id='$labcenter_user_id' and branch_id ='' order by id asc");
                ...
                if ($branch_count > 0) {
                    foreach ($branch_query->result_array() as $branch_row) {
                        $branch_id                              = $branch_row['branch_user_id'];
                       ...
                       /* $branch_details = $branch_row['fnac_sample_pickup_price_option '];
                        ...
                        $package_query = $this->db->query("SELECT * FROM `lab_packages1` WHERE user_id='$branch_id' order by id asc");
                        $package_count = $package_query->num_rows();
                        if ($package_count > 0) {
                            foreach ($package_query->result_array() as $package_row) {
                                $package_id = $package_row['id'];
...
$area_expertise       = array();
        ...
        if ($total_category > 0) {
            ...
            if ($listing_type == '6') {
                ...
                $query = $this->db->query("SELECT `user_id`, `center_name`, `contact`, `email`, `image` FROM `fitness_center` WHERE user_id='$listing_id'");
            ...
            if ($listing_type == '8') {
                ...
                $query = $this->db->query("SELECT `id`, `user_id`, `name_of_hospital`,`phone`,`about_us`, `certificates_accred`, `category`, `speciality`, `surgery`,`services`, `address`, `pincode`, `city`, `state`, `email`, `lat`, `lng`, `image`, `rating`, `review`,  `date`, `is_active`,`establishment_year` FROM hospitals WHERE user_id='$listing_id'");
