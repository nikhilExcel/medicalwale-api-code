...
class VideoModel extends CI_Model
{
    ...
    {
        $yoga_query          = $this->db->query("SELECT id,video,details,video_thumbnail,title FROM exercise_details WHERE cat_id='2' order by id asc limit 10");
        $physiotherapy_query = $this->db->query("SELECT id,video,details,video_thumbnail,title FROM exercise_details WHERE cat_id='1' order by id asc limit 10");
        $other_query         = $this->db->query("SELECT * FROM med_tube order by type asc");
     ...
     public function med_tube_pagination($user_id,$page,$category,$keyword)
    {
        ...
        if($category == 'yoga')
        {
            if($keyword != '')
            {
          $yoga_query  = $this->db->query("SELECT id,video,details,video_thumbnail,title FROM exercise_details WHERE cat_id='2' AND title LIKE '%$keyword%' order by id desc  limit $start,$limit");
            }
            else
            {
          $yoga_query  = $this->db->query("SELECT id,video,details,video_thumbnail,title FROM exercise_details WHERE cat_id='2' order by id desc  limit $start,$limit");      
            }
          $yoga_count          = $yoga_query->num_rows();
         if ($yoga_count > 0) {
            foreach ($yoga_query->result_array() as $row) {
        ...
        else if($category == 'physiotherapy')
        {
            if($keyword != '')
            {
                $physiotherapy_query = $this->db->query("SELECT id,video,details,video_thumbnail,title FROM exercise_details WHERE cat_id='1' AND title LIKE '%$keyword%' order by id asc limit $start,$limit");
            }
            else
            {
        $physiotherapy_query = $this->db->query("SELECT id,video,details,video_thumbnail,title FROM exercise_details WHERE cat_id='1' order by id asc limit $start,$limit");
            } 
        $physiotherapy_count = $physiotherapy_query->num_rows();
    ...
    public function med_tube_views($user_id, $video_id, $type)
    {
        ...
        if ($type == 'yoga' || $type == 'physiotherapy') {
            $exercise_views_array = array(
                'user_id' => $user_id,
                'post_id' => $video_id
            );
            $this->db->insert('exercise_views', $exercise_views_array);
            
            $total_views = $this->db->select('id')->from('exercise_views')->where('post_id', $video_id)->get()->num_rows();
        ...
        if ($type == 'social_events' || $type == 'special_days' || $type == 'advertising_campaign') {
            ...
            $med_tube_views = array(
                'user_id' => $user_id,
                'video_id' => $video_id
            );
            $this->db->insert('med_tube_views', $med_tube_views);
            
            $total_views = $this->db->select('id')->from('med_tube_views')->where('video_id', $video_id)->get()->num_rows();
    ...
    public function med_spread($user_id,$page,$keyword)
    {
        ...
        if($keyword != '')
        {
        $med_spread_query = $this->db->query("SELECT * FROM med_spread where title LIKE '%$keyword%' order by id asc limit $start,$limit");
        }
        else
        {
        $med_spread_query = $this->db->query("SELECT * FROM med_spread order by id asc limit $start,$limit");    
        }
