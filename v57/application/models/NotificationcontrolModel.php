...
class NotificationcontrolModel extends CI_Model {
    ...
    public function Notification_All_list($user_id,$page)
    {
      ...
      //  $radius = $page*5;
        ...
        $query = $this->db->query("SELECT * FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications') order by id desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
            
            $query1 = $this->db->query("SELECT * FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications') ");
