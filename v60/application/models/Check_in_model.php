...
if($product['day']===$dfinal)
{
 $current_day_final = $product['time'][0];
    ...
    $distances = str_replace(',', '.', GetDrivingDistance($mlat, $lat, $mlng, $lng));
     ...
     public function addCheckInDetails($user_id,$vendor_id,$amount,$tags,$show_image)
    {
        ...
        $date = date('Y-m-d H:i:s');
         ...
         $p_results = $this->db->query("INSERT INTO `check_in_data`(`vendor_id`, `user_id`, `bill_image`, `tags`, `amount`, `created_at`,`showimage`) VALUES('$vendor_id','$user_id','$profile_pic' ,'$tags', '$amount','$date','$show_image')");
