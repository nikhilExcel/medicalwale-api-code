...
class NotificationcontrolModel extends CI_Model {
    ...
    public function Notification_All_list($user_id,$page)
    {
      ...
      //  $radius = $page*5;
        ...
        $count = $query->num_rows();
         $query1 = $this->db->query("SELECT count(id) as id FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order') ");
            $count1 = $query1->row()->id;
       // print_r($count1);
     ...
     public function Notification_All_list_test($user_id,$page)
    {
        if($page==""){
         $page=1;   
        }
