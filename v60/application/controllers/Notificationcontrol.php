...
class Notificationcontrol extends CI_Controller {
    ...
    public function Notification_All_list() {
	 ...
	 public function Notification_All_list_test() {
        $this->load->model('NotificationcontrolModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array(
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $resp = $this->NotificationcontrolModel->Notification_All_list_test($user_id,$page);
                    }
                    simple_json_output($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $user_id = $params['user_id'];
                        $id  = $params['id'];
                        $resp = $this->NotificationcontrolModel->Notification_Delete($user_id,$id);
                    }
                    simple_json_output($resp);
                }
            }
