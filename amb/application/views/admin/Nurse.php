...
<?php echo $this->session->unset_userdata("type"); ?>
        ...
        $(document).on('click', '.btnaction', function () {
            ...
            if (action == 'view') {
                $.ajax({
                    type: 'post',
                    url: '<?php echo $this->config->base_url() ?>admin/viewNurse',
                    data: "user_id=" + id,
            ...
            if (action == 'edit') {
                $.ajax({
                    type: 'post',
                    url: '<?php echo $this->config->base_url() ?>admin/getNurse',
                    data: "user_id=" + id,
                    success: function (data) {
                        $("#confirm").modal("show");
                        $("#response").html(data);
                    ...
                    <div class="panel-body">
                        <div class="panel widget">
                            <div  class="panel-body table-responsive">
                                <div class="col-md-12">
                                    <form action="<?= $this->config->base_url() . 'admin/Nurse_serch' ?>" method="post">
                                      ...
                                      <div class="col-md-2">
                                          ...
                                          <div class="col-md-2">
                                            ...
                                            <select class="form-control" name="is_onlines">
                                                <option value="">Select</option>
                                               
                                                 <option <?php echo isset($data['is_onlines']) ? $data['is_onlines'] == '1' ? 'selected' : '' : ''; ?>  value="1">Online</option>
                                                  <option <?php echo isset($data['is_onlines']) ? $data['is_onlines'] == '0' ? 'selected' : '' : ''; ?>  value="0">offline</option>
                                            </select>
                                        </div>
                                        ...
                                        <?php if (!empty($chk)) { ?>
                                            <div class="col-md-1" style="margin-top:29px;width:105px;margin-left:14px">
                                                <a href="<?= $this->config->base_url() . 'admin/Nurse' ?>" style="color: red">Clear search</a>
                                            </div>
                                        <?php } ?>
                            ...
                            <div class="panel-heading vd_bg-grey">
                                <h3 class="panel-title" style="color:white"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> Nurse </h3>
                            </div>
                            <div  class="panel-body table-responsive">
                                        ...
                                        if (!empty($result)) {
                                            foreach ($result as $val) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?= $val->price ?></td>                                                    <td><?= $val->status == 1 ? '<span id="span" class="label label-success" style="background-color:green;color:white;">Active</span>' : '<span id="span" class="label label-success" style="background-color:red;color:white;">Deactive</span>'; ?></td>
                                                    <td><?= $val->is_online == 1 ? '<span id="span" class="label label-success" style="background-color:green;color:white;">Online</span>' : '<span id="span" class="label label-success" style="background-color:red;color:white;">offline</span>'; ?></td>
                                                    <td><span class="menu-action hiderow<?= $val->user_id ?>"><a id="<?= $val->user_id ?>" data-original-title="edit" data-toggle="tooltip" data-placement="top" class="btnaction btn menu-icon vd_bd-yellow vd_yellow"> <i class="fa fa-pencil"></i> </a> <a id="<?= $val->user_id ?>" data-original-title="delete" data-toggle="tooltip" data-placement="top" class="btnaction btn menu-icon vd_bd-red vd_red"> <i class="fa fa-times"></i> </a><a id="<?= $val->user_id ?>" data-original-title="view" data-toggle="tooltip" data-placement="top" class="btnaction btn menu-icon vd_bd-green vd_green"> <i class="fa fa-eye"></i> </a></span></td>
                                                </tr>
                                                <?php
                                            }
                                        }
