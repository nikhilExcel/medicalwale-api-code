   ...
   else
   {
    ...
    public function notifyMethod1($listing_id, $user_id, $description, $booking_date, $booking_time, $appointment_id, $connect_type, $user_name)
    {
        ...
        if ($customer_token_count > 0) {
            ...
            $msg          = "Your Appointment for ".$booking_date." at ".$booking_time." for ".$usr_name." is confirmed";
            $type = 'Booking';
...
public function doctor_pic_list111($group_user_id) 
{
         ...
         $disCount = $discountQuery->num_rows();
            ...
            if ($disCount > 0) {
                ...
                if ($row['image'] != '') {
                    $profile_pic = $row['image'];
                    $profile_pic = str_replace(' ', '%20', $profile_pic);
                    $profile_pic = 'https://medicalwale.s3.amazonaws.com/images/healthwall_avatar/' . $profile_pic;
                } else {
                    $profile_pic = 'https://d2c8oti4is0ms3.cloudfront.net/images/img/default_profile_pic_user.png';
                }
    ...
    public function list_patient($doctor_id) 
    {
            ...
            $query1 = $this->db->query("SELECT *  FROM `doctor_patient` WHERE `doctor_id`='$doctor_id' and status = 0 group by contact_no order by created_date desc ");
            $count1 = $query1->num_rows();
            if ($count1 > 0) 
                {
                       ...
                       foreach($qRows as $qRow)
                            {
                                     ...
                                     $prescription_date = $qRow['created_date'];
                                      ...
                                      $resultpost[] = array(
                                            ...
                                            'prescription_id' => 'https://doctor.medicalwale.com/doctor/doctor_prescription.php?id='.$prescription_id,
                                            'doctor_id' => $doctor_id,
                                            'patient_id' => $patient_id,
                                            'prescription_note' => $prescription_note,
                                            'prescription_date' => $prescription_date,
                                            'prescription_pdf'=> "https://doctor.medicalwale.com/doctor/prescription/".$prescription_id.".pdf"
                                        );
        ...
        if ($count_appointment_slot > 0) {
            foreach ($Appointment_dataList->result_array() as $row) 
          {
           ...
           $is_user='1';
            ...
            {
                $url="https://doctor.medicalwale.com/doctor/doctor_prescription.php?id=".$pr_id;
                $pdf="https://doctor.medicalwale.com/doctor/prescription/".$pr_id.".pdf";                           
 ...
 public function insert_doctor_users_feedback($doctor_id,$user_id,$type,$feedback,$ratings,$recommend,$booking_id,$booking_type)
    ...
    public function doctor_edit_timings($data, $doctor_id, $consultation_type) {
     ...
     public function doctor_view_timings($doctor_id, $consultation_type) {
      ...
      $CosultQuery = $this->db->query("SELECT consultation_type FROM `doctor_list` WHERE `user_id` = '$doctor_id'");  
      //  echo "SELECT consultation_type FROM `doctor_list` WHERE `user_id` = '$doctor_id'";
         ...
         if($disCount>0)
         {
              $consultation_typee = $consultrow1->consultation_type;
         }
         else
         {
             $consultation_typee = $consultation_type;
         }
         //end
            if ($disCount > 0) {
                foreach ($discountQuery->result_array() as $rowDis) {
        ...
        $finalData['consultation_type'] = $consultation_typee;
         ...
         $count = $query->num_rows();
          if($count>0)
          {
           ...
           /* echo $_FILES['invoice_file']['name'];
            ...
            include('../s3_config.php'); 
            $img_url ="";
             $image_file_count = count($_FILES['stroy']['name']);
      ...
      //  echo "SELECT id from doctor_dating_status where (user_id='$user_id' and dating_id='$following_id') OR (user_id='$following_id' and dating_id='$user_id')";
        ...
        if ($count > 0) {
         ...
         $this->db->insert('myactivity_notification', $notification_array);
                    ...
                    $this->send_gcm_notify_doctor_friend_request($following_id, $user_id, $title, $reg_id, $msg, $img_url, $tag, $agent);
                }
               return array(
                    'status' => 201,
                    'message' => 'Inserted'
     ...
     public function all_event_list_new($user_id,$status)
    {
         ...
         if($status == 'upcoming')
         {
           /*  $d = date_parse_from_format("Y-m-d", $find_date);
             ...
             $where ="WHERE start_date > '$system_date' ";
       ...
       /* $event_attending = $this->db->query("SELECT event_id,interest FROM event_attending_list where (interest = '1' or interest = '3' )  and user_id = '$user_id' ");
        $event_attending_count = $event_attending->num_rows();
        if($event_attending_count > 0 ) 
        {
           foreach($event_attending->result_array() as $row1)
                        {
            $event_id= $row1['event_id'];
            $event_attending_count_f=$row1['interest'];
            $query = $this->db->query("SELECT * FROM events where id='$event_id' order by id desc");
            $count = $query->num_rows(); 
            if($count>0)
              {
                  foreach($query->result_array() as $row)
                     ...
                     $interested_query = $this->db->query("SELECT id FROM event_attending_list where event_id = '$event_id' and interest = '1' order by id desc");
                   $interested_count = $interested_query->num_rows();
                   
                   $notinterested_query = $this->db->query("SELECT id FROM event_attending_list where event_id = '$event_id' and interest = '2' order by id desc");
                   $notinterested_count = $notinterested_query->num_rows();
                   
                    $maybe_query = $this->db->query("SELECT id FROM event_attending_list where event_id = '$event_id' and interest = '3' order by id desc");
                    $maybe_count = $maybe_query->num_rows();
...
$html .= '
        ...
        // $pdfFilePath ='/home/medicalwale/public_html/vendorsandbox.medicalwale.com/doctor/prescription/'.$output;
         $pdfFilePath = '/data/web/vendor.medicalwale.com/doctor/prescription/'.$output;
