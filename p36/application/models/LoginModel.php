...
class LoginModel extends CI_Model
{
    ...
    public function fetch_comment_reply_edit($comment_id, $comment_type, $comment) {
	 ...
	 function sub_v1($items, $id, $user_id) {
        $ar1 = array();
        $ar = array();
        foreach ($items as $item) {
            $ar = array();
		    ...
		    $question = $item['question'];
                $ID = $item['id'];
                $ar['que_id'] = $item['id'];
                $query = $this->db->query("SELECT * FROM users WHERE health_condition='$question' and id='$user_id'");

                //echo "SELECT * FROM userprofile_question_answer WHERE question_id='$ID'";
                $items2 = $query->row_array();

                $r = $this->sub_v1($items, $item['id'], $user_id);

                if (!empty($r)) {
                    $ar['sub_que'] = $r;
                }
                if ($items2['health_condition'] == null) {
                    $ar['ans'] = "";
                } else {
