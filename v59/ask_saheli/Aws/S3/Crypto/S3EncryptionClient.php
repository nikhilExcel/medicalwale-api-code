...
class S3EncryptionClient extends AbstractCryptoClient {
    ...
    S3Client $client, $instructionFileSuffix = null
    ) {
        $this->client = $client;
    ...
    public function putObjectAsync(array $args) {
        ...
        return Promise\promise_for($this->encrypt(
                                Psr7\stream_for($args['Body']), $args['@CipherOptions'] ?: [], $provider, $envelope
                ))->then(
                        function ($encryptedBodyStream) use ($args) {
                    $hash = new PhpHash('sha256');
                    $hashingEncryptedBodyStream = new HashingStream(
                            $encryptedBodyStream, $hash, self::getContentShaDecorator($args)
                    );
        ...
        $promise = $this->client->getObjectAsync($args)
                        ->then(
                            ...
                            $result['Body'] = $this->decrypt(
                                    $result['Body'], $provider, $envelope, isset($args['@CipherOptions']) ? $args['@CipherOptions'] : []
                            );
                            return $result;
                        }
                        )->then(
            ...
            if (!empty($saveAs)) {
                file_put_contents(
                        $saveAs, (string) $result['Body'], LOCK_EX
                );
            }
            return $result;
        }
