...
class Substitute_model extends CI_Model {
    ...
    public function substitute_list($mname) {
        ...
        $sql = "SELECT id,group_id,product_name,product_price FROM product WHERE product_name like '%$mname1%' ";
        ...
        if ($count > 0) {
            ...
            if($g!=""){
            $sql1 = "SELECT * FROM product WHERE group_id = '$g' ORDER BY CAST(product_price AS DECIMAL(18,2)) ASC";
        ...
        if ($count1 > 0) {
            foreach ($query1->result_array() as $row) {
                      ...
                      $resultpost[] = array(
                                          ...
                                          'medicine_name' =>  stripslashes($row['product_name']),
                                         // 'qty' => $row['qty'],
             ...
             $id = $query->row()->id;
              $group_id = $query->row()->group_id;
               $medicine_name = $query->row()->product_name;
                $price = $query->row()->product_price;
            $resultpost[] = array(
                                           'id' => $id,
                                           'group_id' => $group_id,
                                          'medicine_name' =>  stripslashes($medicine_name),
                                          'price' => $price,
    ...
    public function substitute_search($mname) {
        $mname1 = addslashes($mname);
        $sql = "SELECT * FROM product WHERE product_name like '%$mname1%'  GROUP BY product_name ORDER BY CAST(product_price AS DECIMAL(18,2)) ASC";
        $query = $this->db->query($sql);
        $count = $query->num_rows();
        if ($count > 0) {
