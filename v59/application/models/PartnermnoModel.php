...
class PartnermnoModel extends CI_Model {
    ...
    public function pharmacy_list($mno_id, $lat, $lng, $page_no, $per_page){
        ...
        if($count>0){
            foreach ($query->result_array() as $row) {
                ...
                $drivingDistance = $this->PartnermnoModel->GetDrivingDistance($mlat, $lat, $mlng, $lng);
            ...
            $data['data_count'] = strval($pharmacy_count);
   ...
   public function GetDrivingDistance($lat1, $lat2, $long1, $long2) {
    ...
    public function order_list($mno_id, $page_no, $per_page){
        ...
        $data['data_count'] = strval($orders_count);
        $data['per_page'] = $per_page;
        $data['current_page'] = $page_no;
        $data['first_page'] = 1;
        $data['last_page'] = "$last_page";
    ...
    public function order_accept_reject($mno_id,$invoice_no, $accepted, $cancel_reason){
        ...
        if ($count > 0) {
            $current_status = $this->db->query("SELECT status FROM `mno_orders` WHERE `invoice_no` LIKE '$invoice_no'")->row_array();
     ...
     public function assign_mno($user_id,$invoice_no,$lat,$lng){
        ...
        $busyMnos = $this->db->query("SELECT DISTINCT ml.`mno_id` FROM `mno_list` as ml left join mno_orders as mo on (ml.mno_id = mo.mno_id) where ml.mno_id NOT IN (SELECT ml.`mno_id`  FROM mno_list as  ml left join mno_orders as mo on (ml.mno_id = mo.mno_id) where mo.ongoing = 1)")->result_array();
            ...
            if(!empty($busyMnos) &&  sizeof($busyMnos) > 0){
                ...
                if(!empty($mno_id) &&  sizeof($mno_id) > 0){
                    ...
                    $listing_name = '';
                    $invoice_no = '';
                    send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $key_count, $order_status, $order_date, $invoice_no, $invoice_no, $name, $listing_name, $agent);
    ...
    public function order_details($invoice_no){
        ...
        // echo "select action_by,order_id,order_type,delivery_time,listing_id,cancel_reason,action_by,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status,rxId from user_order where `order_id` = '$order_id' group by invoice_no order by order_id desc";
        // die();
        foreach ($query->result_array() as $row) {
            $user_id = $row['user_id'];
...
// add_new_pharmacy
    public function add_new_pharmacy($details){
        ...
        $pharmacy_exists = $this->db->query("SELECT * FROM `mno_suggested_pharmacies` WHERE `gst` LIKE '123'")->result_array();
        ...
        if(sizeof($pharmacy_exists) > 0){
            ...
            $data = array("insert_id" => $insert_id, "phamacy" => $phamacy, "exists" => 0);
    ...
    public function suggested_pharmacy_list($mno_id,$page_no,$per_page){
            $offset = $per_page*($page_no - 1);
            $limit = "LIMIT $per_page OFFSET $offset"; 
        ...
        if ($count > 0) {
            $data['mno_exists'] = 1;
            $suggested_pharmacies = $this->db->query("SELECT * FROM `mno_suggested_pharmacies` WHERE `mou_signed` = 0 $limit")->result_array();
            $all_stores = $this->db->query("SELECT COUNT(id) as total_count FROM mno_suggested_pharmacies WHERE `mou_signed` = 0")->row_array();
            ...
            $data['data_count'] = strval($store_count);
            $data['per_page'] = $per_page;
            $data['current_page'] = $page_no;
            $data['first_page'] = 1;
            $data['last_page'] = $last_page;
...
// order_to_pharmacy
    public function order_to_pharmacy($mno_id, $invoice_no){
