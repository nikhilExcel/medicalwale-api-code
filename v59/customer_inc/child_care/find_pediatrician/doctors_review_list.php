...
if (isset($_POST['gym_id'])) {
    ...
    $doctors_reviewQuery = mysqli_query($conn, "SELECT find_pediatrician_review.id,find_pediatrician_review.user_id,find_pediatrician_review.product_id,find_pediatrician_review.rating,find_pediatrician_review.review,find_pediatrician_review.service,find_pediatrician_review.date as review_date,timelines.id as user_id,timelines.name as firstname FROM `find_pediatrician_review` INNER JOIN `timelines` ON find_pediatrician_review.user_id=timelines.id WHERE find_pediatrician_review.product_id='$product_id' order by find_pediatrician_review.id desc");
    ...
    if ($doctors_review_count > 0) {
        while ($row = mysqli_fetch_array($doctors_reviewQuery)) {
            ...
            $count_query = mysqli_query($conn, "SELECT id FROM `find_pediatrician_review_likes` where post_id='$id'");
            $like_count = mysqli_num_rows($count_query);

            $comment_query = mysqli_query($conn, "SELECT id FROM `find_pediatrician_review_comment` where post_id='$id'");
            $post_count = mysqli_num_rows($comment_query);

            $user_ids = $_POST['user_id'];
            $like_count_query = mysqli_query($conn, "SELECT id FROM `find_pediatrician_review_likes` where user_id='$user_ids' and post_id='$id'");
