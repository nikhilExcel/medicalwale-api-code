...
// 		 print_r($query1); die();
    ...
    //    print_r($finalArray[0]); die();   
       ...
       foreach($finalArray[0] as $key => $value) {
		...
		$stmt = "SELECT d.pd_id,vd.v_id,vd.v_name,vd.v_company_logo,vd.v_delivery_charge,d.item_type,d.total_units,d.directions,d.dim_length,d.dim_width,d.dim_height,d.product_weight,d.ingredients,d.gender_type, d.origin_country   ,d.pd_name,d.pd_pc_id,d.pd_psc_id,d.pd_added_v_id,d.pd_photo_1,d.pd_photo_2,d.pd_photo_3,d.pd_photo_4,d.pd_mrp_price,d.legal_disclaimer,d.pd_vendor_price,d.safety_warnings,d.is_container_glass,d.speciality,d.is_liquid,d.indications,d.servings,d.variable_product,vd.cap_available,vd.cap_charge,d.pd_quantity,d.pd_long_desc,avg(r.pr_rating) as ratting,COUNT(r.pr_rating) as total_rating,d.pd_view_count as total_view FROM `product_details_hm` d 
		LEFT JOIN product_reviews r ON d.pd_id = r.pr_pd_id 
		LEFT join vendor_details_hm vd 
        on vd.v_id=d.pd_added_v_id		
		...
		foreach($query->result_array() as $query1 ){
	        ...
	        $pd_vendor_price = $query1['pd_vendor_price'];
            $vendor_id = $query1['pd_added_v_id'];
            $product_weight = $query1['product_weight'];
           
            $pd_length = $query1['dim_length'];
            $pd_width = $query1['dim_width'];
            $pd_height = $query1['dim_height'];
      
		} 
		
  
        
	    $offersProd = $this->db->query("SELECT * FROM `vendor_offers` WHERE listing_id='34' AND vendor_id = '$vendor_id' AND `status` = 0");
       ...
       // 		added to wishlist or not
	    ...
	    $query['pd_length']=$pd_length;
		$query['pd_width'] = $pd_width;
		$query['pd_height'] = $pd_height;	 
...
// 	get_offers
    public function get_offers(){
        $allOffers = array();
	    $offers = $this->db->query("SELECT * FROM `vendor_offers` WHERE listing_id='34' AND status = '0'");
...
// 	get_user_orders
    public function get_user_orders($user_id){
        $data = array();
        $pro_list = array();
	     $results = $this->db->query("SELECT uo.*,vd.delivery_by_medicalwale FROM `user_order` as uo left join vendor_details_hm as vd on (uo.`listing_id` = vd.v_id) WHERE `user_id` = '$user_id' AND  `listing_type`='34' ORDER BY order_id DESC");
...
// 	get_banner_images
	public function get_banner_images(){
	     $bannerImages = $this->db->query("SELECT * FROM `banner_images` WHERE `status` = 1 ORDER BY `id` DESC");
