		...
		if($insert_id != ""){
         ...
         VALUES ('".$insert_id."','".$data['product_id'.$i]."','".$data['product_qty'.$i]."','".$data['vendor_id'.$i]."','".$data['is_offer'.$i]."','".$data['offer_id'.$i]."','".$data['offer_total'.$i]."')";
    		...
    		foreach($query1 as $q1){
    		    ...
    		    $count = $this->db->query($tmp)->num_rows();
    		    if($count > 0 )
    		    {
...
// 	new place_order add order by vendor
	 ...
	 public function place_order1($data, $allowed, $products){
...
('$invoice_no','$order_status','$v_id',$listing_type,'$v_name','$name','$mobileno','$cust_id','$mrps','$amount','$dis','$chc','$pincode','$address','$landmark','$city','$state','$paym','$order_date','customer')";
    ...
    // 		white here ****************
    		...
    		foreach($query1 as $q1){
    		    $dbmediquery="SELECT delivery_by_medicalwale,v_company_name,v_address1,v_address2,v_city,v_state,v_country,v_pincode,v_phoneno FROM vendor_details_hm WHERE v_id = '".$q1['vendor_id']."'";
    		    ...
    		    $ret_add       =  $this->db->query($dbmediquery)->row_array()['v_address1'].', '.$this->db->query($dbmediquery)->row_array()['v_address2'].', '.$this->db->query($dbmediquery)->row_array()['v_city'].', '.$this->db->query($dbmediquery)->row_array()['v_state'].', '.$this->db->query($dbmediquery)->row_array()['v_country']; //String,
    		    $ship_pin      =  $this->db->query($dbmediquery)->row_array()['v_pincode']; //String
    		    $ship_phone    =  $this->db->query($dbmediquery)->row_array()['v_phoneno']; //String,
    		...
    		if($delivery_by_medicalwale==='1'){
                $rapidRespone = $this->rapidelivery_create($insert_id,$name,$mobileno,$address,$address1,$pincode,$landmark,$city,$state,$amt,$mode,$ret_add,$ship_pin,$ship_phone,$ship_company,$product_description);
                // print_r($rapidRespone);
    ...
    // 		white here ****************
    		...
    		if($delivery_by_medicalwale==='1'){
               ...
               // print_r($rapidRespone); die();
                if(sizeof($rapidRespone) > 0 && $rapidRespone->status==='200'){
                    $waybill = $rapidRespone->waybill;
                    $order_id = $rapidRespone->order_id;
                    $this->db->query("UPDATE user_order SET waybill = '$waybill' WHERE order_id = '$order_id'");
                }
...
// 		print_r($result); die();
	...
	function rapidelivery_create($oid,$name,$mobileno,$address,$address1,$pincode,$landmark,$city,$state,$amount,$mode,$ret_add,$ship_pin,$ship_phone,$ship_company,$product_description)
    {
    ...
    */
	...
	public function get_products($type=0,$id,$term=""){
       ...
       $this->load->library('Elasticsearch');
		...
		public function get_productselastic($type=0,$id,$term=""){
	    
	    return $doctor=$this->elasticsearchdoctor("hm_product",$term);
	...
	public function cancel_order($order_id, $reason){
        ...
        if($count > 0){
            if($checkAvailable->result_array()[0]['order_status'] == 'Order Cancelled'){
                ...
                // Rapid delivery order cancel by ghanshyam parihar starts
                
                // $waybill = $checkAvailable->result_array()->waybill;
                // if(!empty($waybill)){
                //     $rapidCancelRespone = $this->rapidelivery_cancel($waybill);
                //     // print_r($rapidRespone);
                //     if($rapidCancelRespone->status==='200'){
                //         $waybill = $rapidRespone->waybill;
                //         $order_id = $rapidRespone->order_id;
                //         $this->db->query("UPDATE user_order SET waybill = '$waybill' WHERE order_id = '$order_id'");
                //     }
                ...
                // Rapid delivery order cancel by ghanshyam parihar ends
            }
        } else {
...
public function add_filters($filter){
       ...
       $bestSeller = $filtered = $brnd = $finalDataArray = $cats = $subCatsProducts = $subCatsProducts1 = $allSubcategories =$finalData =  array();
        ...
        if($filter['in_stock']>=1){
            $stock = $filter['in_stock'];
            $finalData = "SELECT DISTINCT * FROM `product_details_hm` WHERE (pd_added_v_id IN (".$brnd.") AND (`pd_pc_id` IN (".$all.") OR `pd_psc_id` IN (".$all."))) AND pd_quantity >= 1 AND pd_status = 1 AND pd_mrp_price > 0 	";
        } else {
            $finalData = "SELECT DISTINCT * FROM `product_details_hm` WHERE pd_added_v_id IN (".$brnd.") AND  (`pd_pc_id` IN (".$all.") OR `pd_psc_id` IN (".$all.")) AND pd_status = 1 AND pd_mrp_price > 0 ";
           // $stock = 0;
...
// 	get_sponsored_advertisements
    public function get_sponsored_advertisements($user_id){
        ...
        // print_r($today); die();
        $res = $this->db->query("SELECT * FROM `sponsored_advertisements` WHERE `status` = 1 AND `expiry` >= '$today' AND `vendor_type` = 34 ORDER BY RAND() ");
