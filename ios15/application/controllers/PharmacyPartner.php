            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($product_id == "" || $user_id == "" || $quantity == "") {
                        ...
                        $resp = $this->PharmacyPartnerModel->stock_availability($user_id,$hub_user_id,$product_id,$quantity);
                    }
                    simple_json_output($resp);
                }
            }
     ...
     public function inventory_stock_report_list()
    {
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), TRUE);
                    if ($params['user_id'] == "" || $params['staff_user_id'] == "" || $params['filter']=="") {
                        $resp = array(
                            'status' => 400,
                            'message' => 'please enter fields'
                        );
                        ...
                        if($filter == "surplus" || $filter == "lowstock")
                        {
                            ...
                            {
                                $resp         = $this->PharmacyPartnerModel->inventory_stock_report_list($user_id,$staff_user_id,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to);
                            }
                        }
                        else
                        {
