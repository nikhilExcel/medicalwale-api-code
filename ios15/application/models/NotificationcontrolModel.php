...
SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) order by notification_date  desc  LIMIT $start, $limit");
     ...
     public function Notification_All_list_v1($user_id,$page,$type)
    {
      ...
      //$radius = $page*5;
        ...
        if($type=="promo"){
        $query = $this->db->query("SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) and   notification_type!='article' and notification_type!='HealthDictionary' order by notification_date desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
        ...
        if($type=="learn"){
        $query = $this->db->query("SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) and notification_type='article' || notification_type='HealthDictionary' order by notification_date desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
