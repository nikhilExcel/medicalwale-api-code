        ...
        $count1 = $sql->num_rows();
            if ($count1 > 0) {
                ...
                if($vid=="13" || $vid=="38" || $vid=="44")
                    {
                        $query = $this->db->query("SELECT user_order.*,medical_stores.medical_name,medical_stores.profile_pic FROM user_order LEFT JOIN medical_stores on user_order.listing_id=medical_stores.id WHERE (user_order.listing_type='13' or user_order.listing_type='38' or user_order.listing_type='44') AND user_order.user_id='$user_id' and  user_order.invoice_no='$invoice_no' group by user_order.invoice_no order by user_order.order_id DESC");
                        $count = $query->num_rows();
                        if ($count > 0) 
                           { 
            foreach ($query->result_array() as $row) {
                ...
                if($vid=="34")
                   {
                        ...
                        $pro_list = array();
                        $results = $this->db->query("SELECT vd.delivery_by_medicalwale,vd.v_company_logo,uo.* FROM `user_order` as uo left join vendor_details_hm as vd on (uo.`listing_id` = vd.v_id) WHERE uo.`user_id` = '$user_id' AND  uo.`listing_type`='34'  and uo.invoice_no='$invoice_no' ORDER BY order_id DESC");
                	            ...
                	            if (array_key_exists("order_date",$order))
                                          {
                                            ...
                                            if (array_key_exists("v_company_logo",$order)) 
                                            {
                                            
                                             unset($order['v_company_logo']);
                                           }
                        	        if($value == null){
                        	            $order[$key] = "";
                        	        }
...
$count = $query->num_rows();
            ...
            $resultpost8 = array();
                ...
                $final_array=array_merge($resultpost,$resultpost1, $resultpost2,$resultpost3,$resultpost4,$resultpost5,$resultpost7,$resultpost8);
                        ...
                        $query1 = $this->db->query("SELECT * from life_saving_drugs where user_id='$user_id' order by created_at DESC  LIMIT $start, $limit");
                         $count1 = $query1->num_rows();
                           if ($count1 > 0)
                            ...
                            $resultpost=array_merge($resultpost4,$resultpost5);
                            function cmp($a, $b)
{
    $aDateTime = new DateTime($a["order_date"]);
    $bDateTime = new DateTime($b["order_date"]);
                ...
                if($vid=="34")
                   {
                        ...
                        $pro_list = array();
                        $results = $this->db->query("SELECT vd.delivery_by_medicalwale,vd.v_company_logo, uo.* FROM `user_order` as uo left join vendor_details_hm as vd on (uo.`listing_id` = vd.v_id) WHERE uo.`user_id` = '$user_id' AND  uo.`listing_type`='34'   ORDER BY order_date DESC LIMIT $start, $limit");
                ...
                if($vid=="34")
                   {
                        ...
                        $pro_list = array();
                        $results = $this->db->query("SELECT vd.delivery_by_medicalwale, v_company_logo, uo.* FROM `user_order` as uo left join vendor_details_hm as vd on (uo.`listing_id` = vd.v_id) WHERE uo.`user_id` = '$user_id' AND  uo.`listing_type`='34' and uo.invoice_no='$booking_id' ");
