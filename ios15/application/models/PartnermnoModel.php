...
class PartnermnoModel extends CI_Model {
   ...
   public function GetDrivingDistance($originLat, $destLat, $originLng, $destLng) {
    ...
    public function order_list($mno_id, $page_no, $per_page,$type){
        ...
        $allOrders = $this->db->query("SELECT mo.id,  mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE `mno_id` = '$mno_id' $typeWHere ORDER BY uo.invoice_no desc $limit")->result_array();
        $allOrders_count = $this->db->query("SELECT mo.id FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE `mno_id` = '$mno_id' $typeWHere")->result_array();
                ...
                if($current_status['status'] == 'accepted'){
                    ...
                    return 7;
                     ...
                     if($accepted == false){
                        ...
                        $existingOrder = $this->db->query("SELECT mo.*, u.id, u.phone, u.token,  u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id)  WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
    ...
    public function order_to_pharmacy($mno_id, $invoice_no, $pharmacy_id, $suggested_pharmacy_id){
        ...
        if($pharmacy_id > 0){
            ...
            if(sizeof($sp) > 0){
                ...
                $this->db->query("UPDATE user_order set `suggested_pharmacy_id` = '$suggested_pharmacy_id', `listing_name` = '$spName' WHERE `invoice_no` = '$invoice_no'");
                $data['store_exists'] = 1; 
            } else {
                 $data['store_exists'] = 0; 
...
// remaining 
    ...
    public function order_pickedup($mno_id,$invoice_no, $order_pickedup){
        $this->load->model('OrderModel');
        $existingOrder = $this->db->query("SELECT mo.*, u.id, u.phone, u.token,  u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id)  WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
        if($existingOrder > 0 && $order_pickedup === true){
            ...
            $reg_id = $existingOrder['token'];
            $msg = "Night owl pickedup order from pharmacy store";
            $img_url   = 'https://medicalwale.com/img/noti_icon.png';
            $tag = "text";
            $agent = $existingOrder['agent'];
            $invoice_no = $invoice_no;
    ...
    public function order_delivered($mno_id,$invoice_no, $order_delivered){
        $this->load->model('OrderModel');
        $existingOrder = $this->db->query("SELECT mo.*, u.id, u.phone, u.token,  u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id)  WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
        if($existingOrder > 0 && $order_delivered === true){
            $action_by_status = "Night owl";
    ...
    public function at_store($mno_id,$invoice_no, $at_store){
        $this->load->model('OrderModel');
        $existingOrder = $this->db->query("SELECT mo.*, u.id, u.phone, u.token,  u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id)  WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
        if($existingOrder > 0 && $at_store === true){
            $action_by_status = "Night owl";
   ...
   public function tracker_notifications($invoice_no,$mno_orders_id) {
        ...
        $getNotificationTypes = $this->db->query("SELECT * FROM `mno_notification_types` ")->result_array();
        ...
        $getOrderInfo = $this->db->query("SELECT uo.suggested_pharmacy_id, uo.listing_id FROM `user_order` as uo WHERE `invoice_no` LIKE '$invoice_no' ")->row_array();
        
        
        foreach($getNotificationTypes as $g){
            
            $s['message'] = $g['notification_type_description'];
            $s['step'] = $g['notification_type'];
            $s['type'] = $g['id'];
            $s['time'] = "";
            $s['is_completed'] = 0;
            $statuses[] = $s;
        ...
        if($pharmacy_id > 0 && $pharmacy_id != null && $pharmacy_id != "" ){
            ...
            foreach($statuses as $st){
                ...
                if($find > -1){
                    ...
                    $st['is_completed'] = 1;
                    $st['time'] = $getStatuses[$find]['created_at'];
                    $allStatuses[] = $st;
                } else {
