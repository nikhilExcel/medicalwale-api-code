   ...
   /* public function order_list($listing_id, $listing_type, $order_type)
    ...
    public function order_list($listing_id, $listing_type, $order_type)
    {
        ...
        if($listing_type == 0){
            ...
            $whereListing = "and listing_type='$listing_type'";
        }
    ...
    public function order_status_common($order_id_data, $prescription_id_data,$order_details,$prescription_details)
    {
       ...
       $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',listing_id='$listing_id' where order_id='$order_id_data'");
       ...
       $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',listing_id='$listing_id' where order_id='$prescription_id_data'");
    ...
    public function order_status($order_id, $delivery_time, $order_status, $listing_id, $listing_type, $order_data_list)
    {
       ...
       $this->db->update('user_order',array('listing_name' => $medical_name));
        ...
        $query = $this->db->query("UPDATE user_order SET order_status='$order_status',delivery_time='$delivery_time',listing_id='$listing_id' where order_id='$order_id'");
        ...
        if ($token_status > 0) {
            ...
            if ($type == 'Order Delivered') {
              ...
              //  $msg   = 'Delivered: Order no.'.$invoice_no.'  is delivered. We hope your experience with Medicalwale.com has been satisfactory, for feedback and queries contact us on Medicalwale.com.';
                 $title = 'Order Delivered';
                        //$msg   = 'Delivered: Order no.'.$invoice_no.' of Rs.'.$order_total.' is delivered. We hope your experience with Medicalwale.com has been satisfactory, for feedback and queries contact us on Medicalwale.com.';
                        $msg='Your order has been delivered by '.$order_info->medical_name .' Thanks for choosing Medicalwale.com';
                $notification_type="pharmacy_order_deliver";
...
//$stock =  $this->Inventory_master_m->get_warehouse_stock($q->id,$pack->id); 
	...
	public function pharmacy_logo($listing_id, $licence_pic_file)
   {
     ...
     public function all_order_list($user_id,$staff_user_id,$from_date,$to_date,$filter,$amount_from,$amount_to,$discount_from,$discount_to,$prescription)
    {
     ...
     public function inventory_stock_report_list($user_id,$staff_user_id,$filter,$stock_limit,$brand,$purchase_order,$expiry_date_from,$expiry_date_to)
    {
        if($filter =="all")
        {   
            $resultpost = array();
                   ...
                   /* $ingredients            = $row['ingredients'];
                    ...
                    foreach($warehouse as $q) {
                        if($q->id==0)
            	       	...
            	       	{
                	       	$sql1="SELECT sum(stock_quantity) as stock_quantity FROM inventory_stock where user_id='$user_id'  AND product_id='$id' AND warehouse_id='$w_id' $where";
                            $stock=$this->db->query($sql1)->row();
