...
class PharmacyPartnerModel extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT id,otp_code,name,email,phone,city,vendor_id FROM users WHERE phone='$phone' and vendor_id>'0'");
        $count = $query->num_rows();
        if ($count > 0) {
   ...
   /* public function order_list($listing_id, $listing_type, $order_type)
    ...
    public function order_status_common($order_id_data, $prescription_id_data,$order_details,$prescription_details)
    {
        ...
        $order_info    = $this->db->select('user_id,listing_id,listing_name,delivery_time,invoice_no,name,listing_name,order_total')->from('user_order')->where('order_id', $order_id_data)->get()->row();
    ...
    public function order_deliver_cancel($order_id, $cancel_reason, $type, $notification_type)
    {
        ...
        $order_info    = $this->db->select('user_id,listing_id,listing_name,delivery_time,invoice_no,name,listing_name,sum(order_total) as total')->from('user_order')->where('invoice_no', $order_id)->get()->row();
        $user_id       = $order_info->user_id;
        $listing_id    = $order_info->listing_id;
        $listing_name  = $order_info->listing_name;
        $delivery_time = $order_info->delivery_time;
        ...
        if ($token_status > 0) {
            ...
            if ($type == 'Order Cancelled') {
                ...
                $msg   = 'Cancelled: Order no.'.$invoice_no.' of Rs.'.$order_total.' is cancelled. For any queries contact us on Medicalwale.com';
    ...
    {
       $query = $this->db->query("SELECT * FROM inventory_product_type WHERE user_id='$user_id' OR user_id='$staff_user_id'");
       $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id                     = $row['id'];
    ...
    {
       $query = $this->db->query("SELECT * FROM inventory_category WHERE user_id='$user_id' OR user_id='$staff_user_id'");
       $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $sub_category = array();
                if($row['sub_category'] == 0){
                   ...
                   $query1 = $this->db->query("SELECT * FROM inventory_category WHERE sub_category= '$id' AND (user_id='$user_id' OR user_id='$staff_user_id')");
                   $count1 = $query1->num_rows();
                    if ($count1 > 0) {
                        foreach ($query1->result_array() as $row1) {
  ...
  /*  function sub($items,$id, $user_id,$staff_user_id){
        $ar1 =  array();
        $ar = array();      
       ...
       if($item['sub_category'] == $id){
            ...
            $ID = $item['id'];
             $ar['que_id'] = $item['id'];
            $query = $this->db->query("SELECT * FROM inventory_category WHERE sub_category ='$ID' AND user_id='$user_id' OR user_id='$staff_user_id'");
            
          
            $items2 = $query->row_array();
            
            $r = $this->sub($items,$item['id'], $user_id,$staff_user_id); 
    ...
    {
        $query = $this->db->query("SELECT * FROM inventory_product WHERE user_id='$user_id' OR user_id='$staff_user_id'");
       $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id                     = $row['id'];
                ...
                foreach($warehouse as $q) {
        	       ...
        	       echo	$sql1="SELECT sum(stock_quantity) as stock_quantity FROM inventory_stock where (user_id='$user_id' OR user_id='$staff_user_id') AND product_id='$id' AND warehouse_id='$w_id'";
...
//$stock =  $this->Inventory_master_m->get_warehouse_stock($q->id,$pack->id); 
    ...
    {
        $query = $this->db->query("SELECT * FROM inventory_product WHERE barcode='$barcode' AND ( user_id = '$user_id' OR user_id = '$staff_user_id')");
        $count = $query->num_rows();
        if ($count == 0) {
    ...
    {
       $query = $this->db->query("SELECT * FROM inventory_distributor WHERE user_id='$user_id' OR user_id='$staff_user_id'");
       $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id                     = $row['id'];
    ...
    {
        $query = $this->db->query("SELECT * FROM inventory_distributor WHERE distributor_phone='$distributor_phone' AND ( user_id = '$user_id' OR user_id = '$staff_user_id')");
        $count = $query->num_rows();
        if ($count == 0) {
    ...
    {
       $query = $this->db->query("SELECT * FROM inventory_warehouse WHERE user_id='$user_id' OR user_id='$staff_user_id'");
       $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id                     = $row['id'];
    ...
    {
        $query = $this->db->query("SELECT * FROM inventory_warehouse WHERE wname='$wname' AND ( user_id = '$user_id' OR user_id = '$staff_user_id')");
        $count = $query->num_rows();
        if ($count == 0) {
    ...
    public function product_inventory_bill($user_id,$hub_user_id,$date,$invoice_no,$product_details,$total_quantity,$total_price,$discount,$tax,$net_amount,$payment_method,$customer_name,$customer_phone,$customer_email,$customer_address)
    {
        date_default_timezone_set('Asia/Kolkata');
        $created_date = date('Y-m-d H:i:s');
        //Customer details
        ...
        $data_add=array(        'user_id'           => $hub_user_id,
    	                        'vendor_id'         => '13',
    	                        'bill_no'           => $invoice_no,
    	                        'cust_user_id'      => $cust_user_id,
    	                        'cust_id'           => $cust_id,
        ...
        if(!empty($type))
    	{
    	            ...
    	            $medical_id = $user_id;
    	            if($user_id != $hub_user_id)
    	            {
    	                $medical_id = $hub_user_id;
    	            }
