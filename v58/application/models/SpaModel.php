...
class SpaModel extends CI_Model {
 ...
 $review_list_count = $this->db->select('id')->from('spa_center_review_comment')->where('post_id', $id)->get()->num_rows();
   ...
   {
        $personal_trainer =  $this->db->query("SELECT * FROM product_details_hm WHERE pd_added_v_id='126' ");            $count = $personal_trainer->num_rows();
           if ($count > 0)
           {
               foreach ($personal_trainer->result_array() as $row)
               {
                 echo  $id = $row['pd_id'];
                   echo "<br>";                    $pd_mrp_price = $row['pd_mrp_price'];
