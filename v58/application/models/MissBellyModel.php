...
class MissBellyModel extends CI_Model {
    ...
    public function miss_belly_character($type) {
    ...
    public function miss_belly_add_question($user_id, $user_name, $user_image, $question, $age, $height, $weight, $diet_preference, $post_location) {
        date_default_timezone_set('Asia/Kolkata');
        ...
        $this->db->insert('miss_belly_question', $sex_education_question);
         ...
         $new_post_data=json_encode($post_data);  
            ...
            $url='http://52.66.208.83:8004/miss/belly/auto/answer/forum/';
            $ch = curl_init();
    ...
    public function miss_belly_add_reply($user_id, $doctor_id, $post_id, $type, $answer) {
        ...
        $this->db->insert('miss_belly_answer', $sex_education_question);
    ...
    public function miss_belly_question_list($user_id, $page) {
        ...
        $query = $this->db->query("SELECT miss_belly_question.id,miss_belly_question.age,miss_belly_question.weight,miss_belly_question.diet_preference,miss_belly_question.height,miss_belly_question.user_image,miss_belly_question.user_name,miss_belly_question.user_id,miss_belly_question.question,miss_belly_question.date,IFNULL(miss_belly_question.post_location,'') AS post_location,user_character.image AS c_image  FROM  `miss_belly_question` INNER JOIN `user_character` ON miss_belly_question.user_image=user_character.id  WHERE miss_belly_question.id NOT IN (SELECT post_id FROM miss_belly_hide WHERE post_id=miss_belly_question.id AND user_id='$user_id')  order by miss_belly_question.id DESC limit $start, $limit");

        $count_query = $this->db->query("SELECT miss_belly_question.id,miss_belly_question.age,miss_belly_question.weight,miss_belly_question.height,miss_belly_question.user_image,miss_belly_question.user_name,miss_belly_question.user_id,miss_belly_question.question,miss_belly_question.date,user_character.image AS c_image  FROM  `miss_belly_question` INNER JOIN `user_character` ON miss_belly_question.user_image=user_character.id  WHERE miss_belly_question.id NOT IN (SELECT post_id FROM miss_belly_hide WHERE post_id=miss_belly_question.id AND user_id='$user_id')  order by miss_belly_question.id DESC");
    ...
    public function miss_belly_question_details($user_id, $post_id) {
        ...
        $query = $this->db->query("SELECT miss_belly_question.id,miss_belly_question.age,miss_belly_question.weight,miss_belly_question.height,IFNULL(miss_belly_question.post_location,'') AS post_location,miss_belly_question.user_image,miss_belly_question.user_name,miss_belly_question.user_id,miss_belly_question.question,miss_belly_question.date,user_character.image AS c_image  FROM  `miss_belly_question` INNER JOIN `user_character` ON miss_belly_question.user_image=user_character.id  WHERE miss_belly_question.id='$post_id' order by miss_belly_question.id DESC");
    ...
    public function miss_belly_delete($user_id, $post_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $count_query = $this->db->query("SELECT id from `miss_belly_question` WHERE  user_id='$user_id' and id='$post_id'");
        $count = $count_query->num_rows();

        if ($count > 0) {
            $this->db->query("DELETE FROM `miss_belly_question` WHERE user_id='$user_id' and id='$post_id'");
    ...
    public function miss_belly_reply_delete($post_id, $answer_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $count_query = $this->db->query("SELECT id from `miss_belly_answer` WHERE  post_id='$post_id' and id='$answer_id'");
        $count = $count_query->num_rows();

        if ($count > 0) {
            $this->db->query("DELETE FROM `miss_belly_answer` WHERE post_id='$post_id' and id='$answer_id'");
...
public function add_comment($data,$comment1,$user_id){
		...
		$this->db->insert("miss_belly_answer", $data);
