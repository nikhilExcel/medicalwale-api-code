    ...
    public function prescription_add_quickbook($user_id, $address_id, $chat_id, $payment_method, $delivery_charge, $is_night_delivery,$lat,$lng,$product_id,$is_profile,$listing_id,$night_owls) {
        ...
        if($is_profile != 'yes')
           ...
           // $listing_name = implode(',',$listing_name);
              ...
              if(count($listing_id_new) > 1)
            {
                if($is_profile!= 'yes')
                {
                  $listing_name_insert="Insta Order";    
     ...
     /* echo "SELECT `id`, `user_id`,`medical_name` FROM medical_stores WHERE is_approval='1' AND is_active='1' AND user_id = '$check_id'";
      die;*/
        ...
        if($night_owls > 0){
            // $order_id = '123';
            // $this->OrderModel->assign_mno($order_id,$lat,$lng,$radius);
        }
        ...
        if ($order_id > 0 ) {
           ...
           if($night_owls > 0){
            ...
            $order_info_count = $this->db->select('email, token, agent, token_status')->from('users')->where('id', $user_id)->get()->num_rows();
          ...
          if($night_owls == 0){
           ...
           for($i = 0 ; $i<count($listing_id); $i++)
            ...
            if ($partner_token_status > 0) {
                ...
                $img_url1 = 'https://medicalwale.com/img/medical_logo.png';
                $tag1 = 'text';
                $key_count1 = '3';
                $title1 = 'New Order';
                //web notification starts
      ...
      //    print_r($product_details['medicine_product_data']);
        ...
        else
       {
               ...
               foreach ($query->result_array() as $prow) {
                  ...
                  date_default_timezone_set('Asia/Calcutta');
                       ...
                       // $actual_image_name  = $query_status->prescription_link;
                        $this->db->query("INSERT INTO `prescription_order_details`(`order_id`, `prescription_image`,`order_status`, `uni_id`,`description`) VALUES ('$order_id', '$prescription_name','$order_status', '$invoice_no',' $description')");
        ...
        $order_id1 = $this->db->insert_id();
         ...
         if($night_owls > 0){
            // $order_id = '123';
            // $this->OrderModel->assign_mno($order_id1,$lat,$lng,$radius);
        }
...
// print_r($user_order); die();
       ...
       // $listing_name = explode(',',$listing_name);
          if($night_owls == 0){
           ...
           for($i = 0 ; $i<count($listing_id); $i++)
             {           
            $partner_info = $this->db->query('SELECT token, agent, token_status,phone from users where id= '.$listing_id[$i]);
           // echo 'SELECT web_token, agent, token_status,phone from users where id= '.$listing_id[$i];
            $partner_token_status = $partner_info->num_rows();
            $partner_info = $partner_info->row();
            if ($partner_token_status > 0) {
        ...
        if ($order_id > 0) {
           ...
           // echo 'SELECT web_token, agent, token_status,phone from users where id= '.$listing_id[$i];
            ...
            if ($partner_token_status > 0) {
                ...
                $img_url1 = 'https://medicalwale.com/img/noti_pharmacy.png';
                $tag1 = 'text';
                $key_count1 = '2';
                $title1 = 'New Order';
                //web notification starts
  ...
  /*  public function order_confirm_cancel($order_id, $type, $order_status, $cancel_reason) {
    ...
    */
     ...
     public function order_confirm_cancel($order_id, $type, $order_status, $cancel_reason,$mode) {
        ...
        $date = date('Y-m-d');
        $updated_at = date('Y-m-d H:i:s');
        $order_type_query = $this->db->query("select order_type,user_id from user_order where invoice_no='$order_id' ");
        $get_order_info = $order_type_query->row_array();
        $order_type = $get_order_info['order_type'];
        $user_id = $get_order_info['user_id'];
        ...
        if ($type == 'Order Cancelled') {
            $res_status = $this->db->query("select order_status from user_order where invoice_no='$order_id' limit 1");
            $o_status = $res_status->row_array();
            $check_status = $o_status['order_status'];
            if ($check_status == 'Order Delivered') {
                return array(
                ...
                if ($update) {
                    ...
                    if ($token_status > 0) {
                        ...
                        $tag = 'text';
                        $title = 'Order Cancelled ';
                        $msg = 'Your order '.$invoice_no.' has been cancelled' ;
                        send_gcm_notify_usr($title, $reg_id, $msg, $img_url, $tag, $order_status, $updated_at, $order_id, $order_invoice_no, $name, $listing_name, $agent, $order_type);
                 ...
                 if(!empty($user_email))
                    ...
                    if ($token_status > 0) {
                        ...
                        $tag = 'text';
                        $title = 'Order Cancelled';
                        $msg = 'Your order '.$order_invoice_no.' has been cancelled' ;
                        //web notification starts
                        $pharmacy_notifications = array(
                            'listing_id' => $listing_id,
                            'order_id' => $order_id,
...
else
{
 ...
 $user_info_user = $this->db->query("SELECT medical_name,payment_type FROM medical_stores WHERE user_id='$listing_id'");
   ...
   else
   {
    ...
    public function assign_mno($order_id,$lat,$lng,$radius){
        
        date_default_timezone_set('Asia/Kolkata');
        $order_date = date('Y-m-d H:i:s');
        $busyMnos = $this->db->query("SELECT DISTINCT ml.`mno_id` FROM `mno_list` as ml left join mno_orders as mo on (ml.mno_id = mo.mno_id) where mo.mno_id NOT IN (SELECT ml.`mno_id`  FROM mno_list as  ml left join mno_orders as mo on (ml.mno_id = mo.mno_id) where mo.ongoing = 1)")->result_array();
            
            if(sizeof($busyMnos) > 0){
                $sql = sprintf("SELECT ml.`id`, ml.`mno_id`,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance  FROM mno_list  as ml left join mno_track as mt on (ml.mno_id = mt.mno_id) WHERE mt.logged_in <= '$order_date' AND mt.logged_out = '0000-00-00 00:00:00'  HAVING distance < '%s' ORDER BY distance LIMIT 20", ($lat), ($lng), ($lat), ($radius));
                $query = $this->db->query($sql);
                $count = $query->num_rows();
                $mno = $query->row_array();
                
                $mno_id = $mno['mno_id'];
                $insertInOrder = $this->db->query("INSERT INTO `mno_orders`(`order_id`, `mno_id`, `ongoing`, `created_at`, `updated_at`) VALUES ('$order_id','$mno_id','1','$order_date','$order_date')");
