...
// die();
    ...
    public function sociallogin($email,$phone,$oauth_provider,$oauth_uid,$agent,$name,$gender,$image,$token) {
        if ($email == "") {
            ...
            $user_query  = $this->db->query("SELECT id from users where (oauth_provider='$oauth_provider' and oauth_uid='$oauth_uid') $email_ $phone_");
            $check_user  = $user_query->num_rows();
            //$check_user = $this->db->select('id')->from('users')->where('oauth_provider', $oauth_provider)->where('oauth_uid', $oauth_uid)->where('vendor_id', '0')->get()->row();
            if ($check_user>0) {
...
public function family_tree($user_id) {
        ...
        if ($count > 0) {
			...
			if($row['relation_id']==0 || $row['relation_id']=='')
			{
			    $id=$user_id;
			}
			else
...
//   }
  ...
  public function coupon_code($user_id,$coupon,$listing_id,$listing_type,$amount,$card_type){
      $this->load->model('LabcenterModel_v2');
        ...
        $query1 = $this->db->query("SELECT * FROM vendor_offers WHERE name='$coupon' and (vendor_id='$listing_id' || vendor_id='0') and end_date >= '$current_date' and (min_amount <= '$amount' || min_amount >='$amount')");
        $row_fitness1 = $query1->row_array();
        $count1 = $query1->num_rows();  
          if($count1 > 0)
               {
