...
class Spa extends CI_Controller {
    ...
    public function update_price() {
       $this->load->model('SpaModel');        $method = $_SERVER['REQUEST_METHOD'];
       if ($method != 'POST') {
           json_output(400, array(
               'status' => 400,
               'message' => 'Bad request.'
           ));
       } else {            $check_auth_client = $this->SpaModel->check_auth_client();            if ($check_auth_client == true) {
               $response = $this->LoginModel->auth();
               if ($response['status'] == 200) {
                   $params = json_decode(file_get_contents('php://input'), TRUE);                        $resp = $this->SpaModel->update_price();                  json_outputs($resp);
               }
           }
       }
   }
