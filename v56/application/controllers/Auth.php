    ...
    public function generate_privilage() {
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_user == true) {
                $params = json_decode(file_get_contents('php://input'), TRUE);
                $user_id = $params['user_id'];
                $response = $this->LoginModel->generate_privilage($user_id);
                simple_json_output($response);
            }
        }
    }
        ...
        if ($method != 'POST') {
            ...
            if ($is_mail == 'android_mail' || $is_mail == 'ios_mail') {
                if ($name == "" || $mail == "" || $gender == "") {
                    ...
                    $resp = $this->LoginModel->usersignup($name, $phone, $mail, $gender, $dob, $image, $token, $agent, $is_mail);
                }
            } else {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['name'] == "" || $params['mobile'] == "" || $params['pincode'] == "" || $params['address1'] == "" || $params['city'] == "" || $params['state'] == "" || $params['address_type'] == "" ) {
                        ...
                        $resp = $this->LoginModel->address_add($user_id, $name, $mobile, $pincode, $address1, $address2, $landmark, $city, $state, $address_type,$lat,$lng,$full_address);
                    }
                    simple_json_output($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['address_id'] == "" || $params['name'] == "" || $params['address1'] == "" || $params['address2'] == "" || $params['mobile'] == "" || $params['city'] == "" || $params['state'] == "" || $params['pincode'] == "" || $params['address_type'] == "") {
                        ...
                        $resp = $this->LoginModel->address_update($user_id, $address_id, $name, $address1, $address2, $landmark, $mobile, $city, $state, $pincode, $address_type,$lat,$lng,$full_address);
                    }
                    simple_json_output($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['type'] == "" || $params['reason'] == "") {
                        ...
                        $resp = $this->LoginModel->save_pdf_file($file_name, $user_id, $listing_id, $vendor_type, $type, $reason);
                    }
                    simple_json_output($resp);
                }
            }
