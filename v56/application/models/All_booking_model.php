...
where uo.user_id = '$user_id' and uo.listing_type='34'");
   ...
   // Added by Dhaval all booking details
    public function all_booking_details($user_id )
    {
            ...
            if ($count2 > 0) {
                foreach ($query2->result_array() as $row2) {
                      ...
                      if($status == null)
                    {
                     ...
                     if($prescription_id!="")
                     {
                    $url="http://vendorsandbox.medicalwale.com/doctor.medicalwale.com/prescription/".$prescription_id.".pdf";
                     }
                     else
                     {
                         $url="";
        ...
        $count_query4 = $this->db->query("SELECT lb.*,bm.vendor_id,bm.status from booking_master as bm LEFT JOIN lab_booking_details as lb ON(bm.booking_id = lb.booking_id) where bm.user_id='$user_id' and (bm.vendor_id='10' or bm.vendor_id='31') order by bm.id desc");
            ...
            if($report_path_count > 0){
                ...
                "listing_type"=> "31"
                    ...
                    if($package_id > 0){
                        $LP_query = $this->db->query("SELECT * FROM lab_packages WHERE id='$package_id'");
                        if($LP_query->num_rows() >0)
                        {
                            $lab_pack_name = $LP_query->row()->lab_name;
                            $pack_details = $LP_query->row()->lab_details;
                    ...
                    $resultpost4[] = array(
                        ...
                        'status'=>$status,
                        'listing_type'=>$Lbooked['vendor_id']
                    );
                    }
                }
            }else{
...
where hospital_booking_master.user_id = '$user_id' and (hospitals.name_of_hospital like '%$parts[$i]%' or dl.doctor_name like '%$parts[$i]%'or hospital_booking_master.booking_id like '%$parts[$i]%')ORDER BY hospital_booking_master.id DESC");
       ...
       // IPD
       $booking_details = $this->db->query("SELECT bm.*,bm.id as ids, hp.package_name FROM booking_master as bm LEFT JOIN hospital_packages as hp ON (bm.package_id=hp.id) WHERE user_id='$user_id' and vendor_id='8' and (hp.package_name like '%$keywords%' or bm.booking_id like '%$keywords%') order by id DESC");
