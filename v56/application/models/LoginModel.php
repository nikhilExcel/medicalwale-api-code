...
// die();
    ...
    public function userdetails($listing_id, $user_id) {
        if ($listing_id == "") {
            ...
            if ($check_user != "") {
                ...
                $img_count = $this->db->select('media.id')->from('media')->join('users', 'users.avatar_id=media.id','LEFT')->where('users.id', $user_id)->get()->num_rows();
    ...
    //create new user
    public function usersignup($name, $phone, $mail, $gender, $dob, $image, $token, $agent, $is_mail) {
...
/*	public function update_family($id, $name, $phone, $email, $gender, $dob, $profile_pic, $relationship) {
	...
	public function update_family($id, $name, $phone, $email, $gender, $dob, $profile_pic, $relationship) {
        $Q = "SET source = '$profile_pic', title='$profile_pic'";
        $sql = "SELECT avatar_id FROM health_record WHERE id='$id'";
        $avatar_id = $this->db->query($sql)->row()->avatar_id;
        if ($profile_pic != "") {
            if ($avatar_id == 0) {
                date_default_timezone_set('Asia/Kolkata');
                ...
                $this->db->insert('media', $data);
                $aid = $this->db->insert_id();
                $this->db->query("UPDATE health_record SET avatar_id = '" . $aid . "' WHERE id='$id'");
            } else {
                $this->db->query("UPDATE media $Q WHERE id = '" . $avatar_id . "'");
            }
        }
        $query = $this->db->query("UPDATE health_record SET `gender`='$gender',`date_of_birth`='$dob',`patient_name`='$name',`phone`='$phone',`email`='$email',`relationship`='$relationship' WHERE id='$id'");
	...
	public function delete_family($user_id, $family_id) {
	   // echo "DELETE FROM `users` WHERE id='$family_id' AND parent_id='$user_id'";
        $query = $this->db->query("DELETE FROM `health_record` WHERE id='$family_id' AND user_id='$user_id'");
        return array(
            'status' => 201,
            'message' => 'success'
        );
...
public function family_tree($user_id) {
	    $resultpost = array();
		$childpost = array();
		$query = $this->db->query("SELECT health_record.relation_id as id,health_record.patient_name,health_record.gender,health_record.date_of_birth,health_record.phone,health_record.email,health_record.relationship,media.source FROM `health_record` LEFT JOIN media ON health_record.avatar_id = media.id WHERE health_record.user_id='$user_id'");	
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
			    $id           = $row['id'];
  ...
  public function family_relation_tree($user_id) {
    ...
    public function address_list($user_id) {
        $query = $this->db->query("SELECT address_id,name,mobile,pincode,address1,address2,full_address,landmark,city,state,address_type,lat,lng FROM `user_address` WHERE user_id='$user_id' order by address_id desc");
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
...
/****************************************************************/
    ...
    public function address_add($user_id, $name, $mobile, $pincode, $address1, $address2, $landmark, $city, $state, $address_type,$lat,$lng,$full_address) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $address_data = array(
            'address_type' => $address_type,
    ...
    public function address_update($user_id, $address_id, $name, $address1, $address2, $landmark, $mobile, $city, $state, $pincode, $address_type,$lat,$lng,$full_address) {
        $query = $this->db->query("UPDATE `user_address` SET `address_type`='$address_type',`name`='$name',`mobile`='$mobile',`address1`='$address1',`address2`='$address2',`landmark`='$landmark',`city`='$city',`state`='$state',`pincode`='$pincode',`lat`='$lat',`lng`='$lng',full_address='$full_address' WHERE address_id='$address_id' and user_id='$user_id'");
        return array(
            'status' => 201,
            'message' => 'success'
        );
...
$count = $query->num_rows();
    ...
    public function save_pdf_file($pdf_file_name, $user_id, $listing_id, $vendor_type, $type, $reason) {
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        $profile_views_array = array(
            'user_id' => $user_id,
        ...
        return array(
            'status' => 201,
            'message' => 'success',
            'path' => 'https://sandbox.medicalwale.com/user_pdf/' . $pdf_file_name . ".pdf"
        );
    }
