...
class OfwModel extends CI_Model
{
    ...
    public function add_menstrual_cycle_data($user_id,$name,$relationship,$start_period_day, $cycle_length, $total_period_day, $born_of_year, $tutual_cycle, $tracking_resoan, $profile_id)
    {
        $query = $this->db->query("SELECT id from `users` WHERE id='$user_id' limit 1");
        $count = $query->num_rows();
        if ($count > 0) {
            $query1 = $this->db->query("SELECT id from `menstural_cycle_data` WHERE user_id='$user_id' and profile_id='$profile_id'");
            $count1 = $query1->num_rows();
            if ($count1 > 0) {
                // $this->db->where('user_id'=> $user_id, 'id'=> $profile_id)->update('menstural_cycle_data', array(
                    $this->db->where('user_id', $user_id)->where('profile_id', $profile_id)->update('menstural_cycle_data', array(
                    'user_id' => $user_id,
                    'name' => $name,
                ...
                //added by zak for create profile by default
    ...
    public function get_menstrual_cycle_data($user_id,$profile_id)
    {
        ...
        $count = $query->num_rows();
        if ($count > 0) {
            $query1 = $this->db->query("SELECT * from `menstural_cycle_data`  WHERE user_id='$user_id' AND profile_id='$profile_id'");
            $count1 = $query1->num_rows();
            if ($count1 > 0) {
                foreach ($query1->result_array() as $row) {
                    $user_id          = $row['user_id'];
