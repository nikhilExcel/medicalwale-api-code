...
function get_time_difference_php($created_time)
{
		...
		$doctors_reviewQuery = mysqli_query($conn,"SELECT find_opthamologist_review.id,find_opthamologist_review.user_id,find_opthamologist_review.product_id,find_opthamologist_review.rating,find_opthamologist_review.review,find_opthamologist_review.service,find_opthamologist_review.date as review_date,timelines.id as user_id,timelines.name as firstname FROM `find_opthamologist_review` INNER JOIN `timelines` ON find_opthamologist_review.user_id=timelines.id WHERE find_opthamologist_review.product_id='$product_id' order by find_opthamologist_review.id desc");
		...
		$count_query = mysqli_query($conn,"SELECT id FROM `find_opthamologist_review_likes` where post_id='$id'");
		$like_count = mysqli_num_rows($count_query);
		
		$comment_query = mysqli_query($conn,"SELECT id FROM `find_opthamologist_review_comment` where post_id='$id'");
		$post_count = mysqli_num_rows($comment_query);
		
		$user_ids=$_POST['user_id'];
		$like_count_query = mysqli_query($conn,"SELECT id FROM `find_opthamologist_review_likes` where user_id='$user_ids' and post_id='$id'");
