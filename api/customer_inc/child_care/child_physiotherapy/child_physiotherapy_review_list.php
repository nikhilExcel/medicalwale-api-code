...
function get_time_difference_php($created_time)
{
		...
		$doctors_reviewQuery = mysqli_query($conn,"SELECT child_physiotherapy_review.id,child_physiotherapy_review.user_id,child_physiotherapy_review.product_id,child_physiotherapy_review.rating,child_physiotherapy_review.review,child_physiotherapy_review.service,child_physiotherapy_review.date as review_date,timelines.id as user_id,timelines.name as firstname FROM `child_physiotherapy_review` INNER JOIN `timelines` ON child_physiotherapy_review.user_id=timelines.id WHERE child_physiotherapy_review.product_id='$product_id' order by child_physiotherapy_review.id desc");
		...
		$count_query = mysqli_query($conn,"SELECT id FROM `child_physiotherapy_review_likes` where post_id='$id'");
		$like_count = mysqli_num_rows($count_query);
		
		$comment_query = mysqli_query($conn,"SELECT id FROM `child_physiotherapy_review_comment` where post_id='$id'");
		$post_count = mysqli_num_rows($comment_query);
		
		$user_ids=$_POST['user_id'];
		$like_count_query = mysqli_query($conn,"SELECT id FROM `child_physiotherapy_review_likes` where user_id='$user_ids' and post_id='$id'");
