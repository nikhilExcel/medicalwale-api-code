...
function get_time_difference_php($created_time)
{
		...
		$doctors_reviewQuery = mysqli_query($conn,"SELECT babysitter_review.id,babysitter_review.user_id,babysitter_review.product_id,babysitter_review.rating,babysitter_review.review,babysitter_review.service,babysitter_review.date as review_date,timelines.id as user_id,timelines.name as firstname FROM `babysitter_review` INNER JOIN `timelines` ON babysitter_review.user_id=timelines.id WHERE babysitter_review.product_id='$product_id' order by babysitter_review.id desc");
		...
		$count_query = mysqli_query($conn,"SELECT id FROM `babysitter_review_likes` where post_id='$id'");
		$like_count = mysqli_num_rows($count_query);
		
		$comment_query = mysqli_query($conn,"SELECT id FROM `babysitter_review_comment` where post_id='$id'");
		$post_count = mysqli_num_rows($comment_query);
		
		$user_ids=$_POST['user_id'];
		$like_count_query = mysqli_query($conn,"SELECT id FROM `babysitter_review_likes` where user_id='$user_ids' and post_id='$id'");
