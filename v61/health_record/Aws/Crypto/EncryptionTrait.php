...
trait EncryptionTrait {
    ...
    Stream $plaintext, array $cipherOptions, MaterialsProvider $provider, MetadataEnvelope $envelope
    ) {
        $materialsDescription = $provider->getMaterialsDescription();

        $cipherOptions = array_intersect_key(
                $cipherOptions, self::$allowedOptions
        );
        ...
        $cipherOptions['Iv'] = $provider->generateIv(
                $this->getCipherOpenSslName(
                        $cipherOptions['Cipher'], $cipherOptions['KeySize']
                )
        );
    ...
    Stream $plaintext, $cek, &$cipherOptions
    ) {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                ...
                $cipherTextStream = new AesGcmEncryptingStream(
                        $plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'], $cipherOptions['KeySize']
                );
