...
$open_close_status = "";
 ...
 //********************************************added by jakir on 3 aug 2018 for lab changes*************************************************** 
                ...
                $branch_id ="";
                // $branch_query = $this->db->query("SELECT * FROM `lab_center_branch` WHERE user_id='$labcenter_user_id' order by id asc");
                $branch_query = $this->db->query("SELECT lc.*,u.multi_user_type FROM `lab_center` as lc left join users as u on(lc.`branch_user_id` = u.id) WHERE lc.user_id='$labcenter_user_id' AND `is_active` = 1 AND u.multi_user_type = 'branch' order by lc.id asc");
