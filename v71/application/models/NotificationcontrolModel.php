...
class NotificationcontrolModel extends CI_Model {
    ...
    {
        $cuurent_date = date('Y-m-d H:i:s');
        $final_date=date('Y-m-d', strtotime($cuurent_date. ' - 30 days'));
        // added by nikhil (20 march 2021)
        $user_created_at = $this->db->get_where('users',array('id' => $user_id))->row()->created_at;
        
        $promo_count_status=0;
        ...
        $promo = $this->db->query("SELECT id FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) and   notification_type!='article' and notification_type!='HealthDictionary' AND is_read = '0' and notification_date >='$user_created_at' order by notification_date desc ");
    ...
    /*  echo "SELECT * FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order')
                                UNION ALL
                                  SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) and (noti_type='andriod' || noti_type='android') order by created_at  desc  LIMIT $start, $limit";
      
  */  
    // echo "SELECT * FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications' AND notification_type != 'doctor_appointment_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order' AND notification_type != 'doctor_appointment_notifications') UNION ALL SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) and (noti_type='andriod' || noti_type='android') and created_at >='$user_created_at' order by created_at  desc  LIMIT $start, $limit"; die();
    
/*    $query =$this->db->query("SELECT * FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications' AND notification_type != 'doctor_appointment_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order' AND notification_type != 'doctor_appointment_notifications')
                                UNION ALL
    ...
    $query =$this->db->query("SELECT mo.ongoing, anm.* FROM `All_notification_Mobile` as anm left join mno_orders as mo on (mo.invoice_no = anm.invoice_no AND mo.ongoing = '1' AND mo.mno_id > '0')  where (anm.user_id = '$user_id' AND anm.notification_type != 'user_feedback' AND anm.notification_type != 'healthwall_notifications' AND anm.notification_type != 'doctor_appointment_notifications') OR (anm.listing_id = '$user_id' AND anm.notification_type != 'view_notifications' AND anm.notification_type != 'order' AND anm.notification_type != 'doctor_appointment_notifications')
    UNION ALL 
    SELECT mos.ongoing, otn.* FROM `other_notifications` as otn left join mno_orders as mos on (mos.invoice_no = otn.invoice_no AND mos.ongoing = '1' AND mos.mno_id > '0')  where (otn.user_id = '0' || otn.user_id = '$user_id' ) and (otn.noti_type='andriod' || otn.noti_type='android') and otn.created_at >='$user_created_at' order by created_at desc LIMIT $start, $limit");
     ...
     public function Notification_All_list_v1($user_id,$page,$type)
    {
      ...
      //$radius = $page*5;
        ...
        $query = $this->db->query("SELECT * FROM `other_notifications` where (user_id = '0' || user_id = '$user_id' ) and   notification_type!='article' and notification_type!='HealthDictionary' and  notification_type!='quoteweek' and (noti_type='andriod' || noti_type='android') and created_at >='$user_created_at' and notification_date >='$final_date' order by created_at desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
        ...
        if($type=="trans"){
            /*API changes done by Swapnali Waghunde*/
            /*Old query*/
             // echo "SELECT * FROM `All_notification_Mobile` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order') order by created_at desc LIMIT $start, $limit"; die();
                
                $query = $this->db->query("SELECT mo.ongoing, anm.* FROM `All_notification_Mobile` as anm left join mno_orders as mo on (mo.invoice_no = anm.invoice_no AND mo.ongoing = '1' AND mo.mno_id > '0') where (anm.user_id = '$user_id' AND anm.notification_type != 'user_feedback' AND anm.notification_type != 'healthwall_notifications') OR (anm.listing_id = '$user_id' AND anm.notification_type != 'view_notifications' AND anm.notification_type != 'order') order by anm.created_at desc LIMIT $start, $limit");
        ...
        if($type=="myactivity"){
                ...
                $query = $this->db->query("SELECT * FROM `myactivity_notification` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order') and created_at >='$user_created_at' order by created_at desc LIMIT $start, $limit");
        $count = $query->num_rows();
        if ($count > 0) {
            
            $query1 = $this->db->query("SELECT * FROM `myactivity_notification` where (user_id = '$user_id' AND notification_type != 'user_feedback' AND notification_type != 'healthwall_notifications')  OR (listing_id = '$user_id' AND notification_type != 'view_notifications' AND notification_type != 'order') and created_at >='$user_created_at' ");
            $count1 = $query1->num_rows();
