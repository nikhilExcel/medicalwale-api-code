...
class Dental_clinic_model extends CI_Model {
    ...
    public function featured_dental_clinic_list($user_id,$lat,$lng)
    {
        ...
        $sql = sprintf("SELECT *,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_clinic_list  WHERE is_active='1' AND featured='1'  ORDER BY distance", ($lat), ($lng), ($lat), ($radius));
       ...
       /* if($count == 0){
        ...
        echo $sql = sprintf("SELECT *,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_branch  WHERE is_active='1' AND featured='1'  HAVING distance < '5' ORDER BY distance", ($lat), ($lng), ($lat), ($radius));
    ...
    public function view_package($user_id,$listing_id,$package_id,$type)
    {
            ...
            if($count1>0)
            {
                ...
                $image = $row1['image'];
                  if ($image != '') {
                                ...
                                $discounted_price = strval(round($price - ($price * ($discount / 100))));
        ...
        if($type == 2)
        {
            ...
            if($count1>0)
            {
                ...
                $image = $row1['image'];
                if ($image != '') {
                $image_url = 'https://medicalwale.s3.amazonaws.com/images/Dentist/image/Orthosquare/' . $image;
            } else {
                $image_url = 'https://medicalwale.s3.amazonaws.com/images/Dentist/image/Orthosquare/GIC_filling.jpg';
            } 
                 $sql2 = "SELECT * FROM dentist_services_master where id='$category_id'";
                $query2 = $this->db->query($sql2);
                $count2 = $query2->num_rows();
    ...
    public function nearby_dental_clinic_list($user_id,$lat,$lng,$page,$listing_id1,$type,$keyword)
    {
        ...
        function GetDrivingDistance1($lat1, $lat2, $long1, $long2) {
            $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $lat1 . "," . $long1 . "&destinations=" . $lat2 . "," . $long2 . "&mode=walking&language=pl-PL&key=AIzaSyCMHN__LczEziaYuKXQ-SKdiMfx5AiY66o&callback=initMap";
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        ...
        if($type==1 || $type==3 || $type==4)
        {
            ...
            if($keyword!=""){
               $where=" name_of_hospital LIKE '%%$keyword%%' and";
                ...
                $result_data= array();
                $sql = "SELECT 'no_branch' as id,user_id as user_id,name_of_hospital,about_us,establishment_year,speciality,category,surgery,services,address,address_2,pincode,phone,city,state,email,lat,lng,image,rating,review,medicalwale_discount,description,opening_hours,mba,certified,recommended,featured,free_consultancy,( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_clinic_list  WHERE $wh $where is_active='1'  UNION SELECT 'branch' as id,dentists_branch_user_id as user_id,name_of_hospital,about_us,establishment_year,speciality,category,surgery,services,address,address_2,pincode,phone,city,state,email,lat,lng,image,rating,review,medicalwale_discount,description,opening_hours,mba,certified,recommended,featured,free_consultancy,( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_branch  WHERE $wh is_active='1'  ORDER BY distance LIMIT $start, $limit";
               // $sql = sprintf(" ORDER BY distance LIMIT $start, $limit", ($lat), ($lng), ($lat), ($radius));
                ...
                if($count>0)
                {
                    ...
                    if($product['day']===$dfinal)
                    {
                     $current_day_final = $product['time'][0];
                       ...
                       //-----------------------service-------------------------
                        ...
                        if($id == 'branch')
                        {
                            $sql_p = "SELECT * FROM packages WHERE user_id='$listing_id' AND FIND_IN_SET('" . $id . "',branch)";
                        }
                        else 
                        {
                            $sql_p = "SELECT * FROM packages WHERE user_id='$listing_id'";
                    ...
                    $fav_pharmacy = $this->db->select('id')->from('dentist_favourite')->where('user_id', $user_id)->where('listing_id', $listing_id)->get()->num_rows();
                    $distances = str_replace(',', '.', GetDrivingDistance($lat, $lat1, $lng, $lng1));
                    $distances1 = str_replace(',', '.', GetDrivingDistance1($lat, $lat1, $lng, $lng1));
                    $distance_root =  round(($distances1/1000),1);
                        $result_data[] = array(
                               'listing_id'             => $listing_id,
                               'listing_type'           => '39',
                               'name_of_clinic'         => $name_of_hospital,
               ...
               /* $result_data1= array();
                $sql = sprintf("SELECT *,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_branch  WHERE $wh is_active='1' HAVING distance < '%s' ORDER BY distance LIMIT $start, $limit", ($lat), ($lng), ($lat), ($radius));
                $query = $this->db->query($sql);
                $count = $query->num_rows();
              ...
              //$data1 = array_merge($result_data,$result_data1);  
        ...
        if($type==2)
        {
                ...
                $favDetails = $this->db->query("SELECT * FROM dentist_favourite  WHERE user_id = '$user_id' LIMIT $start, $limit");
                $count_main = $favDetails->num_rows();   
              
                if($count_main > 0) {
                    foreach($favDetails->result_array() as $row) {
                        $list = $row['listing_id'];
                        $sql = sprintf("SELECT *,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_clinic_list  WHERE user_id = '$list' AND  is_active='1' HAVING distance < '%s' ORDER BY distance ", ($lat), ($lng), ($lat), ($radius));
                        $query = $this->db->query($sql);
                        $count = $query->num_rows();
                        ...
                        $sql = sprintf("SELECT *,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM dentists_branch  WHERE dentists_branch_user_id = '$list' AND is_active='1' HAVING distance < '5' ORDER BY distance ", ($lat), ($lng), ($lat), ($radius));
                        $query = $this->db->query($sql);
                        $count = $query->num_rows();
        ...
        if($type==5)
        {
            ...
            $sql = "SELECT * FROM dentists_branch  WHERE hub_user_id='$listing_id1' $where  LIMIT $start, $limit";
            $query = $this->db->query($sql);
    ...
    public function nearby_dental_clinic_list_old($user_id,$lat,$lng,$page,$listing_id1,$type,$keyword)
    {
               ...
               $where=" name_of_hospital LIKE '%%$keyword%%' and";
                ...
                if($count>0)
                {
                    foreach($query->result_array() as $row)
                    {
                      ...
                      //   if($listing_id == 33662) {echo $opening_hours;}
                        $dfinal=date("l");
                        $open_hrs ="";
                        $current_day_final="";
                        $first ="";
               ...
               //-----------------------service-------------------------
                ...
                if(!empty($ser))
                {
                    foreach ($ser as $ser1) {
                        ...
                        $Querys = $this->db->query("SELECT * FROM dental_services_offered WHERE category_id='$category_id' AND branch_id='$listing_id' limit 10");
                        $m_service = $Querys->result();    
                            if(!empty($m_service))
                            {
                                foreach ($m_service as $mser1) {
                ...
                //-----------------------packages-------------------------
                $packages = array();
                $sql_p = "SELECT * FROM packages WHERE user_id='$listing_id' limit 10";
                $query_p = $this->db->query($sql_p);
                $count_p = $query_p->num_rows();
    ...
    public function dental_prescription_booking($user_id, $lat, $lng, $listing_id, $address_line1, $address_line2, $city, $state, $pincode, $user_name, $mobile, $email, $gender, $status, $payment_mode, $booking_date, $booking_location, $booking_address, $booking_mobile, $patient_id, $booking_id, $trail_booking_date, $trail_booking_time, $prescription_remark){
        ...
        //     /////////*******************************************excotel text message vendor ***************************************************
          ...
          //  $vendor_phone = $vendor_info->phone;
            $vendor_phone = $vendor_info->phone;; // temp added for testing by zak
    ...
    public function dental_freeconsultancy_booking($user_id, $lat, $lng, $listing_id, $address_line1, $address_line2, $city, $state, $pincode, $user_name, $mobile, $email, $gender, $status, $payment_mode, $booking_date, $booking_location, $booking_address, $booking_mobile, $patient_id, $booking_id, $trail_booking_date, $trail_booking_time,$package_id,$type,$sub_type,$amount,$transaction_id){
        date_default_timezone_set('Asia/Kolkata');
        $created_at   = date('Y-m-d H:i:s');
        //$joining_date = date('Y-m-d');
        if($patient_id == '')
    ...
    public function dental_booking_v2($user_id, $lat, $lng, $listing_id, $address_line1, $address_line2, $city, $state, $pincode, $user_name, $mobile, $email, $gender, $status, $payment_mode, $booking_date, $booking_location, $booking_address, $booking_mobile, $patient_id, $booking_id, $trail_booking_date, $trail_booking_time,$package_id,$type,$sub_type,$amount,$transaction_id,$coupon_id){        
        date_default_timezone_set('Asia/Kolkata');
        $created_at   = date('Y-m-d H:i:s');
        //$joining_date = date('Y-m-d');
        if($patient_id == '')
        ...
        $coupon_discount = "";
            if($coupon_id>0){
                ...
                $coupon=$cuser['name'];
                $card_type='';
                $cres=$this->LoginModel->coupon_code($user_id,$coupon,$listing_id,$listing_type,$amount,$card_type);
         ...
         //  echo 'insert';
            ...
            $this->db->where('user_id',$user_id);
            $this->db->delete('dental_service_cart');  
    ...
    public function dentist_time_slot_v1($user_id)
    {
        $timings = array();
        $todays_day = date('l');
                                ...
                                if($s_slot != $e_slot){
                                        ...
                                        while($startTime < $endTim){
                                            $slot = strtotime($stm[$i]); 
                                                ...
                                                $slots['time_slot']  =  date("g:i A", strtotime($startTime));
                                                if(date('Y-m-d H:i:s', strtotime($slots['time_slot'])) > $timenow) 
                                                {
                                                $time_slots[] = $slots;
                                                }
                                            $startTime = strtotime($startTime);
...
By using these service, you agree that you are bound by the Terms of Service.<br/>Copyright 2017 AEGIS HEALTH SOLUTIONS PVT. LTD. All rights reserved.
 ...
 $review_list_count = $this->db->select('id')->from('dental_clinic_review_comment')->where('post_id', $id)->get()->num_rows();
    ...
    public function packages_list($user_id ,  $given_listing_id ,  $lat ,  $lng ,  $page_no ,  $per_page ,  $sort_price ,  $search , $special_package)
    {
        $data = $result_data1 = $result_data = array();
        
        $where_conditions = $listing_where_clinic = $listing_where_branch = "";
        ...
        if($given_listing_id > 0){
            $listing_where_clinic = " AND  d.user_id = '$given_listing_id'";
            $listing_where_branch = " AND  d.dentists_branch_user_id  ='$given_listing_id'";
        }
        
        if($search != ""){
            $search_where_clinic = " AND (d.name_of_hospital like '%$search%' OR d.address like '%$search%' OR d.address_2 like '%$search%' OR d.pincode like '%$search%' OR d.city like '%$search%' OR p.package_name like '%$search%' OR p.package_details   like '%$search%' ) ";
            $search_where_branch = " AND (d.name_of_hospital like '%$search%' OR d.address like '%$search%' OR d.address_2 like '%$search%' OR d.pincode like '%$search%' OR d.city like '%$search%' OR p.package_name like '%$search%' OR p.package_details   like '%$search%' ) ";
        } else {
            $search_where_clinic = $search_where_branch = "";
        }
   
        if($special_package == 1){
            $special_package_where = " AND  p.special_package = '$special_package'";
        } else {
            $special_package_where = "";
        }
        
        $sql_query = "SELECT ( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance, d.user_id as listing_id , d.name_of_hospital as name_of_clinic , d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image ,  p.id as package_id , p.image as imagess , p.package_name ,  p.package_details ,  p.price ,  p.discount FROM dentists_clinic_list as d  INNER JOIN packages as p ON (d.user_id=p.user_id AND p.v_id='39') WHERE d.is_active='1' $listing_where_clinic   $search_where_clinic $special_package_where
        UNION 
        SELECT ( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance, d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic , d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image ,   p.id as package_id  , p.image as imagess , p.package_name ,  p.package_details ,  p.price ,  p.discount FROM dentists_branch as d  INNER JOIN packages as p ON (d.dentists_branch_user_id=p.user_id AND p.v_id='39') WHERE d.is_active='1'  $listing_where_branch $search_where_branch $special_package_where $order_by $limit" ;
        $query = $this->db->query($sql_query)->result_array();
       
        $total_sql_query = "SELECT ( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance, d.user_id as listing_id ,  p.id as package_id FROM dentists_clinic_list as d  INNER JOIN packages as p ON (d.user_id=p.user_id AND p.v_id='39') WHERE d.is_active='1'  $listing_where_clinic $search_where_clinic $special_package_where
        UNION 
        SELECT ( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance , d.dentists_branch_user_id as listing_id ,  p.id as package_id  FROM dentists_branch as d  INNER JOIN packages as p ON (d.dentists_branch_user_id=p.user_id AND p.v_id='39') WHERE d.is_active='1' $listing_where_branch $search_where_branch $special_package_where"  ;
        
        $total_query = $this->db->query($total_sql_query)->result_array();
        
        $total_count = sizeof($total_query);
        
        foreach($query as $row){
                  
            $listing_id = $row['listing_id'];
            $name_of_clinic = $row['name_of_clinic'];
            $about_us = $row['about_us'];
            $establishment_year = $row['establishment_year'];
            $address = $row['address'];
            $address_2 = $row['address_2'];
            ...
            $package_details = $row['package_details'];
            $price = $row['price'];
            $discount = $row['discount']; // always in percent
		if($discount == null)
            {
                $discount = "0";
            }
            ...
            $discounted_price = strval(round($price - ($price * ($discount / 100))));
             ...
             if ($image != '') {
                $image_url = 'https://medicalwale.s3.amazonaws.com/images/healthwall_avatar/' . $image;
            } else {
                $image_url = 'https://medicalwale.s3.amazonaws.com/images/Dentist/image/Orthosquare/GIC_filling.jpg';
            }  
           ...
           if($total_count == 0){
            ...
            $data['data_count'] = strval($total_count);
            $data['per_page'] = strval($per_page);
            $data['current_page'] = strval($page_no);
            $data['first_page'] = $first_page;
            $data['last_page'] = "$last_page";
            $data['packages'] = $result_data;
        
           return $data;
    } 
    
    
    public function treatments_list($user_id ,  $given_listing_id ,  $lat ,  $lng ,  $page_no ,  $per_page ,  $sort_price ,  $search, $recommended_treatments)
    {
        // $recommended_treatments - pending
        $data = $result_data1 = $result_data = array();
        $total_count =  $first_page =  $last_page =  "";
        $offset = $per_page*($page_no - 1);
        $limit = "LIMIT $per_page OFFSET $offset"; 
        if($sort_price == "low"){
            $order_by = " order by price asc";
        ...
        if($given_listing_id > 0){
            ...
            $listing_where_clinic = $listing_where_branch = "";
        }
        
        if($search != ""){
            $search_where = " AND (d.about_us like '%$search%' OR dsm.name like '%$search%' OR d.name_of_hospital like '%$search%' OR d.address like '%$search%' OR d.address_2 like '%$search%' OR d.pincode like '%$search%' OR d.city like '%$search%' OR p.service_name like '%$search%' OR p.description   like '%$search%' ) ";
        } else {
            $search_where = "";
        }
        if($recommended_treatments == 1){
           $recommendedWHere = "AND p.recommended = '1'"; 
        } else {
            $recommendedWHere = "";
        }
        
        $sql = "SELECT dsm.name as cat_name,d.user_id as listing_id , d.name_of_hospital as name_of_clinic , d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image ,p.image as imagess ,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dentists_clinic_list as d  INNER JOIN dental_services_offered as p ON d.user_id=p.branch_id  left join dentist_services_master as dsm on ( dsm.id=p.category_id) WHERE d.is_active='1'   $listing_where_clinic $search_where   $recommendedWHere
        UNION
        SELECT dsm.name as cat_name,d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic , d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image,p.image as imagess ,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dentists_branch as d  INNER JOIN dental_services_offered as p ON d.dentists_branch_user_id=p.branch_id  left join dentist_services_master as dsm on ( dsm.id=p.category_id) WHERE d.is_active='1' $listing_where_branch $search_where  $recommendedWHere  $order_by $limit";
        
        $sql_all_count = "SELECT dsm.name as cat_name,d.user_id as listing_id , d.name_of_hospital as name_of_clinic , d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image ,p.image as imagess ,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dentists_clinic_list as d  INNER JOIN dental_services_offered as p ON d.user_id=p.branch_id  left join dentist_services_master as dsm on ( dsm.id=p.category_id) WHERE d.is_active='1'   $listing_where_clinic $search_where $recommendedWHere
        UNION
        SELECT dsm.name as cat_name,d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic , d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image,p.image as imagess ,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dentists_branch as d  INNER JOIN dental_services_offered as p ON d.dentists_branch_user_id=p.branch_id  left join dentist_services_master as dsm on ( dsm.id=p.category_id) WHERE d.is_active='1' $listing_where_branch $search_where $recommendedWHere  $order_by";
        
            $result = $this->db->query($sql)->result_array();
        $total_result = $this->db->query($sql_all_count)->result_array();
        $total_count = sizeof($total_result);
        
        foreach($result  as $row) {
            $listing_id              = $row['listing_id'];
            $name_of_hospital        = $row['name_of_clinic'];
            $about_us                = $row['about_us'];
            $establishment_year      = $row['establishment_year'];
            $address                 = $row['address'];
            $address_2               = $row['address_2'];
        ...
        $data['first_page'] = $first_page;
        $data['last_page'] = "$last_page";
        $data['treatments'] = $result_data;
        return $data;
    } 
    public function treatment_master_list($user_id,$page)
    {
        $resultpost = array();
        if($page==""){
         $limit = " LIMIT 0, 10";   
        ...
        $query = $this->db->query("SELECT * FROM dentist_services_master WHERE status='1' order by id desc $limit");
        if(!empty($query)) {
            foreach ($query->result_array() as $row) {
                $id = $row['id'];
                $name = $row['name'];
                ...
                if($image == "")
                {
                    $image_link = "https://medicalwale.s3.amazonaws.com/images/Dentist/image/Orthosquare/GIC_filling.jpg";
                }
                else
                {
                    $image_link = "https://medicalwale.s3.amazonaws.com/images/Dentist/image/Category/$image";
                }
    ...
    public function treatment_master_list_with_treatments($user_id,$page,$cat_id,$sub_cat_name,$lat,$lng)
    {
        $result_data= array();
        $radius = 1;
        $open_hrs = "";
        ...
        function GetDrivingDistance($lat1, $lat2, $long1, $long2) {
            ...
            if(!empty($response_a['rows'][0]['elements'][0]['status'] != 'NOT_FOUND')) {
            $dist = $response_a['rows'][0]['elements'][0]['distance']['text'];}
            else
            $dist ="";
            return $dist;
        ...
        if($cat_id != "")
        {
            $wh = " AND p.category_id='$cat_id'";
        }
        if($sub_cat_name != "")
        {
            $wh .= " AND p.service_name='$sub_cat_name'";
        }
        $sql = sprintf("SELECT dsm.name as cat_name,d.speciality,d.category,d.services,d.rating,d.description,d.review,d.user_id as listing_id , d.name_of_hospital as name_of_clinic , d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image ,p.image as imagess ,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p LEFT JOIN dentists_clinic_list as d ON p.branch_id=d.user_id LEFT JOIN dentist_services_master as dsm ON ( dsm.id=p.category_id) WHERE d.is_active='1' $wh UNION
                        SELECT dsm.name as cat_name,d.speciality,d.category,d.services,d.rating,d.description,d.review,d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic , d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image,p.image as imagess ,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p LEFT JOIN dentists_branch as d ON p.branch_id=d.dentists_branch_user_id LEFT JOIN dentist_services_master as dsm ON ( dsm.id=p.category_id) WHERE d.is_active='1' $wh ORDER BY distance ASC $limit", ($lat), ($lng), ($lat), ($radius));
    ...
    public function treatment_master_dental_clinic($user_id,$page,$cat_id,$lat,$lng)
    {
        $result_data= array();
        $radius = 1;
        $open_hrs = "";
        ...
        $sql = sprintf("SELECT dsm.name as cat_name,d.speciality,d.category,d.services,d.rating,d.description,d.review,d.user_id as listing_id , d.name_of_hospital as name_of_clinic , d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image, ( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p LEFT JOIN dentists_clinic_list as d ON p.branch_id=d.user_id LEFT JOIN dentist_services_master as dsm ON ( dsm.id=p.category_id) WHERE d.is_active='1' $wh group by d.user_id UNION
                        SELECT dsm.name as cat_name,d.speciality,d.category,d.services,d.rating,d.description,d.review,d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic , d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p LEFT JOIN dentists_branch as d ON p.branch_id=d.dentists_branch_user_id LEFT JOIN dentist_services_master as dsm ON ( dsm.id=p.category_id) WHERE d.is_active='1' $wh group by d.dentists_branch_user_id ORDER BY distance ASC $limit", ($lat), ($lng), ($lat), ($radius));
    ...
    public function treatment_master_dental_clinic_v2($user_id,$page,$service_name,$lat,$lng,$sort)
    {
        $result_data= array();
        $radius = 1;
        $open_hrs = "";
        ...
        $wh="";
        $service_namea = explode(',',$service_name);
        $service_name = implode("','",$service_namea);
        if($service_name != "")
        {
            $wh = " AND p.service_name IN ('$service_name')";
        }
        
        $order_by="";
        if($sort == 'AtoZ')
        {
            $order_by = "ORDER BY name_of_clinic ASC";
        }
        else if($sort == 'ZtoA')
        {
            $order_by = "ORDER BY name_of_clinic DESC";
        }
        else if($sort == '' || $sort=='nearby')
        {
            $order_by = "ORDER BY distance ASC";
        }
        $sql = sprintf("SELECT d.speciality,d.category,d.services,d.rating,d.description,d.review,d.user_id as listing_id , d.name_of_hospital as name_of_clinic ,d.rating, d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image, ( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p LEFT JOIN dentists_clinic_list as d ON p.branch_id=d.user_id LEFT JOIN dentist_services_master as dsm ON ( dsm.id=p.category_id) WHERE d.is_active='1' $wh group by d.user_id UNION
                        SELECT d.speciality,d.category,d.services,d.rating,d.description,d.review,d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic ,d.rating, d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  d.image,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p LEFT JOIN dentists_branch as d ON p.branch_id=d.dentists_branch_user_id LEFT JOIN dentist_services_master as dsm ON ( dsm.id=p.category_id) WHERE d.is_active='1' $wh group by d.dentists_branch_user_id $order_by $limit", ($lat), ($lng), ($lat), ($radius));
        ...
        if($count>0)
        {
           ...
           /* $discound_price = "0";
            ...
            {
               $discound_price = round($price-(($price*$discount)/100));
            }*/
            ...
            $distances = str_replace(',', '.', GetDrivingDistance($lat, $lat1, $lng, $lng1));
            $tot_price=0;
            $tot_discound_price = 0;
           for($i=0;$i<count($service_namea);$i++) {
                    $w= " AND branch_id='$listing_id'";
                $ser_name=$service_namea[$i];
                $Querys1 = $this->db->query("SELECT id,image,price,discount FROM dental_services_offered WHERE service_name='$ser_name' $w LIMIT  1");
                $ser1 = $Querys1->row_array();
                if(!empty($ser1))
                {
                    $offered_id = $ser1['id'];
                    $image  =  $ser1['image'];
                    ...
                    $discount  =  $ser1['discount'];
                    $discound_price = "0";
                    if($discount != 0 && $discount != "")
                    {
                       $discound_price = round($price-(($price*$discount)/100));
                       $tot_discound_price = $tot_discound_price + $discound_price;
                    }
            ...
            $result_data[] = array(
              ...
              'price'                  => $tot_price,
              'discount'               => $discount,
              'discound_price'         => "$tot_discound_price",
              'rating'                 => $rating    
            );
        }
    }
    ...
    public function all_treatment_clinic_list($user_id,$treatment_id,$page,$lat,$lng,$sort_price,$search)
    {
        $result_data = array();
        function GetDrivingDistance($lat1, $lat2, $long1, $long2) {
            $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $lat1 . "," . $long1 . "&destinations=" . $lat2 . "," . $long2 . "&mode=walking&language=pl-PL&key=AIzaSyCMHN__LczEziaYuKXQ-SKdiMfx5AiY66o&callback=initMap";
       ...
       // $query = $this->db->query("SELECT m.id as id,m.name,m.category_id,o.* FROM dental_services_offered as o LEFT JOIN dentist_services_master as m ON m.id = o.category_id  WHERE o.category_id='$treatment_id' order by o.id desc $limit");
         if($sort_price == "low"){
            $order_by = " order by price asc";
        } else if($sort_price == "high"){
            $order_by = " order by price desc";
        ...
        if($search != ""){
           ...
           $recommendedWHere = "AND p.category_id = $treatment_id"; 
      
        $sql = "SELECT dsm.name as cat_name,d.user_id as listing_id , d.name_of_hospital as name_of_clinic , d.about_us , d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  p.image as imagess,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p  INNER JOIN dentists_clinic_list as d ON d.user_id=p.branch_id  left join dentist_services_master as dsm on ( dsm.id=p.category_id) WHERE d.is_active='1'    $search_where   $recommendedWHere
        UNION
        SELECT dsm.name as cat_name,d.dentists_branch_user_id as listing_id , d.name_of_hospital  as name_of_clinic , d.about_us ,  d.establishment_year ,  d.address ,  d.address_2 ,  d.pincode ,  d.phone ,  d.city ,  d.state ,  d.email ,  d.lat ,  d.lng ,  p.image as imagess,p.id as package_id,p.category_id,p.service_name,p.description,p.price,p.discount,( 6371 * acos( cos( radians('$lat') ) * cos( radians( d.lat ) ) * cos( radians( d.lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( d.lat ) ) ) ) AS distance FROM dental_services_offered as p INNER JOIN dentists_branch as d  ON d.dentists_branch_user_id=p.branch_id  left join dentist_services_master as dsm on ( dsm.id=p.category_id) WHERE d.is_active='1'  $search_where  $recommendedWHere  $order_by $limit";
    ...
    public function treatments_services_list($user_id,$page,$cat_id,$listing_id)
    {
        $result_data= array();
        $radius = 1;
        $open_hrs = "";
        ...
        $sub_category_details = array();
        if($listing_id == "") {
            $Querys = $this->db->query("SELECT d.* FROM dental_services_offered as d WHERE d.category_id='$cat_id' group by service_name $limit");
        }
        else
        {
            $Querys = $this->db->query("SELECT d.* FROM dental_services_offered as d WHERE d.branch_id='$listing_id' AND d.category_id='$cat_id' group by service_name $limit");
        }
        $ser = $Querys->result();
        if(!empty($ser))
        {
            foreach ($ser as $mser1) {
                $ids = $mser1->id;
                $service_name = $mser1->service_name;
                $category_id = $mser1->category_id;
                $price = $mser1->price;
                $discount = $mser1->discount;
                $image  =  $mser1->image;
                $discound_price = "0";
               ...
               // $cat_name  =  $mser1->name;
                ...
                $cart = false;
                $Querys = $this->db->query("SELECT * FROM dental_service_cart WHERE user_id='$user_id'  AND service_name='$service_name' AND treatment_id='$category_id'");
                $ser = $Querys->result();
                if(!empty($ser))
                {
                    $cart = true;
    ...
    public function add_to_cart_service($user_id,$service_name,$treatment_id,$listing_id)
    {
        if($listing_id == "") {
            $Querys = $this->db->query("SELECT * FROM dental_service_cart WHERE user_id='$user_id'  AND service_name='$service_name' AND treatment_id='$treatment_id'");
            $ser = $Querys->result();
            if(empty($ser))
            {
                 $data  = array(
                ...
                $this->db->insert('dental_service_cart',$data);  
        ...
        {
            $Querys = $this->db->query("SELECT * FROM dental_service_cart WHERE user_id='$user_id'");
            $ser = $Querys->result();
            if(empty($ser))
            {
                $data  = array(
            ...
            {
                $Querys1 = $this->db->query("SELECT * FROM dental_service_cart WHERE user_id='$user_id' LIMIT 1");
                $pre_listing_id = $Querys1->row()->listing_id;
                if($pre_listing_id == "")
                {
                    $count_rec =0;
                    foreach($ser as $row)
                    {
                        $lat_service_name =$row->service_name; 
                        $Queryss = $this->db->query("SELECT d.* FROM dental_services_offered as d WHERE d.branch_id='$listing_id' AND service_name='$lat_service_name'");
                        if(!empty($Queryss))
                        {
                            $count_rec++;
                        }
                ...
                else if($pre_listing_id != "")
                {
                    if($pre_listing_id==$listing_id)
                    {
                        $data  = array(
                                'user_id'       => $user_id,
                                'listing_id'    => $listing_id,
    ...
    public function delete_from_cart_service($user_id,$cart_id)
    {
        if($cart_id != "") {
            $Querys = $this->db->query("SELECT * FROM dental_service_cart WHERE id='$cart_id'");
            $ser = $Querys->result();
            if(!empty($ser))
            {
    ...
    public function unselect_from_cart_service($user_id,$service_name)
    {
        $Querys = $this->db->query("SELECT * FROM dental_service_cart WHERE user_id='$user_id' AND service_name='$service_name'");
        $ser = $Querys->result();
        if(!empty($ser))
        {
            $this->db->where('user_id',$user_id);
    ...
    public function service_cart_list($user_id)
    {
        $result_data= array();
        $radius = 1;
        $open_hrs = "";
        ...
        $sub_category_details = array();
        //$Querys = $this->db->query("SELECT c.*, d.service_name,d.category_id,d.price,d.discount,d.image,m.name as main_cat_name FROM dental_service_cart as c INNER JOIN dental_services_offered as d ON d.id=c.service_id INNER JOIN dentist_services_master as m ON m.id=d.category_id WHERE c.user_id='$user_id'");
        $Querys = $this->db->query("SELECT c.*,m.name as main_cat_name FROM dental_service_cart as c  INNER JOIN dentist_services_master as m ON m.id=c.treatment_id WHERE c.user_id='$user_id'");
        $ser = $Querys->result();
        $listing_id = "";
        if(!empty($ser))
        {
            foreach ($ser as $mser1) {
                $ids = $mser1->id;
                $main_cat_id = $mser1->treatment_id;
                $listing_id = $mser1->listing_id;
                $main_cat_name = $mser1->main_cat_name;
                $service_name = $mser1->service_name;
                $w ="";
                ...
                $Querys1 = $this->db->query("SELECT id,image,price,discount FROM dental_services_offered WHERE service_name='$service_name' $w LIMIT  1");
                $ser1 = $Querys1->row_array();
                if(!empty($ser1))
                {
                    $offered_id = $ser1['id'];
        ...
        $sub_category_details1 = array();
        if($listing_id!="" && $listing_id !="0")
        {
            $sql = "SELECT  name_of_hospital as name_of_clinic,address,address_2,pincode,city, state ,image,rating,lat,lng,phone FROM dentists_clinic_list WHERE user_id='$listing_id'
            UNION
            SELECT name_of_hospital  as name_of_clinic,address,address_2,pincode,city, state ,image,rating,lat,lng,phone FROM dentists_branch  WHERE dentists_branch_user_id = '$listing_id'";
            $result = $this->db->query($sql);
            if(!empty($sql)) {
              ...
              $row = $result->row_array();  
                ...
                $sub_category_details1[] = array('listing_id'=>$listing_id ,
                                                'name_of_clinic' => $name_of_clinic,
                                                'address' => $address,
                                                'address_2' => $address_2,
                                                'pincode' => $pincode,
        ...
        return array("status"=> 200,
                    "message"=> "success",
                    "count" => count($sub_category_details),
                    "clinic_details" => $sub_category_details1,
                    "data" => $sub_category_details
                    );
    }
    
    public function update_cart_service($user_id,$listing_id)
    {
        $Querys = $this->db->query("SELECT * FROM dental_service_cart WHERE user_id='$user_id'");
        $ser = $Querys->result();
        if(!empty($ser))
        {
            $arr = array('listing_id'=>$listing_id);
            $this->db->where('user_id',$user_id);
            $this->db->update('dental_service_cart',$arr);  
        }
        return array(
                    'status' => 201,
                    'message' => 'success'
