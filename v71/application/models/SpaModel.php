...
class SpaModel extends CI_Model {
    ...
    public function spa_center_list_recommended_v2($user_id, $latitude, $longitude,$api_type, $tab_id, $main_category, $sub_category, $page, $sort,$category_type,$price) {
        ...
        else
        {
         ...
         //Recommended
            ...
            // echo "SELECT fcb.id,fcb.user_id, fcb.branch_name, fc.image as branch_image, fcb.branch_phone, fcb.branch_email, fcb.about_branch, fcb.branch_business_category, fcb.branch_offer, fcb.branch_facilities, fcb.branch_address, fcb.opening_hours, fcb.pincode, fcb.state, fcb.city, fcb.lat, fcb.lng,fcb.user_discount,fcb.massage_service,p.price as package_price,p.discount as package_discount,p.package_name,p.package_details,p.image as package_image,p.package_times,p.deliverytype as package_deliverytype,p.categ_id as package_categ_id,p.spa_package_category,p.id as package_id, ( 6371 * acos( cos( radians($latitude) ) * cos( radians( fcb.lat ) ) * cos( radians( fcb.lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( fcb.lat ) ) ) ) AS distance FROM spa_branch as fcb LEFT JOIN fitness_center as fc ON (fcb.user_id= fc.user_id) INNER JOIN packages as p ON (fc.user_id=p.user_id AND p.v_id='36' AND p.branch=fcb.id) WHERE fc.is_active='1'  $where1 $where $cat_wh $pricesort HAVING distance < '%s' $wh $limit";
            $sql = sprintf("SELECT fcb.id,fcb.user_id, fcb.branch_name, fc.image as branch_image, fcb.branch_phone, fcb.branch_email, fcb.about_branch, fcb.branch_business_category, fcb.branch_offer, fcb.branch_facilities, fcb.branch_address, fcb.opening_hours, fcb.pincode, fcb.state, fcb.city, fcb.lat, fcb.lng,fcb.user_discount,fcb.massage_service,p.price as package_price,p.discount as package_discount,p.package_name,p.package_details,p.image as package_image,p.package_times,p.deliverytype as package_deliverytype,p.categ_id as package_categ_id,p.spa_package_category,p.id as package_id, ( 6371 * acos( cos( radians($latitude) ) * cos( radians( fcb.lat ) ) * cos( radians( fcb.lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( fcb.lat ) ) ) ) AS distance FROM spa_branch as fcb LEFT JOIN fitness_center as fc ON (fcb.user_id= fc.user_id) INNER JOIN packages as p ON (fc.user_id=p.user_id AND p.v_id='36' AND p.branch=fcb.id) WHERE fc.is_active='1'  $where1 $where $cat_wh $pricesort HAVING distance < '%s' $wh $limit", ($latitude), ($longitude), ($latitude), ($radius));
    ...
    */
    ...
    public function spa_category_v3()
    {
        $result_list = array();
        $stmt = "SELECT * FROM spa_category WHERE status='0'";
		$query = $this->db->query($stmt);
    ...
    {
         $result_list = array();
        $stmt = "SELECT * FROM spa_type WHERE status='0'";
		$query = $this->db->query($stmt);
		...
		foreach($query->result_array() as $row){
		     $data = array();
		      $category_id = $row['id'];
                ...
                if(!empty($icon))
                {
                     $icon = 'https://d2c8oti4is0ms3.cloudfront.net/images/fitness_center/' . $icon;
                }
                else
                {
                     $icon = "";
    ...
    public function spa_packages_categorywise($user_id,$category_id,$tab_id,$sort,$latitude,$longitude,$price,$page)
    {
        function GetDrivingDistance($lat1, $lat2, $long1, $long2) {
            $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $lat1 . "," . $long1 . "&destinations=" . $lat2 . "," . $long2 . "&mode=walking&language=pl-PL&key=AIzaSyCMHN__LczEziaYuKXQ-SKdiMfx5AiY66o&callback=initMap";
            $ch = curl_init();
        ...
        {
            if ($sort == 'high'){
                $filter_sort = " order by p.price desc";
            }
            elseif ($sort == 'low'){
                $filter_sort = " order by p.price asc";
            }
            elseif ($sort == 'discount'){
                $filter_sort = " order by p.discount desc";
            }
            else{
                $filter_sort = "";
            }
        ...
        if(!empty($price))
        {
            ...
            $filter_price = " and (p.price BETWEEN '$min_price' and '$max_price') ";
        }
        $filter_category = "";
        if(!empty($category_id))
        {
            $filter_category = " and p.package_type='$category_id'";
        }
       
        $resultpost = array();
        $sql = sprintf("SELECT fcb.id,fcb.user_id, fcb.branch_name, fc.image as branch_image, fcb.branch_phone, fcb.branch_email, fcb.about_branch, fcb.branch_business_category, fcb.branch_offer, fcb.branch_facilities, fcb.branch_address, fcb.opening_hours, fcb.pincode, fcb.state, fcb.city, fcb.lat, fcb.lng,fcb.user_discount,fcb.massage_service,p.price as package_price,p.discount as package_discount,p.package_name,p.package_details,p.image as package_image,p.package_times,p.deliverytype as package_deliverytype,p.categ_id as package_categ_id,p.spa_package_category,p.id as package_id FROM spa_branch as fcb LEFT JOIN fitness_center as fc ON (fcb.user_id= fc.user_id) INNER JOIN packages as p ON (fc.user_id=p.user_id AND p.v_id='36' AND p.branch=fcb.id) WHERE fc.is_active='1' $filter_category $filter_price $filter_sort $limit");
    ...
    */
    ...
    public function spa_packages_typewise($user_id,$category_id,$tab_id,$sort,$latitude,$longitude,$price,$page)
    {
        function GetDrivingDistance($lat1, $lat2, $long1, $long2) {
            $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $lat1 . "," . $long1 . "&destinations=" . $lat2 . "," . $long2 . "&mode=walking&language=pl-PL&key=AIzaSyCMHN__LczEziaYuKXQ-SKdiMfx5AiY66o&callback=initMap";
            $ch = curl_init();
        ...
        $resultpost = array();
        $sql = sprintf("SELECT fcb.id,fcb.user_id, fcb.branch_name, fc.image as branch_image, fcb.branch_phone, fcb.branch_email, fcb.about_branch, fcb.branch_business_category, fcb.branch_offer, fcb.branch_facilities, fcb.branch_address, fcb.opening_hours, fcb.pincode, fcb.state, fcb.city, fcb.lat, fcb.lng,fcb.user_discount,fcb.massage_service,p.price as package_price,p.discount as package_discount,p.package_name,p.package_details,p.image as package_image,p.package_times,p.deliverytype as package_deliverytype,p.categ_id as package_categ_id,p.spa_package_category,p.id as package_id FROM spa_branch as fcb LEFT JOIN fitness_center as fc ON (fcb.user_id= fc.user_id) INNER JOIN packages as p ON (fc.user_id=p.user_id AND p.v_id='36' AND p.branch=fcb.id) WHERE fc.is_active='1' and fcb.massage_service='$tab_id' $filter_category $filter_price $filter_sort $limit");
    ...
    */
    ...
    public function add_bookings($user_id, $patient_id, $listing_id, $package_id, $branch_id, $category_id, $trail_booking_date, $trail_booking_time, $user_name, $user_mobile, $user_email, $user_gender,$user_age,$trainer_id,$address_id,$instruction,$amt,$payment_method,$transaction_id) {
        date_default_timezone_set('Asia/Kolkata');
       ...
       else
       {
        ...
        if ($order_id > 0) {
            if($payment_method != '3' || $payment_method != '14') {
                ...
                $vendor_id = 36;
                $dat = array();
                $ledger_data = $this->LedgerModel->create_ledger($user_id,  $booking_id,  $ledger_owner_type,  $listing_id,  $listing_id_type,  $credit,  $debit,  $payment_method,  $user_comments,  $mw_comments, $vendor_comments, $order_type, $transaction_of, $transaction_id, $trans_status, $transaction_date, $vendor_id, $dat);
        ...
        //     /////////*******************************************excotel text message vendor ***************************************************
            ...
            $type_of_order = 'booking';
             $login_url = 'https://spa.medicalwale.com';
            $message      = 'You have received ' . $type_of_order . ', kindly login to ' . $login_url . '. Thank you.';
            $post_data    = array(
                'From' => '02233721563',
                'To' => $vendor_phone,
                'Body' => $message
        ...
        if($ledger_data['status'] == 1)
        {
            if(!empty($ledger_data['ledger_id'])){
                $lid = $ledger_data['ledger_id'];
                return array(
                'status' => 200,
                'message' => 'success',
                'booking_id' => $booking_id,
