...
class PartnermnoModel extends CI_Model {
    ...
    /*created on 5th may 2020 by swapnali waghunde*/
    /*checking code is live or in sandbox, depending on changing urls in same page*/
    ...
    public function login($phone, $token, $agent, $password, $for_excecutor) {
        ...
        if($for_excecutor == 1){
            $query = $this->db->query("SELECT u.id,u.password,u.name,u.email,u.phone,u.city,u.vendor_id,u.avatar_id,m.source FROM users as u left join media as m on (u.avatar_id = m.id) left join mno_executors as me on (u.id = me.user_id) WHERE u.phone='$phone' and u.vendor_id='44' AND u.status = 1")->row_array();
        } else {
            $query = $this->db->query("SELECT u.id,u.password,u.name,u.email,u.phone,u.city,u.vendor_id,u.avatar_id,m.source FROM users as u left join media as m on (u.avatar_id = m.id) WHERE u.phone='$phone' and u.vendor_id='44' AND u.status = 1")->row_array();    
        }
   ...
   public function GetDrivingDistance($originLat, $destLat, $originLng, $destLng) {
        ...
        $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $originLat . "," . $originLng . "&destinations=" . $destLat . "," . $destLng . "&mode=driving&language=pl-PL&key=AIzaSyCMHN__LczEziaYuKXQ-SKdiMfx5AiY66o&callback=initMap";
        // AIzaSyDg9YiklFpUhqkDXlwzuC9Cnip4U-78N60 : OLD
        // AIzaSyCMHN__LczEziaYuKXQ-SKdiMfx5AiY66o : NEW
        ...
        if(sizeof($response_a['rows']) > 0 && $response_a['rows'][0]['elements'][0]['status'] != 'ZERO_RESULTS' && $response_a['rows'][0]['elements'][0]['status'] != 'NOT_FOUND'){
            // in meter
        //  print_r(); die();
    ...
    /*NOTE : created new function order_list_v1 to handle distributors orders*/
    public function order_list($mno_id, $page_no, $per_page,$type,$search){
      //  date_default_timezone_set('GMT');
        $allOrdersWithInv = array();
        $duration = $distance = $order_date1 = $order_date = null;
        $pickup_lat = $pickup_lng = $delivery_lat = $delivery_lng = null;
        $ongoing = false;
        $tracking_status = $typeWHere = "";
        if($type == 1 || $type == 'accepted'){
            $typeWHere = "AND mo.status = 'accepted'";
        } else if($type == 2 || $type == 'rejected'){
            $typeWHere = "AND mo.status = 'rejected'";
        } else if($type == 3 || $type == 'ongoing'){
            $typeWHere = "AND mo.ongoing = 1 AND mo.status = 'accepted'";
        } else if($type == 'completed'){
            $typeWHere = "AND mo.ongoing = 0 AND mo.status = 'accepted'" ;
        } else if($type == 'pending'){
            $typeWHere = "AND mo.ongoing = 1 AND mo.status = ''" ;
        } else {
            $typeWHere = ""; 
        }
...
//        $allOrders = $this->db->query("SELECT mo.id as mno_order_id,mo.updated_at as last_updated,uo.order_date, uo.listing_name,uo.listing_type, mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE mo.`mno_id` = '$mno_id' $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) ORDER BY uo.invoice_no desc $limit")->result_array();
        ...
        $allOrders = $this->db->query("SELECT uo.mno_bill_no,uo.delivery_note,msp.lat as suggested_pharmacy_lat,msp.lng as suggested_pharmacy_lng  ,ms.lat as pharmacy_lat, ms.lng as pharmacy_lng ,uo.suggested_pharmacy_id , uo.listing_id,uo.lat as delivery_lat, uo.lng as delivery_lng, mo.id as mno_order_id,mo.updated_at as last_updated,uo.order_date, uo.listing_name,uo.listing_type, mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) left join medical_stores as ms on ((ms.user_id = uo.listing_id AND ms.pharmacy_branch_user_id = '0') OR (ms.user_id  > 0 AND ms.pharmacy_branch_user_id =  uo.listing_id )) left join mno_suggested_pharmacies as msp on (msp.id = uo.suggested_pharmacy_id) WHERE mo.`mno_id` = '$mno_id' AND mo.order_type = '0'  $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) group by mo.id ORDER BY mo.created_at desc $limit")->result_array();

        
// ( 6371 * acos( cos( radians(ms.lat) ) * cos( radians( uo.lat ) ) * cos( radians( uo.lng ) - radians(ms.lng) ) + sin( radians(ms.lat) ) * sin( radians( uo.lat ) ) ) ) AS distance_ms


        $allOrders_count = $this->db->query("SELECT mo.id FROM `mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) WHERE mo.`mno_id` = '$mno_id' AND mo.order_type = '0'  $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) group by mo.id ")->result_array();
        ...
        foreach($allOrders as $o){
            ...
            $mno_order_id = intval($o['mno_order_id']);
            $mno_bill_no = $o['mno_bill_no'] != '' ? $o['mno_bill_no'] : null;
            $delivery_note = $o['delivery_note'] != '' ? $o['delivery_note'] : null;
            
            
            $delivery_lat = $o['delivery_lat'] > 0 ? floatval($o['delivery_lat']) : null;
            $delivery_lng = $o['delivery_lng'] > 0 ? floatval($o['delivery_lng']) : null; ;
            
            
            if($o['listing_id'] > 0){
                $pickup_lat = floatval($o['pharmacy_lat']);
                $pickup_lng = floatval($o['pharmacy_lng']);
            } else if ($o['suggested_pharmacy_id'] > 0){
                $pickup_lat = floatval($o['suggested_pharmacy_lat']);
                $pickup_lng = floatval($o['suggested_pharmacy_lng']);
            } else {
                $pickup_lat = $pickup_lng = null;
            }
           
            if($delivery_lat > 0 && $delivery_lng > 0 && $pickup_lat > 0 && $pickup_lng > 0 && $o['ongoing'] == 1){
                $drivingDistanceDurtion = $this->PartnermnoModel->GetDrivingDistance($pickup_lat, $delivery_lat, $pickup_lng, $delivery_lng );
                $distance = $drivingDistanceDurtion['distance'] > -1 ? floatval($drivingDistanceDurtion['distance']) : null;
                $duration = $drivingDistanceDurtion['duration'] > 0 ? floatval($drivingDistanceDurtion['duration']) : null;
            } else {
                $duration = $distance = null;
            }
            
            $location['pickup_lat'] = $pickup_lat;
            $location['pickup_lng'] = $pickup_lng;
            $location['delivery_lat'] = $delivery_lat;
            $location['delivery_lng'] = $delivery_lng;
          ...
          if($o['listing_type'] == 13){
           ...
           /* if(empty($o['status'])){
            ...
            if($oldInv == $inv){
                ...
                $orderData['mno_order_id'] = $mno_order_id;
                $orderData['last_updated'] = $last_updated;
                $orderData['mno_bill_no'] = $mno_bill_no;
                $orderData['delivery_note'] = $delivery_note;
    ...
    /*UPDATE : Done changes WRT disctributor order by swapnali waghunde on 9th may 2020*/
    public function order_accept_reject($mno_id,$invoice_no, $accepted, $cancel_reason_id,$mno_lat,$mno_lng){
        ...
        if ($count > 0) {
            $current_status = $this->db->query("SELECT id as mno_order_id,status FROM `mno_orders` WHERE `mno_id` = '$mno_id' AND `invoice_no` = '$invoice_no' AND `ongoing` = '1' ")->row_array();
            if(sizeof($current_status) > 0){
                $mno_order_id = $current_status['mno_order_id'];
                if($current_status['status'] == 'accepted'){
                    $return['status'] = 6;  /*Order already accepted*/
                    $return['mno_order_id'] = $mno_order_id;
                    return $return;
                } else if($current_status['status'] == 'rejected'){
                    $return['status'] = 7; /*You have already reject this order*/
                    return $return;
                     ...
                     if($accepted == false){
                        ...
                        if(sizeof($cancelReasond) > 0){
                            ...
                            $get_order_sql ="SELECT uo.listing_id,uo.order_date,uo.order_status  , ml.mno_name as mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.order_type = 0 GROUP by mo.id
                            UNION
                            SELECT idi.listing_id,idi.order_date,idi.order_status  , ml.mno_name as mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no) left join users as u on (idi.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.order_type = 1 GROUP by mo.id"; 
                            
                            $existingOrder = $this->db->query($get_order_sql)->row_array();
                            // print_r($existingOrder); die();
                            $rejectOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='0',`status`='rejected',`cancel_reason`='$cancel_reason' WHERE `mno_id` = '$mno_id' AND `invoice_no` = '$invoice_no' AND `ongoing` = '1' ");
                        
                        
                        $mno_name = $existingOrder['mno_name'];
                        $user_mobile = $existingOrder['phone'];
                        ...
                        /*Changes by swapnali waghunde on 11th May 2020. Whatsapp integration*/
                        $whatsapp_msg = "";
                        $whatsapp_body = array();
                        /*Added by Swapnali Waghunde on 19th May 2020*/
                        $user_id = $existingOrder['user_id'];
                        $listing_id = $existingOrder['listing_id'];
                        $order_date = $existingOrder['order_date'];
                        $user_order_status = $existingOrder['order_status'];
            
                        $res = $this->PartnermnoModel->send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status, $whatsapp_body);
                    ...
                    $assignAnotherOrder = $this->PartnermnoModel->assign_pending_order_to_mno($mno_id, $mno_lat, $mno_lng);
                        ...
                        $return['status'] = 3; /*Order   rejected*/
                        return $return;
                      
                        } else {
                            $cancel_reason = "";
                            
                            $return['status'] = 8; /*Order cancel reason not found*/
                            return $return;
                        }
                        ...
                        $acceptedOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='1',`status`='accepted'  WHERE `mno_id` = '$mno_id' AND `invoice_no` = '$invoice_no' AND `ongoing` = '1'");
                        $action_by_status = "Night owl";
                     ...
                     //   $order_status = $this->OrderModel->update_status($invoice_no,$orderStatus, $action_by_status);
                        ...
                        $get_order = "SELECT uo.listing_id,uo.order_date,uo.order_status  , ml.mno_name as mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.order_type = '0' GROUP BY mo.id
                        
                            UNION
    
                            SELECT idi.listing_id,idi.order_date,idi.order_status  , ml.mno_name as mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no) left join users as u on (idi.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.order_type = '1' GROUP BY mo.id";
                        
                        $existingOrder = $this->db->query($get_order)->row_array();
                        // print_r($existingOrder); die();
                        $mno_name = $existingOrder['mno_name'];
                        $user_mobile = $existingOrder['phone'];
                        $action_by_status = "Night owl";
                        ...
                        $reg_id = $existingOrder['token'];
                        $msg = "Night owl $mno_name accepted your order #$invoice_no";
                        $img = $img_url   = 'https://medicalwale.com/img/noti_icon.png';
                        $tag = "text";
                        $agent = $existingOrder['agent'];
                        $invoice_no = $invoice_no;
                        ...
                        $whatsapp_msg = "";
                        $whatsapp_body = array();
                        /*Added by Swapnali Waghunde on 19th May 2020 TO ADD NOTIFICATION IN TABLE WHICH WAS COMMENTED BEFORE */
                        $user_id = $existingOrder['user_id'];
                        $listing_id = $existingOrder['listing_id'];
                        $order_date = $existingOrder['order_date'];
                        $user_order_status = $existingOrder['order_status'];
                        ...
                        $orderDetails = $this->db->query("SELECT * FROM `mno_orders`   WHERE `mno_id` = '$mno_id' AND `invoice_no` = '$invoice_no' AND `ongoing` = '1' ")->row_array();
                        ...
                        $return['status'] = 4; /*Order accepted*/
                        $return['mno_order_id'] = $mno_order_id; 
                        ...
                        $return['status'] = 2; /*Please send either true or false in accepted*/
                        return $return;
                    }
                }
                ...
                $return['status'] = 5; /*No such order exists, please check invoice_no again*/
                return $return;
            }    
           
        } else {
            $return['status'] = 1; /*'Night night owl found'*/
            return $return;
        }
    }
    ...
    /*DOne with handling distributors orders by Swapnali waghunde on 6th MAY 2020*/
     public function assign_mno($user_id,$invoice_no,$lat,$lng){
        ...
        $getPincode = $this->db->query("select listing_type,pincode from user_order where invoice_no = '$invoice_no' UNION select listing_type,pincode from inven_distrub_invoice where invoice_no = '$invoice_no'")->row_array();
        
        $address_id = 0;
        $pincode = $getPincode['pincode'];
        $vendor_type = $getPincode['listing_type'];
        ...
        if($mno_available_delivery['delivery_available'] == 1){
            ...
            // else {
                ...
                if(!empty($busyMnos) &&  sizeof($busyMnos) > 0 && $mno_id == ''){
                    ...
                    if($orderInsertedButNotAssiged == 0){
                        $insertInOrder = $this->db->query("INSERT INTO `mno_orders`(`invoice_no`, `mno_id`, `ongoing`, `created_at`, `updated_at`,`vendor_type`,`order_type`) VALUES ('$invoice_no','$mno_id','1','$order_date','$order_date','$vendor_type','$order_type')");
            ...
            // ask vaibhav regarding delivery charges and delivery by values : do we need to remove the inserted record
    ...
    // order_details
    /*created new function order_details_v1 to handle distributor orders */
    public function order_details($mno_id, $invoice_no,$mno_order_id = null){
        $orderd_pharmacy_details = array();
           $todayDate = date('Y-m-d H:i:s');
        $todayDay = date('l');
        $currenttime = date('h:i A');
        $current_time = date('h : i A');
        ...
        $bill_url = null;
        $delivery_note = "";
        if($mno_order_id != null && $mno_order_id != "" && $mno_order_id > 0){
            $where = " AND  mo.`id` = '$mno_order_id' ";
        }
        ...
        if($acceped_query['status'] == 'accepted'){
            ...
            $query = $this->db->query("select mno_bill_no, delivery_note, bill_url,user_id,gst,chc,discount,order_total,action_by,order_id,order_type,delivery_time,listing_id,suggested_pharmacy_id,cancel_reason,action_by,delivery_charge,delivery_charges_by_customer,delivery_charges_by_vendor,delivery_charges_to_mno,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status,rxId,delivered_sign, order_received_by from user_order where `invoice_no` = '$invoice_no' group by invoice_no order by order_id desc");
            ...
            $gst = $order_info['gst'];
            $bill_url = $order_info['bill_url'];
            $delivery_note = $order_info['delivery_note'];
            $mno_bill_no = $order_info['mno_bill_no'];
            ...
            //   echo $pharmacy_id; die();
                ...
                $search = "";
                 $res = $this->PartnermnoModel->pharmacy_list($mno_id, $lat, $lng, $page_no, $per_page,$pharmacy_id,$search);
                      $parmacyWhere = "WHERE   (`user_id` = '$pharmacy_id'  AND (`pharmacy_branch_user_id` = 0 OR `pharmacy_branch_user_id` = null OR `pharmacy_branch_user_id` = '')) OR ((`user_id` != 0 OR `user_id` != null OR `user_id` != '') AND `pharmacy_branch_user_id` = '$pharmacy_id')";
       
                  $sql = "SELECT mba,recommended,certified,`id`,`store_time`, `user_id`,`pharmacy_branch_user_id`,`reach_area`, `medical_name`, `discount`, `store_manager`, `lat`, `lng`, `address1`, `address2`, `pincode`, `city`, `state`, `contact_no`, IFNULL(whatsapp_no,'') AS whatsapp_no, `profile_pic`,  `day_night_delivery`,  `days_closed`,  `is_24hrs_available`, `payment_type`, `store_open`, `store_close`, ( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance FROM medical_stores  $parmacyWhere ";
                  $row = $pharmacy_details = $this->db->query($sql)->row_array();
               ...
               /* $medical_user_id
                ...
                //  $pharmacy = $res['stores'][0];
                   $pharmacy = $orderd_pharmacy_details;
            ...
            foreach ($query->result_array() as $row) {
                ...
                $delivered_sign = $row['delivered_sign'];
                $order_received_by = $row['order_received_by'];
                        ...
                        if ($product_count1 > 0) {
                            ...
                            if ($prescription_count > 0) {
                                ...
                                foreach ($prescription_query->result_array() as $prescription_row) {
                                    $finalamt=floatval($prescription_row['prescription_price'])*intval($prescription_row['prescription_quantity']);
                                   ...
                                   $sub_total_discount1 += floatval($prescription_discount);
        ...
        */
        if($rxId == "") {
            ...
            $prescription_resultpost_new=array();
               ...
               $total_customer = $order_total + $delivery_charges_by_customer + $chc - $order_total_discount + $gstAmt;
               $total_vendor = $order_total - $delivery_charges_by_vendor + $chc - $order_total_discount + $gstAmt;
                ...
                $cost = array(
                    ...
                    "total_customer" => floatval($total_customer),
                    "total_vendor" => floatval($total_vendor),
                ...
                $resultpost = array(
                    "mno_order_id" => $mno_order_id >= 0 ? intval($mno_order_id) : null,
                    "invoice_no" => $invoice_no,
                    "bill_url" => $bill_url != '' ? $bill_url : null,
                    "delivery_note" => $delivery_note != '' ? $delivery_note : null,
                    "mno_bill_no" => $mno_bill_no != "" ? $mno_bill_no : null,
                    "on_going"  => $ongoin,
                    ...
                    "action_by" => $action_by,
                    "rxid" => $rxId,
                    "delivered_sign" => $delivered_sign !="" ? $delivered_sign : null,
                    "order_received_by" => $order_received_by !="" ? $order_received_by : null,
                    "is_cancel" => $is_cancel,
                    "cost" => $cost,
                    "tracking" => $tracker
                );
    ...
    if ($product_count12 > 0) {
        ...
        if ($token_status > 0) {
          ...
          //  $orderDetails = $this->db->query("SELECT * FROM `mno_orders` WHERE `invoice_no` LIKE '$invoice_no' AND `mno_id` = '$mno_id' ")->row_array();
            $orderDetails = $this->db->query("SELECT uo.listing_id,uo.order_date,uo.order_status  ,ml.mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
    
            $mno_order_id = $orderDetails['id'];
            $user_mobile = $orderDetails['phone'];
            $notification_type = 7;
            ...
            $this->PartnermnoModel->send_gcm_notification_to_mno($mno_id,$invoice_no,$mno_order_id, $notification_type, $img, $title, $msg);
                ...
                $listing_id = $orderDetails['listing_id'];
                $order_date = $orderDetails['order_date'];
                $user_order_status = $orderDetails['order_status'];
                
             
            
                $res = $this->PartnermnoModel->send_gcm_notify_user($title1, $reg_id, $msg1, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status,$whatsapp_body);
        ...
        */
         ...
         $order_details = $this->PartnermnoModel->order_details($mno_id , $invoice_no,$mno_order_id);
     //    echo $invoice_no;
...
// order_to_pharmacy
    public function order_to_pharmacy($mno_id, $invoice_no, $pharmacy_id, $suggested_pharmacy_id){
        ...
        $orderDetails = $this->db->query("SELECT * FROM `mno_orders` WHERE `invoice_no` LIKE '$invoice_no' AND `mno_id` = '$mno_id' AND `ongoing` = '1' AND `status` = 'accepted' ")->row_array();
        ...
        if(array_key_exists('id',$orderDetails)){
            $mno_order_id = $orderDetails['id'];
        } else {
            $mno_order_id = null;
        }
   ...
   /*Swapnali waghunde made changes in function WRT whatsapp messages on 11th May 2020*/
     public function send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile = null, $whatsapp_msg = null,$user_id = null,$listing_id=null,$order_date=null, $order_status=null, $whatsapp_body) {
        ...
        if($reg_id != ""){
            ...
            // print_r($fields); die();
// user_id, order_date, order_status, $listing_id

            $notification_date = date('Y-m-d');
            // $listing_id = 44;
            
            if($user_id > 0){
                  $this->db->query("INSERT INTO `All_notification_Mobile`(`user_id`, `listing_id`, `order_date`, `order_status`,   `invoice_no`, `notification_type`, `notification_date`, `title`, `msg`, `img_url`, `tag`) VALUES ('$user_id', '$listing_id', '$order_date' ,'$order_status','$invoice_no','$notification_type' ,'$notification_date','$title','$msg','$img_url','$tag')"); 
            ...
            $headers = array(
                GOOGLE_GCM_URL,
                'Content-Type: application/json',
                $agent === 'android' ? 'Authorization: key=AAAA3WXM0sY:APA91bHTtTr38NM_lFlgLbVZd0gex74YcLZToH0C4xgiiB0PBiQnDzZ7sYU4Spd-zc0B7cU_DdXqbzCDpMNF8GFFQgc2E3qtik9rIuIA5zeVgKD-X1y2wfBj--eLKxMWxii8Pwxdgym1' : 'Authorization: key=AAAA8fDyFRw:APA91bFGE64KagMB9rL1w7ozOnnSEIkUCU0F9KxOFckGCelJJoycUfAu16vc4HntOMsFJBAPgXjOU_dQFllzPZEbZGWOvvy9e86HVXBGTdsRYjaf6KJebAXaZ0V0rq7ykvkootz75naw'
            );
        ...
        if(!empty($whatsapp_body)) {
            $res = $this->PartnermnoModel->send_whatsapp_msg($whatsapp_body);
        }    
        return true;
    ...
    // cancel_reason
    /*not using in app : written by swapnali waghunde on 9th May 2020*/
...
// remaining 
    ...
    /*Done : distributor orders integration : Swapnali Waghunde on 30th Jun 2020*/
    public function dashboard($reqData){
        ...
        $orderDetails = $this->db->query("SELECT uo.order_total,uo.actual_cost, uo.discount, mo.* FROM `mno_orders` as mo  join user_order as uo on (mo.`invoice_no` = uo.invoice_no) WHERE mo.`mno_id` = '$mno_id' AND mo.`created_at` >= '$from_date' AND mo.`created_at` <= '$to_date'  group by mo.id
        UNION 
        SELECT idi.order_total,idi.actual_cost, idi.discount, mo.* FROM `mno_orders` as mo  join inven_distrub_invoice as idi on (mo.`invoice_no` = idi.invoice_no) WHERE mo.`mno_id` = '$mno_id' AND mo.`created_at` >= '$from_date' AND mo.`created_at` <= '$to_date' group by mo.id ORDER BY created_at ASC")->result_array();
        
        $maxMinOrderTime = $this->db->query("SELECT uot.status,uot.created_at,uot.invoice_no, mo.created_at as mno_assiged_time FROM `mno_orders` as mo left join user_order_tracking as uot on (mo.invoice_no = uot.invoice_no) WHERE mo.`mno_id` = '$mno_id' AND uot.`created_at` >= '$from_date' AND (uot.status like '%Order delivered%' || uot.status like '%Order accepted%')  ORDER BY uot.invoice_no, uot.status ASC")->result_array();
        ...
        foreach($maxMinOrderTime as $mot){
            ...
            if($oldInvoice_no == $invoice_no){
                if(strpos($status, 'Order accepted') !== false){
                    $mt['invoice_no'] = $invoice_no;
                    $mt['accepted_at'] = $created_at;
                    
                } else if(strpos($status, 'Order delivered') !== false ){
                    $mt['invoice_no'] = $invoice_no;
        ...
        foreach($finalMT as $fmt){
            ...
            if(array_key_exists('accepted_at',$fmt)){
                ...
                $onlyAcceptedDate = date("Y-m-d", strtotime($fmt['mno_assiged_time']));
            }
        ...
        // $finalData['ordersFinal'] = $ordersFinal;
    ...
    /*Pending: distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    ...
    public function order_pickedup($mno_id,$invoice_no,$paid_to_pharmacy,$paid_to_pharmacy_comment,$received_from_pharmacy,$received_from_pharmacy_comment,$received_including_delivery_charges,$paid_payment_method, $received_payment_method){
        ...
        $this->load->model('OrderModel');
        $existingOrder = $this->db->query("SELECT uo.order_date,uo.order_status,uo.bill_url,uo.mno_bill_no,uo.listing_name,uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor, uo.delivery_charges_by_mw,  uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id,  uo.delivery_charges_to_mno, uo.suggested_pharmacy_id, ml.mno_name,  ml.phone as mno_phone_no,mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted' AND mo.order_type = '0'
        UNION 
        SELECT idi.order_date,idi.order_status,idi.bill_url,idi.mno_bill_no,idi.listing_name,idi.user_id,idi.delivery as delivery_charge,idi.delivery_charges_by_customer, idi.delivery_charges_by_vendor, idi.delivery_charges_by_mw, null as total_without_dc,idi.listing_type,idi.payment_method,idi.order_total, idi.listing_id, idi.delivery_charges_to_mno, null as suggested_pharmacy_id, ml.mno_name, ml.phone  as mno_phone_no,mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no) left join users as u on (idi.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted' AND mo.order_type = '1'
        ")->row_array();
        ...
        if($existingOrder > 0){
            ...
            if($suggested_pharmacy_id > 0){
                ...
                $listing_id = $listing_id;
                $listing_id_type = $listing_type; // pharmacy vendor type
            }
            ...
            // $vendor_type 52 (distributor) Ledger order type  10 
            // $vendor_type 13 or 44 Ledger order type  1
            $order_type = $listing_id_type == '52' ? 10 : 1 ;
           ...
           // add for each transaction
            ...
            // WORKING
            $transaction_date = "";
             $vendor_id = $phatmacy_id_type;
                    $array_data = array();
                    
             $res = $this->LedgerModel->create_ledger($mno_id, $invoice_no, $mno_id_type, $phatmacy_id, $phatmacy_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
            ...
            // delivery charges debit entry : 
                ...
                if($listing_type == 44){
                    ...
                    if($delivery_charges_by_customer != 0){
                        ...
                        // WORKING
                        $transaction_date = "";
                        $vendor_id = $mno_id_type;
                    $array_data = array();
                        $res1 = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $mno_id, $mno_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
                    ...
                    if($delivery_charges_by_vendor != 0 && $completed != 1){
                        ...
                        $vendor_id = $phatmacy_id_type;
                    $array_data = array();
                        $res2 = $this->LedgerModel->create_ledger($mno_id, $invoice_no, $mno_id_type, $phatmacy_id, $phatmacy_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
                    ...
                    if($delivery_charges_by_mw != 0 && $completed != 1){
                        ...
                        $vendor_id = $mno_id_type;
                    $array_data = array();
                        $res3 = $this->LedgerModel->create_ledger($mno_id, $invoice_no, $mno_id_type, $mw_id, $mw_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
                ...
                if($listing_type == 13 || $listing_type == 52){
                    $completed = 0;
                    ...
                    if($delivery_charges_by_vendor != 0){
                        ...
                        $vendor_id = $phatmacy_id_type;
                    $array_data = array();
                        $res1 = $this->LedgerModel->create_ledger($mno_id, $invoice_no, $mno_id_type, $phatmacy_id, $phatmacy_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
                    ...
                    if($delivery_charges_by_customer != 0 && $completed != 1){
                        ...
                        $vendor_id = $mno_id_type;
                    $array_data = array();
                        $res2 = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $mno_id, $mno_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
            ...
            $listing_name = $existingOrder['listing_name'];
            $mno_bill_no = $existingOrder['mno_bill_no'];
            $bill_url = $existingOrder['bill_url'];
            $mno_name = $existingOrder['mno_name'];
            $phone = $existingOrder['mno_phone_no'];
            $todayDate = date("jS F Y");
            
            /*Changes by swapnali waghunde on 11th May 2020. Whatsapp integration*/
            $download_link = "";
            $pickedup_from_line = $listing_name != "" ? " from ".$listing_name : "";
            $ref_no_line = $mno_bill_no != " with Reference No. ".$mno_bill_no ? "" : "" ;
            $link_line = $download_link != "" ?  " from Google Play or Apple App Store ".$download_link : "" ;
            $bill_url_line = $bill_url != "" ? "Tap on the following link to view your invoice - ".$bill_url : "" ;
            $whatsapp_msg = "Your order with invoice number is #".$invoice_no." has been picked up  ".$pickedup_from_line.$ref_no_line."

            $bill_url_line
            
            Night Owl - ".$mno_name." will deliver your order. You may contact him at ".$phone.".
            
            Your order is expected to reach on ".$todayDate." between 10:00 AM and 08:00 PM
            
            To track your order download Medicalwale Saathi App $link_line.";
            ...
            /*Message to user*/
            $sms_text = "Your order with invoice number is #".$invoice_no." has been picked up  $pickedup_from_line  $bill_url_line Night Owl - ".$mno_name." will deliver your order. You may contact him at ".$phone.".";
            $result = crul_sms($user_mobile, $sms_text);
            
            $whatsapp_body->destinationPhone = "91".$user_mobile;
            $whatsapp_body->template_type ="template_40";
            $whatsapp_body->v1 = $listing_name != "" ? $listing_name : "NA";
            $whatsapp_body->v2 = $invoice_no != "" ? $invoice_no : "NA";
            $whatsapp_body->v3 = $mno_bill_no != "" ? $mno_bill_no : "NA";
            $whatsapp_body->v4 = "NA";
            $whatsapp_body->v5 = "NA";
            $whatsapp_body->v6 = "NA";
    ...
    /*distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    /*Not using , created new API  order_delivered_with_sign */
    public function order_delivered($mno_id,$invoice_no){
        $this->load->model('OrderModel');

       $existingOrder = $this->db->query("SELECT uo.order_date,uo.order_status,uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor, uo.delivery_charges_by_mw,  uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id,  uo.delivery_charges_to_mno, uo.suggested_pharmacy_id, ml.mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
        ...
        if($existingOrder > 0 ){
           ...
           $vendor_id = $mno_id_type;
                    $array_data = array();
            $res = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $mno_id, $mno_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
            ...
            $download_link = "";
            $link_line = $download_link != "" ?  " from Google Play or Apple App Store ".$download_link : "" ;
            $mw_contact_no = "022 68 44 33 22";
            $mw_email_id = "info@medicalwale.com";
            
            $whatsapp_msg = "Your order #".$invoice_no." is delivered. Thank you for choosing Night Owl Service.

 For feedback please contact ".$mw_contact_no." or email ".$mw_email_id."

For more services regarding Health | Beauty | Fitness download Medicalwale Saathi App $link_line and stay healthy.";
            ...
            $whatsapp_body->destinationPhone = "91".$user_mobile;
            $whatsapp_body->template_type ="template_42";
            $whatsapp_body->v1 = "NA";
            $whatsapp_body->v2 = $invoice_no != "" ? $invoice_no : "NA";
            $whatsapp_body->v3 = $mw_contact_no != "" ? $mw_contact_no : "NA";
            $whatsapp_body->v4 = $mw_email_id != "" ? $mw_email_id : "NA";
            $whatsapp_body->v5 = "NA";
            $whatsapp_body->v6 = "NA";
     ...
     // mno_out_for_pickup
    /*distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    /*Not using , created new API  out_for_pickups */
    public function mno_out_for_pickup($mno_id,$invoice_no){
        $this->load->model('OrderModel');
         ...
         $existingOrder = $this->db->query("SELECT uo.listing_id,uo.order_date,uo.order_status,ml.mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
        ...
        if($existingOrder > 0){
            ...
            $reg_id = $existingOrder['token'];
            $msg = "Night owl is on the way to pick-up the order from pharmacy store";
            $img_url   = 'https://medicalwale.com/img/noti_icon.png';
            $tag = "text";
            $agent = $existingOrder['agent'];
            $invoice_no = $invoice_no;
            ...
            $mno_order_id = $existingOrder['id'];
            $title = "On the way to pick-up";
            $msg = "On the way to pick-up for order id $invoice_no";
    ...
    // at_store
    /*distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    /*Not using in APP : Swapnali Waghunde on 9th May 2020*/
    ...
    // at_doorstep
    /*testing Pending: distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    public function at_doorstep($mno_id,$invoice_no){
        $this->load->model('OrderModel');
        
        /*Old query before distributor orders integration*/
        /*$existingOrder = $this->db->query("SELECT uo.listing_id,uo.order_date,uo.order_status  ,ml.mno_name, mo.*, u.id as user_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();*/
       
       
       $sql_query = "SELECT uo.listing_id,uo.order_date,uo.order_status  ,ml.mno_name, mo.*, u.id as user_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'

        UNION
        
        SELECT idi.listing_id,idi.order_date,idi.order_status  ,ml.mno_name, mo.*, u.id as user_id,idi.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no) left join users as u on (idi.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'";
        
        $existingOrder = $this->db->query($sql_query)->row_array();
        ...
        if($existingOrder > 0 ){
            ...
            $reg_id = $existingOrder['token'];
            $msg = "Night owl $mno_name reached at doorstep";
            $img_url   = 'https://medicalwale.com/img/noti_icon.png';
            $tag = "text";
            $agent = $existingOrder['agent'];
            $invoice_no = $invoice_no;
            ...
            /*SMS to user*/
            $result = crul_sms($user_mobile, $msg);
    ...
    // payment_accepted 
    /*distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    /*Not using New APi payment_accepted_by_user*/
    public function payment_accepted($mno_id,$invoice_no, $payment_id,$amount){
            ...
            $existingOrder = $this->db->query("SELECT uo.order_date,uo.order_status  ,uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor, uo.delivery_charges_by_mw,  uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id,  uo.delivery_charges_to_mno, uo.suggested_pharmacy_id,  ml.mno_name, mo.*, u.id as user_id,uo.listing_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
                ...
                if(sizeof($paymentMethodsSelected) > 0){
                    ...
                    $array_data = array();
                            ...
                            // $user_id , $user_id_type , $phatmacy_id, $phatmacy_id_type
                             ...
                             $vendor_id = $phatmacy_id_type;
                    $array_data = array();
                            $res = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $phatmacy_id, $phatmacy_id_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
   ...
   /*distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
   /*Done with distributor order integration by Swapnali Waghunde on 03rd July 2020*/
   ...
   //     echo "SELECT * FROM `mno_notifications` WHERE `invoice_no` LIKE '$invoice_no' AND `mno_orders_id` = '$mno_orders_id' ORDER BY `id` ASC"; die();
        ...
        $getOrderInfo = $this->db->query("SELECT uo.suggested_pharmacy_id, uo.listing_id, mo.pharmacy_didnot_respond, uo.listing_type FROM `user_order` as uo left join mno_orders as mo on (uo.invoice_no = mo.invoice_no ) WHERE mo.`invoice_no` LIKE '$invoice_no' 
            UNION
            SELECT null as suggested_pharmacy_id, idi.listing_id, mo.pharmacy_didnot_respond, idi.listing_type FROM `inven_distrub_invoice` as idi left join mno_orders as mo on (idi.invoice_no = mo.invoice_no ) WHERE mo.`invoice_no` LIKE '$invoice_no' 
     ...
     /*Pendig: distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    public function get_delivery_time_cost($order_id_data){
        ...
        /*query before integration of distributor*/
         /*$getMNODetails = $this->db->query("SELECT uo.listing_type,uo.pincode,uo.order_id,uo.invoice_no, uo.lat as user_lat, uo.lng as user_lng ,uo.`listing_id`, ms.lat as store_lat, ms.lng as store_lng , uo.`suggested_pharmacy_id`, msp.lat as suggested_pharmacy_lat ,  msp.lng as suggested_pharmacy_lng, mo.mno_id, mo.id as mno_order_id, ma.lat as mno_lat, ma.lng as mno_lng ,u.multi_user_type
            ...
            left join mno_suggested_pharmacies as msp on (uo.suggested_pharmacy_id != '' && uo.suggested_pharmacy_id = msp.id ) left join users as u on (uo.listing_id = u.id)
             WHERE uo.`order_id` = '$order_id_data' AND mo.vendor_type != 52  GROUP by uo.order_id")->row_array();*/
             
            $sql_query = "SELECT uo.listing_type,uo.pincode,uo.order_id,uo.invoice_no, uo.lat as user_lat, uo.lng as user_lng ,uo.`listing_id`,
            ms.lat as store_lat, ms.lng as store_lng , uo.`suggested_pharmacy_id`, msp.lat as suggested_pharmacy_lat , msp.lng as
            suggested_pharmacy_lng, mo.mno_id, mo.id as mno_order_id, ma.lat as mno_lat, ma.lng as mno_lng ,u.multi_user_type
            FROM `user_order` as uo
            left join mno_orders as mo on (uo.invoice_no = mo.invoice_no AND mo.`status` = 'accepted' )
            left join mno_actions as ma on (mo.mno_id=ma.mno_id && mo.invoice_no = ma.invoice_no)
            left join medical_stores as ms on (uo.listing_id != '' && (uo.listing_id = ms.user_id || uo.listing_id =
            ms.pharmacy_branch_user_id))
            left join mno_suggested_pharmacies as msp on (uo.suggested_pharmacy_id != '' && uo.suggested_pharmacy_id = msp.id ) left
            join users as u on (uo.listing_id = u.id)
            WHERE uo.`order_id` = '$order_id_data' AND mo.vendor_type != 52 GROUP by uo.order_id
            
            UNION 
            
            SELECT idi.listing_type,idi.pincode,idi.order_id,idi.invoice_no, idi.lat as user_lat, idi.lng as user_lng ,idi.`listing_id`,
            d.lat as store_lat, d.lng as store_lng , null as `suggested_pharmacy_id`, null as suggested_pharmacy_lat , null as
            suggested_pharmacy_lng, mo.mno_id, mo.id as mno_order_id, ma.lat as mno_lat, ma.lng as mno_lng ,u.multi_user_type
            FROM inven_distrub_invoice as idi
            left join mno_orders as mo on (idi.invoice_no = mo.invoice_no AND mo.`status` = 'accepted' )
            left join mno_actions as ma on (mo.mno_id=ma.mno_id && mo.invoice_no = ma.invoice_no)
            left join distributor as d on (idi.listing_id != '' && (idi.listing_id = d.user_id || idi.listing_id =
            d.hub_branch_id)) left join users as u on (idi.listing_id = u.id)
            WHERE idi.`order_id` = '$order_id_data' AND mo.vendor_type = 52 GROUP by idi.order_id
            ";
            $getMNODetails = $this->db->query($sql_query)->row_array(); 
        // print_r($getMNODetails); die();
        $invoice_no = $getMNODetails['invoice_no'];
        $listing_type = $getMNODetails['listing_type'];
        ...
        if($listing_type == 44){
            $delivery_service_charge_per = $delivery_service_charge_rs = 0;
        } else {
            $delivery_service_charge_per = $get_mno_charges['delivery_service_charge_per'];
            $delivery_service_charge_rs = $get_mno_charges['delivery_service_charge_rs'];
        }
        
        
        $data['delivery_charges'] = $get_mno_charges['delivery_charges'] ;
        $data['delivery_service_charge_per'] = $delivery_service_charge_per;
        $data['delivery_service_charge_rs'] = $delivery_service_charge_rs;
...
//   get_ongoing_order
    ...
    public function get_ongoing_order($mno_id){
        ...
        $type = 3; //ongoing 
        $search = "";
          $listOfOrders = $this->PartnermnoModel->order_list_v1($mno_id, $page_no, $per_page,$type,$search);
   ...
   //     return $data;
            ...
            if(sizeof($getAllReasons) > 0){
                $cancel_reason = $getAllReasons['cancel_reason'];
                    ...
                    if ($check_status == 'Order Delivered') {
                       ...
                       $existingOrder = $this->db->query("SELECT ml.mno_name, mo.*, u.id as user_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` > 0 AND mo.`status` = 'accepted'")->row_array();
    ...
    /*distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    /*Done with intergration  by Swapnali Waghunde on 4th Jul 2020*/
            ...
            if(sizeof($mno_track) > 0){
                ...
                if(sizeof($mno_orders) == 0){
                    ...
                    foreach($alreadyAssinedInvs as $a){
                     ...
                     $invs = "";
                    if($allinvs != ""){$invs = 'WHERE mo.invoice_no NOT IN ( '. $allinvs .' ) ' ;   }*/
                    
                    /*Before integration og distributor orders*/
                   /* $sql = sprintf("SELECT uo.invoice_no,( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance  FROM user_order as uo  join mno_orders as mo on (uo.invoice_no = mo.invoice_no &&  mo.mno_id = 0 && mo.ongoing = 1 && uo.order_status != 'Order Cancelled') $invs   ORDER BY distance LIMIT 1", ($lat), ($lng), ($lat));*/
                   
                   
                   $sql = "SELECT uo.invoice_no,( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance  FROM user_order as uo  join mno_orders as mo on (uo.invoice_no = mo.invoice_no &&  mo.mno_id = 0 && mo.ongoing = 1 && uo.order_status != 'Order Cancelled'  AND mo.vendor_type != '52') AND mo.invoice_no NOT IN (SELECT `invoice_no` FROM `mno_orders` WHERE `mno_id` = '$mno_id')   
                UNION
                SELECT idi.invoice_no,( 6371 * acos( cos( radians('$lat') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('$lng') ) + sin( radians('$lat') ) * sin( radians( lat ) ) ) ) AS distance  FROM inven_distrub_invoice as idi  join mno_orders as mo on (idi.invoice_no = mo.invoice_no &&  mo.mno_id = 0 && mo.ongoing = 1 && idi.order_status != 'Order Cancelled' AND mo.vendor_type = '52' AND mo.invoice_no NOT IN (SELECT `invoice_no` FROM `mno_orders` WHERE `mno_id` = '$mno_id'))    
      ...
      $sent_to = 0;
        ...
        if($mno_id  != "" && $mno_name != "" && $mno_id  != null && $mno_name != null && $mno_id  > 0 ){
            ...
            foreach($mno_executors as $me){
                ...
                $result = crul_sms($executor_phone, $message);
    ...
    public function get_mno_charges($store_lat,$store_lng,$user_lat,$user_lng,$mno_lat,$mno_lng, $listing_id,$pincode){
        ...
        $service_charges = 18; // standard 18% service charge
        $service_charges_gst = $service_charges;
        $service_charges_rs = floatval($DeliveryCharges)*($service_charges/100);
        
        /*add service charge on order*/ 
        $data['delivery_charges'] = $DeliveryCharges;
        $data['delivery_service_charge_per'] = $service_charges_gst;
        $data['delivery_service_charge_rs'] = $service_charges_rs;
    ...
    /*NOT USING ANYWHERE : 4th Jul 2020*/
    /* distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    /*First check where we are using it */
    ...
    /*testinf pending : done with distributor orders integration on 9th May 2020 by Swapnali Waghunde*/
    public function assign_mno_v1($user_id,$invoice_no,$lat,$lng,$order_vendor_type){
        $no_delivery_available_invoices = $available_invoices = $mno_details = $data = array();
        $res = $mno_orders_id = 0;
        $orderInsertedButNotAssiged = 0;
        $description = $inv_id = $oid = $mno_id = $mnos = $allMnos = '';
        $mno_id = 0;
        ...
        /*52 : distributor to night owl(inven_distrub_invoice); 13: Pharmacy to night owl(user_order); 44: user to night owl(user_order)*/
        if($order_vendor_type == 52){
            $getPincodes = $this->db->query("SELECT * FROM `inven_distrub_invoice` WHERE `invoice_no` IN ($invoice_no)  AND listing_type = '52'")->result_array();
            foreach($getPincodes as $gp){
                $address_id = 0;
                $pincode = $gp['pincode'];
                $vendor_type = $gp['listing_type'];
                $mno_available_delivery = $this->PartnermnoModel->mno_available_delivery($address_id, $pincode);
                if($mno_available_delivery['delivery_available'] == 1){
                    $available_invoices[] = $gp['invoice_no'];
                } else {
                    $no_delivery_available_invoices[] = $gp['invoice_no'];
                }
            }
        } else {
            $getPincode = $this->db->query("select invoice_no,listing_type,pincode from user_order where invoice_no IN ($invoice_no)")->row_array(); 
            $address_id = 0;
            $pincode = $getPincode['pincode'];
            $vendor_type = $getPincode['listing_type'];
            $mno_available_delivery = $this->PartnermnoModel->mno_available_delivery($address_id, $pincode);
            if($mno_available_delivery['delivery_available'] == 1){
                $available_invoices[] = $getPincode['invoice_no'];
            } else {
                $no_delivery_available_invoices[] = $getPincode['invoice_no'];
            }
        }
        
        if(sizeof($available_invoices) > 0){
            foreach($available_invoices as $ai){
            ...
            if(!empty($isAlreadyAssigned) && sizeof($isAlreadyAssigned > 0) && $isAlreadyAssigned['mno_id'] > 0){
                ...
                if(!empty($busyMnos) &&  sizeof($busyMnos) > 0 && $mno_id == ''){
                    ...
                    if(!empty($mno_id) &&  sizeof($mno_id) > 0){
                        ...
                        if($orderInsertedButNotAssiged == 0){
                            $insertInOrder = $this->db->query("INSERT INTO `mno_orders`(`invoice_no`, `mno_id`, `ongoing`, `created_at`, `updated_at`,`vendor_type`) VALUES ('$invoice_no','$mno_id','1','$order_date','$order_date','$order_vendor_type')");
                            $mno_orders_id = $this->db->insert_id();
            ...
            $res = 2; // MNO delivery not available
            /*$description = $mno_available_delivery['message'];*/
            $description = 'NO night owl delivery available';
        }
    ...
    /*Pendig: distributor orders not integrated : Swapnali Waghunde on 9th May 2020*/
    public function order_delivered_with_sign($mno_id,$invoice_no,$received_by,$sign_file){
        $image_path = "";
        $this->load->model('OrderModel');
        /*
        * Query before distributor integration
        */
/*        $existingOrder = $this->db->query("SELECT uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor, uo.delivery_charges_by_mw,  uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id, uo.order_date,uo.order_status  , uo.delivery_charges_to_mno, uo.suggested_pharmacy_id, ml.mno_name, mo.*, u.id as user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'")->row_array();
*/        


       $sql_query = "SELECT uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor,
        uo.delivery_charges_by_mw, uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id,
        uo.order_date,uo.order_status , uo.delivery_charges_to_mno, uo.suggested_pharmacy_id, ml.mno_name, mo.*, u.id as
        user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no)
        left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` =
        '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted' AND mo.vendor_type != '52'
        
        UNION 
        
        SELECT idi.user_id,null as delivery_charge,idi.delivery_charges_by_customer, idi.delivery_charges_by_vendor,
        idi.delivery_charges_by_mw, null as  total_without_dc,idi.listing_type,idi.payment_method,idi.order_total, idi.listing_id,
        idi.order_date,idi.order_status , idi.delivery_charges_to_mno, null as suggested_pharmacy_id, ml.mno_name, mo.*, u.id as
        user_id, u.phone, u.token, u.agent FROM `mno_orders` as mo left join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no)
        left join users as u on (idi.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` =
        '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted' AND mo.vendor_type = '52'"; 
        ...
        if($existingOrder > 0 ){
            ...
            $delivery_charges_by_vendor = $existingOrder['delivery_charges_by_vendor'];
            $delivery_charges_by_mw = $existingOrder['delivery_charges_by_mw'];
            $vendor_type = $existingOrder['vendor_type'];
            ...
            if($suggested_pharmacy_id > 0){
                ...
                $listing_id = $listing_id;
                $listing_id_type = $listing_type; //  vendor type as per db
            }
...
For more services regarding Health | Beauty | Fitness download Medicalwale Saathi App $link_line and stay healthy.";
            ...
            if(!empty($sign_file)){
                $img_name = $sign_file['signature']['name'];
                $img_size = $_FILES['signature']['size'];
                $img_tmp = $_FILES['signature']['tmp_name'];
                ...
                if (strlen($img_name) > 0) {
                    if (in_array($ext, $img_format)) {
                        ...
                        $actual_image_path = 'images/Signature/' . $actual_image_name;
                        if ($s3->putObjectFile($img_tmp, $bucket, $actual_image_path, S3::ACL_PUBLIC_READ)) {
                            $image_path = "https://medicalwale.s3.amazonaws.com/images/Signature/".$actual_image_name;
                        }
                    }
                }
            }
            $changeOrderStatus = $this->db->query("UPDATE `mno_orders` SET `ongoing`= '0' WHERE `invoice_no` = '$invoice_no' AND `mno_id` = '$mno_id' AND `ongoing` = '1' AND `status` = 'accepted'"); 
            if($listing_type == 52){
                $updateUserOrderTable = $this->db->query("UPDATE inven_distrub_invoice set `order_status` = 'Delivered',`status` = '2',  `cancel_reason`='',`action_by`='Night owl' , `delivered_sign` = '$image_path', `order_received_by` = '$received_by' WHERE `invoice_no` = '$invoice_no'"); 
            } else {
                $updateUserOrderTable = $this->db->query("UPDATE user_order set `order_status` = 'Order Delivered', `cancel_reason`='',`action_by`='night owl' , `delivered_sign` = '$image_path', `order_received_by` = '$received_by' WHERE `invoice_no` = '$invoice_no'"); 
            }
            ...
            $getMnoLocation = $this->db->query("SELECT * FROM `mno_location` WHERE `mno_id` = '$mno_id' ORDER BY `created_at` DESC limit 1 ")->row_array();
    ...
    /*for SE panel*/
    public function mno_list($search,$page_no,$per_page){
        $find = "";
        $offset = $per_page*($page_no - 1);
        $limit = "LIMIT $per_page OFFSET $offset"; 
        if($search != ""){
            $find = " where ml.mno_name LIKE '%$search%' or ml.phone LIKE '%$search%' or ml.email LIKE '%$search%' ";
        }
        /*pending order by*/
        $mno_list = $this->db->query("SELECT ml.mno_id, ml.mno_name, ml.phone, ml.email, ml.approval, u.status  FROM `mno_list` as ml left join users as u on (u.id = ml.mno_id) $find $limit")->result_array();
        return $mno_list;
    }
    
    public function add_mno($name, $mobile_no, $email, $map_location, $lat, $lng, $vechile_no,$city,$gender,$dob){
        ...
        $get_mno = $this->db->query("SELECT * FROM `mno_list` WHERE `phone` = '$mobile_no'")->result_array();
        if(sizeof($get_mno) > 0){
            $status = 3; //mno found
            $msg = "Nightowl already exists";
        } else {
            
            
            //insert in users table
            
            $this->db->query("INSERT INTO `users`(`created_at`, `name`, `phone`, `email`, `agent`, `vendor_id`, `lat`, `lng`, `map_location`, `password`, `city`,  `gender`, `dob`, `is_active`,  `status`) VALUES ('$current_time' , '$name','$mobile_no','$email','$agent','$vendor_id','$lat','$lng','$map_location','$password','$city','$gender','$dob','$is_active','$is_active')");
            $user_id = $this->db->insert_id();
            if($user_id != "" && $user_id > 0){
                //insert in mno_list table    
                $this->db->query("INSERT INTO `mno_list`( `mno_id`, `mno_name`, `phone`, `email`, `vechile_no`, `lat`, `lng`, `address`, `approval`, `createddate`) VALUES ('$user_id','$name','$mobile_no','$email','$vechile_no','$lat','$lng','$map_location','$is_active','$current_time')");
                
                $status = 1; //success
                $msg = 'Night owl successfully added';
            } else {
                $status = 2; //something wrong
    ...
    public function edit_mno($mno_id, $name, $mobile_no, $email, $vechile_no, $approve, $status){
        $set_mno_list = $set_users = "";
        
        if($name != ""){
            $set_mno_list .= " mno_name  = '$name' ";
            $set_users .= " name  = '$name' ";
        }
        
        if($mobile_no != ""){
            $set_mno_list .= $set_mno_list!= "" ? ' , ' : "";
            $set_users .= $set_users!= "" ? ' , ' : "";
            
            $set_mno_list .= " phone  = '$mobile_no' ";
            $set_users .= " phone  = '$mobile_no' ";
        }
        ...
        if($email != ""){
            ...
            $set_mno_list .= " email  = '$email' ";
            $set_users .= " email  = '$email' ";
        }
        
        if($vechile_no != ""){
            $set_mno_list .= $set_mno_list!= "" ? ' , ' : "";
            
            $set_mno_list .= " vechile_no  = '$vechile_no' ";
        }
        ...
        if($status != ""){
            ...
            $set_users .= " status  = '$status' ";
        }
        
        if($set_mno_list != ""){
            $this->db->query("UPDATE mno_list SET $set_mno_list WHERE mno_id = '$mno_id' ");
        }
        if($set_users != ""){
            $this->db->query("UPDATE users SET $set_users WHERE id = '$mno_id' ");
        }
    ...
    /*Done with integrating distributor orders*/
    public function all_orders_list($mno_id, $page_no, $per_page,$type,$b2b,$b2c,$search,$from_date = "",$to_date=""){
      //  date_default_timezone_set('GMT');
      $this->load->model('PaymentModel');  
        $allOrdersWithInv = array();
       $order_date1 = $order_date = null;
        ...
        if($type == 1 || $type == 'accepted'){
            ...
            $typeWHere = "AND mo.ongoing = 1 AND mo.status = '' AND mo.mno_id > '0' " ;
        }else if($type == 'notassign'){
            $typeWHere = "AND mo.ongoing = 1  AND mo.mno_id = '0' " ;
        } else {
            $typeWHere = "";
        }
        
        // $from_date $to_date preffered formate Y-m-d
        if($to_date != "" && $from_date != ""){
            $to_date = date('Y-m-d', strtotime($to_date . ' +1 day'));
            $date_search_uo = " AND uo.order_date BETWEEN '$from_date' AND '$to_date' ";
            $date_search_idi = " AND idi.order_date BETWEEN '$from_date' AND '$to_date' ";
            ...
            $date_search_idi = $date_search_uo = "";
        }
        
        if($mno_id > 0){
            $whereMNO = " AND mo.`mno_id` IN ($mno_id) ";
        } else {
            $whereMNO = "";
        }
        
        if($search != ""){
            $whereSearchUO = " AND (uo.order_status like '%$search%' OR uo.listing_name like '%$search%' OR uo.name like '%$search%' OR uo.mobile like '%$search%' OR mo.invoice_no like '%$search%' OR ml.mno_name like '%$search%' OR uo.address1 like '%$search%' OR uo.address2 like '%$search%'  )  ";
            $whereSearchIDI = " AND (idi.order_status like '%$search%' OR idi.listing_name like '%$search%' OR idi.name like '%$search%' OR idi.mobile like '%$search%' OR mo.invoice_no like '%$search%' OR ml.mno_name like '%$search%' OR idi.address1 like '%$search%' OR idi.address2 like '%$search%'  )  ";
            
        } else {
            $whereSearchIDI = $whereSearchUO = "";
        }
        
        if($b2b == 1 && $b2c == 0){
            $whereOrderType = " AND mo.order_type = '1' ";
        } else if ($b2b == 0 && $b2c == 1){
            $whereOrderType = " AND mo.order_type = '0' ";
        }  else {
            $whereOrderType = "";
        }
        ...
        $data = array();
            ...
            $$orders_sql_query = "SELECT mo.cancel_reason,uvl.payment_method as payment_method_id, uvl.ledger_owner_type,uvl.transaction_id,pm.payment_method, uvl.trans_status, uvl.listing_id_type as payment_received_by, uvl.credit, uvl.debit, uvl.accepted_by, uvl.submitted_by, uo.order_status,uo.mno_bill_no, uo.delivery_note,uo.mobile as
        customer_phone,uo.bill_url,mo.vendor_type,mo.order_type,ml.mno_name,ml.phone as mno_phone,mo.mno_id,mo.id as
        mno_order_id,mo.updated_at as last_updated,uo.order_date, uo.listing_name,uo.listing_type,
        mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name,
        uo.address1, uo.address2, uo.landmark, uo.city, uo.state,uo.pincode 
        FROM `mno_orders` as mo left join user_order as uo
        on (uo.invoice_no = mo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (mo.mno_id =
        ml.mno_id) left join user_vendor_ledger as uvl on (uvl.invoice_no = mo.invoice_no AND uvl.transaction_of = 2 AND (uvl.ledger_owner_type = '0' or uvl.listing_id_type = '0') AND uvl.trans_status = '1') left join payment_method as pm on (uvl.payment_method = pm.id)  WHERE (uo.listing_type = 44 OR uo.listing_type = 13) $whereMNO $typeWHere $whereOrderType $whereSearchUO $date_search_uo
        
        
        UNION
        
        SELECT  mo.cancel_reason,uvl.payment_method as payment_method_id,uvl.ledger_owner_type,uvl.transaction_id,pm.payment_method, uvl.trans_status, uvl.listing_id_type as payment_received_by, uvl.credit, uvl.debit, uvl.accepted_by, uvl.submitted_by,  idi.order_status, null as mno_bill_no, null as delivery_note,idi.mobile as
        customer_phone,null as bill_url,mo.vendor_type,mo.order_type,ml.mno_name,ml.phone as mno_phone,mo.mno_id,mo.id as
        mno_order_id,mo.updated_at as last_updated,idi.order_date, idi.listing_name,idi.listing_type,
        mo.status,mo.ongoing,idi.order_id,idi.order_type,idi.invoice_no ,idi.name, idi.user_id, u.id, u.name as user_name,
        idi.address1, idi.address2, idi.landmark, idi.city, idi.state,idi.pincode 
        FROM `mno_orders` as mo left join inven_distrub_invoice as idi
        on (idi.invoice_no = mo.invoice_no) left join users as u on (idi.user_id = u.id) left join mno_list as ml on (mo.mno_id =
        ml.mno_id) left join user_vendor_ledger as uvl on (uvl.invoice_no = mo.invoice_no AND uvl.transaction_of = 2 AND (uvl.ledger_owner_type = '0' or uvl.listing_id_type = '0')  AND uvl.trans_status = '1') left join payment_method as pm on (uvl.payment_method = pm.id) WHERE (idi.listing_type = 52) $whereMNO $typeWHere $whereOrderType $whereSearchIDI $date_search_idi
        
        
        ORDER BY invoice_no desc "; 
        
//          echo $orders_sql_query ;  die();
        
        $allOrders = $this->db->query($orders_sql_query.$limit)->result_array();
        $allOrders_count = $this->db->query($orders_sql_query)->result_array();
        $orders_count = sizeof($allOrders_count);
        ...
        foreach($allOrders as $o){
          ...
          // send pharmacy name or siggested pharmacy name if order from pharmacy
            ...
            if($o['vendor_type'] == '52'){
                $order_from = 'Distributor';
                $type = 'b2b';
            } else if($o['vendor_type'] == '13') {
                $order_from = 'Pharmacy';
                $type = 'b2c';
            } else {
                $order_from = 'User';
          ...
          if($o['listing_type'] == 13){
           ...
           $invoice_no = $o['invoice_no'];
            ...
            $get_invoice_costing = $this->PaymentModel->get_invoice_costing($inv);
            if($get_invoice_costing['order_found'] == 1) {
                $cost = $get_invoice_costing['data'];
            }else {
                $cost = (object)[];
            }
            ...
            // } else {
                ...
                if(!empty($o['name'])){
                 ...
                 $orderData['customer_phone'] = $o['customer_phone'];
                $orderData['mno_id'] = $mno_id;
                $orderData['mno_name'] = $o['mno_name'];
                $orderData['delivery_note'] =  $o['delivery_note'];
                $orderData['mno_bill_no'] =  $o['mno_bill_no'];
                $orderData['order_status'] =  $o['order_status'];
                
                $orderData['mno_phone'] = $o['mno_phone'];
                $orderData['order_from'] = $order_from;
                $orderData['type'] = $type;
                $orderData['address1'] = $o['address1'];
                $orderData['address2'] = $o['address2'];
                
                $orderData['landmark'] = $o['landmark'];
                $orderData['city'] = $o['city'];
                
                $orderData['state'] = $o['state'];
                $orderData['pincode'] = $o['pincode'];

                $orderData['transaction_id'] = $o['transaction_id'];
                $orderData['payment_method'] = $o['payment_method'];
                $trans_status = $o['trans_status'];
                $payment_received_by = $o['payment_received_by'];
                $ledger_owner_type = $o['ledger_owner_type'];
                $credit  = $o['credit'];
                $debit  = $o['debit'];
                $orderData['accepted_by'] = $o['accepted_by'];
                $orderData['submitted_by'] = $o['submitted_by'];
                $orderData['payment_method_id'] = $o['payment_method_id'];
                $orderData['cancel_reason'] = $o['cancel_reason'];
                
                $received_by = $accepted_amount = "";
                if($ledger_owner_type == '0'){
                    
                    if($payment_received_by == '13'){
                        $received_by = 'Pharmacy';
                    } else if($payment_received_by == '44'){
                        $received_by = 'Night owl';
                    } else if($payment_received_by == '52'){
                        $received_by = 'Distributor';
                    } else {
                        $received_by = '';
                    }  
                    $accepted_amount = $debit;
                } else if($payment_received_by == '0'){
                    
                    if($ledger_owner_type == '13'){
                        $received_by = 'Pharmacy';
                    } else if($ledger_owner_type == '44'){
                        $received_by = 'Night owl';
                    } else if($ledger_owner_type == '52'){
                        $received_by = 'Distributor';
                    } else {
                        $received_by = '';
                    }
                    $accepted_amount = $credit;
                }
                
                $orderData['amount_received_by'] = $received_by;
                $orderData['amount_from_user'] = $accepted_amount;
                ...
                $orderData['transaction_status'] = $transaction_status;
    ...
    /*call_night_owl*/
    /*DOne with ditributor order integration by Swapnali Waghunde*/
    public function call_night_owl($invoices, $vendor_type,$user_id){
        $this->load->model('PaymentModel');  
        $status = 0;
        $return_msg = "";
        $return = array();
        ...
        $delivery_by_mno = "";
        $update_user_order = $get_upload_invices = $upload_invices = $insert_data = array();
        $invs = implode(',', $invoices);
        $alreadyAddedInvs = $this->db->query("SELECT invoice_no from mno_orders where invoice_no IN ($invs) GROUP BY invoice_no")->result_array();
        $existing_invoice_no = array_column($alreadyAddedInvs, 'invoice_no');
        $remaining = array_diff($invoices,$existing_invoice_no);
        // GET  invs which are already insetrted
        // 1: added all 2: added partially 3: not added
        // 
        
        if(sizeof($remaining) > 0){
            if($vendor_type == 52){
            // its b2b
                $remaining_invs = implode(',', $remaining);
               
                $upload_invices = $this->db->query("SELECT null as delivery_charges_by_customer,null as order_id , invoice_no from inven_distrub_invoice where invoice_no IN ($remaining_invs)  ")->result_array();
                // $get_upload_invices = array_column($upload_invices, 'invoice_no');
                
                $order_type  = 1;
            } else {
                // its b2c
                $remaining_invs = implode(',', $remaining);
                $upload_invices = $this->db->query("SELECT delivery_charges_by_customer, order_id,invoice_no from user_order where invoice_no IN ($remaining_invs) AND order_status != 'Order Cancelled' AND order_status != 'Order Delivered' ")->result_array();
                // $get_upload_invices = array_column($upload_invices, 'invoice_no');
                $order_type  = 0;
            ...
            foreach($upload_invices as $ui){
                ...
                if($vendor_type != 52){
                    $delivery_by_mno .= $delivery_by_mno != "" ? " OR " : " ";
                    $delivery_by_mno .= " `invoice_no` = '$in' " ;
                    
                }
                
            }
            if(sizeof($insert_data) > 0){
                $this->db->insert_batch('mno_orders', $insert_data);
                if($delivery_by_mno != ""){
                    $update_user_sql = "UPDATE user_order SET order_deliver_by = 'mno' WHERE $delivery_by_mno";
                    $this->db->query($update_user_sql);
                }
                ...
                if($vendor_type != 52){
                    foreach($upload_invices as $ui){
                        $in = $ui['invoice_no'];
                        $order_id = $ui['order_id'];
                        $delivery_charges_by_customer = $ui['delivery_charges_by_customer'];
                        $offer_id = 0; 
                       
                        $deliveryTimeAndCostMno = $this->PartnermnoModel->get_delivery_time_cost($order_id);
                        $delivery_charges = $deliveryTimeAndCostMno['delivery_charges'];
                        
                        $addDeliveryCharges  = $this->PaymentModel->add_delivery_charges($delivery_charges, $in, $delivery_charges_by_customer, $offer_id);
        ...
        if(sizeof($invoices) == sizeof($upload_invices)){
            $status = 1; // all 
            $return_msg = 'All orders are sent to night owl';
        } else if(sizeof($upload_invices) > 0 && sizeof($upload_invices) < sizeof($invoices)) {
            $status = 2; // partially
            $return_msg = 'Some orders are sent to night owl';
        } else {
            $status = 3; // none
            $return_msg = 'No order sent to night owl';
        }
    ...
    public function get_ongoing_orders($mno_id){
        $invoice_no = "";
        $orders = $data = $order_details = array();
        $page_no = 1;   
        $per_page = 10;
        ...
        foreach($listOfOrders['orders'] as $o){
            $invoice_no = $o['invoice_no'];
            $mno_order_id = $o['mno_order_id'];
            $order_details  = $this->PartnermnoModel->order_details_v1($mno_id , $invoice_no, $mno_order_id);
            if(!empty($order_details['data'])){
                $orders[] = $order_details['data'];
            }
        }
    ...
    /*Handled distributor orders by Swapnali Waghunde 9th May 2020*/
    public function assign_mno_manually($mno_id,$invoices){
        $status = 0;
        $msg = "";
        $update_invs = "";
        $updated_data = $insert_data = array();
        $invs = implode(',', $invoices);
        $alreadyAddedInvs = $this->db->query("SELECT id, invoice_no from mno_orders where invoice_no IN ($invs) AND mno_id = '0' and ongoing = '1' GROUP BY invoice_no")->result_array();
        // $existing_invoice_no = array_column($alreadyAddedInvs, 'invoice_no');
        // $remaining = array_diff($invoices,$existing_invoice_no);
        ...
        if(sizeof($mno_details) > 0){
            ...
            foreach($alreadyAddedInvs as $rem){
                $inv_no = $rem['id'];
                $update_invs .= $update_invs != "" ? ' OR ' : " "; 
                $update_invs .= " id = '$inv_no' ";
                
                $invs_no = $rem['invoice_no'];
                $po_id = $rem['id'];
                
                
                
                $updated['invoice_no'] = $invs_no;
                $updated['id'] = $po_id;
            ...
            if($update_invs != ""){
                $query = "UPDATE mno_orders SET mno_id = '$mno_id' WHERE $update_invs";
                $this->db->query($query);
                
                
                $reg_id = $mno_details['token'];
                $agent = $mno_details['agent'];
                $msg = 'You have received bulk orders. Please check';
                $img_url = 'https://medicalwale.com/img/noti_pharmacy.png';
                $tag = 'text';
                $key_count = '1';
                $title = 'Bulk orders received';
                $title_for_tracker = 'Order received';
                $order_status = 'Awaiting confirmation';
                $order_date = '';
                $name = '';
                $notification_type = 1;
                ...
                $notification = $this->PartnermnoModel->bulk_notification_to_mno($mno_id,$updated_data, $notification_type, $img, $title, $msg,$title_for_tracker); 
            ...
            // no mno found
            $status = 3;
            $msg = "Orders are already assigned";
        }    
        $return['status'] = $status;
        $return['msg'] = $msg;
        return $return;    
    ...
    /*Done with distributor integration */
    public function orders_accept($mno_id,$invoices,$lat,$lng){
        $this->load->model('OrderModel');
        $return_status = 0;
        $return_msg = "";
        $invs = implode(',', $invoices);
        ...
        $pending_orders = $this->db->query("SELECT uo.user_id,uo.listing_id,uo.order_date,uo.order_status  ,u.phone,u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join user_order as uo on (uo.invoice_no = mo.invoice_no) join users as u on (u.id = uo.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = '' AND mo.invoice_no IN ($invs) AND mo.order_type = '0'
            UNION
            SELECT idi.user_id,idi.listing_id,idi.order_date,idi.order_status  ,u.phone,u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join inven_distrub_invoice as idi on (idi.invoice_no = mo.invoice_no) join users as u on (u.id = idi.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = '' AND mo.invoice_no IN ($invs) AND mo.order_type = '1'")->result_array();
        if (sizeof($pending_orders) > 0) {
            ...
            $token = "";
            $agent = "";
            $updated_data = $updated = $mno_order_ids = $updated_invs = array();
            $mno_actions = array();
            // mno_notification_types table order accepted by mno type id
            $action_type = 2;
            
            foreach($pending_orders as $po){
                $inv_no = $po['invoice_no'];
                $po_id = $po['id'];
                $mno_name = $po['mno_name'];
                
                
                
                $updated['invoice_no'] = $inv_no;
                $updated['id'] = $po_id;
                $updated_data[] = $updated;
                ...
                $action_by_status = "Night owl";
                $orderStatus = "Order accepted by night owl ". $mno_name;
                 $order_status = $this->OrderModel->update_status($inv_no,$orderStatus, $action_by_status);
                ...
                $mno_actions[] = $ma;
                      ...
                      // mno acceoted the order
            if($update_ids != ""){
                  $acceptedOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='1',`status`='accepted'  WHERE $update_ids");
                  $this->db->insert_batch('mno_actions', $mno_actions);
                ...
                foreach($pending_orders as $po){
                    ...
                    $reg_id = $po['token'];
                    $agent = $po['agent'];
                    $user_mobile = $po['phone'];
                    $mno_name = $po['mno_name'];
                    
                    $msg1 = $title = "Order accepted by night owl";
                    $msg = "Night owl $mno_name accepted your order #$inv_no";
                    $title_for_tracker = 'Order accepted';
                    $img = $img_url   = 'https://medicalwale.com/img/noti_icon.png';
                    $tag = "text";
                    $invoice_no = $inv_no;
                    $notification_type_user = "mno"; 
                    ...
                    /*Added by Swapnali Waghunde on 19th May 2020 TO ADD NOTIFICATION IN TABLE WHICH WAS COMMENTED BEFORE */
                    $user_id = $po['user_id'];
                    $listing_id = $po['listing_id'];
                    $order_date = $po['order_date'];
                    $user_order_status = $po['order_status'];
                
                    $res = $this->PartnermnoModel->send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type_user,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status,$whatsapp_body);
                    
                }
                // accepted notification to user - end
                        
                
                
                $notification_type = 2; 
                $notification = $this->PartnermnoModel->bulk_notification_to_mno($mno_id,$updated_data, $notification_type, $img, $title, $msg1,$title_for_tracker);    
                if(array_key_exists('orders',$notification)){
            ...
            $return_status = 2;
            $return_msg = 'Selected orders are either accepted or rejected already';
        }
    ...
    public function bulk_notification_to_mno($receiver_id,$updated_data, $notification_type, $img, $title, $msg,$title_for_tracker) {
        $detailsData = $return = $result = $updated_invs = $order_details = array();
        date_default_timezone_set('Asia/Kolkata');
        $date = date('j M Y h:i A');
        ...
        if($token != ""){
            ...
            foreach($updated_data as $ud){
                $invoice_no = $ud['invoice_no'];
                $mno_orders_id = $ud['id'];
                $updated_invs[] = $invoice_no;
                 $notificationCheck = $this->db->query("SELECT * FROM `mno_notifications` WHERE `invoice_no` LIKE '$invoice_no' AND `mno_orders_id` = '$mno_orders_id' AND `notification_type` = '$notification_type' ORDER BY `id` ASC")->row_array();
            
            
                 if(sizeof($notificationCheck) == 0){
                     $this->db->query("INSERT INTO `mno_notifications`( `receiver_id`, `invoice_no`,`mno_orders_id`,`notification_type`, `img`, `title`, `msg`) VALUES ('$receiver_id','$invoice_no','$mno_orders_id','$notification_type','$img','$title_for_tracker','$msg')");
                 }
            
                
                // $order_details[] = $this->PartnermnoModel->order_details($receiver_id , $invoice_no,$mno_order_id);
          ...
          curl_close($ch);
          $return['notification'] = $fields['data'];
          $return['orders'] = $detailsData;
        }
        return $return;    
   }
   
   /*Done with distributor orders integration on 9th May 2020*/
   public function orders_reject($mno_id,$invoices,$lat,$lng,$cancel_reason_id){
        ...
        //  $cancel_reason_id
        $cancelReason = $this->PartnermnoModel->get_cancel_reason($cancel_reason_id); 
        if(sizeof($cancelReason) > 0){
            $cancel_reason = $cancelReason['cancel_reason'];
            
            $pending_query = "SELECT uo.user_id,uo.listing_id,uo.order_date,uo.order_status  ,u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join user_order as uo on (uo.invoice_no = mo.invoice_no) join users as u on (u.id = uo.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = '' AND mo.invoice_no IN ($invs) AND mo.order_type = '0'
                                UNION
                                SELECT idi.user_id,idi.listing_id,idi.order_date,idi.order_status  , u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join inven_distrub_invoice as idi on (idi.invoice_no = mo.invoice_no) join users as u on (u.id = idi.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = '' AND mo.invoice_no IN ($invs) AND mo.order_type = '1'";
            
            $pending_orders =$this->db->query($pending_query)->result_array();
                ...
                if (sizeof($pending_orders) > 0) {
                    ...
                    $updated_data = $updated = $mno_order_ids = $updated_invs = array();
                    $mno_actions = array();
                    // mno_notification_types table order reject by mno type id
                    $action_type = 3;
                    ...
                    if($update_ids != ""){
                        $rejectedOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='0',`status`='rejected' ,`cancel_reason`='$cancel_reason' WHERE $update_ids");
                        
                        $this->db->insert_batch('mno_actions', $mno_actions);
                        if($cancelReason['redirect_to_another_mno'] > 0){
                            
                            $this->db->insert_batch('mno_orders', $mno_orders);
                        } 
                        ...
                        $title_for_tracker = $msg1 = $title = "Order rejected";
                            $msg = "Night owl rejected your order";
                            $img = $img_url   = 'https://medicalwale.com/img/noti_icon.png';
                            $tag = "text";
                ...
                $return_status = 3;
                $return_msg = 'No cancel reason found';
            }                    
   ...
   /*DOne with distributor order handling by Swapnali on 9th May 2020*/
    public function out_for_pickups($mno_id,$invoices){
        $this->load->model('OrderModel');
        $return_status = 0;
        $invs_comma = $return_msg = "";
        $invs = implode(',', $invoices);
        $updated_data = $orders = $data = array();
           
        $pending_query = "SELECT uo.user_id,uo.listing_id,uo.order_date,uo.order_status  ,u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join user_order as uo on (uo.invoice_no = mo.invoice_no) join users as u on (u.id = uo.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = 'accepted' AND mo.invoice_no IN ($invs) AND mo.order_type = '0'
                                UNION
                                SELECT idi.user_id,idi.listing_id,idi.order_date,idi.order_status  ,u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join inven_distrub_invoice as idi on (idi.invoice_no = mo.invoice_no) join users as u on (u.id = idi.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = 'accepted' AND mo.invoice_no IN ($invs) AND mo.order_type = '1'";
            
        
        $orders = $this->db->query($pending_query)->result_array();    
        ...
        foreach($orders as $o){
            ...
            $order_status = $this->OrderModel->update_status($invoice_no,$orderStatus, $action_by_status);
            $invs_comma .= $invs_comma != "" ? ', ' : ''; 
            $invs_comma .= $invoice_no;
            
            $updated['invoice_no'] = $invoice_no;
            $updated['id'] = $mno_order_id;
            ...
            $user_order_status = $o['order_status'];
            
            $res = $this->PartnermnoModel->send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $invoice_no,$notification_type,$user_mobile,$whatsapp_msg,$user_id,$listing_id,$order_date, $user_order_status,$whatsapp_body);
            
        }
        
        if(sizeof($updated_data) > 0){
            $receiver_id = $mno_id;
            $invoice_no = $invoice_no;
            $notification_type = 10; //OUT_FOR_PICKUP refer mno_notification_types
            $img = "https://s3.amazonaws.com/medicalwale/images/img/pharmacy.png";
            $title_for_tracker = $title = "On the way to pick-up";
            $msg = "On the way to pick-up for $invs_comma orders";
...
//   reahced_store
    /*Done with Distributor orders integration By swapnali on 9th may 2020*/
    public function reahced_store($mno_id,$invoices){
        $this->load->model('OrderModel');
        $return_status = 0;
        $invs_comma = $return_msg = "";
        $invs = implode(',', $invoices);
        ...
        if(sizeof($updated_data) > 0){
            ...
            $notification_type = 11; //ARRIVED_AT_PHARMACY refer mno_notification_types
            $img = "https://s3.amazonaws.com/medicalwale/images/img/pharmacy.png";
            $title_for_tracker = $title = "Reached at store";
            $msg = "Night owl arrived at pharmacy";
    ...
    /*cancel_orders*/
    /*Done with distributo Orders integration on 9th may 2020 by Swapnali Waghudne*/
    public function cancel_orders($mno_id,$invoices,$cancel_reason_id,$lat,$lng){
        ...
        $invs = implode(',', $invoices);
        $orders = $data = array();
        $update_distributor_inv_ids = $update_user_inv_ids = $update_inv_ids = $invs_comma = "";
        ...
        if(sizeof($cancelReason) > 0){
            ...
            $orders_query = "SELECT uo.user_id,uo.listing_id,uo.order_date,uo.order_status  , u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join user_order as uo on (uo.invoice_no = mo.invoice_no) join users as u on (u.id = uo.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = 'accepted' AND mo.invoice_no IN ($invs) AND mo.order_type = '0'
                                UNION
                                SELECT idi.user_id,idi.listing_id,idi.order_date,idi.order_status  ,u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join inven_distrub_invoice as idi on (idi.invoice_no = mo.invoice_no) join users as u on (u.id = idi.user_id)  WHERE mo.`mno_id` = '$mno_id' AND mo.`ongoing` = '1' AND mo.`status` = 'accepted' AND mo.invoice_no IN ($invs) AND mo.order_type = '1'";
            
            $orders =$this->db->query($orders_query)->result_array();
                ...
                if (sizeof($orders) > 0) {
                    ...
                    foreach($orders as $po){
                        $invoice_no = $inv_no = $po['invoice_no'];
                        $po_id = $po['id'];
                        $mno_name = $po['mno_name'];
                        ...
                        $update_inv_ids .= $update_inv_ids != "" ? " OR " : "";
                        $update_inv_ids .= " invoice_no = '$inv_no' ";
                        
                        if($order_type == 1){
                            $update_distributor_inv_ids .= $update_distributor_inv_ids != "" ? " OR " : "";
                            $update_distributor_inv_ids .= " invoice_no = '$inv_no' ";
                        } else {
                            $update_user_inv_ids .= $update_user_inv_ids != "" ? " OR " : "";
                            $update_user_inv_ids .= " invoice_no = '$inv_no' ";
                        }
                    ...
                    if($update_ids != ""){
                         $rejectedOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='0' ,`cancel_reason`='$cancel_reason' WHERE $update_ids");
                        ...
                        if($cancelReason['redirect_to_another_mno'] > 0){
                             ...
                             $this->db->insert_batch('mno_orders', $mno_orders);
                            $msg1 = $title = "Redirecting order";
                            $msg = "Order will redirect to another night owl";
                        
                        } 
                          
                        if($cancelReason['cancel_order'] > 0){
                            $rejectedOrder = $this->db->query("UPDATE `mno_orders` SET `order_cancel` = '1'  WHERE $update_ids");
                            $msg1 = $title = "Order cancelled";
                            $msg = "Night owl cancelled your order";
                            
                            if($update_user_inv_ids != ""){
                                 $this->db->query("UPDATE user_order SET order_status = 'Order Cancelled' WHERE $update_user_inv_ids");
                            }
                            
                            if($update_distributor_inv_ids != ""){
                                $updateUserOrderTable = $this->db->query("UPDATE inven_distrub_invoice set `order_status` = 'Cancelled',`status` = '3',  `cancel_reason`='$msg',`action_by`='Night owl' WHERE `invoice_no` = '$invoice_no'"); 
                            }
                        }
                        ...
                        $title_for_tracker = $msg1 = $title = "Order cancelled";
                        $msg = "Night owl cancelled $invs_comma orders";
                        $notification_type = 27; 
                        $notification = $this->PartnermnoModel->bulk_notification_to_mno($mno_id,$updated_data, $notification_type, $img, $title, $msg,$title_for_tracker);    
                        if(array_key_exists('orders',$notification)){
                            $orders = $notification['orders'];
    ...
    /*Done with Distributor order handling*/
    public function redirect_order($mno_id,$invoices,$reason_id){
        $this->load->model('OrderModel');
        $lat = "19.126959"; // Eco space office address
        $lng = "72.849984"; // Eco space Office address
        $return_status = 0;
        $return_msg = "";
        $update_inv_ids = $invs_comma = "";
        $orders = $data = array();
        $invs = implode(',', $invoices);
        // $mno_id,$invoices,$reason_id
        /*check reson exists*/
        /*check order is ongoing*/
        /*user cancel order logic and assign given mno to another order*/
        // echo $mno_id; die();
        
        //  $cancel_reason_id
        $cancelReason = $this->PartnermnoModel->get_cancel_reason($reason_id); 
        if(sizeof($cancelReason) > 0){
            $cancel_reason = $cancelReason['cancel_reason'];
            
            $orders_query = "SELECT mo.mno_id,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join user_order as uo on (uo.invoice_no = mo.invoice_no) join users as u on (u.id = uo.user_id)  WHERE mo.mno_id != '$mno_id' AND mo.`ongoing` = '1'  AND mo.invoice_no IN ($invs) AND mo.order_type = '0'
                                UNION
                                SELECT mo.mno_id, mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo join mno_list as ml on (mo.mno_id = ml.mno_id) join inven_distrub_invoice as idi on (idi.invoice_no = mo.invoice_no) join users as u on (u.id = idi.user_id)  WHERE mo.mno_id != '$mno_id' AND mo.`ongoing` = '1'  AND mo.invoice_no IN ($invs) AND mo.order_type = '1'";
            ...
            if(sizeof($orders) > 0){
                    ...
                    $new_mno_details = $this->db->query("SELECT * FROM mno_list WHERE mno_id = '$mno_id'")->row_array();
                    // print_r($new_mno_details); die();
                    foreach($orders as $po){
                        $invoice_no = $inv_no = $po['invoice_no'];
                        $po_id = $po['id'];
                        $mno_name = $new_mno_details['mno_name'];
                        
                        $old_mno_id = $po['mno_id'];
                        $vendor_type = $po['vendor_type'];
                        $order_type = $po['order_type'];
                        ...
                        $action_by_status = "Support executor";
                        $orderStatus = "Order redirected to ". $mno_name ." by support executor";
                        $order_status = $this->OrderModel->update_status($inv_no,$orderStatus, $action_by_status);
                        ...
                        $msg1 = "Support executive redirected your order";
                        $this->PartnermnoModel->send_gcm_notification_to_mno($old_mno_id,$inv_no,$po_id, $notification_type, $img, $title, $msg1);
                    ...
                    if($update_ids != ""){
                         ...
                         $rejectedOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='0',`redircted_to` = '$mno_id' ,`cancel_reason`='$cancel_reason' WHERE $update_ids");
                        ...
                        $img = $img_url   = 'https://medicalwale.com/img/noti_icon.png';
                      /*  $title_for_tracker = $msg1 = $title = "Order cancelled";
                        $msg = "Support executive redirected $invs_comma orders";
                        $notification_type = 27; 
                        $notification = $this->PartnermnoModel->bulk_notification_to_mno($old_mno_id,$updated_data, $notification_type, $img, $title, $msg,$title_for_tracker);    
                      */  
                        ...
                        if(sizeof($mno_orders) > 0){
                            ...
                            $new_data = $this->db->query("SELECT id, invoice_no from mno_orders WHERE invoice_no IN ($invs_comma) AND mno_id = '$mno_id'")->result_array();
                            ...
                            if(sizeof($new_data) > 0){
                                ...
                                $notification = $this->PartnermnoModel->bulk_notification_to_mno($mno_id,$new_data, $notification_type, $img, $title, $msg,$title_for_tracker); 
                            }
                    ...
                    $return_msg = $invs_comma != "" ?  'Updated invoices are '.$invs_comma : "o orders to redirect";
            } else {
                $return_status = 2;    
                $return_msg = 'Selected orders are not ongoing';
            }        
    ...
    /*Created by swapnali waghunde on 5TH may 2020*/
    /*cREATING FUNCTION TO SEND WHATSAPP MSGS to user*/
    public function send_whatsapp_msg( $body)
    {
        $is_live = $this->check_environment();
        $data = json_encode($body, JSON_UNESCAPED_UNICODE);

        if($is_live == true){
            /*change this url to live*/
            $url = "https://vendor.medicalwale.com/whatsapp/public/whatsapp";
        } else {
            $url = "http://vendorsandbox.medicalwale.com/whatsapp/public/whatsapp";
        }
      
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
        curl_setopt($ch, CURLOPT_HTTPHEADER,['User-ID:1','Authorizations:25iwFyq/LSO1U','Client-Service:frontend-client','Auth-Key:medicalwalerestapi','Content-Type:application/json','Cookie:XSRF-TOKEN=; laravel_session=%3D']);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $result = curl_exec($ch);
        curl_close($ch);
    ...
    /*Created by swapnali waghunde on 6th MAY 2020 to handle distributors orders as well*/
    public function order_list_v1($mno_id, $page_no, $per_page,$type,$search){
      //  date_default_timezone_set('GMT');
    //   mno_bill_no
    // delivery_note
       ...
       $pickup_lat = $pickup_lng = $delivery_lat = $delivery_lng = null;
        ...
        if($search != ""){
            $whereSearch = " AND mo.invoice_no like '%$search%'  ";
        } else {
            $whereSearch = "";
        }
        $orderData = $allOrdersWithInv = array();
        ...
        $all_orders_query = "SELECT uo.mno_bill_no,uo.delivery_note,msp.lat as suggested_pharmacy_lat,msp.lng as suggested_pharmacy_lng ,ms.lat as pharmacy_lat,
ms.lng as pharmacy_lng ,uo.suggested_pharmacy_id , uo.listing_id,uo.lat as delivery_lat, uo.lng as delivery_lng, mo.id
as mno_order_id,mo.updated_at as last_updated,uo.order_date, uo.listing_name,uo.listing_type,
mo.status,mo.ongoing,uo.order_id,uo.order_type,uo.invoice_no ,uo.name, uo.user_id, u.id, u.name as user_name FROM
`mno_orders` as mo left join user_order as uo on (uo.invoice_no = mo.invoice_no AND (uo.listing_type = 44 OR
uo.listing_type = 13)) left join users as u on (uo.user_id =
u.id) left join medical_stores as ms on ((ms.user_id = uo.listing_id AND ms.pharmacy_branch_user_id = '0') OR
(ms.user_id > 0 AND ms.pharmacy_branch_user_id = uo.listing_id )) left join mno_suggested_pharmacies as msp on (msp.id =
uo.suggested_pharmacy_id) WHERE mo.`mno_id` = '$mno_id' AND mo.order_type = '0'  $typeWHere AND (uo.listing_type = 44 OR uo.listing_type = 13) $whereSearch group by mo.created_at

UNION

SELECT null as mno_bill_no, null as delivery_note, null as suggested_pharmacy_lat, null as suggested_pharmacy_lng, d.lat as pharmacy_lat, d.lng as pharmacy_lng, null as suggested_pharmacy_id, idi.listing_id, idi.lat as delivery_lat, idi.lng as delivery_lng, mo.id as mno_order_id, mo.updated_at as last_updated,idi.order_date, idi.listing_name,idi.listing_type, mo.status, mo.ongoing, idi.order_id,
idi.order_type, idi.invoice_no , idi.name, idi.user_id, u.id, u.name as user_name from `mno_orders` as mo left join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no AND idi.listing_type = 52 ) left join users as u on (idi.user_id = u.id) left join distributor as d on (d.user_id = idi.listing_id) WHERE mo.`mno_id` = '$mno_id' AND mo.order_type = '1' $typeWHere $whereSearch group by mo.created_at 


order by invoice_no desc ";

        $allOrders = $this->db->query($all_orders_query.$limit)->result_array();

        $orders_count = $this->db->query($all_orders_query)->num_rows();
        ...
        foreach($allOrders as $o){
            ...
            if($inv != "" && $inv != null){
                ...
                /*geting duration and distance of ongoing orders only, its taking more time for all orders*/
                if($delivery_lat > 0 && $delivery_lng > 0 && $pickup_lat > 0 && $pickup_lng > 0 && $o['ongoing'] == 1){
                    $drivingDistanceDurtion = $this->PartnermnoModel->GetDrivingDistance($pickup_lat, $delivery_lat, $pickup_lng, $delivery_lng );
                    $distance = $drivingDistanceDurtion['distance'] > 0 ? floatval($drivingDistanceDurtion['distance']) : null;
                    $duration = $drivingDistanceDurtion['duration'] > 0 ? floatval($drivingDistanceDurtion['duration']) : null;
                } else {
                    $duration = $distance = null;
                }
              ...
              if($o['listing_type'] == 13 || $o['listing_type'] == 52){
                  $pharmacy_name = $o['listing_name'];
              } else {
                  $pharmacy_name = null;
              }
               ...
               // print_r($o); die();
                ...
                if($tracking_status == null || $tracking_status == ""){
                    $tracking_status = "No actions yet";
                }
    ...
    /*Created by Swapnali Waghunde 27th May 2020 to handle distributor orders*/
    /*Pending : order product details -> Need to talk to dhaval*/
    public function order_details_v1($mno_id, $invoice_no,$mno_order_id = null){
        $orderd_pharmacy_details = array();
        $todayDate = date('Y-m-d H:i:s');
        $todayDay = date('l');
        $currenttime = date('h:i A');
        ...
        $statusdata = $data = array();
        $acceped_query = $this->db->query("SELECT mn.created_at as order_delivery_time,ma.created_at as order_acceped_time, mo.* FROM `mno_orders` as mo left join mno_actions as ma on (mo.mno_id = ma.mno_id && mo.invoice_no = ma.invoice_no && ma.action_type = 2) left join mno_notifications as mn on (mo.mno_id = mn.receiver_id && mo.invoice_no = mn.invoice_no && mn.notification_type = 15) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id'  $where ")->row_array();
        $order_type_b2b = $acceped_query['order_type'];
        if($acceped_query['status'] == 'accepted'){
            $mno_orders_id = $acceped_query['id'];
            $start_time = strtotime($acceped_query['order_acceped_time']);
            $end_time = strtotime($acceped_query['order_delivery_time']);
            ...
            $query = $this->db->query("select uo.mno_bill_no, uo.delivery_note,
uo.bill_url,uo.user_id,uo.gst,uo.chc,uo.discount,uo.order_total,uo.action_by,uo.order_id,uo.order_type,uo.delivery_time,uo.listing_id,uo.suggested_pharmacy_id,uo.cancel_reason,uo.action_by,uo.delivery_charge,uo.delivery_charges_by_customer,uo.delivery_charges_by_vendor,uo.delivery_charges_to_mno,uo.listing_name,uo.listing_type,uo.invoice_no,uo.chat_id,uo.address_id,uo.name,uo.mobile,uo.pincode,uo.address1,uo.address2,uo.landmark,uo.city,uo.state,uo.order_total,uo.payment_method,uo.order_date,uo.order_status,uo.rxId,uo.delivered_sign,
uo.order_received_by from user_order as uo where uo.`invoice_no` = '$invoice_no' group by uo.invoice_no
union
select null as mno_bill_no, null as delivery_note, null as bill_url,user_id,gst,null as
chc,idi.discount,idi.order_total,idi.action_by,idi.order_id,idi.order_type,null as delivery_time,idi.listing_id,null as
suggested_pharmacy_id,idi.cancel_reason,idi.action_by,null as delivery_charge,delivery as delivery_charges_by_customer,null as
delivery_charges_by_vendor,null as
delivery_charges_to_mno,idi.listing_name,idi.listing_type,idi.invoice_no,idi.chat_id,idi.address_id,idi.name,idi.mobile,idi.pincode,idi.address1,idi.address2,idi.landmark,idi.city,idi.state,idi.order_total,pm.payment_method,idi.order_date,idi.order_status,null
as rxId,idi.delivered_sign, idi.order_received_by from inven_distrub_invoice as idi left join payment_method as pm on (pm.id = idi.payment_method) where idi.`invoice_no` = '$invoice_no' group by
idi.invoice_no order by order_id desc
            ");
            ...
            // echo $pharmacy_id; die();
            if($pharmacy_id != "" && $pharmacy_id != 0 && $pharmacy_id != null && ($listing_type == 44 || $listing_type == 13)){
            //   echo $pharmacy_id; die();
                $page_no = 1;
                $per_page = 1;
                $lat = $lng = "";
            ...
            } else if($suggested_pharmacy_id != "" && $suggested_pharmacy_id != 0  && $suggested_pharmacy_id != null && ($listing_type == 44 || $listing_type == 13)){
                // print_r($suggested_pharmacy_id); die();
                $page_no = 1;
                $per_page = 1;
                $lat = $lng = "";
            ...
            } else if($listing_type == 52){
                $lat = $lng = "";
                
            //   echo $pharmacy_id; die();
                $sql = "SELECT null as mba, null as recommended,null as certified,`id`, null as `store_time`, `user_id`,null as `pharmacy_branch_user_id`,null as `reach_area`, name as medical_name,null as `discount`, null as `store_manager`, `lat`, `lng`, `address` as address1 , null as address2, `pincode`, `city`, `state`, `phone` as contact_no, null as whatsapp_no , null as `profile_pic`, null as `day_night_delivery`,null as `days_closed`, null as `is_24hrs_available`,null as `payment_type`, null as `store_open`, null as `store_close`, ( 6371 * acos( cos( radians('') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('') ) + sin( radians('') ) * sin( radians( lat ) ) ) ) AS distance FROM distributor where user_id = '$pharmacy_id'  ";
            ...
            foreach ($query->result_array() as $row) {
                ...
                if($row['listing_type'] == "52"){
                    
                    
                    $order_id1            = $row['order_id'];
                    
                     $disttributor_product_query = $this->db->query("select did.discount,did.product_mrp,did.gst_type,did.gst_amount,did.id as product_order_id, did.product_id, did.discount_rupess as product_discount, p.product_name, null as product_unit, null as product_unit_value, p.image as product_img, did.product_qty as product_quantity, did.price as product_price, null as sub_total, 'Available' as product_status, null as product_status_type, null as product_status_value, 'Available' as order_status from inven_distrub_invoice_details as did left join product as p on (p.id = did.product_id) where did.order_id='$order_id1' order by did.id asc")->result_array();
                    
                        foreach ($disttributor_product_query as $product_row) {
                                ...
                                //$product_price        = floatval($product_row['product_price']);
                                $gst_type = $product_row['gst_type'];
                                $gst_amount = $product_row['gst_amount'];
                                $product_mrp = $product_row['product_mrp'];
                                $discount_per = $product_row['discount'];
                                if($gst_type == 1){
                                    $product_price = floatval($product_mrp);
                                } else {
                                    $product_price = floatval($product_mrp+$gst_amount);
                                }
                            
                                // discount_amount
                                
                                $discount_rs = $product_price*$discount_per/100;
                                
                                $product_unit         = $product_row['product_unit'];
                                $product_unit_value   = floatval($product_row['product_unit_value']);
                                $product_quantity     = intval($product_row['product_quantity']);
                                $product_discount     = floatval($discount_rs);
                                $sub_total            = floatval($product_row['sub_total']);
                                $product_status       = $product_row['product_status'];
                                $product_status_type  = $product_row['product_status_type'];
                                $product_status_value = $product_row['product_status_value'];
                                ...
                                if($product_order_status == 'Available'){
                                    $sub_total_sum1      += $product_price*$product_quantity;
                                    $sub_total_discount +=$product_discount*$product_quantity;
                                }
                                
                                // $single_product_price = $product_quantity > 0 ? $product_price / $product_quantity : $product_price ;
                ...
                $order_total_discount = 0;
                 $query1 = $this->db->query("select discount,user_id,order_id,order_type,action_by,delivery_time,cancel_reason,action_by,listing_id,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status from user_order where invoice_no='$invoice_no'");
                  $count1 = $query1->num_rows();
                  ...
                  if ($count1 > 0) {
                    foreach ($query1->result_array() as $row1) {
                        $order_id1            = $row1['order_id'];
                        $order_total_discount = $row1['discount'];
                        $product_query = $this->db->query("select id as product_order_id,product_id,product_discount,product_name,product_unit,product_unit_value,product_img,product_quantity,product_price,sub_total,product_status,product_status_type,product_status_value,order_status from user_order_product where order_id='$order_id1'  order by product_order_id asc");
                        $product_count = $product_query->num_rows();
                        if ($product_count > 0) {
                            foreach ($product_query->result_array() as $product_row) {
    ...
    public function ditributor_info($pharmacy_id){
    ...
    // Swapnali Waghunde on 11th Jun 2020
    // enhancement in payment accepted , getting image and payment accepted by whom from MNO
    public function payment_accepted_by_user($mno_id,$invoice_no, $payment_id, $amount,$received_by,$transaction_id,$payment_image){
        $this->load->model('OrderModel');
        
        $sql = "SELECT uo.order_date,uo.order_status  ,uo.user_id,uo.delivery_charge,uo.delivery_charges_by_customer, uo.delivery_charges_by_vendor, uo.delivery_charges_by_mw,  uo.total_without_dc,uo.listing_type,uo.payment_method,uo.order_total, uo.listing_id,  uo.delivery_charges_to_mno, uo.suggested_pharmacy_id,  ml.mno_name, mo.*, u.id as user_id,uo.listing_id,uo.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo left join user_order as uo on (mo.invoice_no = uo.invoice_no) left join users as u on (uo.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted' AND mo.order_type = '0'
            UNION
            SELECT idi.order_date,idi.order_status  ,idi.user_id,null as  delivery_charge,idi.delivery_charges_by_customer, idi.delivery_charges_by_vendor, idi.delivery_charges_by_mw,  null as total_without_dc,idi.listing_type,idi.payment_method,idi.order_total, idi.listing_id,  idi.delivery_charges_to_mno, null as suggested_pharmacy_id,  ml.mno_name, mo.*, u.id as user_id,idi.listing_id,idi.name as user_name, u.phone, u.token, u.agent FROM `mno_orders` as mo  join inven_distrub_invoice as idi on (mo.invoice_no = idi.invoice_no) left join users as u on (idi.user_id = u.id) left join mno_list as ml on (ml.mno_id = mo.mno_id) WHERE mo.`invoice_no` = '$invoice_no' AND mo.`mno_id` = '$mno_id' AND mo.`ongoing` = 1 AND mo.`status` = 'accepted'  AND mo.order_type = '1'
        ";
        // echo $sql; die();
        ...
        /*payment id is given and amount is not 0*/
        if($payment_id != "" &&  $amount > 0){
        
            $paymentMethods = $this->db->query("SELECT * FROM payment_method where id not in (SELECT parent_id FROM `payment_method` WHERE `parent_id` = '$payment_id') AND id = '$payment_id'")->row_array(); 
            
            if(sizeof($paymentMethods) > 0){
            
               
                /*Payment accepted by MNO and payment type is also present*/
                
                $paymentMethodsSelected = $this->db->query("SELECT * FROM `payment_method` WHERE `id` = '$payment_id'")->row_array(); 
                
                   
                    $paymentMethosName = $paymentMethods['payment_method'];
                    ...
                    if(sizeof($existingOrder) > 0 ){
                        ...
                        if($suggested_pharmacy_id > 0){
                            ...
                            $listing_id = $listing_id;
                            $listing_id_type = $listing_type; // pharmacy or distributor vendor type
                        }
                        ...
                        if($received_by == 'mno'){
                            $paid_to_id = $mno_id;
                            $paid_to_type = $mno_id_type;
                            
                        } else if($received_by == 'medicalwale'){
                            $paid_to_id = $mw_id;
                            $paid_to_type = $mw_id_type;
                        } else {
                            $paid_to_id = $listing_id;
                            $paid_to_type = $listing_id_type;
                        } 
                        
                        $vendor_id = $paid_to_type;
                        $invoice_no = $invoice_no; 
                        // $order_type = 1;
                       ...
                       // add for each transaction
                        ...
                        $array_data['accepted_by'] = "";
                        $array_data['submitted_by'] = "";
                        
                        $res = $this->LedgerModel->create_ledger($user_id, $invoice_no, $user_id_type, $paid_to_id, $paid_to_type, $credit, $debit, $payment_method, $user_comments, $mw_comments, $vendor_comments,$order_type,$transaction_of,$transaction_id,$trans_status,$transaction_date,$vendor_id,$array_data);
                        //print_r($res); die();
                        ...
                        if(!empty($payment_image)){
                            $img_name = $payment_image['image']['name'];
                            ...
                            if (strlen($img_name) > 0) {
                                if (in_array($ext, $img_format)) {
                                    include('s3_config.php');
                                    $actual_image_name = uniqid() . date("YmdHis") . "." . $ext;
                                    $actual_image_path = 'images/Payments/' . $actual_image_name;
                                    if ($s3->putObjectFile($img_tmp, $bucket, $actual_image_path, S3::ACL_PUBLIC_READ)) {
                                        $paid_payment_image = "https://medicalwale.s3.amazonaws.com/images/Payments/".$actual_image_name;
                                    }
                                }
                            }
                        }
                        
                        $changeOrderPaymentMethod = $this->db->query("UPDATE `user_order` SET `payment_method`= '$paymentMethosName', `paid_payment_image` = '$paid_payment_image' WHERE `invoice_no` = '$invoice_no' ");
            ...
            if(sizeof($existingOrder) > 0){
                ...
                /*PAyment not accepted by MNO while delivery*/
    ...
    /*Created by Swapnali Waghunde on 16th July 2020,
    * Handling cancel requests of ditributor order
    */
    public function cancel_distributor_orders($invoices){
        $status = "";
        $return = array();
        $cancel_reason = "Cancelled by vendor"; // get cancel reason id from cancel reasons table
        
        $invs = implode(',', $invoices);
        $fertch_invs_query = "SELECT mo.mno_id,idi.user_id,idi.listing_id,idi.order_date,idi.order_status  ,u.phone,mo.vendor_type, mo.order_type, u.token,u.agent, ml.mno_name,mo.id, mo.invoice_no FROM `mno_orders` as mo left join mno_list as ml on (mo.mno_id = ml.mno_id) join inven_distrub_invoice as idi on (idi.invoice_no = mo.invoice_no) left join users as u on (u.id = mo.mno_id)  WHERE  mo.`ongoing` = '1' AND mo.invoice_no IN ($invs) AND mo.order_type = '1' GROUP BY mo.id";
        
        $fetch_invoices = $this->db->query($fertch_invs_query)->result_array();
        
        
        $fetch_invoice_nos = array_column($fetch_invoices, 'id');
        $invoice_nos_comma = implode(',', $fetch_invoice_nos);
        
        if(sizeof($fetch_invoice_nos) > 0){
            $rejectedOrder = $this->db->query("UPDATE `mno_orders` SET  `ongoing`='0' ,`cancel_reason`='$cancel_reason', `order_cancel` = '1' WHERE id IN ($invoice_nos_comma)");
        ...
        $orders = ""; 
        foreach($fetch_invoices as $fi){
            $updated_data = array();
            $mno_id = $fi['mno_id'];
            $invoice_no = $fi['invoice_no'];
                
            if($mno_id > 0 && $mno_id != null){
                
                $msg = "Distributor cancelled $invoice_no order";
        ...
        if($orders != ""){
            $message = "Updated invoices are $orders";
            $status = 1;
        } else {
            $message = "No invoice to update";
            $status = 2;
        }
