...
if($product['day']===$dfinal)
{
 $current_day_final = $product['time'][0];
      ...
      public function fitness_center_recommended_list($user_id, $latitude, $longitude, $category,$term) {
        $radius = '5';
        if(empty($category))
        {
           $category=0; 
        ...
        $query = $this->db->query("SELECT fcb.id,fcb.user_id, fcb.branch_name, fcb.branch_image, fcb.branch_phone, fcb.branch_email, fcb.about_branch, fcb.branch_business_category, fcb.branch_offer, fcb.branch_facilities, fcb.branch_address, fcb.opening_hours, fcb.pincode, fcb.state, fcb.city, fcb.lat, fcb.lng, fcb.is_free_trail, fcb.from_trail_time,fcb.user_discount, fcb.to_trail_time,( 6371 * acos( cos( radians($latitude) ) * cos( radians( fcb.lat ) ) * cos( radians( fcb.lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( fcb.lat ) ) ) ) AS distance  FROM fitness_center_branch as fcb LEFT JOIN fitness_center as fc ON(fcb.user_id= fc.user_id) WHERE fc.is_active='1' AND fcb.recommended ='1'  HAVING distance <= '$radius' ORDER BY distance LIMIT 0 , 20");
...
WHERE booking_master.booking_id='".$booking_id."'";
    ...
    public function fitness_center_category_list($user_id)
    {
        $category_array = array();
        $Query = $this->db->query("SELECT * FROM business_category WHERE category_id='6'");
        $category_val = $Query->result_array();
     ...
     public function fitness_center_category_wise_list($user_id, $latitude, $longitude, $category,$term,$page_no,$per_page) {
        $radius = '5';
        ...
        if(!empty($category))
        {
            $wh .= "AND FIND_IN_SET('" . $category . "', branch_business_category) ";
        }
        
        if(!empty($term))
        {
            $wh .= "AND fcb.branch_name LIKE '%$term%'";
        }
        $query = $this->db->query("SELECT fcb.id,fcb.user_id, fcb.branch_name, fcb.branch_image, fcb.branch_phone, fcb.branch_email, fcb.about_branch, fcb.branch_business_category, fcb.branch_offer, fcb.branch_facilities, fcb.branch_address, fcb.opening_hours, fcb.pincode, fcb.state, fcb.city, fcb.lat, fcb.lng, fcb.is_free_trail, fcb.from_trail_time,fcb.user_discount, fcb.to_trail_time,( 6371 * acos( cos( radians($latitude) ) * cos( radians( fcb.lat ) ) * cos( radians( fcb.lng ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians( fcb.lat ) ) ) ) AS distance  FROM fitness_center_branch as fcb LEFT JOIN fitness_center as fc ON(fcb.user_id= fc.user_id) WHERE fc.is_active='1' $wh  HAVING distance <= '$radius' ORDER BY distance $limit");
