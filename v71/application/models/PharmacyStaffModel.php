...
class PharmacyStaffModel extends CI_Model
{
    ...
    function SumHours($smon,$fmon) {
          $diff = 0 ;
          if ($smon && $fmon) {
            $smon = $this->ConvertToSeconds($smon);
            $fmon = $this->ConvertToSeconds($fmon);
            $diff = abs( $fmon - $smon ) ;
            return $time_diff = $this->secondsTohhmmss($diff);
          }
    ...
    public function attendanceView($hospital_id,$staff_id,$month,$year)
    {
        
        
        date_default_timezone_set('Asia/Kolkata');
        $number = cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y')); // 31
        // echo "There were {$number} days in ".date('M')." ".date('Y')."";
         
        $now = new DateTime(date('Y-m-d H:i:s'));
        cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y'));
        $start = new DateTime( $now->format('Y-m-00'), new DateTimeZone( 'Asia/Kolkata' ) );
        $end = new DateTime( $now->format('Y-m-'.cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y'))), new DateTimeZone( 'Asia/Kolkata' ) );
        ...
        $query1 = $this->db->query("SELECT users.name,users.phone,media.source,m.address1 
        FROM users LEFT JOIN media ON(media.id=users.avatar_id)
        LEFT JOIN medical_stores m ON(m.user_id=users.id)
        WHERE users.id='$hospital_id'");
        ...
        $query3 = $this->db->query("SELECT salary,working_hours,weekly_off,check_in,check_out
        FROM pharmacy_staff
        WHERE staff_user_id='$staff_id'");
        $count3 = $query3->num_rows();
        foreach ($query3->result_array() as $row) {
            $monthlySalaryArray = $row['salary'];
            $w_hours = $row['working_hours'];
            $w_off = $row['weekly_off'];
            $c_in = $row['check_in'];
            $c_out = $row['check_out'];
        }
        
        $query = $this->db->query("SELECT ps.working_hours,ps.weekly_off,ps.salary,users.is_active,users.avatar_id,users.id user_id,users.name staff_name,users.phone,users.email 
        FROM users 
        INNER JOIN pharmacy_staff ps ON(ps.staff_user_id=users.id) 
        WHERE users.staff_hub_user_id='$hospital_id'
        AND users.id='$staff_id' 
        AND users.vendor_id='13'");
        //$staffList[] = array();
        ...
        if ($count > 0) {
            ...
            // working hours by ghanshyam parihar starts
                
                $weekly_off_s = explode(',',$weekly_off);
                $leaves_s = 0; $cs = 0;
                $weeklyOffHours_ = '';
                foreach( $period as $date ){
                	if(in_array($date->format('w'),$weekly_off_s) ){
                		$leaves_s++;
                		$weeklyOffHours_ = $weeklyOffHours_ + $this->h2mss($working_hours);
                	}
                	else{
                	    $cs++;
                	}
                ...
                $finalweeklyOffHours = explode('.', $this->convertToHoursMins($weeklyOffHours_));
                $weeklyOffHours      = $this->convert($finalweeklyOffHours[0], $finalweeklyOffHours[1]);
                
                date_default_timezone_set('Asia/Kolkata');
                $days = cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y')); // 31
                $workingHours = 0; $perHoursSalary = 0; $mth = 0;
                if(!empty($salary) && !empty($working_hours)){
                    $perDaySalary = number_format((float)$salary/$days, 2, '.', '');
                    $wh = explode('.',$working_hours);
                    if(isset($wh[1])){
                        $workingHours = $this->convert($wh[0], $wh[1]);
                        $perHoursSalary = number_format((float)$perDaySalary/$workingHours, 2, '.', '');
                    }
                    else{
                        $workingHours = $this->convert($wh[0], 0);
                        $perHoursSalary = number_format((float)$perDaySalary/$workingHours, 2, '.', '');
                    }
                }
                
                $query1 = $this->db->query("SELECT duration,check_in,check_out,present_day 
                FROM pharmacy_attendance 
                WHERE status='1' 
                AND user_id='$hospital_id' 
                AND duration!=''
                ...
                if ($count1 > 0) {
                    foreach ($query1->result_array() as $row1) {
                        if(!empty($row1['duration'])){
    					 $mth = $mth + $this->h2mss($row1['duration']);
    				    }
                    }
                    
                } 
                
                $wh = $this->h2ms($this->convertToHoursMinss($mth));
                
            // working hours by ghanshyam parihar ends
                
            // calculate leaves starts
                $weekly_off1 = explode(',',$weekly_off);
                $leaves = 0; $present1 =0;
                foreach( $period as $date ){
                	if(in_array($date->format('w'),$weekly_off1) ){
                		$leaves++;
                	}
                	else{
                	    $present1++;
                ...
                $wo = $present1-$count;
            // calculate leaves ends
                
            // calculate weekly off starts
                $weekly_off_ = explode(',',$weekly_off);
                $leaves_ = 0;
                foreach( $period as $date ){
                	if(in_array($date->format('w'),$weekly_off_) ){
                		$leaves_++;
                	}
                	else{
                	    $c++;
                ...
                $l = $leaves_;
            // calculate weekly off ends
                
            // expected working hours starts
                if (!empty($w_hours)) {
                    $t_hours = $this->h2m($this->convertToHoursMins($this->h2mss($w_hours) * ($number - $leaves_)));
                } else {
                    $t_hours = 0;
                }
                $expectedHours = $t_hours;
            // expected working hours ends
                
            // calculate ot hours starts
                $overtime = 0;
                $otm      = 0;
                
                $query3 = $this->db->query("SELECT pharmacy_staff.working_hours,pharmacy_staff.first_name,pharmacy_staff.last_name,pharmacy_attendance.*,TIMESTAMPDIFF(HOUR,pharmacy_attendance.check_in,pharmacy_attendance.check_out) total_hours
                FROM pharmacy_attendance 
                INNER JOIN pharmacy_staff ON(pharmacy_staff.staff_user_id=pharmacy_attendance.staff_id)
                WHERE pharmacy_attendance.status='1' 
                AND pharmacy_attendance.user_id='$hospital_id' 
                AND pharmacy_attendance.staff_id = $user_id
                AND pharmacy_attendance.attendance_month = $month
                order BY pharmacy_attendance.present_date ASC");
                
                $count1 = $query1->num_rows();
                
                if (!empty($w_hours)) {
                    foreach ($query3->result_array() as $mwh) {
                        if (!empty($mwh['duration']) && ($mwh['duration'] > $w_hours)) {
                            $otm = $otm + ($this->h2mss($mwh['duration']) - $this->h2mss($w_hours));
                        }
                    }
                    $overtime = $otm;
                } else {
                ...
                // $present = $this->Attendance_m->calculatePresent($month,$year,$admission_info['attendance']['id']);
                $otdayp  = 0;
                $othours = 0;
                if ($otm > $this->h2mss($w_hours)) {
                    $otdayp = floor($otm / $this->h2mss($w_hours));
                    $othours = $this->h2mss($w_hours);
                } else {
                    // echo $present;
                }
                
                $othourss = $this->h2mrecentOT($this->convertToHoursMins($overtime - ($othours * $otdayp)));
            // calculate ot hours ends
                
            // ot days starts
                $otday = 0;
                if ($otm > $this->h2mss($w_hours)) {
                    $otday     = floor($otm / $this->h2mss($w_hours));
                    $restHours = $otm % $this->h2mss($w_hours);
                }
            // ot days ends
                
            // total leave starts
                $weekly_off = explode(',', $w_off);
                $leaves1    = 0;
                $present1   = 0;
                foreach ($period as $date) {
                if (in_array($date->format('w'), $weekly_off)) {
                    $leaves1++;
                } else {
                    $present1++;
                    }
                }
                $tleaves = $present1 - $count;
            // total leaves ends
                
            // lat coming days starts
                
                $query5 = $this->db->query("SELECT duration,check_in,check_out,present_day
                FROM pharmacy_attendance 
                WHERE status='1' 
                AND user_id='$hospital_id' 
                AND staff_id = $user_id
                ...
                $mthLateComingDays = 0;
                foreach ($query5->result_array() as $s) {
                    if ($s['check_in'] > $c_in) {
                        $mthLateComingDays = $mthLateComingDays + $this->h2mss($this->SumHours($c_in, $s['check_in'])) / $this->h2mss($w_hours);
                    }
                }
                $latcdays = floor($mthLateComingDays);
            // lat coming days ends
                
            // late coming hours starts
                $mthLateComingHours = 0;
                foreach ($query5->result_array() as $s) {
                    if ($s['check_in'] > $c_in) {
                        $mthLateComingHours = $mthLateComingHours + $this->h2mss($this->SumHours($c_in, $s['check_in']));
                    }
                }
                $lchours = $this->h2mrecentOT($this->convertToHoursMins($mthLateComingHours));
            // late coming hours esds
            // early going days starts
                $mthEarlyGoingDays = 0;
                foreach ($query5->result_array() as $s) {
                    if ($s['check_out'] < $c_out) {
                        $mthEarlyGoingDays = $mthEarlyGoingDays + $this->h2mss($this->SumHours($s['check_out'],$c_out)) / $this->h2mss($w_hours);
                    }
                }
                $egdays = floor($mthEarlyGoingDays);
            // early going days ends
            // ealry going hours starts
                $mthEarlyGoingHours = 0;
                foreach ($query5->result_array() as $s) {
                    if ($s['check_out'] < $c_out) {
                        $mthEarlyGoingHours = $mthEarlyGoingHours + $this->h2mss($this->SumHours($s['check_out'], $c_out));
                    }
                }
                $eghours = $this->h2mrecentOT($this->convertToHoursMins($mthEarlyGoingHours));
            // ealry going hours ends
            ...
            // weekly off starts
                ...
                $WeeklyOffPresent     = 0;
                $present1   = 0;
                foreach($query1->result_array() as $key=>$egh){
                    if (in_array($daysArray[$egh['present_day']],$weekly_off)) {
                        $WeeklyOffPresent++;
                    } else {
                        $present1++;
                    }
                ...
                $weekly_off_     = explode(',', $w_off);
                $leaves_         = 0;
                $weeklyOffHours_ = '';
                foreach ($period as $date) {
                    if (in_array($date->format('w'), $weekly_off_)) {
                        $leaves_++;
                        $weeklyOffHours_ = $weeklyOffHours_ + $this->h2mss($w_hours);
                    } else {
                        $c++;
                    }
                }
                $weeklyOff = $leaves_-$WeeklyOffPresent;
            // weekly off ends
            // weekly off present starts
                $finalweeklyOffHours = explode('.', $this->convertToHoursMins($weeklyOffHours_));
                $weeklyOffHours      = $this->convert($finalweeklyOffHours[0], $finalweeklyOffHours[1]);
            // weekly off present ends
            
            // salary as per presnt days starts
            date_default_timezone_set('Asia/Kolkata');
            ...
            $perHoursSalary = 0;
            $mth            = 0;
            if (!empty($monthlySalaryArray) && !empty($w_hours)) {
                $perDaySalary   = number_format((float) $monthlySalaryArray / $days, 2, '.', '');
                $wh             = explode('.', $w_hours);
                if(isset($wh[1])){
                    $wh1 = $wh[1];
                }
                ...
                $workingHours   = $this->convert($wh[0], $wh1);
                $perHoursSalary = number_format((float) $perDaySalary / $workingHours, 2, '.', '');
            }
            foreach ($query1->result_array() as $s) {
                if (!empty($s['duration'])) {
                    $mth = $mth + $this->h2mss($s['duration']);
                }
            }
            if(count($query1->result_array())>0){
                $salary_as_per_present_day = number_format((float)(((count($query1->result_array())+$leaves_)*$monthlySalaryArray)/$days), 2, '.', '');
            }
            else{
                $salary_as_per_present_day = 0;
            }
            // salary as per present days ends
            
            // salary addition overtime starts
            $addition = $overtime - ($othours * $otdayp);
                                                                
            if(!empty($addition)){
                $finalHours    = explode('.', $this->convertToHoursMins($addition));
                $workingHours_d = $this->convert($finalHours[0], $finalHours[1]);

                $workingHoursotd = '';
                $overtime_addition_salary = number_format((float) ($workingHours_d) * $perHoursSalary, 2, '.', ''); // .'('.h2mrecent($workingHours).')';
            }
            else{
                $overtime_addition_salary = 0.0;
            }
            // salary addition overtime ends
            
            // salary deduction starts
            $HSF2 =  $HSF1 = 0;
            foreach ($query1->result_array() as $s) {
                if (!empty($s['duration']) && ($this->h2mss($s['duration'])<$this->h2mss($w_hours))) {
                    $HSF1 = $HSF1 + ($this->h2mss($w_hours) - $this->h2mss($s['duration']));
                }
            }
            
            if(!empty($HSF1)){
                $finalHours1    = explode('.', $this->convertToHoursMins($HSF1));
                $workingHours_d = $this->convert($finalHours1[0], $finalHours1[1]);

                $workingHoursotd = '';
                $salary_deduction = number_format((float) ($workingHours_d) * $perHoursSalary, 2, '.', ''); // .'('.h2mrecent($workingHours).')';
            }
            else{
                $salary_deduction = 0.0;
            }
            // salary deduction ends
            
            // gross salary starts
            if(!empty($mth)){
                $finalHours    = explode('.', $this->convertToHoursMins($mth));
                $workingHours_ = $this->convert($finalHours[0], $finalHours[1]);
                $gross_salary = number_format((float) ($workingHours_ + $weeklyOffHours) * $perHoursSalary, 2, '.', ''); // .'('.h2mrecent($workingHours).')';
            }
            else{
                $gross_salary = 0.0;
            }
            ...
            // calculate salary starts
                $finalHours = explode('.',$this->convertToHoursMins($mth));
                $workingHours = $this->convert($finalHours[0], $finalHours[1]);
                if(!empty($workingHours)){
                    $salary1 = number_format((float)($workingHours+$weeklyOffHours)*$perHoursSalary, 2, '.', ''); // .'('.h2mrecent($workingHours).')';
                }
                else{
                    $salary1 = 0.0;
                }
            // calculate salary ends
                ...
                $resultpost[] = array(
                    ...
                    'monthlySalaryArray' => $monthlySalaryArray,
                    "user_id"            => $user_id,
                    "present_day"        => $count,
                    "staff_name"         => $staff_name,
                    "monthly_working_hours"    => $wh,
                    "ot_hours"           => $othourss,
                    "ot_days"            => $otday,
                    "total_leaves"       => $tleaves,
                    "late_coming_days"   => $latcdays,
                    "late_coming_hours"  => $lchours,
                    "ealry_going_days"   => $egdays,
                    "ealry_going_hours"  => $eghours,
                    "working_hours"      => $this->h2ms($this->convertToHoursMinss($this->h2mss($w_hours))),
                    "salary"             => $salary1,
                    "leaves"             => $l, 
                    "expectedHours"      => $expectedHours,
                    "weekly_off"         => $weeklyOff,
                    "weekly_off_present" => $WeeklyOffPresent,
                    "salary_as_per_present_day" => $salary_as_per_present_day,
                    "overtime_addition_salary" => $overtime_addition_salary,
        ...
        // $monthly = date('m');
        // $query = $this->db->query("SELECT `pharmacy_staff`.`working_hours`, `pharmacy_staff`.`first_name`, `pharmacy_staff`.`last_name`, `pharmacy_attendance`.*, TIMESTAMPDIFF(HOUR, `pharmacy_attendance`.`check_in`, pharmacy_attendance.check_out) total_hours FROM `pharmacy_attendance` INNER JOIN `pharmacy_staff` ON `pharmacy_staff`.`staff_user_id`=`pharmacy_attendance`.`staff_id` WHERE `pharmacy_attendance`.`status` = 1 AND `pharmacy_attendance`.`user_id` = '$user_id' AND `pharmacy_attendance`.`attendance_month` = '$monthly' AND `pharmacy_attendance`.`staff_id` = '$staff_id' ORDER BY `pharmacy_attendance`.`present_date` ASC");
        // //$staffList[] = array();
        // $count = $query->num_rows();
        // if ($count > 0) {
        //     foreach ($query->result_array() as $row) {
    ...
    public function salaryList($hospital_id,$month,$year)
    {
        if(empty($month)){
            $month = date('m');
        }
        ...
        // echo "There were {$number} days in ".$month." ".$year."";
         
        $now = new DateTime(date('Y-m-d H:i:s'));
        cal_days_in_month(CAL_GREGORIAN, $month, $year);
        $start = new DateTime( $now->format('Y-m-00'), new DateTimeZone( 'Asia/Kolkata' ) );
        $end = new DateTime( $now->format('Y-m-'.cal_days_in_month(CAL_GREGORIAN, $month, $year)), new DateTimeZone( 'Asia/Kolkata' ) );
        $interval = new DateInterval('P1D');
        $period = new DatePeriod( $start, $interval, $end );
        $c = 0;
        $days = 0;
    ...
    function convertToHoursMinss($time, $format = '%02d:%02d') {
        if ($time < 1) {
            return '0.0';
        }
        $hours = floor($time / 60);
    ...
    function convert($hours, $minutes) {
        $n = $hours.round($minutes / 60, 2);
        $whole = floor($n);      // 1
        $fraction = $n - $whole; // .25
        return $hours+($hours.$fraction-$whole);
    }
    ...
    function h2mrecent($hours) { 
        ...
        if($h==0 && $m != 0){
            ...
            if($h>1){
                $mm = ($h).' Hrs '.(round($m * 60)).' Min'; 
            }
            else{
                $mm = ($h).' Hr '.(round($m * 60)).' Min'; 
            } 
        }
        else{
            $mm = '0';
    ...
    public function addAttendanceSave($staff_id,$user_id,$month,$year,$date,$check_in_,$check_out_,$duration_)
    {
        date_default_timezone_set("Asia/Kolkata");
        if(sizeof($date)>0){
            for($i = 0;$i<sizeof($date);$i++){
                $present_date = date("Y-m-d", strtotime($date[$i])); // $date[$i];
                
                $datetime           = DateTime::createFromFormat('Y-m-d', $date[$i]);
                $present_day        = $datetime->format('D');
                $staff_id           = $staff_id;
                $attendance_month   = $month;
                $attendance_year    = $year;
                $check_in           = $check_in_[$i];
                $check_out          = $check_out_[$i];
                $duration           = $duration_[$i];
                $this->db->query("INSERT INTO `pharmacy_attendance` (`present_date`, `staff_id`, `user_id`, `attendance_month`,
    			`attendance_year`, `check_in`, `check_out`, `duration`, `present_day`) VALUES ('$present_date', '$staff_id', '$user_id', '$attendance_month', '$attendance_year', '$check_in', '$check_out', '$duration', '$present_day')");
            }
    ...
    public function searchattendanceList($id,$user_id,$month,$year,$date)
    {
        ...
        if(!empty($date) && (empty($month) || empty($year))){
            $subQuery = "AND `pharmacy_attendance`.`present_date` = '$date'";
        }
        else if(!empty($date) && empty($month) && !empty($year)){
            $subQuery = "AND `pharmacy_attendance`.`present_date` = '$date' AND `pharmacy_attendance`.`attendance_year` LIKE '%$year%'";
        }
        else if(empty($date) && !empty($month) && !empty($year)){
            $subQuery = "AND (`pharmacy_attendance`.`attendance_month` = '$month' AND `pharmacy_attendance`.`attendance_year` = '$year')";
        }
        else if(empty($date) && !empty($month) && empty($year)){
            $subQuery = "AND (`pharmacy_attendance`.`attendance_month` = '$month')";
        }
        else if(!empty($year) && (empty($date) || empty($month))){
            $subQuery = "AND `pharmacy_attendance`.`attendance_year` = '$year'";
        }
        else if(!empty($date) && !empty($month) && !empty($year)){
            $subQuery = "AND `pharmacy_attendance`.`present_date` = '$date' AND `pharmacy_attendance`.`attendance_month` = '$month' AND `pharmacy_attendance`.`attendance_year` = '$year'";
        }
        
        $query = $this->db->query("SELECT `pharmacy_staff`.`working_hours`, `pharmacy_staff`.`first_name`, `pharmacy_staff`.`last_name`,
			`pharmacy_attendance`.*, TIMESTAMPDIFF(HOUR, `pharmacy_attendance`.`check_in`,
			pharmacy_attendance.check_out) total_hours
			FROM `pharmacy_attendance`
			INNER JOIN `pharmacy_staff` ON `pharmacy_staff`.`staff_user_id`=`pharmacy_attendance`.`staff_id`
			WHERE `pharmacy_attendance`.`status` = $isactive
			$subQuery
			AND `pharmacy_attendance`.`user_id` = '$user_id'
			AND `pharmacy_attendance`.`staff_id` = '$id'
			ORDER BY `pharmacy_attendance`.`present_date` DESC");
    ...
    {
        $isactive =  0;
        $sql="UPDATE `pharmacy_attendance` SET `status`='$isactive' WHERE id = '$id'";
    ...
    {
        date_default_timezone_set("Asia/Kolkata");
        $current_time = date("H:i", strtotime(date('h:i a')));
        
        $query = $this->db->query("SELECT check_in FROM pharmacy_attendance WHERE id='$id'");
        $count = $query->num_rows();
        if($count>0){
            foreach ($query->result_array() as $row) {
                $check_in = $row['check_in'];
            }
            $duration = $this->SumHours($check_in,$current_time); // die;
            $check_out = $current_time;
            // print_r($data); die;
            
            $this->db->query("UPDATE `pharmacy_attendance` SET `check_out` = '$check_out', `duration` = '$duration' WHERE `id` = '$id'");
    ...
    public function checkin($staff_id,$user_id)
    {
        date_default_timezone_set("Asia/Kolkata");
        $hospital_id = $user_id;
        $staff_id = $staff_id;
        ...
        $present_day = $datetime->format('D');
        $current_time = date("H:i", strtotime(date('h:i a')));
        $present_date = date("Y-m-d", strtotime($current_date));
        
        $this->db->select('COUNT(*)');
        $this->db->from('pharmacy_attendance');
        $this->db->where('staff_id',$staff_id);
        $this->db->where('present_date',$current_date);
        $sqlattendance = $this->db->get()->row_array();
        if($sqlattendance['COUNT(*)']>0){
            return array(
                    'status' => 201,
            ...
            $this->db->query("INSERT INTO `pharmacy_attendance` (`present_date`, `staff_id`, `user_id`, `attendance_month`,
			`attendance_year`, `check_in`, `present_day`) VALUES ('$present_date', '$staff_id', '$hospital_id', '$month', '$year', '$current_time', '$present_day')");
    ...
    public function editAttendanceSave($id,$check_in,$check_out,$duration)
    {
        $this->db->query("UPDATE `pharmacy_attendance` SET `check_in` = '$check_in', `check_out` = '$check_out', `duration` = '$duration' WHERE `id` = '$id'");
    ...
    public function attendanceList($user_id,$staff_id)
    {
        date_default_timezone_set('Asia/Kolkata');
        $now = new DateTime(date('Y-m-d'));
        $start = new DateTime( $now->format('Y-m-01'), new DateTimeZone( 'Asia/Kolkata' ) );
        $end = new DateTime( $now->format('Y-m-'.cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y'))), new DateTimeZone( 'Asia/Kolkata' ) );
        $interval = new DateInterval('P1D');
        $period = new DatePeriod( $start, $interval, $end );
        $c = 0;
        ...
        $number = cal_days_in_month(CAL_GREGORIAN, date('m'), date('Y')); // 31
        // for complete month by ghanshyam parihar ends
        
        // echo $c;
        // if(!empty($list->data[0]->working_hours)){
        //     $t_hours = h2mrecent(convertToHoursMins(h2mss($list->data[0]->working_hours)*($number-$c)));
        // }
        // else{
        //     $t_hours = 0;
        // }
        ...
        $monthly = date('m');
        $query = $this->db->query("SELECT `pharmacy_staff`.`working_hours`, `pharmacy_staff`.`first_name`, `pharmacy_staff`.`last_name`, `pharmacy_attendance`.*, TIMESTAMPDIFF(HOUR, `pharmacy_attendance`.`check_in`, pharmacy_attendance.check_out) total_hours FROM `pharmacy_attendance` INNER JOIN `pharmacy_staff` ON `pharmacy_staff`.`staff_user_id`=`pharmacy_attendance`.`staff_id` WHERE `pharmacy_attendance`.`status` = 1 AND `pharmacy_attendance`.`user_id` = '$user_id' AND `pharmacy_attendance`.`staff_id` = '$staff_id' AND `pharmacy_attendance`.`attendance_month` = '$monthly' ORDER BY `pharmacy_attendance`.`present_date` DESC");
        //$staffList[] = array();
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                ...
                $staffList[] = array(
                    ...
                    "check_in"           => $check_in,
                    "check_out"          => $check_out,
                    "duration"           => $this->h2mrecent1($duration),
        ...
        $today = date('Y-m-d');
        $query = $this->db->query("SELECT `pharmacy_staff`.`working_hours`, `pharmacy_staff`.`first_name`, `pharmacy_staff`.`last_name`, `pharmacy_attendance`.*, TIMESTAMPDIFF(HOUR, `pharmacy_attendance`.`check_in`, pharmacy_attendance.check_out) total_hours FROM `pharmacy_attendance` INNER JOIN `pharmacy_staff` ON `pharmacy_staff`.`staff_user_id`=`pharmacy_attendance`.`staff_id` WHERE `pharmacy_attendance`.`status` = 1 AND `pharmacy_attendance`.`user_id` = '$user_id' AND `pharmacy_attendance`.`present_date` = '$today' AND `pharmacy_attendance`.`staff_id` = '$staff_id' ORDER BY `pharmacy_attendance`.`present_date` ASC");
        //$staffList[] = array();
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
        ...
        $query = $this->db->query("SELECT `pharmacy_staff`.`working_hours`, `pharmacy_staff`.`first_name`, `pharmacy_staff`.`last_name`, `pharmacy_staff`.`weekly_off` FROM `pharmacy_staff` WHERE `pharmacy_staff`.`staff_user_id` = '$staff_id'");
        //$staffList[] = array();
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                // print_r($row);
                $working_hours1          = $row['working_hours'];
                $staff_name             = $row['first_name'];
                $weekly_off             = $row['weekly_off'];
        ...
        if(!empty($check_in) && empty($check_out)){
		    $today_overtime = $this->h2mrecent1($this->convertToHoursMinss($this->h2mss($this->SumHours($check_in,date("H:i", strtotime(date('h:i a')))))-$this->h2mss($working_hours)));; 
		}
		else if(!empty($check_in) && !empty($check_out)){
		    $today_overtime = $this->h2mrecent1($this->convertToHoursMinss($this->h2mss($this->SumHours($check_in,$check_out))-$this->h2mss($working_hours)));;
		}
		else{
		    $today_overtime = 0;
		}
        ...
        // today, weekly , monthly and remaining hours starts
        if(empty($check_in) && empty($check_out)){
            $hours2 = $this->h2ms($this->convertToHoursMinss($this->h2msss(0)-$this->h2msss(0)));
            $valuenow = $this->convertToHoursMinss($this->h2msss($check_out)-$this->h2msss($check_in));
            $recent_today_time = $this->h2mrecent($this->convertToHoursMinss($this->h2msss($check_out)-$this->h2msss($check_in)));
	    }
	    else if(!empty($check_in) && !empty($check_out)){
            $hours2 = $this->h2ms($this->convertToHoursMinss($this->h2msss($check_out)-$this->h2msss($check_in)));
	        $valuenow = $this->convertToHoursMinss($this->h2msss($check_out)-$this->h2msss($check_in));
	        $recent_today_time = $this->h2ms($this->convertToHoursMinss($this->h2msss($check_out)-$this->h2msss($check_in)));
	    }
	    else{
            $valuenow = $this->convertToHoursMinss($this->h2msss(date("H:i", strtotime(date('h:i a'))))-$this->h2msss($check_in));
            $recent_today_time = $this->h2ms($this->convertToHoursMinss($this->h2msss(date("H:i", strtotime(date('h:i a'))))-$this->h2msss($check_in)));
            $hours2 = $this->h2ms($this->convertToHoursMinss($this->h2msss(date("H:i", strtotime(date('h:i a'))))-$this->h2msss($check_in)));
	    }
	    
	    $today_working_hours = $recent_today_time;
	    
	    // today, weekly , monthly and remaining hours ends
        ...
        $j = date('d');
        if($weekly_off!=''){
        $wday = explode(',',$weekly_off);
        $dayArray = array("0"=>"sun","1"=>"mon","2"=>"tue","3"=>"wed","4"=>"thu","5"=>"fri","6"=>"sat");
        
        $dayArray[end($wday)];
        $monday = date('Y-m-d',strtotime('previous '.$dayArray[end($wday)]));
        // 			echo date('l');
        if($dayArray[current($wday)] != date('l') && $dayArray[end($wday)] != date('l')){
        	$date1 = date_create($monday);
            $date2 = date_create(date('Y-m-d'));
            
            //difference between two dates
            $diff = date_diff($date1,$date2);
            //count days
            $days = $dayss = $diff->format("%a");
            
        	for($i = 1;$i<=$days;$i++){
                
                $date = date('Y-m-'.$j);
        	    $query1 = $this->db->query("SELECT pharmacy_attendance.*,TIMESTAMPDIFF(HOUR,pharmacy_attendance.check_in,pharmacy_attendance.check_out) total_hours
        	    FROM `pharmacy_attendance` 
        	    INNER JOIN `pharmacy_staff` ON `pharmacy_staff`.`staff_user_id`=`pharmacy_attendance`.`staff_id` 
        	    WHERE `pharmacy_attendance`.`status` = 1 
        	    AND `pharmacy_attendance`.`user_id` = '$user_id'
        	    AND pharmacy_attendance.present_date = '$date'
        	    AND `pharmacy_attendance`.`staff_id` = '$staff_id' 
        	    ORDER BY pharmacy_attendance.present_date ASC");
        	    $cnt1 = $query->num_rows();
        	    if ($cnt1 > 0) {
                    foreach ($query1->result_array() as $row) {
                        $mth = $mth + $this->h2mss($row['duration']);
                    }
        ...
        //   $weeklyHours = $this->Attendance_m->weeklyHours_(date('Y-m-d'),$hospital_id,$id);
        }
        $weeklyHours = $this->h2mrecent1($this->convertToHoursMins($mth));
        }
        else{
            $weeklyHours = 0;
        }
        // weekly hours ends
        // weekly hours progressbar starts
        $weeklyProgressbar = 0;
        if(!empty($working_hours)){
            $weeklyProgressbar = $mth*100/($this->h2mss($working_hours)*6);
        }
        // weekly hours progressbar ends
        // expected weekly hours starts
        if(!empty($working_hours)){
			    $expectedWeeklyHours = $this->h2mrecent($this->convertToHoursMins($this->h2mss($working_hours)*6));
		}
		else{
		    $expectedWeeklyHours = 0;
		}
        // expected weekly hours ends
        
        // MONTHLY HOURS STARTS
        $mth = 0; $mtm = 0;
		$day = 0;
        // 		print_r($monthly_working_hours);
		foreach($monthly_working_hours as $mwh){
		    if(!empty($mwh['duration'])){
			 $mth = $mth + $this->h2mss($mwh['duration']);
		    }
		}
		$monthlyHours = $this->h2mrecent1($this->convertToHoursMins($mth));
		// MONTHLY HOURS ENDS
		
        // expected monthly hours starts
        $weekly_off_ = explode(',',$weekly_off);
        $leaves_ = 0;
        foreach( $period as $date ){
        	if(in_array($date->format('w'),$weekly_off_) ){
        ...
        $expectedMonthlyHours = $this->h2mrecent1($this->convertToHoursMins($this->h2mss($working_hours)*($number-$leaves_)));
        // expected monthly hours ends
        
        // monthly hours progressbar starts
        if(!empty($monthlyHours)){ 
            $monthlyHoursProgressbar = $mth*100/($this->h2mss($working_hours)*$number-$leaves_).'%'; 
        } else { 
            $monthlyHoursProgressbar = '0%'; 
        }
        // monthly hours progressbar ends
        
        // monhtly completed hours hours starts
        $monthlyCompletedHours = $this->h2mrecent1($this->convertToHoursMins($mth));
        // monhtly completed hours ends
        
        // monhtly remaining hours hours starts
        $monthlyRemainingHours = $this->h2mrecent1($this->convertToHoursMins(($this->h2mss($working_hours)*($number-$leaves_))-$mth));
        // monhtly remaining hours ends
        // monhtly remaining hours progressbar starts
        if(!empty($monthlyHours)){ 
            $monthlyRemainingHoursProgressbar = $mth*100/($this->h2mss($working_hours)*($number)).'%'; 
            
        } else { 
            $monthlyRemainingHoursProgressbar = '0%'; 
        }
        // monhtly remaining hours progressbar ends
        
        // monthly overtime 
        if(!empty($working_hours)){
			   $otm = 0;
               foreach($monthly_working_hours as $mwh){
                   if(!empty($mwh['duration']) && ($mwh['duration']>$working_hours)){
                        $otm = $otm+($this->h2mss($mwh['duration'])-$this->h2mss($working_hours));
                   }
				} 
				$monthlyOvertime = $this->h2mrecent1($this->convertToHoursMins($otm));
		}
		else { $monthlyOvertime = 0; }
        // 		monthly overtime ends
        
        // monthly overtime progressbar 
        if(!empty($working_hours)){ 
            $monthlyOvertimeProgressbar = number_format((float)($otm*100/($this->h2mss($working_hours)*2)), 2, '.', '').'%';
        }
        else{
            $monthlyOvertimeProgressbar = 0;
        }
        ...
        $resultpost[] = array( 
                    ...
                    "expected_today_working_hours"   => $this->h2mrecent1($this->convertToHoursMins($this->h2mss($working_hours))),
                    "weeklyHours"           => $weeklyHours,
                    "weeklyProgressbar"     => $weeklyProgressbar,
                    "expectedWeeklyHours"   => $expectedWeeklyHours,
                    "monthlyHours"          => $monthlyHours,
                    "monthlyHoursProgressbar"   => $monthlyHoursProgressbar,
                    "expectedMonthlyHours"  => $expectedMonthlyHours,
                    "monthlyCompletedHours" => $monthlyCompletedHours,
                    "monthlyRemainingHours" => $monthlyRemainingHours,
                    "monthlyRemainingHoursProgressbar" => $monthlyRemainingHoursProgressbar,
                    "monthlyOvertime"       => $monthlyOvertime,
                    "monthlyOvertimeProgressbar" => $monthlyOvertimeProgressbar,
                    "today_overtime"        => $today_overtime,
                    "working_hours"         => $working_hours,
                    "weekly_off"            => $weekly_off,
                    "monthly_working_hours" => $monthly_working_hours,
                    "attendance_list"            => $staffList
                    );
        return $resultpost;
        
    }
    
    function h2mrecent1($hours) { 
        $t = explode(".", $hours); 
        $h = $t[0]; 
        if (isset($t[1])) { 
            $m = $t[1];
     ...
     public function prescription_status($order_id, $order_status, $prescription_order, $delivery_time,$listing_id,$delivery_charges,$delivery_charges_by_customer,$sub_all_total,$discount,$gst,$order_deliver_by)
    {
       ...
       // echo $invoice_no; die();
        ...
        foreach ($prescription_order as $result) {
            ...
            $query = $this->db->query("INSERT INTO `prescription_order_list`(`order_id`, `prescription_name`, `prescription_quantity`, `prescription_price`, `prescription_discount`, `prescription_status`,`stock_id`,`barcode`,`batch_no`) VALUES ('$order_id', '$prescription_name', '$prescription_quantity', '$prescription_price', '$prescription_discount', '$prescription_status','$stock_id','$barcode','$batch_no')");
            // $grand_total = $grand_total + $prescription_price;
    ...
    public function order_deliver_cancel($order_id, $cancel_reason, $type, $notification_type,$user_id)
    {
        ...
        if ($type == 'Order Delivered') {
            $invNoArray = $this->db->query("select  `invoice_no` , `listing_id` from `user_order` WHERE invoice_no = '$order_id'")->row_array();
           ...
           // add for each transaction
            ...
            $vendor_comments = "";
            $payment_method = "";
            $credit = $invNoArray['total_without_dc'];
            $debit = "";
            $transaction_of = 1; // entry for amount
             $transaction_id = "";
  ...
  public function pharmacy_partner_profile_list($listing_id)
    ...
    public function order_on_call($delivery_by,$service,$delivery_time,$delivery_charge,$listing_id,$listing_name,$staff_user_id,$product_details,$total_quantity,$total_price,$discount,$net_amount,$payment_method,$gst,$customer_name,$customer_phone,$customer_email,$address1,$address2,$city,$state,$pincode,$lat,$lng,$map_location,$address_id,$delivery_charges_by_customer,$doctor_name)
    {
        ...
        $query1 = $this->db->query("SELECT name,id,email FROM users WHERE phone='$customer_phone' AND vendor_id='0'")->row();
        if(!empty($query1))
        {
        ...
        if(!empty($order_id))
    	{
    	    ...
    	    if(count($ex)==1) {
    	        ...
    	        $sql="SELECT * FROM medical_stores WHERE user_id = '".$medical_id."' || pharmacy_branch_user_id = '".$medical_id."'";
                    ...
                    if($order_type == "order" || $order_type == "")
                    {
                        $bill_detail1 = $this->db->select('l.*,u.gst')->from('user_order_product as l')->join('user_order as u','u.order_id=l.order_id','left')->where('l.order_id',$order_id)->get()->result();
                        // echo $this->db->last_query();
                    }
                    else
                    {
                        $bill_detail = $this->db->select('l.*,u.gst')->from('prescription_order_list as l')->join('user_order as u','u.order_id=l.order_id','left')->where('l.order_id',$order_id)->get()->result();
                        //echo $this->db->last_query();
                    }
    ...
    public function product_find($user_id,$hub_user_id,$find)
    {
        ...
        if($check_table_existance['status'] == true){
            $table_name = $check_table_existance['table_name'];
            $query = $this->db->query("SELECT p.id as id,p.product_name,p.image,ins.barcode,ins.selling_price,ins.id as stock_id, ins.batch_no FROM product as p inner join $table_name as ins on ins.product_id=p.id  WHERE (p.created_by='$user_id' OR p.created_by='0') AND p.product_name LIKE '%" .$find."%'  AND ins.quantity > 0  GROUP BY ins.barcode limit 1 ");
            $count = $query->num_rows();
                if ($count > 0) {
                    foreach ($query->result_array() as $row) {
                        $id                     = $row['id'];
   ...
   public function order_tracking_mno($user_id, $invoice_no){
    ...
    public function product_list($user_id,$page_no,$per_page,$search)
    {
        ...
        if($check_table_existance['status'] == true){
            ...
            $search_where = "";
            if($search != ""){
                $search_where = "AND (inr.rack like '%$search%' OR iw.wname like '%$search%' OR ind.distributor_name like '%$search%' OR ind.manufacturer_name like '%$search%' OR st.product_name  like '%$search%' OR  st.barcode  like '%$search%' OR st.size like '%$search%' OR st.ptr like '%$search%' OR st.mrp like '%$search%' OR st.expiry_date like '%$search%' OR st.quantity like '%$search%' OR st.selling_price like '%$search%' OR   st.bill_no like '%$search%')";
            }
            $get_all_products = $this->db->query("Select inr.rack as rack_name, inr.details as rack_details, iw.wname as warehouse_name, ind.distributor_name, ind.manufacturer_name, st.id as product_id, st.user_id, st.quantity ,st.vendor_type, st.stock_id, st.product_id, st.product_name, st.barcode, st.size, st.rx, st.ptr, st.mrp, st.selling_price, st.po_id,  st.po_date, st.bill_no, st.stock_request_id, st.warehouse_id, st.batch_no, st.mfg_id, st.distributor_id, st.expiry_date, st.mgf_date, st.rack_id, st.status from $table_name  as st left join inventory_distributor as ind on (st.distributor_id = ind.id) left join inventory_warehouse as iw on (st.warehouse_id = iw.id) left join inventory_rack as inr on (st.rack_id = inr.id) where st.user_id = '$user_id' $search_where order by st.id desc $limit")->result_array();
    ...
    public function edit_product($user_id,$product_id, $size, $ptr, $mrp, $selling_price, $distributor_id, $statuss, $rack_id,$quantity,$expiry_date){
        $status = 0;
        $data = array();
        $set = "";
        $check_table_existance = $this->PharmacyPartnerModel->stock_table_existance($user_id);
