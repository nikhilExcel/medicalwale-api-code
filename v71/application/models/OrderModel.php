   ...
   function whatsapp($phone,$template_type,$whatsapp_body)
{
   ...
   //start
    public function prescription_add_quickbook($user_id, $address_id, $chat_id, $payment_method, $delivery_charge, $is_night_delivery,$lat,$lng,$product_id,$is_profile,$listing_id,$night_owls,$bachat_service_id) 
    {
     ...
     /* echo "SELECT `id`, `user_id`,`medical_name` FROM medical_stores WHERE is_approval='1' AND is_active='1' AND user_id = '$check_id'";
      die;*/
        ...
        {
           $bachat_service= $bachat_service_id;
        }
        $user_order = array(
            'user_id' => $user_id,
            'order_type' => 'prescription',
            ...
            'bachat_service_id'=>$bachat_service
        );
            ...
            if ($key_count == '2') {
              ...
              if ($key_count == '4') {
               ...
               $fields = array(
                'to' => $reg_id,
                'priority' => "high",
               $agent === 'android' ? 'data' : 'data' => array("title" => $title, "body" => $msg, "icon" => $img_url, "tag" => $tag, "notification_image" => $img_url,"notification_date" => $order_date, "notification_type" => "prescription", "order_status" => $order_status, "order_date" => $order_date, "order_id" => $order_id, "invoice_no" => $invoice_no, "name" => $name, "listing_name" => $listing_name,"sound" => "https://medicalwale.s3.amazonaws.com/images/Sound/Vendor_Panel.mp3")
            );
              // print_r($fields);
                $headers = array(
                    GOOGLE_GCM_URL,
                    'Content-Type: application/json',
                    $agent === 'android' ? 'Authorization: key=AAAASZs-aSM:APA91bEQ0SZFS7YnRRK95O2Jp_PdzVC8t_osgDK0UdWXeQRo06fEedknj2b26yfX_YPkLq85h0lc3hUBlnSSPD1qvfikzFYiHd8tlt7BeuOg3f5WRwQxmz19WbEGbZEX894zZEF5iRO5' : 'Authorization: key=AAAASZs-aSM:APA91bEQ0SZFS7YnRRK95O2Jp_PdzVC8t_osgDK0UdWXeQRo06fEedknj2b26yfX_YPkLq85h0lc3hUBlnSSPD1qvfikzFYiHd8tlt7BeuOg3f5WRwQxmz19WbEGbZEX894zZEF5iRO5'
...
//        print_r($listing_id_new); die();
           ...
           for($i = 0 ; $i<count($listing_id); $i++)
            ...
            if ($partner_token_status > 0) {
               ...
               // $this->whatsapp($mobile,$temp_type,$whatsapp_msg);
                ...
                $staff_info = $this->db->query("SELECT web_token from users where (staff_hub_user_id= '$listing_id[$i]' OR admin_hub_user_id= '$listing_id[$i]') AND web_token!='' ");
                    $staff_token_status = $staff_info->num_rows();
                    $staff_array_info = $staff_info->result();
                    
                    foreach($staff_array_info as $staff_n) {
                        $staff_web_token = $staff_n->web_token;
                        send_gcm_notify($title, $staff_web_token, $msg, $img_url, $tag, '4', $order_status, $order_date, $order_id, $invoice_no, $name, $listing_name, $agent);
                    }
     ...
     public function order_from_stack($user_id,$address_id, $payment_method, $delivery_charges , $is_night_delivery, $lat, $lng, $product_details,$is_profile,$listing_id,$night_owls)
       {
        ...
        $check_cart = $this->db->query("SELECT * FROM `user_card_order` WHERE user_id = '$user_id'")->result_array();
    ...
    // print_r($user_order); die();
        ...
        //   b4 -> no nightowls
              ...
              $this->db->insert('All_notification_Mobile', $notification_array);
                    ...
                    foreach($staff_array_info as $staff_n) {
                        $staff_web_token = $staff_n->web_token;
                        send_gcm_notify($title, $staff_web_token, $msg, $img_url, $tag, '4', $order_status, $order_date, $order_id1, $invoice_no, $name, $listing_name[$i], $agent);
                    }
      ...
      // echo $pharmacy_listing_count ; die();
            ...
            $return_data['invoice_no'] = "";
               $return_data['message'] = 'Your cart is empty';
        }      
        return $return_data;
       }
  ...
  /* public function order_from_stack($user_id,$address_id, $payment_method, $delivery_charges , $is_night_delivery, $lat, $lng, $product_details,$is_profile,$listing_id)
    ...
    public function order_details_by_id($user_id,  $order_id)
    {
        ...
        $tracker = array();
         $query = $this->db->query("select actual_cost,chc,gst,discount,delivery_charges_by_customer,action_by,order_id,order_type,delivery_time,listing_id,cancel_reason,action_by,delivery_charge,listing_name,listing_type,invoice_no,chat_id,address_id,name,mobile,pincode,address1,address2,landmark,city,state,order_total,payment_method,order_date,order_status,rxId,bill_url,delivered_sign,order_received_by from user_order where user_id='$user_id'  and invoice_no='$order_id' group by invoice_no order by order_id desc");
        ...
        if ($count > 0) { 
            ...
            foreach ($query->result_array() as $row) {
                ...
                $action_by = $row['action_by'];
                $rxId = $row['rxId'];
                $delivered_sign = $row['delivered_sign']; //Hitesh
                $order_received_by = $row['order_received_by']; //Hitesh
