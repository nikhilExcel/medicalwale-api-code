...
public function get_product_list($brands,$page_no,$per_page,$category_ids,$sort,$user_id,$price_min,$price_max,$offer_count,$best_seller,$gender){
        ...
        $finalData = "SELECT DISTINCT * FROM `product_details_hm` WHERE pd_added_v_id IN (".$brnd.") AND  (`pd_pc_id` IN (".$all.") OR `pd_psc_id` IN (".$all."))  AND pd_status = 1 $wh_price $wh_best_seller $wh_gender $sort_a_zh ORDER BY RAND() $limit";
        $productArray = array();
        $offers=array();
...
public function get_user_wishlist($user_id,$per_page,$page_no){
        $offset = $per_page*($page_no - 1);
        $limit = "LIMIT $per_page OFFSET $offset";
        $finalDataNew = $this->db->query("SELECT DISTINCT * FROM `customer_wishlist` WHERE `customer_id` = '$user_id' $limit")->result_array();
        $checkAvailable = $this->db->query("SELECT * FROM `customer_wishlist` WHERE `customer_id` = '$user_id' $limit");
...
//echo $query_for_order; die();
    ...
    //    $pd_weight = '';
        ...
        if($delivery_by_medicalwale==='1'){
            ...
            /*notification integration by nikhil - 20/04/2021*/
            // cust_id = user_id
            
            
            $reg_id_token       = $this->db->query("SELECT name,token,agent,token_status FROM users WHERE id = $cust_id")->row_array();
            $title              = 'Order Placed';
            $msg                = 'Your order Id '.$insert_id.' of '.$data['total_products'].' item(s) is placed. We will inform you when it is confirmed.';
            ...
            $date1              = date("Y-m-d");
            $booking_id         = '';
            $notification_type  = 'HealthMall';
            //USER
            $this->db->query("INSERT INTO `trans_notification` (`user_id`, `notification_type`, `notification_date`, `title`, `msg`,`order_id`,`order_status`,`img_url`, `tag`,`noti_type`) VALUES ('$cust_id','$notification_type','$date1','$title','$msg','$insert_id','$title','$img_url','$tag','trans')");
            $this->db->query("INSERT INTO `All_notification_Mobile` (`user_id`, `notification_type`, `notification_date`, `title`, `msg`,`order_id`,`order_status`,`img_url`, `tag`) VALUES ('$cust_id','$notification_type','$date1','$title','$msg','$insert_id','$title','$img_url','$tag')");
            ...
            $title = 'HealthMall';
            $msg   = 'You have received a new '.$paym.' order '.$insert_id.' of '.$data['total_products'].' item(s). Please confirm and process the order .';
            send_gcm_notify_vendor($title,$reg_id,$msg,$img_url,$tag,$order_status,$updated_at,$insert_id,$invoice_no,$name,$listing_name,$agent);*/
...
//    print_r($result); die();
   ...
   public function  get_filter_list($user_id,$brand_id,$offer_id,$cat_id)
   {
    ...
    public function hm_menu_list4($user_id){
       ...
       date_default_timezone_set("Asia/Calcutta");
        ...
        $datetime1 = date('Y-m-d H:i:s'); 
        $limit = "LIMIT 10 OFFSET 1"; 
        $finalData = "SELECT DISTINCT * FROM `product_details_hm` WHERE tryon_status='1' AND  pd_status = 1 order by rand() $limit";
        $productArray = array();
        $gender1=array();
        $gender2=array();
        $offers=array();
    ...
    public function Product_home($id,$title,$user_id)
    {
        ...
        $datetime1 = date('Y-m-d H:i:s'); 
        $limit = "LIMIT 0,10"; 
        $finalData = "SELECT DISTINCT * FROM `product_details_hm` WHERE pd_pc_id = '$id'  AND  pd_status = 1 order by rand() $limit";
        $productArray = array();
        $gender1=array();
        $gender2=array();
        $offers=array();
  ...
  $this11->page_no=$page_no;    
    ...
    public function get_product_list_hm5_front($user_id,$page_no,$per_page,$type,$response)
    {
        ...
        $finalData = "SELECT DISTINCT * FROM `product_details_hm` WHERE $wh pd_status = 1 ORDER BY rand(),is_featured=0,is_featured,pd_quantity DESC $limit";
        $productArray = array();
        $gender1=array();
        $gender2=array();
        $offers=array();
...
public function get_product_list_hm5($user_id,$page_no,$per_page,$type,$response,$brands,$category_ids,$sort,$price_min,$price_max,$offer,$best_seller,$gender)
    {
        ...
        {
           $sort_a_zh .= "ORDER BY 	rand(),is_featured=0,is_featured,pd_quantity DESC "; 
        }
        //column_name BETWEEN value1 AND value2
        $wh_price ="";
        if(!empty($price_min) and !empty($price_max))
        ...
        else
        {
         ...
         $finalDataNew = $this->db->query("SELECT DISTINCT * FROM `product_details_hm` WHERE pd_added_v_id IN (".$brnd.") AND  (`pd_pc_id` IN (".$all.") OR `pd_psc_id` IN (".$all.")) AND $wh pd_status = 1 $wh_price $wh_best_seller $wh_gender ORDER BY 	rand(),is_featured=0,is_featured,pd_quantity DESC")->result_array();
    ...
    public function cancel_order($order_id, $reason){
        ...
        if($count > 0){
            if($checkAvailable->result_array()[0]['order_status'] == 'Order Cancelled'){
                ...
                /*notification integration by nikhil - 20/04/2021*/
            // cust_id = user_id
            $user_order_array = $this->db->query("SELECT order_id,invoice_no,order_status,user_id,payment_method,order_date,cancel_reason FROM user_order WHERE order_id = '$order_id'")->row();
            $cust_id = $user_order_array->user_id;
            $reg_id_token = $this->db->query("SELECT name,token,agent,token_status FROM users WHERE id = $cust_id")->row_array();
            $title              = 'Order Cancel';
            $msg = 'Your order '.$order_id.' is cancelled by you - '.$reason.'. Your amount will be credited into your ledger in 24 hours, if already paid. You can use this ledger balance to buy/book anything with Medicalwale.com.';
            $reg_id = $reg_id_token['token'];
            $img_url = 'https://d2c8oti4is0ms3.cloudfront.net/images/img/logo.png';
            $tag = 'text';
            $agent = 'android';
            ...
            $notification_type = 'HealthMall';
            //USER
           $this->db->query("INSERT INTO `trans_notification` (`user_id`, `notification_type`, `notification_date`, `title`, `msg`,`order_id`,`order_status`,`img_url`, `tag`,`noti_type`) VALUES ('$cust_id','$notification_type','$date1','$title','$msg','$order_id','$title','$img_url','$tag','trans')");
            $this->db->query("INSERT INTO `All_notification_Mobile` (`user_id`, `notification_type`, `notification_date`, `title`, `msg`,`order_id`,`order_status`,`img_url`, `tag`) VALUES ('$cust_id','$notification_type','$date1','$title','$msg','$order_id','$title','$img_url','$tag')");
            $this->send_gcm_notify_user($title, $reg_id, $msg, $img_url, $tag, $agent, $booking_id,$notification_type);
        
            $data['message'] = "Order Cancelled!!!";
        
            //vendor
            /* vendor send notification --------- working------- do not remove
            $agent    = $customer_token['agent'];
            $reg_id    = $customer_token['token'];
            $web_token    = $customer_token['web_token'];
            $img_url   = 'https://s3.amazonaws.com/medicalwale/images/img/pharmacy.png';
            ...
            $title = 'HealthMall';
            $msg   = 'Your order '.$order_id.' is cancelled by customer - '.$reason.'.\n Please do not process this order.';
            send_gcm_notify_vendor($title,$reg_id,$msg,$img_url,$tag,$order_status,$updated_at,$insert_id,$invoice_no,$name,$listing_name,$agent);    
            */
   ...
   public function get_main_categories($user_id) {
        
        $results = $this->db->query("SELECT id,cat_name,photo FROM `categories` WHERE parent_cat_id='0' AND pc_status='1'");
        
        foreach ($results->result_array() as $result) {
            $data['id'] = $result['id'];
            $data['banner_image'] = $result['photo'];
            $data['cat_name'] = $result['cat_name'];
            $data['url_name'] = $this->slugify($result['cat_name']);
    ...
    public function get_about_data($user_id){
        
        $sub_category = $this->db->query("SELECT * FROM `about_us_data` WHERE `status` = 1 ORDER BY id ASC");
        
        foreach ($sub_category->result_array() as $result) {
            $data['id'] = $result['id'];
