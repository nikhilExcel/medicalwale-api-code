...
class DoctorModel extends CI_Model
{
    ...
    public function doctor_category($doctors_type_id,$doctors_type_name)
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            date_default_timezone_set('Asia/Kolkata');
                    $current_date = date('H:i:s');
                    $day=date('l');
                   $sql   =  sprintf("SELECT * FROM doctor_online  WHERE FIND_IN_SET('$id', category) and  doctor_online.starttime <= '$current_date'
                    AND doctor_online.endtime >= '$current_date' AND FIND_IN_SET('" . $day . "',doctor_online.day) and doctor_online.status='0' and doctor_online.block='0'");
	...
	if (count($time1) > 0)
                ...
                date_default_timezone_set('Asia/Kolkata');
                    $current_date = date('H:i:s');
                    $day=date('l');
                   $sql   =  sprintf("SELECT * FROM doctor_online  WHERE doctor_id='$doctor_user_id' and  doctor_online.starttime <= '$current_date'
                    AND doctor_online.endtime >= '$current_date' AND FIND_IN_SET('" . $day . "',doctor_online.day) and doctor_online.status='0' and doctor_online.block='0'");
                ...
                else
                {
                  ...
                  date_default_timezone_set('Asia/Kolkata');
                    $current_date = date('H:i:s');
                     ...
                     $day=date('l');
                   $sqldi   =  sprintf("SELECT * FROM doctor_online  WHERE doctor_id='$doctor_user_id' and  doctor_online.starttime <= '$current_date'
                    AND doctor_online.endtime >= '$current_date' AND FIND_IN_SET('" . $day . "',doctor_online.day) and doctor_online.status='0' and doctor_online.block='0'");
   ...
   public function doctor_details($user_id, $listing_id)
    {
           $this->un_block_doctor();
          // Doctor
          //echo "SELECT user_id FROM doctor_list WHERE hospital_doctor_id='$listing_id'";
            $sql0 = sprintf("SELECT user_id FROM doctor_list WHERE hospital_doctor_id='$listing_id'");
            ...
            foreach ($query->result_array() as $row) 
            {
                ...
                $patient_att           = $this->db->select('id')->from('doctor_booking_master')->where('listing_id', $doctor_user_id)->get()->num_rows();
                $followers           = $this->db->select('id')->from('follow_user')->where('parent_id', $doctor_user_id)->get()->num_rows();
                $check_id            = $this->db->select('id')->from('check_in_data')->where('vendor_id','5')->where('listing_id',$doctor_user_id)->get()->num_rows();
                $following           = $this->db->select('id')->from('follow_user')->where('user_id', $doctor_user_id)->get()->num_rows();
                $is_follow           = $this->db->select('id')->from('follow_user')->where('user_id', $user_id)->where('parent_id', $doctor_user_id)->get()->num_rows();
                if ($is_follow > 0) {
                    $is_follow = 'Yes';
	...
	if (count($time1) > 0)
                  ...
                  $day=date('l');
                   $sql   =  sprintf("SELECT * FROM doctor_online  WHERE doctor_id='$doctor_user_id' and  doctor_online.starttime <= '$current_date'
                    AND doctor_online.endtime >= '$current_date' AND FIND_IN_SET('" . $day . "',doctor_online.day) and doctor_online.status='0' and doctor_online.block='0'");
                     ...
                     $count1_doc_online = $query_doct_online->num_rows();
                     if($count1_doc_online >0){
                        $rows_online=$query_doct_online->row_array();
                        $online_status= 1;  
                        $chat_online=$rows_online['chat'];
                        $video_online=$rows_online['video'];
                        $chat_fee=$rows_online['chat_fee'];
                        $video_fee=$rows_online['video_fee'];
                     }else{
                ...
                $resultpost[] = array(
                    ...
                    'recommended' => $recommended,
                    'online_status'=> $online_status,
                    'chat'=>(int)$chat_online,
                    'video'=>(int)$video_online,
                    'chat_fee'=>$chat_fee,
                    'video_fee'=>$video_fee
                );
            }
    ...
    public function add_review($user_id, $listing_id, $rating, $review, $service)
    {
        ...
        $query2 = $this->db->query("select booking_id from doctor_booking_master where user_id = '$user_id' and listing_id='$listing_id' and status='6'");
  ...
  public function add_bookings_v2($user_id, $listing_id, $clinic_id, $booking_date, $booking_time, $user_name, $user_mobile, $from_time, $to_time, $user_email, $user_gender, $is_user, $is_patient, $patient_id,$allergies, $heradiatry_problem, $description, $relationship, $date_of_birth, $connect_type,$coupon_id,$status,$listing_type,$type)
    {
       ...
       // $status               = '1'; //awaiting for confirmation  // pending
        ...
        if ($insert) 
        {
            $message      = 'Your appointment with ' . $doctor_name . ' at ' . $clinic_name . ' for ' . $final_booking_date . ', ' . $booking_time . ' is confirmed and your booking id is '.$booking_id.'. Thanks Medicalwale.com';
            $post_data    = array(
                'From' => '02233721563',
                'To' => $user_mobile,
                'Body' => $message
       ...
       elseif($type=="2")
       {
           ...
           // $status               = '1'; //awaiting for confirmation  // pending
            ...
            $sql0_id = sprintf("SELECT id FROM hospital_ipd_patient WHERE `listing_id` = '$clinic_id' and booking_type='0' and user_id='$patient_id_new'");
            $query_practices0_id  = $this->db->query($sql0_id);
            $total_practices0_id  = $query_practices0_id->num_rows();
            if($total_practices0_id > 0)
            {
              $get_pract0_id=$query_practices0_id->row_array();
              $patient_id_final=$get_pract0_id['id']; 
            }
            else
            {
             $query2_new= $this->db->query("SELECT * FROM `users` WHERE `id` = '$patient_id_new'");
             $queryopd_new = $query2_new->row();
             $age=$queryopd_new->age;
             if(empty($age))
             {
                 $age="";
             }
             $data['user_id']        = $patient_id_new;
             $data['listing_id']        = $clinic_id;
             $data['name'] = $queryopd_new->name;  
             $data['address']        = $queryopd_new->map_location; 
             $data['adharcard']       = ""; 
             $data['booking_type']  = "0";
             $data['email']    = $queryopd_new->email;
             $data['phone']    = $queryopd_new->phone;
             $data['sex']      = $queryopd_new->gender;
             $data['age']      = $age;
             $date1 = $queryopd_new->dob;
             $d1 = date("Y-m-d", strtotime($date1));
             $data['date_of_birth']  =$d1;
             $data['blood_group']  = $queryopd_new->blood_group;
             $data['city']        = $queryopd_new->city;
             $data['pincode']        = "";
             $data['listing_id']=$clinic_id;
             $data['staff_id']="";
             $data[' is_active']="1";
             ...
             $data[' is_active']="1";
             $query2= $this->db->query("SELECT COUNT(*) AS `numrows` FROM `hospital_ipd_patient` WHERE `listing_id` = '$clinic_id'");
             $queryopd = $query2->row_array();
             $pcount=$queryopd['numrows'];
             $pcount=$pcount+1;
             $ddyy=date("m/y");
             $myStr=$this->session->userdata('uname');
             $result = substr($myStr, 0, 2);
             $data11=$result;
             $final_data=$data11.$pcount."/".$ddyy;
             $data['uhidno'] = $final_data;    
             $this->db->insert('hospital_ipd_patient',$data);
             $patient_id_final       = $this->db->insert_id();
            }
       ...
       elseif($type=="3")
       {
        ...
        if($listing_type =="5")
        {
           ...
           // $status               = '1'; //awaiting for confirmation  // pending
            ...
            $appointment_id       = $this->db->insert_id();
             $sql0 = sprintf("SELECT hospital_doctor_id FROM doctor_list WHERE user_id='$listing_id'");
            // echo "SELECT hospital_doctor_id FROM doctor_list WHERE user_id='$listing_id'";
            $query_practices0  = $this->db->query($sql0);
            $total_practices0  = $query_practices0->num_rows();
            $get_pract0=$query_practices0->row_array();
             $ids=$get_pract0['hospital_doctor_id'];  
            ...
            $query_dr_2         = $this->db->query("SELECT name_of_hospital FROM hospitals where user_id='$listing_id'");
            $data_2             = $query_dr_2->row_array();
            $clinic_name        = $data_2['name_of_hospital'];
            $doctor_phone       = $data['telephone'];
            $final_booking_date = date('M-d', strtotime($booking_date));
    ...
    */
     ...
     public function notifyMethod_doctor($listing_id, $user_id, $description, $booking_date, $booking_time, $appointment_id, $connect_type, $user_name,$from)
    {
        ...
        if ($customer_token_count > 0) {
            ...
            $tag          = 'text';
            $key_count    = '1';
             //$title        = "  $connect_type appointment For $newdate at $newtime."; //$user_name . ' has booked an appointment'; 
     ...
     public function notifyMethod_doctor_view($listing_id, $user_id, $user_name, $notyCount)
    {
        ...
        if ($customer_token_count > 0) {
            ...
            $tag          = 'text';
            $key_count    = '1';
            $title        = $user_name . ' has viewed your profile';
            $msg          = $user_name . ' has viewed your profile';
    ...
    */
    public function cancel_status($user_id, $booking_id, $listing_id)
    {
        ...
        if ($count_user > 0) {
           ...
           // $title       = $patient_name . ' has Cancel an Payment1';
           // $msg         = $patient_name . '  has Cancel an Payment1.';
            
            $title       = 'Your '.$from_time1.' appointment '.$consultation_type.' has been cancelled';
            $msg         = 'Tap for the details';
    ...
    public function notifyDoctorMethod($listing_id, $user_id,$title,$msg, $booking_date, $booking_time, $booking_id, $consultation_type, $patient_name)
    {
          ...
          $customer_token_count = $customer_token->num_rows();
            if ($customer_token_count > 0) {
              ...
              $connect_type='0';
              $from="0";
                    $this->send_gcm_notify_doctor($listing_id, $user_id, $title, $reg_id, $msg, $img_url, $tag, $agent, $booking_date, $booking_time, $booking_id,$connect_type,$from);
...
//added for doctor prescription list 
  ...
  public function doctor_prescription_list($user_id){
        ...
        if($pre_count>0)
        {
                ...
                else
                {
                    $doctor_name = "";
                       ...
                       $resultpost[] = array(
                            ...
                            //'prescription_pdf'=> "https://medicalwale.s3.amazonaws.com/images/doctor_prescription_sandbox/".$prescription_id.".pdf"
                            'prescription_pdf'=> "https://medicalwale.s3.amazonaws.com/images/doctor_prescription_live/".$prescription_id.".pdf"
                        );
                }
          ...
          //  $resultpost1 = array();
         $doctorprescriptionRows = $this->db->query("SELECT dp.* FROM   doctor_prescription as dp  where dp.patient_id= '$user_id' AND dp.post_id IS NOT NULL ");
                ...
                else
                {
                    $doctor_name = "";
                       ...
                       $resultpost[] = array(
                            ...
                            'doctor_name' => $doctor_name,
                            'prescription_date' => $prescription_date,
                            'prescription_pdf'=> "https://asktheexpert.medicalwale.com/prescription/".$prescription_id.".pdf"
                        );
                }
...
$notification_array = array(
 ...
 public function doctor_random_cat($user_id,$mlat,$mlng,$type)
     ...
     public function view_doctor_profile($user_id,$view_yourself,$self_user_id)
    {
     ...
     //Api following,followers,pat_attended & profile_views  List End
    public function view_doctor_follower($user_id,$doctor_id,$type,$page_no, $per_page)
    {
        $offset = $per_page*($page_no - 1);
        $limit = "LIMIT $per_page OFFSET $offset"; 
        $resultpost = array();
        // Type = 1 is followers list && Type = 2 is following List
        if($type=="followers")
        {
            $finalData = "SELECT * FROM `follow_user` WHERE parent_id ='$doctor_id' $limit";
                ...
                foreach($query->result_array() as $q1)
                   {
                       ...
                       $name="";
                       $gender="";
                       $user_data = "SELECT * FROM `users` WHERE id ='$users_id'";
                       $query1 = $this->db->query($user_data);
                       $rows_num1 = $query1->num_rows();
                       if($rows_num1 > 0)
                       {
                         $q=$query1->row_array();  
                         $name = $q['name'];
                         $gender = $q['gender'];
                         $img_count ="SELECT media.id FROM `media` INNER JOIN users ON users.avatar_id=media.id  WHERE users.id ='$users_id'"; 
                         $query2 = $this->db->query($img_count);
                         $rows_num2 = $query2->num_rows();
                         if ($rows_num2 > 0) 
                          {
                            $media = $this->db->select('media.source')->from('media')->join('users', 'users.avatar_id=media.id')->where('users.id', $users_id)->get()->row();
                       ...
                       $resultpost[]=array('user_id'=>$users_id,
                                           'name'=>$name,
                                           'gender'=>$gender,
                                           'image'=>$image);
        ...
        elseif($type=="following")
        {
          ...
          $finalData = "SELECT * FROM `follow_user` WHERE user_id ='$doctor_id' $limit";
            $query = $this->db->query($finalData);
            $rows_num = $query->num_rows();
            if($rows_num > 0)
              {
        ...
        elseif($type=="profile_views")
        {
          
          $finalData = "SELECT * FROM `doctor_views` WHERE listing_id ='$doctor_id' $limit";
            $query = $this->db->query($finalData);
            $rows_num = $query->num_rows();
            if($rows_num > 0)
              {
        ...
        elseif($type=="pat_attended")
        {
          
          $finalData = "SELECT * FROM `doctor_booking_master` WHERE listing_id ='$doctor_id' $limit";
            $query = $this->db->query($finalData);
            $rows_num = $query->num_rows();
            if($rows_num > 0)
              {
