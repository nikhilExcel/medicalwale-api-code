...
3. UI enhancements and bugs resolved';
    ...
    public function login($username, $password) {
     ...
     public function check_recent_login($user_id,$vendor_id,$listing_id,$product_id) {
        date_default_timezone_set('Asia/Kolkata');
       
        $created_at = date('Y-m-d');
         $updated_at = date('Y-m-d H:i:s');
         $query = $this->db->query("SELECT * FROM `daily_login_user_details` WHERE user_id = '$user_id' ");
           $row = $query->row_array() ;
           $total=$row['total']+1;
             $count = $query->num_rows();
                if(empty($row))
                {
                 $insert_details = array(
                   ...
                   $this->db->insert('daily_login_user_details',$insert_details);
                   
                    $insert_details1 = array(
                        'user_id'  => $user_id,
                        'total'  => '1',
                   ...
                   $this->db->insert('footprint_user_details',$insert_details1); 
                    ...
                    $this->db->update('daily_login_user_details',$data);
            ...
            if ($check_user>0) {
                ...
                if($type=='email'){
                    ...
                    $sms_response = $this->my_sms_integration($username,$otp_code);
		    ...
		    /*OTP message Integration by nikhil STARTS- 17/04/2021*/
                    ...
                    /*OTP message Integration by nikhil ENDS- 17/04/2021**/			 
                   /* $result        = crul_sms($username, $message);*/
                  /*  $post_data = array(
                        'From' => '02233721563',
                        'To' => $username,
                        'Body' => $message
        ...
        if (empty($username) && empty($password)) {
            ...
            $user_query  = $this->db->query("SELECT id from users where (email='$username' or phone='$username') AND vendor_id='0' AND is_active='1'");
            $check_user  = $user_query->num_rows();			
            if ($check_user>0) {
                $user_querys = $user_query->row();
                $id = $user_querys->id;
    ...
    public function generate_privilage($user_id,$coupon_id) {
        ...
        if ($count > 0) {
            ...
            if ($card_count > 0) {
                ...
                $card_insert_id = $this->db->insert_id();
...
//die();
            if ($card_count > 0) {
                ...
                if ($count > 0) {
                    ...
                    $result        = crul_sms($customer_phone, $message);
...
// die();
    ...
    public function userdetails($listing_id, $user_id) {
        if ($listing_id == "") {
            ...
            if ($check_user != "") {
                ...
                {
                    $height_cm_ft_ne=$height_cm_ft;
                    $height_ne=$height;
                }
                return array(
                    'height_cm_ft' => $height_cm_ft_ne,
                    'name' => $name,
                    'email' => $email,
                    'phone' => $phone,
                    'image' => $image,
    ...
    public function sociallogin($email,$phone,$oauth_provider,$oauth_uid,$agent,$name,$gender,$image,$token,$referral_code) {
      
        
	if($oauth_provider=="apple")
        {
           $email_='';
            $phone_='';
            if(!empty($oauth_provider)){
              
                 $user_query  = $this->db->query("SELECT id from users where  oauth_uid = '$oauth_uid' AND vendor_id=0 ");
            }
            ...
            if ($check_user>0) {
              ...
              */
                ...
                if($referral_code!=''){
                    ...
                    $this->referral_model->add_new_referral_history($id,$referral_code);
                }
                $q = $this->db->select('password,IFNULL(height_cm_ft,"") AS height_cm_ft,id,name,phone,email,gender,dob,height,weight,marital_status,blood_group,diet_fitness,organ_donor,sex_history,agent')->from('users')->where('users.id', $id)->get()->row();
                $id = $q->id;
                $name = $q->name;
        ...
        */
        $check_user_oauth_uid = $this->db->query("SELECT id from users where (oauth_provider='$oauth_provider' and oauth_uid='$oauth_uid')");
        $check_user_count  = $check_user_oauth_uid->num_rows();
        
        if($check_user_count > 0){
                $user_querys = $check_user_oauth_uid->row();
                $id = $user_querys->id;
                $q = $this->db->select('password,IFNULL(height_cm_ft,"") AS height_cm_ft,id,name,phone,email,gender,dob,height,weight,marital_status,blood_group,diet_fitness,organ_donor,sex_history,agent')->from('users')->where('users.id', $id)->get()->row();
                $id = $q->id;
                $name = $q->name;
   ...
   if ($phone == '9833381096' || $phone == '7834277784' || $phone == '8655369076' || $phone == '9824242411' || $phone == '9733331211' || $phone == '8676008901' || $phone == '9107766551' || $phone == '7545032401' || $phone == '8500005213' || $phone == '9822212411' || $phone == '8080262411' || $phone == '9323465785' || $phone == '8097014524' || $phone=='7021493253' || $phone == '8879398756')  {
    ...
    public function checkUserDetails($user_id) {
        /*added by swapnali on 13th March 2020 */
        $display_corona_map = 1; // send 1 to display send 0 to remove
        
        /*added by swapnali - end*/
        /* $q = $this->db->select('avatar_id,password,height_cm_ft,id,name,IFNULL(activity_level,"") AS activity_level,phone,email,gender,dob,IFNULL(height,"") AS height,IFNULL(weight,"") AS weight,IFNULL(marital_status,"") AS marital_status,IFNULL(blood_group,"") AS blood_group,IFNULL(blood_is_active,"") AS blood_is_active,IFNULL(diet_fitness,"") AS diet_fitness,IFNULL(organ_donor,"") AS organ_donor,sex_history,exercise_level,health_condition,health_insurance,addiction,allergies,IFNULL(bmi,"") AS bmi ,IFNULL(heradiatry_problem,"") AS heradiatry_problem,IFNULL(lat,"") AS lat,IFNULL(lng,"") AS lng,IFNULL(map_location,"") AS map_location')->from('users')->where('users.id', $user_id)->get()->row();
        ...
        date_default_timezone_set('Asia/Kolkata');
	     $gmtdate= gmdate('D, d M Y H:i:s \G\M\T', time()); 
	      $gmt=date('d M Y | H:i A');
	      $today_date = date('d M Y');
	     if($gmt >='03 Sep 2019 | 00:00 AM' and $gmt >='31 Dec 2019 | 11:59 PM' )
	     {
        return array(
            'profile_percentage' => $profile_completed, 
            'is_Miss_belly_activated' => $Miss_belly_status, 
    		'ServerTime'=>$gmtdate, 
              'OfferDate'=>$today_date, 
        	'OfferStartTime'=>'01 Mar 2020 | 11:59 PM',
        	'OfferEndTime'=>'31 Dec 2020 | 11:59 PM', 
        	'OfferImageUrl'=>'https://medicalwale.s3.amazonaws.com/images/medicalwale_mobile_app_icons/dashboard/New_Design/New/CORONA.jpg',
        	'display_corona_map' => $display_corona_map,
        	'corona_centers_pdf' => 'https://medicalwale.com/Corona.pdf'
            );
	     }
	     else
	     {
...
if(empty($q->password)){
   $passoword_set=0;
    ...
    //create new user
    public function usersignup($name, $phone, $mail, $gender, $dob, $image,$referral_code, $token, $agent, $is_mail, $deviceId,$oauth_provider,$oauth_uid) {
        ...
        if ($is_mail == 'android_mail' || $is_mail == 'ios_mail') {
            ...
            if ($mail == "") {
                ...
                $count_user2=0;
                if($oauth_provider == 'facebook' || !empty($oauth_uid)){
                $query = $this->db->query("SELECT id FROM `users` WHERE oauth_provider='facebook' and oauth_uid = '$oauth_uid'  and vendor_id='0' ");
                ...
                if($count_user > 0){
                    ...
                    $user_id=$row_id['id'];
                     if ($name != '' && $mail != '' && $gender != '') {
                        ...
                        if($referral_code!=''){
                            $this->load->model('referral_model');
                            $this->referral_model->add_new_referral_history($user_id,$referral_code);
                        }
                         $id = $user_id;
 ...
 $user_data1 = array(
    ...
    */
    // Dhaval Edited add new family member
       public function add_family($user_id,$name, $phone, $email, $gender, $dob, $image, $token, $agent, $relationship) {
        $query = $this->db->query("SELECT users.id FROM `users` WHERE users.phone='$phone'");
        ...
        if ($count > 0) {
            ...
            $query2 = $this->db->query("SELECT * FROM health_record_folder WHERE health_record_id='$user_id' and user_id='$user_id' order by created_at desc");
                $folder_array=['Labs','Prescriptions'];
                $count2 = $query2->num_rows();
          
                  if (empty($count2)) {          
                
                         foreach($folder_array as $a){
                              $health_record = array(
                                'user_id' => $user_id,
                                'health_record_id' => $user_id,
                                'folder_name' => $a,
                                'date' => $updated_at,
                                'created_at' => $updated_at
                            );
            ...
            return array(
             ...
             $query = $this->db->query("SELECT users.id FROM `users` WHERE users.email='$email'");
        ...
        if ($count > 0) {
            ...
            $query2 = $this->db->query("SELECT * FROM health_record_folder WHERE health_record_id='$member_id' and user_id='$user_id' order by created_at desc");
                $folder_array=['Labs','Prescriptions'];
                $count2 = $query2->num_rows();
...
$count = $query->num_rows();
    ...
    public function all_booking_cancel($user_id, $listing_id, $booking_id, $status) {
        ...
        if ($this->db->affected_rows()) {
            ...
            else if ($row_branch['vendor_id'] == '36') {
                $description = 'Dear Spa Center, booking is cancelled by customer ' . $user_name . '. Booking id is ' . $booking_id . '. Date ' . $date_ . '. Thank you.';
            }
...
//   }
  ...
  public function coupon_code($user_id,$coupon,$listing_id,$listing_type,$amount,$card_type){
      $this->load->model('LabcenterModel_v2');
        ...
        {
             // Bachat Health Card
        $query1 = $this->db->query("SELECT * FROM vendor_offers WHERE name='$coupon' and offer_on='$card_type' and end_date >= '$current_date' AND `status` = '0' AND listing_id = '$listing_type'  ");
          ...
          if($count1 > 0)
            ...
            else
            {
                ...
                if($offer_on == 2 ){
                    ...
                    $labs_stack_test = $this->db->query("SELECT ltd.price,ltd.discounted_price,ltd.discount,ss.* FROM `services_stack` as ss left join lab_test_master_details as ltd on (ss.`service_id` = ltd.test_id) WHERE ss.`user_id` = '$user_id' AND ss.`service_type` = 'test' GROUP by ss.id")->result_array();
                    $labs_stack_package = $this->db->query("SELECT lp.Price,lp.discount_type,lp.discount,ss.* FROM `services_stack` as ss left join lab_package_master as lp on (ss.`service_id` = lp.id) WHERE ss.`user_id` = '$user_id' AND ss.`service_type` = 'package' GROUP by ss.id")->result_array();
        ...
        {
           // Missbelly 
/*fOR *th dec event added $amount = 99 : Please change the logic and connect it with coupon code - Swapnali */
//removing 99 amount condition to apply new coupon  - Mrunalini */  
        $query1 = $this->db->query("SELECT * FROM vendor_offers WHERE name='$coupon' and (vendor_id='$listing_id' || vendor_id='0') and end_date >= '$current_date' and min_amount <= '$amount' AND `status` = '0' ");
     ...
     //  echo "SELECT * FROM vendor_offers WHERE name='$coupon' and (vendor_id='$listing_id' || vendor_id='0') and end_date >= '$current_date' and min_amount <= '$amount' ";
        $query1 = $this->db->query("SELECT * FROM vendor_offers WHERE name='$coupon' and (vendor_id='$listing_id' || vendor_id='0') and end_date >= '$current_date' and min_amount <= '$amount' ");
        $row_fitness1 = $query1->row_array();
        $count1 = $query1->num_rows();  
          if($count1 > 0)
...
$notification_array = array(
    ...
    public function privilage_card_detail($user_id) 
       {
     
        $sql = "SELECT `id`, `amt`, `msg`,`main_amt` from user_card_detail where status='0'";
        $query = $this->db->query($sql);
        $count = $query->num_rows();
        if ($count > 0) 
            {
     ...
     public function Profile_menu($user_id,$listing_type){
       
        $query = $this->db->query("SELECT * FROM profile_menu WHERE listing_type='$listing_type' and status='0'");
        $count = $query->num_rows();
        if ($count > 0) {
            foreach ($query->result_array() as $row) {
                $id = $row['id'];
     ...
     public function get_coupon($user_id,$listing_type){
        $allOffers = array();
        $today = $created_date = date("Y-m-d");
        $offers = $this->db->query("SELECT * FROM `vendor_offers` WHERE listing_id='$listing_type' AND status = '0' AND end_date >= '$today' AND (vendor_id='$user_id' or vendor_id='0') ");
	      $count = $offers->num_rows();
        if ($count > 0) {
	 ...
	 public function remove_user_profile($user_id, $patient_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        ...
        if ($count > 0) {
           ...
           $user_query = $this->db->query("UPDATE `users` SET `avatar_id` = '0' WHERE `users`.`id` = $user_id");
            ...
            $user_query = $this->db->query("UPDATE `users` SET `avatar_id` = '0' WHERE `users`.`id` = $patient_id");
            $user_query = $this->db->query("UPDATE `health_record` SET `avatar_id` = '0' WHERE `health_record`.`relation_id` = $patient_id and `health_record`.`user_id` = $user_id ");  
            }
     ...
     public function view_user_social_profile($user_id,$view_yourself,$self_user_id)
    {
        
        $resultpost = array();
        $sql = $this->db->query("SELECT COUNT(`id`) as user_count,`user_id`,`bio`,`relationship_status`,`languages_known` FROM `user_social_profile` WHERE `user_id` = '$user_id'")->row();
        $count_status = 1;
        
            if($count_status > 0)
            {
                    $bio = $sql->bio;
                    $relationship_status = $sql->relationship_status;
                    $languages_known = $sql->languages_known;
                    ...
                    /*****************************************************************/     
                        ...
                        if($view_yourself == 0) 
                        {
                            //echo "SELECT * from doctor_privacy where user_id='$doctor_user_id'";
                             $total_privacy = $this->db->query("SELECT * from doctor_privacy where user_id='$user_id'");
    ...
    public function add_user_social_profile($user_id,$bio,$relationship_status,$languages_known)
    {
        $social_array = array(
            'user_id' => $user_id,
            'bio'  => $bio,
        ...
        $count = $this->db->select('id')->from('user_social_profile')->where('user_id', $user_id)->get()->num_rows();
        
        if($count <1){
            $this->db->insert('user_social_profile', $social_array);  
            return array(
                'status' => 200,
            ...
            $this->db->where('user_id',$user_id);
            $this->db->update('user_social_profile',$social_array);
            return array(
                'status' => 200,
	...
	/**************************add movies Pic***********************************/
    public function add_user_fav_details($user_id,$fav_details,$type)
    {
    ...
    public function delete_users_fav_details($user_id,$media_id){
        
        $delete = $this->db->query("DELETE FROM `doctor_fav_details` WHERE user_id='$user_id' and  pic_id='$media_id'");
        return array(
            'status' => 200,
            'message' => 'success'
        );
    ...
    public function user_privacy_profile_list($user_id)
    {
    ...
    public function user_privacy_profile($user_id, $field, $privacy)
    {
        $sql = $this->db->query("SELECT * FROM doctor_privacy WHERE user_id='$user_id'");
        if($sql->num_rows()>0)
        {
