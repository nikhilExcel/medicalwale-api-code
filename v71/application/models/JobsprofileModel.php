...
class JobsprofileModel extends CI_Model {
    ...
    /*user profile educational details
    id == "" --add
    else edit
    */
...
//JOBS-USER-KEYSKILLS
 ...
 //JOBS-preferred-job-details
    ...
    /*  $type = 1 ----sorting acc to salary
        $sort_salary = 0(low to high salary sorting)
        $sort_salary = 1(high to low salary sorting)
        $type = 2 ---- favourite jobs
        $type = 3 ---- Applied jobs
        $type = 4 ---- After specified posted date
    */
    ...
    {
        $p = $this->db->select('job_id')->from('jobs_description')->where('job_id', $job_id)->get()->num_rows();
        if($p > 0)
        {
            $a = explode(',',$job_id);
          $j=sizeof($a);
    ...
    public function user_profile($user_id, $first_name, $last_name, $mobile,$email,$dob,$gender,$marital_status,$languages_known,$nationality, $address_line1, $address_line2, $city,$city_id, $state,$state_id,$country,$pincode,$c_address_line_1, $c_address_line_2, $c_city, $c_state,$c_city_id, $c_state_id,$c_country,$c_pincode, $mother_tongue) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $job_data = array(
            'user_id' => $user_id,
            ...
            'country' => $country,
            'pincode' => $pincode,
            'c_address_line_1' => $c_address_line_1,
            'c_address_line_2' => $c_address_line_2,
            'c_city' => $c_city,
            'c_state' => $c_state,
             'c_city_id' => $c_city_id,
    ...
    public function user_profile_background($user_id,$resume_head) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $job_data = array(
            'user_id' => $user_id,
    ...
    public function jobs_company_profile($company_id){
        $resultpost = array();
        $job_count = array();
        $query = $this->db->query("SELECT * FROM `company_profile` LEFT JOIN company_industry_type ON company_profile.company_industry_type = company_industry_type.id WHERE `user_id`=$company_id");
        // profile view count
        ...
        foreach ($query->result_array() as $row) {
            ...
            $company_name = $row['company_name'];
            $company_size = $row['company_size'];
            $company_industry_type = $row['category'];
    ...
    // jobs listing based on catergory 
    ...
    // view user complete details 
     public function view_user_profile($user_id){
         
        $query = $this->db->query("SELECT * FROM `jobs_user_profile_master` WHERE user_id=$user_id ORDER BY id DESC LIMIT 1");
        $resultpost = array();
        $personal = array();
        $key_skills = array();
        $certificate = array();
        ...
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
            $image	 = $this->db->get_where('users',array('id' => $user_id))->row()->avatar_id;
            $video = $row['video'];
            $first_name	 = $row['first_name'];
            $last_name = $row['last_name'];
            $mobile = $row['mobile'];
            ...
            $pincode = $row['pincode'];
            $country = $row['country'];
            $c_address_line_1 = $row['c_address_line_1'];
            $c_address_line_2 = $row['c_address_line_2'];
            $c_city = $row['c_city'];
            $c_state = $row['c_state'];
            $c_city_id = $row['c_city_id'];
            $c_state_id = $row['c_state_id'];
            ...
            if($last_name=='null' || $country=='null' || $c_address_line_1=='null' || $address_line2=='null' || $c_city=='null' || $c_state=='null' || $c_country=='null' || $marital_status =='null' )
            {
              $last_name ="" ; 
              $country = "";
              $c_address_line_1 = "";
            ...
            if($image =="0"){
                ...
                $image ='https://d2c8oti4is0ms3.cloudfront.net/images/healthwall_avatar/'.$previous_image1;
            }
            if($video =="0"){
                $video ="";
            }
            ...
            if($background =="0"){
               ...
               $background = 'https://d2c8oti4is0ms3.cloudfront.net/images/job_image/'.$row['background'];
            }
     ...
     public function view_user_profile_edu($user_id){
        ...
        foreach ($query->result_array() as $row) {
            if($row['job_education_id'] == 1)
            {
                $edu = 'SSC(X)';
            }
            elseif($row['job_education_id'] == 2)
            {
                $edu = 'HSC(XII)';
            }
            elseif($row['job_education_id'] == 3)
            {
                 $edu = 'Diploma';
            }
            elseif($row['job_education_id'] == 4)
            {
                 $edu = 'Graduate';
            }
            elseif($row['job_education_id'] == 5)
            {
                 $edu = 'Post-graduate';
            }
            else
    ...
    public function user_dashboard($user_id){
        ...
        $query = $this->db->query("SELECT `id`,`image`,`first_name`,`last_name`,`mobile`,`email`,`city`,`state`,`recruiter_action`,`profile_view_count`,`background` FROM `jobs_user_profile_master` WHERE `user_id`=$user_id");
        $resultpost = array();
        $percentage = array();
       ...
       if($p>0){
        ...
        $min_salary = (min($qwe)) * 12;
            $query3 = $this->db->query("SELECT juj.applied_status,juj.favourite_status,jd.`job_id`,jd.`job_position`,jd.`company_name`, jd.`required_exp`,jd.`location`,jd.`job_desc`,jd.posted_on,jd.`favourite`,jd.income,jd.max_salary FROM `jobs_description` as jd left join jobs_user_job as juj on (jd.job_id = juj.job_id AND juj.user_id = '$user_id' ) WHERE jd.job_position LIKE '%$a%'|| jd.city LIKE '%$b%' || jd.income > $min_salary || jd.max_salary <  $max_salary  AND juj.applied_status !=1 group by jd.job_id ORDER BY jd.job_id DESC");
        ...
        {
            // new jobs
            $query5 = $this->db->query("SELECT juj.applied_status,juj.favourite_status,jd.`job_id`,jd.`job_position`,jd.`company_name`, jd.`required_exp`,jd.`state`,jd.`job_desc`,jd.posted_on,jd.`favourite`,jd.income,jd.max_salary FROM `jobs_description` as jd left join jobs_user_job as juj on (jd.job_id = juj.job_id AND juj.user_id = '$user_id' ) WHERE juj.applied_status=1 group by jd.job_id");
            $count_applied_job += $query5->num_rows();
        }
        ...
        $percentage = ($comper + $comkey + $r1 + $r2 + $r3 + $r4 + $r5 +$s+ $t+ $u +$v);
        $count_alert_no = $this->db->select('id')->from('jobs_alert')->where('user_id', $user_id)->get()->num_rows();
        $updated_date = $this->db->get_where('jobs_user_profile_master',array('user_id' => $user_id))->row()->updated_at;
    ...
    type = 1 ->job skills
    type = 2 -> education details
    type = 3 ->certification details
    type = 4 ->preferred job
    type = 5 ->previous job details
    */
    ...
    //particular comapant job listing
    public function company_job_listing($user_id,$company_id,$vendor_type,$page)
    {
        if($page==""){
         $limit = " LIMIT 0, 10";   
        }
    ...
    // list of industry job 
    public function industry_type($user_id)
    {
        $personal = array();
        $query = $this->db->query("SELECT * FROM `company_industry_type` WHERE `sub_category`=0 ORDER BY category ASC");
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
            $category = $row['category'];
    ...
    public function company_review($id,$user_id,$company_id,$current_employ,$employment_status, $review_title, $company_pros, $company_cons,$comment_or_advise,$rating,$designation,$base_salary,$anonymously )
    {
     ...
     //old api ------- (completely working fine)  
    ...
    //     if($page == 1)
    //     {
    //       $data = $this->db->query("SELECT jobs_review.*,jobs_user_profile_master.first_name,jobs_user_profile_master.last_name,jobs_user_profile_master.image FROM jobs_review LEFT JOIN jobs_user_profile_master ON jobs_review.user_id = jobs_user_profile_master.user_id WHERE jobs_review.user_id = $user_id AND jobs_review.company_id=$company_id  $limit")->result_array();
    //       $array1 = $data;
    ...
    //new api due to new design change
    public function view_company_reviews($user_id,$company_id,$page)
    {
        ...
        $query = $this->db->query("SELECT jobs_review.*,jobs_user_profile_master.first_name,jobs_user_profile_master.last_name,jobs_user_profile_master.image FROM jobs_review INNER JOIN jobs_user_profile_master ON jobs_review.user_id = jobs_user_profile_master.user_id WHERE jobs_review.company_id=$company_id $limit");
        if(sizeof($query) > 0)
        {
            foreach ($query->result_array() as $row) {
            $id = $row['id'];
            ...
            $company_id	 = $row['company_id'];
            $current_employ = $row['current_employ'];
            $employment_status = $row['employment_status'];
            $review_title = $row['review_title'];
            $company_pros = $row['company_pros'];
            $company_cons = $row['company_cons'];
            $comment_or_advise = $row['comment_or_advise'];
            $rating = $row['rating'];
            $designation = $row['designation'];
            $base_salary = $row['base_salary'];
            $anonymously = $row['anonymously'];
            $first_name = $row['first_name'];
            $last_name = $row['last_name'];
            // $salary = $row['first_name'];
            // $salary = $row['first_name'];
            // $salary = $row['first_name'];
            $like_count   = $this->db->select('id')->from('jobs_review_like')->where('review_id', $id)->get()->num_rows();
            $comment_count   = $this->db->select('id')->from('jobs_review_comment')->where('review_id', $id)->get()->num_rows();
            $like_yes_no  = $this->db->select('id')->from('jobs_review_like')->where('review_id', $id)->where('user_id', $user_id)->get()->num_rows();
            $share_comment = 'https://medicalwale.com/share/comment/'.$id;
            $date         = $row['created_at'];
    ...
    // view company review and share url
    public function view_company_review_share($user_id,$review_id)
    {
        $personal = array();
        $image	 = $this->db->get_where('users',array('id' => $user_id))->row()->avatar_id;
        $query = $this->db->query("SELECT * FROM `jobs_review` WHERE `id`=$review_id");
        if(sizeof($query) > 0)
        {
            foreach ($query->result_array() as $row) {
            $id = $row['id'];
            ...
            $base_salary = $row['base_salary'];
            $anonymously = $row['anonymously'];
            $first_name = $this->db->get_where('jobs_user_profile_master',array('user_id' => $user_id1))->row()->first_name;
            $last_name = $this->db->get_where('jobs_user_profile_master',array('user_id' => $user_id1))->row()->last_name;
            // $salary = $row['first_name'];
            // $salary = $row['first_name'];
            // $salary = $row['first_name'];
            $like_count   = $this->db->select('id')->from('jobs_review_like')->where('review_id', $id)->get()->num_rows();
    ...
    //delete review    
    public function delete_review($id,$user_id)
    {
         
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        $this->db->query("DELETE FROM `jobs_review` WHERE id='$id' AND user_id='$user_id'");
        return array(
            'status' => 200,
            'message' => 'deleted successfully'
        );
    ...
    public function delete_profile_resume($user_id)
    {
         
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        $this->db->set('resume','0');
            $this->db->where('user_id', $user_id); //which row want to upgrade  
            $this->db->update('jobs_user_profile_master');
        return array(
            'status' => 200,
    ...
    public function delete_background($user_id)
    {
         
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        $this->db->set('background','0');
            $this->db->where('user_id', $user_id); //which row want to upgrade  
            $this->db->update('jobs_user_profile_master');
        return array(
            'status' => 200,
    ...
    public function delete_profile_video($user_id)
    {
        date_default_timezone_set('Asia/Kolkata');
        $date = date('Y-m-d H:i:s');
        $this->db->set('video','0');
    ...
    public function university_list($user_id,$name){
        $resultpost = array();
        $query = $this->db->query("SELECT * FROM `jobs_university` WHERE `name` LIKE '%%$name%%'");
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
            $name	 = $row['name'];
    ...
    public function board_list($user_id,$name){
        $resultpost = array();
        $query = $this->db->query("SELECT * FROM `jobs_boards` WHERE `name` LIKE '%%$name%%'");
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
            $name	 = $row['name'];
    ...
    // state list and filter acc to name 
    public function state_list($user_id,$name){
        $resultpost = array();
        $query = $this->db->query("SELECT * FROM `states` WHERE `state_name` LIKE '%%$name%%' AND state_id < 42");
        foreach ($query->result_array() as $row) {
            $state_id = $row['state_id'];
            $state_name	 = $row['state_name'];

            $resultpost[] = array('id' => $state_id,
                'name' => $state_name,
    ...
    //city list  
    public function city_list($user_id,$state_id,$name){
        $resultpost = array();
        $query = $this->db->query("SELECT * FROM `cities` WHERE `state_id`=$state_id AND `city_name` LIKE '%%$name%%' ");
        foreach ($query->result_array() as $row) {
            $city_id = $row['city_id'];
            $city_name	 = $row['city_name'];
            $state_id	 = $row['state_id'];

            $resultpost[] = array('id' => $city_id,
                'name' => $city_name,
                );
        }
    ...
    // country list based on name 
    public function country_list($user_id,$name){
        $resultpost = array();
        $query = $this->db->query("SELECT * FROM `sex_store_country` WHERE `country` LIKE '%%$name%%' ORDER BY country ASC ");
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
            $country	 = $row['country'];
    ...
    // religion list 
    public function religion_list($user_id,$page,$name){
        $resultpost = array();
        if($page==""){
        $limit = " LIMIT 0, 10";   
        }
        ...
        $query = $this->db->query("SELECT * FROM `jobs_religion` WHERE `religion` LIKE '%%$name%%'");
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
    ...
    // select data from master table 
    public function jobs_filter_cri($user_id){
        $resultpost = array();
        $experience = array();
        $location = array();
        $salary = array();
        
        $query = $this->db->query("SELECT * FROM `job_req_exp`");
        foreach ($query->result_array() as $row) {
            $id = $row['id'];
            $req_exp = $row['req_exp'];
           $experience[] = array('id' => $id,
                'req_exp' => $req_exp,
                );
        }
        
        $query1 = $this->db->query("SELECT * FROM `jobs_filter_cities`");
        foreach ($query1->result_array() as $row) {
            $id = $row['id'];
            $name = $row['name'];
        ...
        $query2 =$this->db->query("SELECT * FROM `jobs_filter_salary`");
        foreach ($query2->result_array() as $row) {
            $id = $row['id'];
        ...
        $resultpost = array('experience' =>$experience,
        'location' =>$location,
        'salary' =>$salary,
            );
    ...
    //like-dislike jobs's reviews
    public function review_like($user_id, $post_id, $post_user_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $count_query = $this->db->query("SELECT id from jobs_review_like where review_id='$post_id' and user_id='$user_id'");
        ...
        if ($count > 0) {
            $this->db->query("DELETE FROM `jobs_review_like` WHERE user_id='$user_id' and review_id='$post_id'");
            $like_query = $this->db->query("SELECT id from jobs_review_like where review_id='$post_id'");
            ...
            $this->db->insert('jobs_review_like', $post_likes);
            ...
            // } else {
                ...
                $title = $usr_name . ' Beats on your Review';
                $msg = $usr_name . ' Beats on your review click here to view review.';
                $postid = $post_user_id;
            //}
            ...
            if ($customer_token_count > 0) {
               ...
               // $this->send_gcm_notify_doctor($title, $reg_id, $msg, $img_url, $tag, $agent, $post_id);
    ...
    //insert jobs's review comment
    public function review_comment($user_id, $post_id, $comment, $post_user_id) {
        date_default_timezone_set('Asia/Kolkata');
        $created_at = date('Y-m-d H:i:s');
        $comments = array(
            'user_id' => $user_id,
        ...
        $this->db->insert('jobs_review_comment', $comments);
        ...
        $comments_query = $this->db->query("SELECT id from jobs_review_comment where review_id='$post_id'");
    ...
    public function jobs_review_all_reply_list($comment_id, $user_id) {
        ...
        $review_list_count = $this->db->select('id')->from('jobs_review_comment')->where('review_id', $comment_id)->get()->num_rows();
        if ($review_list_count) {
            $query = $this->db->query("SELECT jobs_review_comment.id,jobs_review_comment.review_id,jobs_review_comment.description as comment,jobs_review_comment.created_at,users.name,jobs_review_comment.user_id as post_user_id FROM jobs_review_comment INNER JOIN users on users.id=jobs_review_comment.user_id WHERE jobs_review_comment.review_id='$comment_id' order by jobs_review_comment.id asc");
            foreach ($query->result_array() as $row) {
                $id      = $row['id'];
                ...
                //$like_count   = $this->db->select('id')->from('jobs_review_like')->where('review_id', $comment_id)->get()->num_rows();
                //$like_yes_no  = $this->db->select('id')->from('jobs_review_like')->where('review_id', $comment_id)->where('user_id', $post_user_id)->get()->num_rows();
    ...
    public function get_user_profile_data($user_id){
        $query = $this->db->query("SELECT `name`,`phone`,`email`,`city`,`gender`,`age`,`dob`,`marital_status` FROM `users` WHERE id = $user_id");
        if(sizeof($query) > 0){
            foreach($query->result_array()  as $row ){
                 $name = $row['name'];
                 $phone = $row['phone'];
                 ...
                 $dob = $row['dob'];
                 $marital_status = $row['marital_status'];
                 if (strpos($dob,'/') !== false) { //first we check if the dob contains the string '/'
                    $dob = str_replace('/', '-', $dob); //if yes, we simply replace it with -
                    $dob =  date('Y-m-d', strtotime($dob)); // format
                }
                
                $presence = $this->db->select('id')->from('jobs_user_profile_master')->where('user_id', $user_id)->get()->num_rows();
                if($presence < 1){
                  $this->db->query("INSERT INTO `jobs_user_profile_master`(`user_id`, `first_name`,`gender`, `dob`, `email`,`marital_status`,`mobile`) VALUES ('$user_id','$name','$gender','$dob','$email','$marital_status','$phone')"); 
                  return array(
    ...
    public function get_user_profile_status($user_id){
        $key_skills = $this->db->select('id')->from('jobs_skils_job')->where('user_id', $user_id)->get()->num_rows();
        $education = $this->db->select('id')->from('jobs_user_education')->where('user_id', $user_id)->get()->num_rows();
        $certificate = $this->db->select('id')->from('jobs_user_certifications')->where('user_id', $user_id)->get()->num_rows();
        $preferred_job = $this->db->select('id')->from('jobs_preferred_job')->where('user_id', $user_id)->get()->num_rows();
        $previous_job = $this->db->select('id')->from('jobs_previous_job')->where('user_id', $user_id)->get()->num_rows();
        $resume = $this->db->get_where('jobs_user_profile_master',array('user_id' => $user_id))->row()->resume;
        
        
        if($key_skills < 1){
            $key_skills_array[] = array(
                    'type'=>1,
                    'heading_text' =>'Add your Key skills',
                    'description_text' => 'Get jobs update based on your technical key skills.',
                    'icon_url' => '',
                    'click_text' => 'Add'
        ...
        if ($education < 1) {
            $education_array[]= array(
                    'type'=>2,
                    'heading_text' =>'Add your education',
                    'description_text' => 'Employer need to know your education history.',
                    'icon_url' => '',
                    'click_text' => 'Add'
        ...
        if($certificate < 1) {
            $certificate_array[] = array(
                    'type'=>3,
                    'heading_text' =>'Add your certificates',
                    'description_text' => 'Get relevant jobs update from top recruiter based on your technical skills certificate.',
                    'icon_url' => '',
                    'click_text' => 'Add'
        ...
        if($preferred_job < 1) {
            $preferred_job_array[] = array(
                    'type'=>4,
                    'heading_text' =>'Add your preferred job',
                    'description_text' => 'Get relevant jobs based on your desired career profile from the top recruiter.',
                    'icon_url' => '',
                    'click_text' => 'Add'
        ...
        if($previous_job < 1) {
            $previous_job_arrray[] = array(
                    'type'=>5,
                    'heading_text' =>'Add your work experience',
                    'description_text' => 'Employer need to know yourupdate past jobs history.',
                    'icon_url' => '',
                    'click_text' => 'Add'
        ...
        if($resume == 0) {
            $resume_array[] = array(
                    'type'=>6,
                    'heading_text' =>'Upload your resume',
                    'description_text' => 'Upload your resume to get best job',
                    'icon_url' => '',
                    'click_text' => 'Upload'
         ...
         $resultpost[] =  array(
                'key_skills_array' =>$key_skills_array,
                 'education_array' =>$education_array,
                'certificate_array' =>$certificate_array,
                'preferred_job_array' =>$preferred_job_array,
                'previous_job_arrray' =>$previous_job_arrray,
