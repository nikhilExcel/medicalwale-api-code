...
class BloodbankModel extends CI_Model {
   ...
   public function blood_request($user_id, $donor_id) {
       ...
       if ($this->db->affected_rows() > 0) {
           ...
           function send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $agent, $donor_id) {
               date_default_timezone_set('Asia/Kolkata');
           ...
           foreach ($donor_ids as $donor_ids_array) {
               ...
               $customer_token = $this->db->query("SELECT token,agent,id token_status FROM users WHERE id='$donor_ids_array'");
               ...
               if ($customer_token_count > 0) {
                   ...
                   $msg = 'Someone needs a blood donation near you. Please let them know if you can help.';
                   $donor_id = $token_status['id'];
                  ...
                  $this->db->insert('All_notification_Mobile', $notification_array);
                   ...
                   send_gcm_notify($title, $reg_id, $msg, $img_url, $tag, $agent, $donor_id);
               }
           }
       }
       return array(
