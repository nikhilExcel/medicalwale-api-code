...
{ 
    ...
    public function report_list($user_id,$page)
    {
        $report_list = array();
        $limit = 10;
        $start = 0;
        ...
        $count_query = $this->db->query("SELECT lt.booking_id,lt.listing_id as vendor_id,lab_booking_details.created_at,lab_booking_details.report_pdf,lab_center.lab_name from booking_master lt LEFT JOIN lab_booking_details on lab_booking_details.booking_id=lt.booking_id LEFT JOIN lab_center on lab_center.user_id=lt.listing_id where lt.user_id='$user_id' and lab_booking_details.report_pdf<>''");
        $query       = $this->db->query("SELECT lt.booking_id,lt.listing_id as vendor_id,lab_booking_details.created_at,lab_booking_details.report_pdf,lab_center.lab_name from booking_master lt LEFT JOIN lab_booking_details on lab_booking_details.booking_id=lt.booking_id LEFT JOIN lab_center on lab_center.user_id=lt.listing_id where lt.user_id='$user_id' and lab_booking_details.report_pdf<>'' limit $start, $limit");
    ...
    public function update_order($booking_id,$ledger_id,$transaction_id,$payment_status,$ledger_type)
    {
        ...
        $this->db->where('booking_id', $booking_id);
        $rst = $this->db->update('booking_master', $update_data);
        $user = $this->db->query("SELECT user_id,listing_id from booking_master where booking_id='$booking_id' limit 1")->row_array();
        $user_id = $user['user_id'];
        $vendor_id = $user['listing_id'];
        ...
        if($ledger_id>0){
            $res = $this->LedgerModel->update_ledger($user_id,$ledger_id,$booking_id,$transaction_id,$payment_status,$ledger_type);
            if($payment_status=='1'){
                //$this->lalpathlabs_CreateOrder($booking_id,$vendor_id);
            }
        }
        return array(
            'status' => 200,
    ...
    public function add_order($user_id,$vendor_id,$address_id,$vendor_type,$booking_date,$booking_time,$package_id,$test_id,$member_id,$payment_type,$coupon_id,$bachat_service_id,$user_email_id,$payment_gateway)
    {
        ...
        if(count($test_id>0) || count($package_id>0)){
            ...
            $bachat_status=0;
            $status=0;
            if($bachat_service_id!="0")
            {
             $bachat_status=$bachat_service_id; 
             $status=1;
            }
	    // razorpay lib
	    if($payment_gateway=='razorpay'){
            ...
            $lab_test_order = array(
                ...
                'user_gender' => $user['gender'],
                'status' => $status,
		'bachat_service_id'=>$bachat_status,    
            );
            ...
            $ledger_type='user';
            $this->db->insert('lab_booking_details', $lab_test_order_details);
            if($invoice_no>0 && $payment_type=='3' && $vendor_id=='59370'){
                $this->lalpathlabs_CreateOrder($invoice_no,$vendor_id);
            }
	    if($bachat_service_id=="0")
            {	
    ...
    public function lalpathlabs_CreateOrder($invoice_no,$vendor_id)
    {
        $row1     = $this->db->query("SELECT user_id,booking_date,test_id,total_cost,total_discount,amount,address_line1,address_line2,pincode,city from lab_booking_details where booking_id='$invoice_no' limit 1")->row_array();
        $locality = $row1['address_line1'];
        $address_line2 = $row1['address_line2'];
        $pincode = $row1['pincode'];
        $city     = $row1['city'];
        $total_cost     = $row1['total_cost'];
        $total_discount = $row1['total_discount'];
        $amount         = $row1['amount'];
        $test_id         = $row1['test_id'];
        $user_id         = $row1['user_id'];
        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => "http://lalpathlabs.com/partner/api/v1/GetLabDetails",
          ...
          CURLOPT_POSTFIELDS => "{\r\t\"Locality\":\"$locality\",\r\t\"City\":\"$city\"\r}\r",
          CURLOPT_HTTPHEADER => array(
            "apikey: 5ccbbff691f74c30b0a611a16d72c6ef",
        ...
        if ($err) {
            ...
            $InvoiceCode= $array["Response"]['InvoiceAccount'];
            $WareHouseCode= $array["Response"]['WarehouseCode'];
            $LabCode= $array["Response"]['LabCode'];
        }
        $row4        = $this->db->query("SELECT dob from users where id='$user_id' limit 1")->row_array();
        $dob   = $row4['dob'];
        
        $row2        = $this->db->query("SELECT booking_date,user_name,user_mobile,user_email,user_gender from booking_master where booking_id='$invoice_no' limit 1")->row_array();
		...
		$address= $locality.','.$address_line2;
        ...
        if($test_list>0){
            foreach ($test_list as $tid) {
                $cart_query = $this->db->query("SELECT code,price,discounted_price from lab_test_master_details where test_id='$tid' and user_id='$vendor_id'");
                $count  = $cart_query->num_rows();
                if($count>0){
                    $row3      = $this->db->query("SELECT name from lab_test_master where id='$tid' limit 1")->row_array();
                    $get_test  = $cart_query->row_array();
                    $TestName  = $row3['name'];
		            $TestCode  = $get_test['code'];
		            $mrp       = $get_test['price'];
		            $discount  = $get_test['price']-$get_test['discounted_price'];
		            $price     = $get_test['discounted_price'];
                    $test_cart[] = array(
                        "TestName"  => $TestName,
                		"TestCode"  => $TestCode,
                		"mrp"       => $mrp,
            ...
            $order_array = array(
                "Order"           => $order,
        		"Patient"         => $patient,
        		"TestDetails"     => $test_cart,
        		"isEmailRequired" => $isEmailRequired
            );
            $curl = curl_init();
            curl_setopt_array($curl, array(
              CURLOPT_URL => "http://lalpathlabs.com/partner/api/v3/CreateOrder",
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => "",
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 30,
    ...
    public function all_lab_test($page,$category,$most_common_risk,$sort_type,$home_delivery,$vendor_id,$keyword,$per_page,$type){
	    ...
	    public function vendor_list_by_test_id($lat,$lng,$test_id)
    {
        $filter='';
        $pre='and ';
        $vendor_list=array();
        ...
        $test_id_array = explode(",", $test_id);
        $numcount = count($test_id_array);
        $querys = $this->db->query("SELECT lab_test_master_details.user_id,count(lab_test_master_details.test_id) as test_count FROM lab_test_master_details where FIND_IN_SET(lab_test_master_details.test_id,'$test_id') GROUP BY lab_test_master_details.user_id");
		...
		$homedelivery='';
		$radius = '5000';
        $sql    = sprintf("SELECT ld.home_delivery,lab_center.user_id as vendor_id,lab_center.lab_name as vendor_name,lab_center.address1,lab_center.address2,lab_center.pincode,lab_center.city,lab_center.state,lab_center.contact_no,lab_center.latitude,lab_center.profile_pic,lab_center.longitude, ( 6371 * acos( cos( radians('%s') ) * cos( radians( latitude) ) * cos( radians( longitude) - radians('%s') ) + sin( radians('%s') ) * sin( radians( latitude) ) ) ) AS distance FROM lab_test_master_details ld INNER JOIN lab_center on lab_center.user_id=ld.user_id where (FIND_IN_SET(ld.test_id,'$test_id')) $filter  group by ld.user_id HAVING distance < '%s'  $filter_sort limit $start,$limit", ($lat), ($lng), ($lat), ($radius));
		$image='';
        ...
        $num_count   = $test_query->num_rows(); 
            if ($num_count > 0) {
                foreach ($test_query->result() as $vrow) {
                        ...
                        $report_availability=0;
                        $home_collection_charges=0;
                        $vendor_test_list=array();
                        
                        $price_query = $this->db->query("SELECT lab_test_master_details.test_id,lab_test_master_details.home_delivery,lab_test_master_details.code,lab_test_master_details.report_availability,lab_test_master_details.home_collection_charges,lab_test_master_details.price,lab_test_master_details.discounted_price,lab_test_master_details.discount,lab_test_master_details.medicalwale_discount FROM lab_test_master_details where FIND_IN_SET(lab_test_master_details.test_id,'$test_id') and lab_test_master_details.user_id='$vendor_id' order by lab_test_master_details.test_id asc");
                        $price_list  = $price_query->result();
                        foreach ($price_list as $prow) {
                            $lab_test_id  = $prow->test_id;
                            
                            $lab_query      = $this->db->query("SELECT lab_test_master.name FROM `lab_test_master` Where id='$lab_test_id' limit 1");
			                $lab_row = $lab_query->row_array();	
                            $lab_test_name  = $lab_row["name"];
