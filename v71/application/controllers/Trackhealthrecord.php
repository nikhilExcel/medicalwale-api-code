...
class Trackhealthrecord extends CI_Controller {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['type'] == "") {
                        ...
                        $resp       = $this->TrackhealthrecordModel->record_list($user_id, $type,$from,$record);
                    }
                    json_outputs($resp);
                }
            }
    ...
    public function tracker_information_details() {
        $this->load->model('TrackhealthrecordModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $user_id = $params['user_id'];
                        $key = $params['key'];
                        $resp = $this->TrackhealthrecordModel->tracker_information_details($user_id,$key);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    public function tracker_value_details() {
        $this->load->model('TrackhealthrecordModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "") {
                        ...
                        $user_id = $params['user_id'];
                        $key = $params['key'];
                        $resp = $this->TrackhealthrecordModel->tracker_value_details($user_id,$key);
                    }
                    simple_json_output($resp);
                }
            }
