...
class Pharmacy extends CI_Controller {
 ...
 public function pharmacy_list_v2() {
    ...
    public function pharmacy_front_v1() {
        $this->load->model('PharmacyModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                     ...
                     if ($params['user_id'] == "")
                    {
                        ...
                        $lat = $params['lat'];
                        $lng = $params['lng'];
                        $resp = $this->PharmacyModel->pharmacy_front_v1($user_id, $lat, $lng);
                    }
                    json_outputs($resp);
                }
            }
     ...
     public function pharmacy_front_v2() {
        $this->load->model('PharmacyModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                     ...
                     if ($params['user_id'] == "")
                    {
                        ...
                        $re_mb=$params['type'];
                        $page=$params['page'];
                        $resp = $this->PharmacyModel->pharmacy_front_v2($user_id, $lat, $lng,$re_mb,$page);
                    }
                    json_outputs($resp);
                }
            }
    ...
    public function pharmacy_order_v1() {
        $this->load->model('PharmacyModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                     ...
                     if ($params['user_id'] == "")
                    {
                        ...
                        $lat = $params['lat'];
                        $lng = $params['lng'];
                        $resp = $this->PharmacyModel->pharmacy_order_v1($user_id, $lat, $lng);
                    }
                    json_outputs($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['listing_id'] == "" || $params['rating'] == "" || $params['review'] == "" || $params['service'] == "") {
                        ...
                        $review = $params['review'];
                        $service = $params['service'];
                        $resp = $this->PharmacyModel->edit_review($review_id,$user_id, $listing_id, $rating, $review, $service);
                    }
                    simple_json_output($resp);
                }
            }
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['lat'] == "" || $params['lng'] == "" || $params['page'] == "") {
                        ...
                        /*Added by swapnali waghunde on 26th may 2020*/
                        $search = array_key_exists("search" , $params) ? $params['search'] : "" ;
                        $resp = $this->PharmacyModel->favourite_pharmacy_list($user_id, $lat, $lng, $page,$search);
                    }
                   json_outputs($resp);
                }
            }
    ...
    public function life_saving_drugs() {
        $this->load->model('PharmacyModel');
         	$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
	      	...
	      	if ($this->input->post('user_id') == "" || count($_FILES) == 0 ) {
                        ...
                        $Final = $this->input->post('urgent');
                        $number = $this->input->post('number');
                        $email_id = $this->input->post('email');
                        $resp1 = $this->PharmacyModel->life_saving_drugs($user_id,$member_id,$qty,$Final,$number,$email_id);
					    $ne=(object)[];
					  $resp = array(
                            'status' => 200,
                            'message' => 'Success',
    ...
    public function life_saving_drugs_web() {
        $this->load->model('PharmacyModel');
         	$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
		} else {
	      	if ($this->input->post('user_id') == "" || $this->input->post('image') == "" ) {
						$resp = array('status' => 400,'message' =>  'please enter fields');
			} else {
                        ...
                        $resp1 = $this->PharmacyModel->life_saving_drugs_web($user_id,$member_id,$qty,$Final,$number,$email_id,$image);
					    $ne=(object)[];
					  $resp = array(
                            'status' => 200,
                            'message' => 'Success',
    ...
    public function life_saving_drugs_cancel() {
        $this->load->model('PharmacyModel');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
            json_output(400, array(
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['order_id'] == "" ) {
                        ...
                        $resp = $this->PharmacyModel->life_saving_drugs_cancel($order_id, $cancel_reason);
                    }
                    simple_json_output($resp);
                }
            }
    ...
    // Added by swapnali on 24th july 2k19 on 3:51pm
    // order_tracking_mno
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    if ($params['user_id'] == "" || $params['invoice_no'] == "") {
                        ...
                        $data =  array();
                        $res = $this->PharmacyModel->order_tracking_mno($user_id, $invoice_no);
                        $data = $res['data'];
                        if($res['status'] == 1){
                            $resp = array('status' => 200, 'message' => 'success',  'data' => $data);
                        } else if($res['status'] == 2){
                            $resp = array('status' => 204, 'message' => 'No data found');
                        } else if($res['status'] == 3){
                            $resp = array('status' => 201, 'message' => 'Waiting for night owl to accept the order', 'data' => $data);
                        } else {
